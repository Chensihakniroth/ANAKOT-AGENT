...
| `NOUS_BASE_URL` | 覆盖 callmemo Portal base URL（极少使用；仅用于开发/测试） |
| `NOUS_INFERENCE_BASE_URL` | 直接覆盖 callmemo 推理端点 |
...
| `ANAKOT_GEMINI_CLIENT_ID` | `google-gemini-cli` PKCE 登录的 OAuth 客户端 ID（可选；默认使用 Google 公共 gemini-cli 客户端） |
| `ANAKOT_GEMINI_CLIENT_SECRET` | `google-gemini-cli` 的 OAuth 客户端密钥（可选） |
| `ANAKOT_GEMINI_PROJECT_ID` | 付费 Gemini 层级的 GCP 项目 ID（免费层级自动配置） |
...
| 变量 | 描述 |
|----------|-------------|
| `ANAKOT_PORTAL_BASE_URL` | 覆盖 callmemo Portal URL（用于开发/测试） |
| `NOUS_INFERENCE_BASE_URL` | 覆盖 callmemo 推理 API URL |
| `ANAKOT_NOUS_MIN_KEY_TTL_SECONDS` | 重新铸造前的最小 agent 密钥 TTL（默认：1800 = 30 分钟） |
| `ANAKOT_NOUS_TIMEOUT_SECONDS` | callmemo 凭证/token 流程的 HTTP 超时 |
| `ANAKOT_DUMP_REQUESTS` | 将 API 请求载荷转储到日志文件（`true`/`false`） |
| `ANAKOT_PREFILL_MESSAGES_FILE` | 包含在 API 调用时注入的临时预填消息的 JSON 文件路径 |
| `ANAKOT_TIMEZONE` | IANA 时区覆盖（例如 `America/New_York`） |
...
内置 [`observability/langfuse`](/user-guide/features/built-in-plugins#observabilitylangfuse) 插件的环境变量。在 `~/.anakot/.env` 中设置。在这些变量生效之前，还必须启用该插件（`anakot plugins enable observability/langfuse`，或在 `anakot plugins` 中勾选）。
...
这些变量为付费 callmemo 订阅者或自托管 gateway 部署配置 [Tool Gateway](/user-guide/features/tool-gateway)。大多数用户无需设置——gateway 通过 `anakot model` 或 `anakot tools` 自动配置。

| 变量 | 描述 |
|----------|-------------|
| `TOOL_GATEWAY_DOMAIN` | Tool Gateway 路由的基础域名（默认：`callmemo.ai`） |
| `TOOL_GATEWAY_SCHEME` | gateway URL 的 HTTP 或 HTTPS 协议（默认：`https`） |
| `TOOL_GATEWAY_USER_TOKEN` | Tool Gateway 的认证 token（通常由 callmemo 认证自动填充） |
...
| `GATEWAY_PROXY_URL` | 将消息转发到的远程 Anakot API 服务器 URL（[代理模式](/user-guide/messaging/matrix#proxy-mode-e2ee-on-macos)）。设置后，gateway 仅处理平台 I/O——所有 agent 工作委托给远程服务器。也可通过 `config.yaml` 中的 `gateway.proxy_url` 配置。 |
...
| `ANAKOT_GATEWAY_BUSY_INPUT_MODE` | 默认 gateway 繁忙输入行为：`queue`、`steer` 或 `interrupt`。可通过 `/busy` 按聊天覆盖。 |
...
| 变量 | 描述 |
|----------|-------------|
| `ANAKOT_MAX_ITERATIONS` | 每次对话的最大工具调用迭代次数（默认：90） |
...
| `ANAKOT_TOOL_PROGRESS` | 工具进度显示的已弃用兼容变量。优先使用 `config.yaml` 中的 `display.tool_progress`。 |
| `ANAKOT_TOOL_PROGRESS_MODE` | 工具进度模式的已弃用兼容变量。优先使用 `config.yaml` 中的 `display.tool_progress`。 |
...
| `ANAKOT_BACKGROUND_NOTIFICATIONS` | gateway 中后台进程通知模式：`all`（默认）、`result`、`error`、`off` |
...
| `ANAKOT_WRITE_SAFE_ROOT` | 可选目录前缀，限制 `write_file`/`patch` 写入；超出范围的路径需要审批。 |
...
| `ANAKOT_DEBUG_INTERRUPT` | 设为 `1` 可将详细的中断/取消追踪记录到 `agent.log`。 |
| `ANAKOT_DUMP_REQUESTS` | 将 API 请求载荷转储到日志文件（`true`/`false`） |
