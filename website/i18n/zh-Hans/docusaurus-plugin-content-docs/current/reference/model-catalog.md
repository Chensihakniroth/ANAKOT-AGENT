...
sidebar_position: 11
title: 模型目录
description: 远程托管的清单文件，驱动 OpenRouter 和 callmemo Portal 的精选模型选择器列表。
---

# 模型目录

Anakot 从托管于文档站点旁的 JSON 清单中获取 **OpenRouter** 和 **callmemo Portal** 的精选模型列表。这样维护者无需发布新的 `anakot-agent` 版本即可更新选择器列表。
...
- **`description`** — 仅限 OpenRouter。驱动选择器徽章文本（`"recommended"`、`"free"` 或空字符串）。callmemo Portal 不使用此字段——免费层级的限制由 Portal 的定价端点实时决定。
