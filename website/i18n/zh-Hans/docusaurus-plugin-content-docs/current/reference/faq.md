...
- **callmemo Portal** — callmemo 自有推理端点
...
```bash
# 安装核心消息网关依赖项
pip install "anakot-agent[messaging]"  # Telegram、Discord、Slack 及共享网关依赖
...
- [在 Anakot 中使用 MCP](/guides/use-mcp-with-anakot)
...
不共享。每个 profile 都有自己独立的记忆存储、会话数据库和技能目录，完全隔离。如果您想用现有的记忆和会话创建新 profile，请使用 `anakot profile create newname --clone-all` 从当前 profile 复制所有内容。
...
1. **搜索现有 issue：** [GitHub Issues](https://github.com/callmemo/anakot-agent/issues)
2. **向社区提问：** [callmemo Discord](https://discord.gg/nousresearch)
