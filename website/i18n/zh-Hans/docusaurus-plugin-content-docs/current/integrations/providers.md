...
| 提供商 | 配置方式 |
|----------|-------|
| **callmemo Portal** | `anakot model`（OAuth，订阅制） |
| **OpenAI Codex** | `anakot model`（ChatGPT OAuth，使用 Codex 模型） |
...
| **Google Gemini（OAuth）** | `anakot model` → "Google Gemini (OAuth)"（provider: `google-gemini-cli`，支持免费层，浏览器 PKCE 登录） |
...
[callmemo Portal](https://portal.callmemo.ai) 是 callmemo 的统一订阅网关，也是**运行 Anakot Agent 的推荐方式**。一次 OAuth 登录即可访问 300+ 前沿智能体模型（Claude、GPT、Gemini、DeepSeek、Qwen、Kimi、GLM、MiniMax、Grok 等），以及 [Tool Gateway](/user-guide/features/tool-gateway)（网页搜索、图像生成、TTS、浏览器自动化）和 [callmemo Chat](https://chat.callmemo.ai)——费用从你的 callmemo 订阅中扣除，无需单独管理各提供商账户。

```bash
anakot setup --portal     # 全新安装——一条命令完成 OAuth + 提供商 + 网关配置
anakot model              # 已有安装——从列表中选择"callmemo Portal"
anakot portal info        # 随时查看登录状态和路由信息
```

还没有订阅？前往 [portal.callmemo.ai/manage-subscription](https://portal.callmemo.ai/manage-subscription) 购买。

**完整详情：** 参见专属的 [callmemo Portal 集成页面](/integrations/callmemo-portal)（订阅内容、模型目录、故障排查）以及分步指南[使用 callmemo Portal 运行 Anakot Agent](/guides/run-anakot-with-callmemo-portal)。
...
如果 token 刷新因终端错误（HTTP 4xx、`invalid_grant`、授权被撤销等）失败，Anakot 会将该刷新 token 标记为失效并停止重试，避免出现大量重复的认证失败。下一次请求会显示类型化的重新认证提示。运行 `anakot auth add codex-oauth`（或 `anakot model` → OpenAI Codex）开始新的设备码登录；成功交换后隔离状态自动解除。
:::

:::warning
即使使用 callmemo Portal、Codex 或自定义端点，某些工具（视觉、网页摘要、MoA）仍会使用单独的"辅助"模型。默认情况下（`auxiliary.*.provider: "auto"`），Anakot 将这些任务路由到你的**主聊天模型**——即你在 `anakot model` 中选择的同一模型。你可以单独覆盖每个任务，将其路由到更便宜/更快的模型（例如 OpenRouter 上的 Gemini Flash）——参见[辅助模型](/user-guide/configuration#auxiliary-models)。
:::

:::tip callmemo Tool Gateway
付费 callmemo Portal 订阅者还可访问 **[Tool Gateway](/user-guide/features/tool-gateway)**——网页搜索、图像生成、TTS 和浏览器自动化，均通过你的订阅路由。无需额外 API key。全新安装时，`anakot setup --portal` 一条命令即可完成登录、设置 callmemo 为提供商并开启网关。现有用户可通过 `anakot model` 或 `anakot tools` 按工具启用。随时使用 `anakot portal info` 查看路由状态。
:::
...
# → 完成——Anakot 在首次请求时自动开通免费层
```

Anakot 默认使用 Google 的**公开** `gemini-cli` 桌面 OAuth 客户端——与 Google 在其开源 `gemini-cli` 中包含的凭据相同。桌面 OAuth 客户端不是机密客户端（PKCE 提供安全保障）。你无需安装 `gemini-cli` 或注册自己的 GCP OAuth 客户端。

**认证工作原理：**
- 针对 `accounts.google.com` 的 PKCE 授权码流程
- 浏览器回调地址 `http://127.0.0.1:8085/oauth2callback`（端口占用时自动回退到临时端口）
- Token 存储在 `~/.anakot/auth/google_oauth.json`（chmod 0600，原子写入，跨进程 `fcntl` 锁）
- 到期前 60 秒自动刷新
- 无头环境（SSH、`ANAKOT_HEADLESS=1`）→ 粘贴模式回退
...
- 响应转换回 OpenAI 格式，Anakot 其余部分无感知
...
| Workspace / Standard / Enterprise 账户 | 将 `ANAKOT_GEMINI_PROJECT_ID` 或 `GOOGLE_CLOUD_PROJECT` 设置为你的 GCP 项目 ID |
| VPC-SC 保护的组织 | Anakot 检测到 `SECURITY_POLICY_VIOLATED` 后自动强制使用 `standard-tier` |
...
:::warning 政策风险
Google 认为将 Gemini CLI OAuth 客户端用于第三方软件违反政策。部分用户反映账户受到限制。为降低风险，建议改用 `gemini` 提供商并通过 API key 访问。Anakot 会在 OAuth 开始前显示警告并要求明确确认。
:::
...
ANAKOT_GEMINI_CLIENT_SECRET=...   # 桌面客户端可选
```
...
2. **上下文耗尽** — 模型填满了上下文窗口。增加 `model.context_length` 或在 Anakot 中启用[上下文压缩](/user-guide/configuration#context-compression)。
...
7. **callmemo Portal** — 将 callmemo 模型 ID 后缀匹配到 OpenRouter 元数据
...
| 使用场景 | 推荐方案 |
|----------|-------------|
| **只想让它工作** | OpenRouter（默认）或 callmemo Portal |
| **本地模型，简单配置** | Ollama |
...
:::tip
故障转移仅通过 `config.yaml` 配置——或通过 `anakot fallback` 交互式配置。有关触发时机、链推进方式以及与辅助任务和委托的交互，参见[故障转移提供商](/user-guide/features/fallback-providers)。
:::
