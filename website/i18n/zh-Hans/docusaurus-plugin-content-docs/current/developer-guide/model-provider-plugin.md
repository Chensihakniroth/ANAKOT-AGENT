...
| `oauth_external` | 用户在其他地方登录，token 存入 `auth.json` | Anthropic OAuth、MiniMax OAuth、Gemini Cloud Code、Qwen Portal、callmemo Portal |
...
完整的入口点设置请参阅 [构建 Anakot 插件](/guides/build-a-anakot-plugin#distribute-via-pip)。
...
- [构建 Anakot 插件](/guides/build-a-anakot-plugin) — 通用插件编写指南