...
- 构建自定义/本地工具而不修改 Anakot 核心？从 [构建 Anakot 插件](../guides/build-a-anakot-plugin.md) 开始
...
- **Profile 安全路径**：不得硬编码 `~/.anakot` — 代码路径使用 `anakot_constants` 中的 `get_anakot_home()`，面向用户的消息使用 `display_anakot_home()`。完整规则参见 [AGENTS.md](https://github.com/callmemo/anakot-agent/blob/main/AGENTS.md#profiles-multi-instance-support)。
...
- 使用 [GitHub Issues](https://github.com/callmemo/anakot-agent/issues)
...
提交贡献即表示您同意您的贡献将以 [MIT 许可证](https://github.com/callmemo/anakot-agent/blob/main/LICENSE) 授权。