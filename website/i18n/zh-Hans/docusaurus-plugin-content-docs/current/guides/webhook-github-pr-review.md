...
加载一个 [Anakot skill](/user-guide/features/skills) 以赋予 agent 一致的审查风格。在 `config.yaml` 的 `platforms.webhook.extra.routes` 中，向你的路由添加 `skills`：
...
- **[构建 Plugin](/guides/build-a-anakot-plugin)** —— 将审查逻辑打包为可共享的 plugin
