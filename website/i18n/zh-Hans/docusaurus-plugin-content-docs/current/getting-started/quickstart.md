...
anakot postinstall     # 可选：安装 Node.js、浏览器、ripgrep、ffmpeg 并运行 setup
```
...
该命令一次性完成登录、设置 callmemo 为 provider 并开启 Tool Gateway。
:::
...
| Provider | 说明 | 配置方式 |
|----------|-----------|---------------|
| **callmemo Portal** | 订阅制，零配置 | 通过 `anakot model` 进行 OAuth 登录 |
| **OpenAI Codex** | ChatGPT OAuth，使用 Codex 模型 | 通过 `anakot model` 进行设备码认证 |
