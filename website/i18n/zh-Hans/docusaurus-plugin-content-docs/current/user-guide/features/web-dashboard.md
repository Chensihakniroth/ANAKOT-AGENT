...
`web` 扩展会引入 FastAPI/Uvicorn；`pty` 扩展会引入 `ptyprocess`（POSIX）或 `pywinpty`（原生 Windows——注意嵌入式 TUI 本身仍需要 WSL）。`pip install anakot-agent[all]` 包含两个扩展，如果你还需要消息/语音等功能，这是最简便的方式。
...
- `ptyprocess`——由 `pty` 扩展安装（`pip install 'anakot-agent[web,pty]'`，或 `[all]` 同时包含两者）
