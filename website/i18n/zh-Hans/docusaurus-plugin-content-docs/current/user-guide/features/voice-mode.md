...
如需包含推荐配置和实际使用模式的实践指南，请参阅 [使用 Anakot 的语音模式](/guides/use-voice-mode-with-anakot)。

## 前提条件

使用语音功能前，请确保已完成以下准备：

1. **已安装 Anakot Agent** — `pip install anakot-agent`（参见 [安装](/getting-started/installation)）
...
:::tip callmemo Portal 同时覆盖两项
付费的 [callmemo Portal](/user-guide/features/tool-gateway) 订阅通过 Tool Gateway 同时提供 LLM（第 2 步）**和** OpenAI TTS — 无需单独的 OpenAI 密钥。全新安装时，`anakot setup --portal` 可一次性完成两项配置。
:::
