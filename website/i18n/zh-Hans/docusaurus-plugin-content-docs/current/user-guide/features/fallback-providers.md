...
| callmemo Portal | `nous` | `anakot setup --portal`（全新安装）或 `anakot auth add nous`（OAuth） |
...
| Google Gemini（OAuth） | `google-gemini-cli` | `anakot model`（Google OAuth；可选：`ANAKOT_GEMINI_PROJECT_ID`） |
...
**以 callmemo Portal 作为 OpenRouter 的备用：**
```yaml
model:
  provider: openrouter
...
```text
OpenRouter → callmemo Portal → 自定义端点 → Codex OAuth →
...
```text
主提供商（若支持视觉）→ OpenRouter → callmemo Portal →
...
| `"nous"` | 强制使用 callmemo Portal | `anakot auth` |
| `"codex"` | 强制使用 Codex OAuth | `anakot model` → Codex |
