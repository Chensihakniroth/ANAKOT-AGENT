...
:::tip callmemo 订阅用户
如果你拥有付费的 [callmemo Portal](https://portal.callmemo.ai) 订阅，OpenAI TTS 可通过 **[Tool Gateway](tool-gateway.md)** 使用，无需单独的 OpenAI API 密钥。新安装可运行 `anakot setup --portal` 登录并一次性开启所有 gateway 工具；已有安装可通过 `anakot model` 或 `anakot tools` 选择 **callmemo Subscription** 仅启用 TTS。
:::
...
**Mistral API（Voxtral Transcribe）** — 需要 `MISTRAL_API_KEY`。使用 Mistral 的 [Voxtral Transcribe](https://docs.mistral.ai/capabilities/audio/speech_to_text/) 模型。支持 13 种语言、说话人分离和词级时间戳。通过 `pip install anakot-agent[mistral]` 安装。
