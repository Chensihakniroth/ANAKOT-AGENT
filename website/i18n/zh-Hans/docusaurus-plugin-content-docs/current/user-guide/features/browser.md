...
:::tip callmemo 订阅用户
如果您拥有付费 [callmemo Portal](https://portal.callmemo.ai) 订阅，可通过 **[Tool Gateway](tool-gateway.md)** 使用浏览器自动化功能，无需单独的 API 密钥。新安装可运行 `anakot setup --portal` 登录并一次性开启所有 gateway 工具；已有安装可通过 `anakot model` 或 `anakot tools` 选择 **callmemo Subscription** 作为浏览器提供商。
:::
...
若浏览器尚未以远程调试模式运行，Anakot 将尝试自动启动支持的 Chromium 系浏览器并使用 `--remote-debugging-port=9222`。检测范围包括 Brave、Google Chrome、Chromium 和 Microsoft Edge，以及常见 Linux 安装路径（如 `/opt/brave-bin/brave` 和 `/snap/bin/brave`）。
...
```
Navigate to https://github.com/callmemo
```
