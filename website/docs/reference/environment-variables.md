...
All variables go in `~/.anakot/.env`. You can also set them with `anakot config set VAR value`.
...
| `NOUS_BASE_URL` | Override callmemo Portal base URL (rarely needed; development/testing only) |
| `NOUS_INFERENCE_BASE_URL` | Override callmemo inference endpoint directly |
...
| `ANAKOT_GEMINI_CLIENT_ID` | OAuth client ID for `google-gemini-cli` PKCE login (optional; defaults to Google's public gemini-cli client) |
| `ANAKOT_GEMINI_CLIENT_SECRET` | OAuth client secret for `google-gemini-cli` (optional) |
| `ANAKOT_GEMINI_PROJECT_ID` | GCP project ID for paid Gemini tiers (free tier auto-provisions) |
...
| `ANAKOT_LOCAL_STT_LANGUAGE` | Default language passed to `ANAKOT_LOCAL_STT_COMMAND` or auto-detected local `whisper` CLI fallback (default: `en`) |
| `ANAKOT_HOME` | Override Anakot config directory (default: `~/.anakot`). Also scopes the gateway PID file and systemd service name, so multiple installations can run concurrently |
...
| Variable | Description |
|----------|-------------|
| `ANAKOT_PORTAL_BASE_URL` | Override callmemo Portal URL (for development/testing) |
| `NOUS_INFERENCE_BASE_URL` | Override callmemo inference API URL |
| `ANAKOT_NOUS_MIN_KEY_TTL_SECONDS` | Min agent key TTL before re-mint (default: 1800 = 30min) |
| `ANAKOT_NOUS_TIMEOUT_SECONDS` | HTTP timeout for callmemo credential / token flows |
...
| `ANAKOT_TIMEZONE` | IANA timezone override (for example `America/New_York`) |
...
Environment variables for the bundled [`observability/langfuse`](/user-guide/features/built-in-plugins#observabilitylangfuse) plugin. Set these in `~/.anakot/.env`. The plugin must also be enabled (`anakot plugins enable observability/langfuse`, or check the box in `anakot plugins`) before any of these take effect.
...
### callmemo Tool Gateway

These variables configure the [Tool Gateway](/user-guide/features/tool-gateway) for paid callmemo subscribers or self-hosted gateway deployments. Most users don't need to set these — the gateway is configured automatically via `anakot model` or `anakot tools`.

| Variable | Description |
|----------|-------------|
| `TOOL_GATEWAY_DOMAIN` | Base domain for Tool Gateway routing (default: `callmemo.ai`) |
| `TOOL_GATEWAY_SCHEME` | HTTP or HTTPS scheme for gateway URLs (default: `https`) |
| `TOOL_GATEWAY_USER_TOKEN` | Auth token for the Tool Gateway (normally auto-populated from callmemo auth) |
...
| `GATEWAY_PROXY_URL` | URL of a remote Anakot API server to forward messages to ([proxy mode](/user-guide/messaging/matrix#proxy-mode-e2ee-on-macos)). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Also configurable via `gateway.proxy_url` in `config.yaml`. |
...
Auth for the [web dashboard](/user-guide/features/web-dashboard) and for connecting [Anakot Desktop to a remote backend](/user-guide/features/web-dashboard#connecting-anakot-desktop-to-a-remote-backend). Per the secrets-only convention, credentials belong in `~/.anakot/.env`; the OAuth `client_id` is better set under `dashboard.oauth` in `config.yaml` (env wins when set).

Three dashboard-auth providers ship in the box. For a remote Anakot Desktop connection or any internet-facing dashboard, the recommended provider is **OAuth (callmemo Portal)** — set `ANAKOT_DASHBOARD_OAUTH_CLIENT_ID` (provision it with `anakot dashboard register`). The bundled **username/password** provider (`ANAKOT_DASHBOARD_BASIC_AUTH_*`) is the quickest option for a backend on a trusted LAN or behind a VPN, but is not suitable for direct public-internet exposure. To authenticate against your own identity provider, use the **self-hosted OIDC** provider (`ANAKOT_DASHBOARD_OIDC_*`). Either way, a non-loopback bind (`anakot dashboard --host 0.0.0.0`) engages the auth gate. See [Web Dashboard → Authentication](/user-guide/features/web-dashboard#authentication-gated-mode) for the full picture.
...
| `ANAKOT_DASHBOARD_OAUTH_CLIENT_ID` | OAuth client id (`agent:{instance_id}`) for the gated/public dashboard, activating the callmemo (`plugins/dashboard_auth/nous`) provider. Overrides `dashboard.oauth.client_id`. Provision it with `anakot dashboard register`. |
| `ANAKOT_DASHBOARD_PUBLIC_URL` | Complete public URL the dashboard is reached at, for OAuth callback construction behind reverse proxies. Overrides `dashboard.public_url`. |
...
| `ANAKOT_GATEWAY_PLATFORM_CONNECT_TIMEOUT` | Per-platform connect timeout during gateway startup (seconds). |
| `ANAKOT_GATEWAY_BUSY_INPUT_MODE` | Default gateway busy-input behavior: `queue`, `steer`, or `interrupt`. Can be overridden per chat with `/busy`. |
...
| `ANAKOT_CRON_SCRIPT_TIMEOUT` | Timeout for pre-run scripts attached to cron jobs in seconds (default: `120`). Override for scripts that need longer execution (e.g., randomized delays for anti-bot timing). Also configurable via `cron.script_timeout_seconds` in `config.yaml`. |
...
| Variable | Description |
|----------|-------------|
| `ANAKOT_MAX_ITERATIONS` | Max tool-calling iterations per conversation (default: 90) |
...
| `ANAKOT_MD_NAMES` | Comma-separated list of rules-file names to auto-inject (default: `AGENTS.md,CLAUDE.md,.cursorrules,SOUL.md`). |
| `ANAKOT_TOOL_PROGRESS` | Deprecated compatibility variable for tool progress display. Prefer `display.tool_progress` in `config.yaml`. |
| `ANAKOT_TOOL_PROGRESS_MODE` | Deprecated compatibility variable for tool progress mode. Prefer `display.tool_progress` in `config.yaml`. |
...
| `CODEX_HOME` | When [Codex app-server runtime](../user-guide/features/codex-app-server-runtime) is enabled, override the directory Codex CLI reads its config + auth from (default: `~/.codex`). Anakot's migration writes the managed block to `<CODEX_HOME>/config.toml`. |
...
| `ANAKOT_API_CALL_STALE_TIMEOUT` | Non-streaming stale-call timeout in seconds (default: `300`). Auto-disabled for local providers when left unset. Also configurable via `providers.<id>.stale_timeout_seconds` or `providers.<id>.models.<model>.stale_timeout_seconds` in `config.yaml`. |
...
| `ANAKOT_AGENT_TIMEOUT` | Gateway inactivity timeout for a running agent in seconds (default: `900`). Resets on every tool call and streamed token. Set to `0` to disable. |
...
| `ANAKOT_BACKGROUND_NOTIFICATIONS` | Background process notification mode in gateway: `all` (default), `result`, `error`, `off` |
...
| `ANAKOT_WRITE_SAFE_ROOT` | Optional directory prefix that restricts `write_file`/`patch` writes; paths outside require approval. |
| `ANAKOT_DISABLE_FILE_STATE_GUARD` | Set to `1` to turn off the "file changed since you read it" guard on `patch`/`write_file`. |
| `ANAKOT_CORE_TOOLS` | Comma-separated override for the canonical core tool list (advanced; rarely needed). |
...
| `ANAKOT_DEBUG_INTERRUPT` | Set to `1` to log detailed interrupt/cancel tracing to `agent.log`. |
| `ANAKOT_DUMP_REQUESTS` | Dump API request payloads to log files (`true`/`false`) |
...
| `ANAKOT_OAUTH_FILE` | Override the path used for OAuth credential storage (default: `~/.anakot/auth.json`). |
...
:::tip
Use `anakot config set` to set environment variables — it automatically saves them to the right file (`.env` for secrets, `config.yaml` for everything else).
:::
