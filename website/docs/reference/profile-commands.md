...
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `anakot update` runs won't re-seed the bundled set, and refuses to combine with `--clone` / `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.anakot`), use `anakot skills opt-out` / `anakot skills opt-in`. |
...
anakot profile create work2 --clone --clone-from work
```
...
:::info
`anakot profile export` / `import` are still the right commands for
