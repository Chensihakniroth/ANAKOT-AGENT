...
Behavior:
- Anakot uses the MCP SDK's OAuth 2.1 PKCE flow (metadata discovery, dynamic client registration, token exchange, and refresh)
- On first connect, a browser window opens for authorization
- Tokens are persisted to `~/.anakot/mcp-tokens/<server>.json` and reused across sessions
