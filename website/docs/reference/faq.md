...
pip install "anakot-agent[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `anakot profile create newname --clone-all` to copy everything from the current profile.
...
There is no hard limit. Each profile is just a directory under `~/.anakot/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
