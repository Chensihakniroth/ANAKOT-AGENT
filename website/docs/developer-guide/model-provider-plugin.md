...
| `oauth_external` | User signs in elsewhere, tokens land in `auth.json` | Anthropic OAuth, MiniMax OAuth, Gemini Cloud Code, Qwen Portal, callmemo Portal |
...
`auth_type` gates which codepaths treat your provider as a "simple api-key provider" — if it's not `api_key`, the PluginManager still records the manifest but Anakot's CLI-level automation (doctor checks, `--provider` flag, setup wizard delegation) may skip over it.
...
Like any Anakot plugin, model providers can ship as a pip package. Add an entry point to your `pyproject.toml`:

```toml
[project.entry-points."anakot.plugins"]
acme-inference = "acme_anakot_plugin:register"
```

…where `acme_anakot_plugin:register` is a function that calls `register_provider(profile)`. The general PluginManager picks up entry-point plugins during `discover_and_load()`. For `kind: model-provider` pip plugins, you still need to declare the kind in your manifest (or rely on the source-text heuristic).

See [Building a Anakot Plugin](/guides/build-a-anakot-plugin#distribute-via-pip) for the full entry-points setup.
...
- [Building a Anakot Plugin](/guides/build-a-anakot-plugin) — general plugin authoring
