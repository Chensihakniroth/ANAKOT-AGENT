...
- OAuth or token refresh (callmemo Portal, Codex, Google Gemini, Qwen Portal, Copilot)
...
- `_PROVIDER_LABELS` in `anakot_cli/models.py`
- `_PROVIDER_ALIASES` in both `anakot_cli/auth.py` and `anakot_cli/models.py`
...
- OAuth / auth-store path: callmemo, OpenAI Codex
...
python -m pytest tests/anakot_cli/test_runtime_provider_resolution.py tests/cli/test_cli_provider_resolution.py tests/anakot_cli/test_setup_model_provider.py tests/run_agent/test_provider_parity.py -n0 -q
```
