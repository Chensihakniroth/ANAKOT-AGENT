...
The ACP adapter wraps Anakot's synchronous `AIAgent` in an async JSON-RPC stdio server.
...
The Zed ACP Registry path launches the same adapter through `uvx --from 'anakot-agent[acp]==<version>' anakot-acp`, pointed at the `anakot-agent` PyPI release.
...
Instead it reuses Anakot's runtime resolver:

- `acp_adapter/auth.py`
- `anakot_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Anakot provider/credentials. It also always advertises a terminal setup auth method (`anakot-setup`, args `--setup`) so first-run registry clients can open Anakot's interactive model/provider configuration before starting a normal ACP session.
