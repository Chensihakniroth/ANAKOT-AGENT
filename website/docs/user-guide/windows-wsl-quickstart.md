...
- Using the **OpenAI-compatible API server** (exposed by `anakot gateway` when `API_SERVER_ENABLED=true`) from a Windows-side tool. See the [API Server feature page](/user-guide/features/api-server).
...
The Anakot [Tool Gateway](/user-guide/features/tool-gateway) and the API server are long-lived processes. In WSL2 you have a few options for keeping them up.
...
- **[MCP guide → WSL → Windows Chrome](/guides/use-mcp-with-anakot#wsl2-bridge-anakot-in-wsl-to-windows-chrome)** — controlling your signed-in Windows Chrome from Anakot in WSL.
