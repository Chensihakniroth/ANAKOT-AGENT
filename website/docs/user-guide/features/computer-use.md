...
Anakot Agent can drive your Mac's desktop — clicking, typing, scrolling,
...
Use `anakot computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `anakot tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
window-focus bug for UTM workflows). Anakot refreshes the binary in two
places so you don't get stuck on a stale release:

- **`anakot update`** — when you update Anakot itself, if `cua-driver` is
...
- **`anakot computer-use install --upgrade`** — manual force-refresh.
...
`anakot computer-use status` shows the installed version next to the
binary path.
...
Pair with `approvals.mode: manual` in `~/.anakot/config.yaml` if you want every action confirmed.
...
- **Private SPI risk.** Apple can change SkyLight's symbol surface in any
  OS update. Pin the driver version with the `ANAKOT_CUA_DRIVER_VERSION`
...
ANAKOT_CUA_DRIVER_VERSION=0.5.0    # optional pin
```
...
`anakot computer-use install` to fetch the cua-driver binary, or run
`anakot tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/Chensihakniroth/ANAKOT-AGENT/blob/main/skills/apple/macos-computer-use/SKILL.md)
