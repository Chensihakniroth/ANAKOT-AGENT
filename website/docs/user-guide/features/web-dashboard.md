...
:::tip
Hosted-mode auth uses callmemo Portal OAuth; if you also want the dashboard to talk to a real backend, `anakot setup --portal` wires up the model and tool gateway too. See [callmemo Portal](/integrations/callmemo-portal).
:::
...
```bash
pip install 'anakot-agent[web,pty]'
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `pip install anakot-agent[all]` includes both extras and is the easiest path if you also want messaging/voice/etc.
...
- `ptyprocess` — installed by the `pty` extra (`pip install 'anakot-agent[web,pty]'`, or `[all]` covers both)
...
**Catalog:** browse the callmemo-approved MCP servers (the bundled `optional-mcps/`
...
- **Restart gateway** — credentials are written to `~/.anakot/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page.
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and Anakot versions, CPU core count + utilization, memory, disk usage of the Anakot home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The Anakot version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git or pip install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `anakot update` in the background. On Docker/Nix/Homebrew installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
- **callmemo Portal** — login status, the active inference provider, and the Tool Gateway routing table (which tools run via the Portal vs. locally), with a link to manage your subscription. Read-only mirror of `anakot portal`.
...
![System admin page — host stats and callmemo Portal status](/img/dashboard/admin-system-top.png)
...
| `GET /api/mcp/catalog` | Browse the callmemo-approved MCP catalog |
...
| `GET /api/anakot/update/check` | Report update availability (commits behind, install method) without applying. `?force=1` busts the 6h cache |
| `GET /api/curator` · `PUT .../paused` · `POST .../run` | Skill-curator status + pause/resume + run |
| `GET /api/portal` | callmemo Portal auth + Tool Gateway routing (read-only) |
...
- **[OAuth (callmemo Portal)](#default-provider-nous-research)** — for hosted deployments and any dashboard reachable over the public internet, and the recommended path for a [remote Anakot Desktop connection](#connecting-anakot-desktop-to-a-remote-backend). Every login is verified against your callmemo account, so this is the provider suitable for internet-facing use.
- **[Self-hosted OIDC](#self-hosted-oidc-provider)** — for bringing your own identity provider via standard OpenID Connect (Keycloak, Auth0, Okta, Google, GitHub via an OIDC bridge, etc.). No callmemo Portal involved; suitable for public-internet exposure when fronted by a conformant OIDC server.
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no callmemo plugin, no custom plugin), `anakot dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.

### Default provider: callmemo
...
Because every login is verified against callmemo Portal and protected by your callmemo account, **the callmemo provider is the one suitable for exposing a dashboard to the public internet.**

#### Registering a dashboard

To use the callmemo provider you need an OAuth client ID (shape `agent:{id}`). There are two ways to get one:

- **CLI — `anakot dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing callmemo login (run `anakot setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `ANAKOT_DASHBOARD_OAUTH_CLIENT_ID` into `~/.anakot/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).
...
Bundled providers reported these issues:
  • nous: ANAKOT_DASHBOARD_OAUTH_CLIENT_ID is not set (and
    dashboard.oauth.client_id in config.yaml is empty). The callmemo Portal
    provisions this env var (shape 'agent:{instance_id}') when it
    deploys a Anakot Agent instance — set it to your provisioned
...
#### Worked example: callmemo

From a logged-in Anakot install to a callmemo-gated dashboard in three steps.

**1. Log in and register the dashboard.** `anakot dashboard register` uses your existing callmemo login to provision an OAuth client and writes `ANAKOT_DASHBOARD_OAUTH_CLIENT_ID` into `~/.anakot/.env` for you:

```bash
anakot setup            # if you're not already logged into callmemo Portal
anakot dashboard register
# ✓ Registered dashboard "swift_falcon"
# …writes ANAKOT_DASHBOARD_OAUTH_CLIENT_ID to ~/.anakot/.env
```
...
**3. Log in.** Open `http://<host>:9119/`, you'll be bounced to `/login`. Click **Sign in with callmemo** → authenticate at the Portal → land back on the authenticated dashboard. Verify the gate from any machine:
...
`GET /api/auth/me` then returns the verified session (`provider: basic`). Keep this behind a VPN — see the warning above; for a public host use the [callmemo](#default-provider-nous-research) or [self-hosted OIDC](#self-hosted-oidc-provider) provider instead.
...
If you run your own identity provider, the bundled `plugins/dashboard_auth/self_hosted` plugin authenticates the dashboard against it using **standard OpenID Connect** — no per-IDP code, no callmemo Portal involved. It's verified against and works with any conformant OIDC server:

> **Authentik · Keycloak · Zitadel · Authelia · Auth0 · Okta · Google · …**

Like the callmemo provider, it auto-loads and only registers itself once it's configured, so it's a no-op for loopback / `--insecure` dashboards.
...
The provider implements the [callmemo Portal OAuth contract v1](https://github.com/callmemo/nous-account-service/blob/main/docs/agent-dashboard-oauth-contract.md) — authorization-code grant with PKCE (S256):

1. User hits `/` without a session cookie → gate redirects to `/login`.
2. Login page shows a "Continue with callmemo" button → `/auth/login?provider=nous`.
3. Server stashes PKCE state in a short-lived cookie, redirects user to `https://portal.callmemo.ai/oauth/authorize?…`.
...
| `anakot_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip |
...
To plug a non-callmemo OAuth provider (e.g. Google, GitHub, custom OIDC), create a plugin that registers a `DashboardAuthProvider`:
...
Anakot Desktop can drive a Anakot backend running on another machine (a VPS, a home server, a Mini behind Tailscale). In the app this lives under **Settings → Gateway → Remote gateway**, which asks for a **Remote URL** and a way to **Sign in**. (For the desktop app itself — install, settings, chat — see the [Anakot Desktop](/user-guide/desktop) page.)

You protect the remote dashboard with one of the bundled auth providers, and the desktop app signs in against whichever one the backend advertises. For a backend reachable beyond your own machine — a VPS, a public host, anything internet-facing — the recommended provider is **OAuth (callmemo Portal)** (register it with [`anakot dashboard register`](#registering-a-dashboard) and sign in with *Sign in with callmemo*). The bundled [username/password provider](#usernamepassword-provider-no-oauth-idp) is the quickest option when the backend is on a trusted LAN or reachable only over a VPN, but is **not suitable for direct public-internet exposure**. Binding the dashboard to a non-loopback address engages its auth gate; once signed in, Desktop reuses the session for the chat WebSocket automatically — there is no token to copy or paste.

The recipe below uses the username/password path because it's the quickest to stand up on a trusted network; for the OAuth path see [Default provider: callmemo](#default-provider-nous-research).
