...
If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Anakot](/guides/use-voice-mode-with-anakot).

## Prerequisites

Before using voice features, make sure you have:

1. **Anakot Agent installed** — `pip install anakot-agent` (see [Installation](/getting-started/installation))
...
:::tip callmemo Portal covers both
A paid [callmemo Portal](/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `anakot setup --portal` wires both up at once.
:::
...
pip install "anakot-agent[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
pip install "anakot-agent[messaging]"

# Premium TTS (ElevenLabs)
pip install "anakot-agent[tts-premium]"
...
If you are running Anakot inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
