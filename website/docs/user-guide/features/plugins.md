...
[Adding Tools](/developer-guide/adding-tools) page is for built-in Anakot
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Anakot Plugin](/guides/build-a-anakot-plugin)** — step-by-step guide with a complete working example.
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Anakot — see [Built-in Plugins](/user-guide/features/built-in-plugins) |
...
| Nix | `services.anakot-agent.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/getting-started/nix-setup#plugins) |
...
User plugins at `~/.anakot/plugins/model-providers/<name>/` and `~/.anakot/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.
...
Several categories of plugin bypass `plugins.enabled` — they're part of Anakot's built-in surface and would break basic functionality if gated off by default:
...
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Anakot Plugin](/guides/build-a-anakot-plugin) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Anakot Plugin](/guides/build-a-anakot-plugin) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Anakot Plugin](/guides/build-a-anakot-plugin) · [Extending the CLI](/developer-guide/extending-the-cli) |
| A **subcommand** for `anakot <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/developer-guide/extending-the-cli) |
...
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Anakot auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `anakot skills tap add <repo>` | [Skills Hub](/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.anakot/hooks/<name>/` | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `anakot plugins install` needed. See the **[Nix Setup guide](/getting-started/nix-setup#plugins)** for full details.
...
anakot plugins update my-plugin              # pull latest
...
See the **[full guide](/guides/build-a-anakot-plugin)** for handler contracts, schema format, hook behavior, error handling, and common mistakes.
