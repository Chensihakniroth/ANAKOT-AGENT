...
The curator **never touches** bundled skills (shipped with the repo) or hub-installed skills (from [agentskills.io](https://agentskills.io)). It only reviews skills the agent itself authored. It also **never auto-deletes** — the worst outcome is archival into `~/.anakot/skills/.archive/`, which is recoverable.

Tracks [issue #7816](https://github.com/Chensihakniroth/ANAKOT-AGENT/issues/7816).

## How it runs

The curator is triggered by an inactivity check, not a cron daemon. On CLI session start, and on a recurring tick inside the gateway's cron-ticker thread, Anakot checks whether:
...
1. **Automatic transitions** (deterministic, no LLM). Skills unused for `stale_after_days` (30) become `stale`; skills unused for `archive_after_days` (90) are moved to `~/.anakot/skills/.archive/`.
...
Before every real curator pass, Anakot takes a tar.gz snapshot of `~/.anakot/skills/` at `~/.anakot/skills/.curator_backups/<utc-iso>/skills.tar.gz`. If a pass archives or consolidates something you didn't want touched, you can undo the whole run with one command:
...
Only **agent-created** skills can be pinned — bundled and hub-installed skills are never subject to curator mutation in the first place, and `anakot curator pin` will refuse with an explanatory message if you try.
...
- [Issue #7816](https://github.com/Chensihakniroth/ANAKOT-AGENT/issues/7816) — original proposal and design discussion
