...
| callmemo Portal | `nous` | `anakot setup --portal` (fresh) or `anakot auth add nous` (OAuth) |
| OpenAI Codex | `openai-codex` | `anakot model` (ChatGPT OAuth) |
...
| Google Gemini (OAuth) | `google-gemini-cli` | `anakot model` (Google OAuth; optional: `ANAKOT_GEMINI_PROJECT_ID`) |
...
**callmemo Portal as fallback for OpenRouter:**
```yaml
model:
  provider: openrouter
...
When a task's provider is set to `"auto"` (the default), Anakot tries providers in order until one works:

**For text tasks (compression, web extract, etc.):**

```text
OpenRouter → callmemo Portal → Custom endpoint → Codex OAuth →
...
```text
Main provider (if vision-capable) → OpenRouter → callmemo Portal →
Codex OAuth → Anthropic → Custom endpoint → give up
```

If the resolved provider fails at call time, Anakot also has an internal retry: if the provider is not OpenRouter and no explicit `base_url` is set, it tries OpenRouter as a last-resort fallback.
...
| `"codex"` | Force Codex OAuth | `anakot model` → Codex |
