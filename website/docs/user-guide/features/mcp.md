...
## Catalog: one-click install for callmemo-approved MCPs

Anakot ships a curated catalog of MCP servers that callmemo staff has reviewed
...
anakot mcp install n8n    # install a catalog entry by name
```

The picker shows each entry with its current status:

```
n8n          available              Manage and inspect n8n workflows from Anakot
...
callmemo approval. There is no community submission tier; entries are added by
merging a PR.
...
the anakot-agent repo, so callmemo has reviewed each entry before it shipped —
...
Manifests live at
[`optional-mcps/<name>/manifest.yaml`](https://github.com/Chensihakniroth/ANAKOT-AGENT/tree/main/optional-mcps)
...
[`optional-mcps/`](https://github.com/Chensihakniroth/ANAKOT-AGENT/tree/main/optional-mcps).
...
Most hosted MCP servers (Linear, Sentry, Atlassian, Asana, Figma, Stripe, …) require OAuth 2.1 instead of a static bearer token. Set `auth: oauth` and Anakot handles discovery, dynamic client registration, PKCE, token exchange, refresh, and step-up auth via the MCP Python SDK.
...
**Remote / headless hosts.** When Anakot runs on a different machine than your browser, the loopback callback can't reach your laptop. Two ways to complete the flow:
...
The MCP server reads conversation data directly from Anakot's session store (`~/.anakot/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same `send_message` infrastructure as the Anakot agent itself.
...
- [Use MCP with Anakot](/guides/use-mcp-with-anakot)
