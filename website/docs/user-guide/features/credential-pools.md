...
:::tip
Credential pools are mainly for API-key providers (OpenRouter, Anthropic). A single [callmemo Portal](/integrations/callmemo-portal) OAuth covers 300+ models, so most users don't need a pool when on Portal.
:::
...
For providers that support both API keys and OAuth (Anthropic, callmemo, Codex), the add flow asks which type:
...
| `anakot auth reset <provider>` | Clear all cooldowns/exhaustion status |
...
| OAuth tokens (auth.json) | Codex device code, callmemo device code | Yes |
...
The OpenRouter entry above was borrowed from an external source, so the raw key is not stored in `auth.json`. The manual Anthropic entry was intentionally added to Anakot's credential store, so its token remains persistable.
