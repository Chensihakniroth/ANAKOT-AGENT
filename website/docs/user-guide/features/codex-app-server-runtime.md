...
:::tip
Not using OpenAI Codex? `anakot setup --portal` configures a non-Codex backend with Claude/Gemini/etc. in one step. See [callmemo Portal](/integrations/callmemo-portal).
:::
...
- **Anakot's richer tools come along** — web_search, web_extract, browser automation, vision, image generation, skills, and TTS work via an MCP callback. Codex calls back into Anakot for tools it doesn't have built in.
- **Memory and skill nudges keep working** — Codex's events are projected into Anakot's message shape so the self-improvement loop sees a normal-looking transcript.
...
- **`update_plan`** — codex's internal todo / plan tracker. Equivalent of Anakot's `todo` tool, but managed entirely inside codex's runtime.
...
- **`image_generate`** — image generation through Anakot's image_gen plugin chain.
- **`skill_view` / `skills_list`** — read from Anakot's skill library.
- **`text_to_speech`** — TTS through Anakot's configured provider.

When the model wants one of these, codex spawns the `anakot_tools_mcp_server` subprocess via stdio MCP, the call is dispatched through `model_tools.handle_function_call()` (same code path as Anakot's default runtime), and the result is returned to codex like any other MCP response.
...
- **`memory`** — Anakot's persistent memory store
- **`session_search`** — cross-session search
- **`todo`** — Anakot's todo store (codex's `update_plan` is the in-runtime equivalent)
...
2. **Codex OAuth login.** The codex subprocess reads `~/.codex/auth.json`. Two ways to populate it:
   ...
   ```
   Anakot's own `anakot auth login codex` writes to `~/.anakot/auth.json` — that's a separate session. **Run `codex login` separately** if you haven't.
...
- **Registers Anakot's own tools as an MCP server** so the codex subprocess can call back for tools codex doesn't ship with.
...
Anakot's background self-improvement fires on counter thresholds:
...
- `:workspace` — writes within the current workspace allowed without prompts (Anakot's default when you enable the runtime)
- `:danger-no-sandbox` — no sandbox at all (don't use this unless you understand it)

You can override the default in `~/.codex/config.toml` outside Anakot's managed block:
...
When this runtime is on with the `openai-codex` provider, **auxiliary tasks (title generation, context compression, vision auto-detect, the background self-improvement review fork) also flow through your ChatGPT subscription by default**, because Anakot's auxiliary client uses the main provider/model when no per-task override is set.
...
The self-improvement review fork inherits the main runtime via `_current_main_runtime()` and Anakot downgrades it from `codex_app_server` to `codex_responses` automatically (so the fork can actually call `memory` and `skill_manage` — Anakot's own agent-loop tools). That fork still uses your subscription auth unless you've routed aux tasks elsewhere.
...
Anakot's `mcp_servers` config is auto-translated to the TOML format Codex expects. The migration runs every time you enable the runtime and is idempotent — re-runs replace the managed section but preserve any user-edited Codex config.
...
This means: when your friend says "I have Calendar and GitHub set up in my Codex CLI" and they enable Anakot's codex runtime, Anakot activates those automatically. No re-configuration needed.
...
- **Anakot auth and codex auth are separate sessions.** You need both `codex login` AND `anakot auth login codex` for the cleanest UX (the runtime uses codex's session for the LLM call). This is a deliberate design choice in Anakot's `_import_codex_cli_tokens` — Anakot won't share OAuth state with codex CLI to avoid clobbering each other on token refresh.
...
If you find a bug, [open an issue](https://github.com/Chensihakniroth/ANAKOT-AGENT/issues) with the output of `anakot logs --since 5m`. Mention `codex-runtime` in the title so it's easy to triage.
...
For implementation details, see [PR #24182](https://github.com/Chensihakniroth/ANAKOT-AGENT/pull/24182) and the [Codex app-server protocol README](https://github.com/openai/codex/blob/main/codex-rs/app-server/README.md).
