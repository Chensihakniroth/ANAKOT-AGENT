...
description: "Use your callmemo Portal subscription (or other OAuth provider) as an OpenAI-compatible endpoint for external apps"
---
...
This opens your browser for the callmemo Portal OAuth flow. Anakot stores
...
Currently shipped: `nous` (callmemo Portal) and `xai` (xAI / Grok). More
...
The proxy only forwards paths the upstream actually serves. For callmemo
Portal:
...
supports; check `portal.callmemo.ai/models`.
...
[portal.callmemo.ai](https://portal.callmemo.ai).
