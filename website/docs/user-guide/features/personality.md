...
- "Edit `~/.anakot/SOUL.md` to change Anakot's default personality."
...
If the file is empty, whitespace-only, or cannot be read, Anakot falls back to a built-in default identity ("You are Anakot Agent, an intelligent AI assistant created by callmemo..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
You can also define named custom personalities in `~/.anakot/config.yaml` under `agent.personalities`.
...
- [SOUL.md Guide](/guides/use-soul-with-anakot)
