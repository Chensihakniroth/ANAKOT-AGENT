...
12. **execute_code sandbox has no env vars** — When Anakot runs auto_jailbreak via execute_code, the sandbox doesn't inherit `~/.anakot/.env`. Load dotenv explicitly: `from dotenv import load_dotenv; load_dotenv(os.path.expanduser("~/.anakot/.env"))`
