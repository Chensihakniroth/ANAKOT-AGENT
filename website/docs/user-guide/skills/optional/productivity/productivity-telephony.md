---
title: "Telephony — Give Anakot phone capabilities without core tool changes"
sidebar_label: "Telephony"
description: "Give Anakot phone capabilities without core tool changes"
---
...
Give Anakot phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.
...
- save provider credentials into `~/.anakot/.env`
...
3. Buy it and save it into `~/.anakot/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
3. persist that number to `~/.anakot/.env`
