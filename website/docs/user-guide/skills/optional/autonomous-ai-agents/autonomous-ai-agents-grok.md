...
| Related skills | [`codex`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-codex), [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`anakot-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-anakot-agent) |
...
2. **Don't conflate Anakot's xAI auth with the `grok` CLI's auth.** Anakot's
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Anakot's xAI auth carries
   over.
