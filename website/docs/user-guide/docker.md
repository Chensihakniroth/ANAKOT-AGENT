...
:::tip
Inside the container, run `anakot setup --portal` once — the refresh token persists in the mounted `~/.anakot` volume. See [callmemo Portal](/integrations/callmemo-portal).
:::
...
| `ANAKOT_DASHBOARD_INSECURE` | Set to `1` (or `true` / `yes`) to bind without the OAuth auth gate. Only use on trusted networks behind a reverse proxy without the OAuth contract — the dashboard exposes API keys and session data | *(unset — gate enforced when a `DashboardAuthProvider` is registered)* |
...
- **OAuth (callmemo Portal)** — for hosted/public deploys: the `dashboard_auth/nous` provider activates whenever `ANAKOT_DASHBOARD_OAUTH_CLIENT_ID` is set.
...
- Python 3 with all Anakot dependencies (`uv pip install -e ".[all]"`)
...
- `docker restart` preserves running gateways: the cont-init reconciler reads `$ANAKOT_HOME/profiles/<name>/gateway_state.json` and brings the slot back up if the last recorded state was `running`. Stopped gateways stay stopped.
