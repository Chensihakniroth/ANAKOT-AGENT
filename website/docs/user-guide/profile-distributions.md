...
- **You just want to back up a profile on your own machine.** Use [`anakot profile export` / `import`](../reference/profile-commands.md#anakot-profile-export) — that's what those are for.
...
5. Copies distribution-owned files into `~/.anakot/profiles/research-bot/` (or wherever the manifest's `name` resolves).
...
- [`anakot profile export` / `import`](../reference/profile-commands.md#anakot-profile-export) — local backup / restore (not distribution)
