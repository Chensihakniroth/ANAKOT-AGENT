...
(`ai.anakot.gateway-<name>.plist`) or systemd user service
(`anakot-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
```bash
anakot logs --tail              # follow default profile
anakot -p coder logs --tail     # follow one profile
