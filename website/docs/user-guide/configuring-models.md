...
[callmemo Portal](/user-guide/features/tool-gateway) provides 300+ models under one subscription. On a fresh install, run `anakot setup --portal` to log in and set callmemo as your provider in one command. Inspect what's wired up with `anakot portal info`.
...
Every auxiliary task defaults to `auto` — meaning Anakot uses your main model for that job too. Override a specific task when you want a cheaper or faster model for a side-job.
...
`provider: auto` with `model: ''` tells Anakot to use the main model for that task.
...
**Short string form (`model.aliases.<name>: provider/model`)** — convenient from the shell because `anakot config set` only writes scalar values, but it can't carry a custom `base_url`:
...
`anakot model` walks you through picking a provider, authenticating (OAuth flows open a browser; API-key providers prompt for the key), and then choosing a specific model from that provider's curated catalog. The choice is written to `model.provider` and `model.model` in `~/.anakot/config.yaml`.

To list providers/models without launching the picker, use the dashboard or the REST endpoints below. To inspect what the CLI will actually use right now: `anakot config show | grep '^model\.'` and `anakot status`.
