...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`anakot insights`](/reference/cli-commands#anakot-insights).
...
| Gateway routing index | `~/.anakot/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |
...
Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.anakot/config.yaml`:
