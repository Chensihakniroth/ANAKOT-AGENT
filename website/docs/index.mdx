...
slug: /
sidebar_position: 0
title: "Anakot Agent Documentation"
description: "The self-improving AI agent built by callmemo. A built-in learning loop that creates skills from experience, improves them during use, and remembers across sessions."
...
The self-improving AI agent built by [callmemo](https://callmemo.ai). The only agent with a built-in learning loop — it creates skills from experience, improves them during use, nudges itself to persist knowledge, and builds a deepening model of who you are across sessions.
...
>
  ...
  <a
    href="https://github.com/Chensihakniroth/ANAKOT-AGENT/releases"
    style={{
      display: "inline-block",
      padding: "0.6rem 1.2rem",
...
2. Download the latest installer for your platform (`.exe`, `.dmg`, or `.AppImage`)
3. Run it

See the full **[Installation Guide](/getting-started/installation)** for platform-specific setup, prerequisites, and troubleshooting.

:::tip Fastest path to a working agent
After installing, run `anakot setup` to configure your LLM provider and start chatting.
:::
...
| 🔌 **[MCP Integration](/user-guide/features/mcp)**                      | Connect to MCP servers, filter their tools, and extend Anakot safely  |
| 🧭 **[Use MCP with Anakot](/guides/use-mcp-with-anakot)**               | Practical MCP setup patterns, examples, and tutorials                 |
| 🎙️ **[Voice Mode](/user-guide/features/voice-mode)**                    | Real-time voice interaction in CLI, Telegram, Discord, and Discord VC |
| 🗣️ **[Use Voice Mode with Anakot](/guides/use-voice-mode-with-anakot)** | Hands-on setup and usage patterns for Anakot voice workflows          |
| 🎭 **[Personality & SOUL.md](/user-guide/features/personality)**        | Define Anakot's default voice with a global SOUL.md                    |
...
| 💡 **[Tips & Best Practices](/guides/tips)**                            | Quick wins to get the most out of Anakot                              |
...
- **Built by model trainers** — Created by [callmemo](https://callmemo.ai), the lab behind Anakot, Nomos, and Psyche. Works with [callmemo Portal](https://portal.callmemo.ai), [OpenRouter](https://openrouter.ai), OpenAI, or any endpoint
...
- **Full web control** — Search, extract, browse, vision, image generation, TTS — one subscription via [callmemo Portal](/integrations/callmemo-portal) bundles all of them
- **MCP support** — Connect to any MCP server for extended tool capabilities
- **Research-ready** — Batch processing, trajectory export, RL training with Atropos. Built by [callmemo](https://callmemo.ai) — the lab behind Anakot, Nomos, and Psyche models
