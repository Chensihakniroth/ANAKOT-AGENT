...
Copy-paste recipes for common automation patterns. Each template uses Anakot's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
anakot cron create "0 2 * * *" \
  "You are a project manager triaging the callmemo/anakot-agent GitHub repo.

1. Run: gh issue list --repo callmemo/anakot-agent --state open --json number,title,labels,author,createdAt --limit 30
...
anakot cron create "0 9 * * 1" \
  "Scan the callmemo/anakot-agent repo for documentation drift.

1. Run: gh pr list --repo callmemo/anakot-agent --state merged --json number,title,files,mergedAt --limit 30
