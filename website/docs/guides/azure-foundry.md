...
Context length for the chosen model is resolved via Anakot's standard metadata chain (`models.dev`, provider metadata, and hardcoded family fallbacks) and stored in `config.yaml` so the model can size its own context window correctly.
...
`azure-identity` is installed automatically on first use via Anakot's lazy-install path. To pre-install:
...
- Azure OpenAI v1 endpoints (`<resource>.openai.azure.com/openai/v1`) expose `GET /models` with the resource's **available** model catalog. Anakot uses this list to prefill the model picker.
