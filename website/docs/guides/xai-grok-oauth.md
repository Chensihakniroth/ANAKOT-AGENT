...
# → Anakot opens your browser to accounts.x.ai
...
On servers, containers, or SSH sessions where no browser is available, Anakot detects the remote environment and prints the authorization URL instead of opening a browser.
...
If you don't have a regular SSH client (e.g. you're running Anakot inside GCP Cloud Shell, GitHub Codespaces, AWS EC2 Instance Connect, Gitpod, or another browser-based console), the `ssh -L` recipe above isn't available. Use `--manual-paste` instead — Anakot skips the loopback listener and lets you paste the failed callback URL straight from your browser:

```bash
anakot auth add xai-oauth --manual-paste
# Or via the model picker:
anakot model --manual-paste
```

See [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md#browser-only-remote-cloud-shell--codespaces--ec2-instance-connect) for the full walkthrough. Regression fix for [#26923](https://github.com/Chensihakniroth/ANAKOT-AGENT/issues/26923).

If the consent page renders the authorization code directly on the page (xAI's current behavior on browser-based consoles) instead of redirecting to your `127.0.0.1:56121/callback`, paste **just the bare code value** at the `Callback URL:` prompt — Anakot accepts the full URL, a bare `?code=...&state=...` query fragment, or a bare code interchangeably.

## How the Login Works

1. Anakot opens your browser to `accounts.x.ai`.
2. You sign in (or confirm your existing session) and approve access.
3. xAI redirects back to Anakot and the tokens are saved to `~/.anakot/auth.json`.
4. From then on, Anakot refreshes the access token in the background — you stay signed in until you `anakot auth remove xai-oauth` or revoke access from your xAI account settings.
...
```bash
anakot config set model.default grok-4.3
...
The loopback listener has a finite expiry window (default 180 s). If you don't approve the login in time, Anakot raises a timeout error.

**Fix:** re-run `anakot auth add xai-oauth` (or `anakot model`). The flow starts fresh.

### State mismatch (possible CSRF)

Anakot detected that the `state` value returned by the authorization server doesn't match what it sent.
...
On SSH or container sessions Anakot prints the authorization URL instead of opening a browser. The loopback callback listener still binds `127.0.0.1:56121` on the remote host — your laptop's browser can't reach it without an SSH local-forward:
...
This is **not** a stale-token problem — re-running `anakot model` won't change it. xAI's backend has been seen to restrict OAuth API access to specific SuperGrok tiers despite the in-app subscription being active (issue [#26847](https://github.com/Chensihakniroth/ANAKOT-AGENT/issues/26847)).
...
- [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md) — required reading if Anakot is on a different machine than your browser
