...
The GitHub webhook payload includes PR metadata (title, description, branch names, URLs) but **not the diff**. The prompt above instructs the agent to run `gh pr diff` to fetch the actual changes. The `terminal` tool is included in the default `anakot-webhook` toolset, so no extra configuration is needed.
:::
...
Load a [Anakot skill](/user-guide/features/skills) to give the agent a consistent review persona. Add `skills` to your route inside `platforms.webhook.extra.routes` in `config.yaml`:
...
- **[Build a Plugin](/guides/build-a-anakot-plugin)** — package review logic into a shareable plugin
