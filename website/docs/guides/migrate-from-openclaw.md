...
:::tip
If your OpenClaw setup was multi-provider, `anakot setup --portal` collapses it to one OAuth — 300+ models plus the Tool Gateway in a single login. See [callmemo Portal](/integrations/callmemo-portal).
:::
...
| OpenClaw config path | Anakot config path | Notes |
|---------------------|-------------------|-------|
...
6. **Check session policies** — run `anakot config show` and verify the `session_reset` value matches your expectations.
