...
| Provider | Setup |
|----------|-------|
| **callmemo Portal** | `anakot model` (OAuth, subscription-based) |
| **OpenAI Codex** | `anakot model` (ChatGPT OAuth, uses Codex models) |
...
| **OpenRouter** | `OPENROUTER_API_KEY` in `~/.anakot/.env` |
...
| **Qwen Cloud (Alibaba DashScope)** | `DASHSCOPE_API_KEY` in `~/.anakot/.env` (provider: `alibaba`) |
...
| **Google Gemini (OAuth)** | `anakot model` → "Google Gemini (OAuth)" (provider: `google-gemini-cli`, free tier supported, browser PKCE login) |
...
[callmemo Portal](https://portal.callmemo.ai) is callmemo's unified subscription gateway and **the recommended way to run Anakot Agent**. One OAuth login covers 300+ frontier agentic models (Claude, GPT, Gemini, DeepSeek, Qwen, Kimi, GLM, MiniMax, Grok, ...) plus the [Tool Gateway](/user-guide/features/tool-gateway) (web search, image generation, TTS, browser automation) plus [callmemo Chat](https://chat.callmemo.ai) — billed against your callmemo subscription instead of separate per-provider accounts.

```bash
anakot setup --portal     # fresh install — OAuth + provider + gateway in one command
anakot model              # existing install — pick "callmemo Portal" from the list
anakot portal info        # inspect login + routing at any time
```

Don't have a subscription yet? Get one at [portal.callmemo.ai/manage-subscription](https://portal.callmemo.ai/manage-subscription).

**For full details:** see the dedicated [callmemo Portal integration page](/integrations/callmemo-portal) (what's in the subscription, model catalog, troubleshooting) and the step-by-step [Run Anakot Agent with callmemo Portal guide](/guides/run-anakot-with-callmemo-portal).
...
:::info Codex Note
The OpenAI Codex provider authenticates via device code (open a URL, enter a code). Anakot stores the resulting credentials in its own auth store under `~/.anakot/auth.json` and can import existing Codex CLI credentials from `~/.codex/auth.json` when present. No Codex CLI installation is required.

If a token refresh fails with a terminal error (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Anakot marks the refresh token as dead and stops replaying it so you don't see a flood of identical auth failures. The next request surfaces a typed re-auth message instead. Run `anakot auth add codex-oauth` (or `anakot model` → OpenAI Codex) to start a fresh device-code login; the quarantine clears on the next successful exchange.
:::

:::warning
Even when using callmemo Portal, Codex, or a custom endpoint, some tools (vision, web summarization, MoA) use a separate "auxiliary" model. By default (`auxiliary.*.provider: "auto"`), Anakot routes these tasks to your **main chat model** — the same model you picked in `anakot model`. You can override each task individually to route it to a cheaper/faster model (e.g. Gemini Flash on OpenRouter) — see [Auxiliary Models](/user-guide/configuration#auxiliary-models).
:::

:::tip callmemo Tool Gateway
Paid callmemo Portal subscribers also get access to the **[Tool Gateway](/user-guide/features/tool-gateway)** — web search, image generation, TTS, and browser automation routed through your subscription. No extra API keys needed. On a fresh install, `anakot setup --portal` logs you in, sets callmemo as your provider, and turns the gateway on in one command. Existing users can enable it from `anakot model` or per-tool from `anakot tools`. Inspect routing at any time with `anakot portal info`.
:::
...
```bash
# StepFun
anakot chat --provider stepfun --model step-3-mini
...
# → done — Anakot auto-provisions your free tier on first request
```

Anakot ships Google's **public** `gemini-cli` desktop OAuth client by default —
...
- Tokens stored at `~/.anakot/auth/google_oauth.json` (chmod 0600, atomic write, cross-process `fcntl` lock)
- Automatic refresh 60 s before expiry
- Headless environments (SSH, `ANAKOT_HEADLESS=1`) → paste-mode fallback
...
- Responses translated back to OpenAI shape so the rest of Anakot works unchanged
...
| Workspace / Standard / Enterprise account | Set `ANAKOT_GEMINI_PROJECT_ID` or `GOOGLE_CLOUD_PROJECT` to your GCP project ID |
| VPC-SC-protected org | Anakot detects `SECURITY_POLICY_VIOLATED` and forces `standard-tier` automatically |
...
experience, use your own API key via the `gemini` provider instead. Anakot shows
...
```bash
ANAKOT_GEMINI_CLIENT_ID=your-client.apps.googleusercontent.com
ANAKOT_GEMINI_CLIENT_SECRET=...   # optional for Desktop clients
```
...
Anakot will automatically load a LM Studio model with 64K context length
...
**Possible causes:**
1. **Low output cap (`max_tokens`) on the server** — SGLang defaults to 128 tokens per response. Set `--default-max-tokens` on the server or configure Anakot with `model.max_tokens` in config.yaml. Note: `max_tokens` controls response length only — it is unrelated to how long your conversation history can be (that is `context_length`).
2. **Context exhaustion** — The model filled its context window. Increase `model.context_length` or enable [context compression](/user-guide/configuration#context-compression) in Anakot.
...
7. **callmemo Portal** — suffix-matches callmemo model IDs against OpenRouter metadata
...
custom_providers:
  ...
  - name: work
    ...
    api_mode: chat_completions   # set explicitly by `anakot model` → Custom Endpoint wizard; auto-detection still happens as a fallback
...
The `anakot model` → Custom Endpoint wizard now prompts for `api_mode` explicitly and persists your answer to `config.yaml`. URL-based auto-detection (e.g. `/anthropic` paths → `anthropic_messages`) still happens as a fallback when the field is left blank.
...
The cloud providers listed in [Other Compatible Providers](#other-compatible-providers) all speak OpenAI's REST dialect, so they wire up the same way under `custom_providers:`. Three worked recipes follow. Each drops into `~/.anakot/config.yaml` and the matching API key goes in `~/.anakot/.env`.
...
|----------|-------------|
| **Just want it to work** | OpenRouter (default) or callmemo Portal |
...
:::tip
Fallback is configured exclusively through `config.yaml` — or interactively via `anakot fallback`. For full details on when it triggers, how the chain advances, and how it interacts with auxiliary tasks and delegation, see [Fallback Providers](/user-guide/features/fallback-providers).
:::
