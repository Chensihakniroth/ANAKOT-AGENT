...
:::tip Start here
If you only have time to set up one integration, set up [callmemo Portal](/integrations/callmemo-portal) — a single OAuth login covers 300+ models plus the four Tool Gateway tools (web search, image generation, TTS, and browser automation).
:::
...
- **[AI Providers](/user-guide/features/provider-routing)** — OpenRouter, Anthropic, OpenAI, Google, and any OpenAI-compatible endpoint. Anakot auto-detects capabilities like vision, streaming, and tool use per provider.
...
- **[MCP Servers](/user-guide/features/mcp)** — Connect Anakot to external tool servers via Model Context Protocol. Access tools from GitHub, databases, file systems, browser stacks, internal APIs, and more without writing native Anakot tools. Supports both stdio and SSE transports, per-server tool filtering, and capability-aware resource/prompt registration.

## Web Search Backends

The `web_search` and `web_extract` tools support four backend providers, configured via `config.yaml` or `anakot tools`:
...
- **[IDE Integration (ACP)](/user-guide/features/acp)** — Use Anakot Agent inside ACP-compatible editors such as VS Code, Zed, and JetBrains. Anakot runs as an ACP server, rendering chat messages, tool activity, file diffs, and terminal commands inside your editor.

## Programmatic Access

- **[API Server](/user-guide/features/api-server)** — Expose Anakot as an OpenAI-compatible HTTP endpoint. Any frontend that speaks the OpenAI format — Open WebUI, LobeChat, LibreChat, NextChat, ChatBox — can connect and use Anakot as a backend with its full toolset.
...
- **[Plugin System](/user-guide/features/plugins)** — Extend Anakot with custom tools, lifecycle hooks, and CLI commands without modifying core code. Plugins are discovered from `~/.anakot/plugins/`, project-local `.anakot/plugins/`, and pip-installed entry points.
- **[Build a Plugin](/guides/build-a-anakot-plugin)** — Step-by-step guide for creating Anakot plugins with tools, hooks, and CLI commands.
