...
Copies bundled skills from the repo's skills/ directory into ~/.anakot/skills/
...
The manifest lives at ~/.anakot/skills/.bundled_manifest.
"""
...
# Marker file written by `anakot profile create --no-skills` (named profiles)
# and by the installer's `--no-skills` flag (the default ~/.anakot profile).
# When present in ANAKOT_HOME, sync_skills() is a no-op so neither the
# installer, `anakot update`, nor a direct sync re-injects bundled skills.
# Delete the file to opt back in. Mirrors
# anakot_cli.profiles.NO_BUNDLED_SKILLS_MARKER (kept as a literal here to
...
def _get_bundled_dir() -> Path:
    ...
    Checks ANAKOT_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.anakot/skills/.curator_suppressed`` directly if that import
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.anakot/skills/mlops/axolotl
    """
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.anakot/skills/ using the manifest.

    Returns:
        dict with keys: copied (list), updated (list), skipped (int),
                        user_modified (list), cleaned (list), total_bundled (int)
    """
    # Opt-out: a profile (named or the default ~/.anakot) that wrote the
    ...
    if (ANAKOT_HOME / NO_BUNDLED_SKILLS_MARKER).exists():
        if not quiet:
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.anakot/skills/.curator_suppressed) is written when the curator
        # archives a bundled skill with curator.prune_builtins enabled. Without
        # this skip, every `anakot update` would resurrect a skill the user
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `anakot skills reset {skill_name}` "
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if restore and deleted_user_copy:
        ...
        message = (
            f"Cleared manifest entry for '{name}'. Future `anakot update` runs "
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    When ``enabled`` is True, writes ANAKOT_HOME/.no-bundled-skills so the
    installer, ``anakot update``, and any direct sync stop seeding bundled
    skills. When False, removes the marker so seeding resumes on the next
    sync. This is the on-disk-state half of ``anakot skills opt-out`` /
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = ANAKOT_HOME / NO_BUNDLED_SKILLS_MARKER
    ...
    try:
        if enabled:
            ...
            changed = existed
            message = (
                "Opted back in. The next `anakot update` (or `anakot skills "
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (ANAKOT_HOME / NO_BUNDLED_SKILLS_MARKER).exists()
