...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``anakot tools`` repaint, tool-registration scans,
    ...
    2. ``~/.anakot/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.anakot/.env`` first, then ``os.environ``.

    Wraps :func:`anakot_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from anakot_cli.config import get_env_value as _anakot_get_env_value

        value = _anakot_get_env_value(name)
...
def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    ...
    Prefers Anakot-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``anakot_cli.config.get_env_value`` so keys
    stored in ``~/.anakot/.env`` (the standard Anakot location) are honored —
