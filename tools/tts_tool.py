...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.anakot/config.yaml``. Anakot
...
Configuration is loaded from ~/.anakot/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``anakot_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from anakot_cli.config import get_env_value as _get_env_value
    except ImportError:
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.anakot/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.anakot/config.yaml.
...
# custom voice-cloning scripts, etc.) into Anakot without any Python code
...
#           output_format: wav
#
# Anakot writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__ANAKOT_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    # output_format when Anakot actually needs a non-default format/override.
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Anakot caches Piper voice models.

    Resolves to ``~/.anakot/cache/piper-voices/`` under the active
    ANAKOT_HOME so voice downloads follow profile boundaries.
    """
    from anakot_constants import get_anakot_dir
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.anakot/config.yaml (tts: section).
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'anakot-agent[mistral]'"
                }, ensure_ascii=False)
...
def _resolve_openai_audio_client_config() -> tuple[str, str]:
    ...
    if managed_gateway is None:
        ...
        if managed_callmemo_tools_enabled() or prefers_gateway("tts"):
            message += (
                ". "
                + nous_tool_gateway_unavailable_message(
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_anakot_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
