...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Anakot exit
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of anakot directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "anakot-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = os.getenv("ANAKOT_SESSION_KEY", "")
    if session_key:
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (ANAKOT_INTERACTIVE=1).
    ...
    try:
        ...
        if "ANAKOT_SPINNER_PAUSE" in os.environ:
            del os.environ["ANAKOT_SPINNER_PAUSE"]
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (ANAKOT_INTERACTIVE=1):
    ...
    # interactive Anakot password prompt or the sudo -S password-pipe path.
    ...
    # silently without Anakot prompting.
    if not has_configured_password and not sudo_password and _sudo_nopasswd_works():
        return command, None

    if not has_configured_password and not sudo_password and env_var_enabled("ANAKOT_INTERACTIVE"):
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so Anakot can track lifecycle and output.
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``anakot-agent=1`` for the current
    profile that match the issue #20561 leak class — containers left behind
    by Anakot processes that exited without firing ``atexit`` (SIGKILL,
    ...
    * ``terminal.docker_orphan_reaper: false`` disables it entirely (the
      ...
      Anakot processes in the same profile and don't trust the
    ...
    * ``_docker_orphan_reaper_ran`` flag — sweep runs once per Python
    ...
    # 2 × lifetime_seconds gives sibling Anakot processes a generous grace
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, AnakotSweEnv, ...)
...
def _get_env_config() -> Dict[str, Any]:
    ...
    return {
        ...
        # Startup orphan reaper for anakot-tagged containers left behind by
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        # prior Anakot processes that hit SIGKILL / OOM / a closed terminal
        ...
        if modal_state["selected_backend"] != "direct":
            if modal_state["managed_mode_blocked"]:
                raise ValueError(
                    ...
                    "callmemo Tool Gateway access is not currently available and no direct "
            ...
            if managed_callmemo_tools_enabled():
                message = (
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "Use terminal(background=true) so Anakot can track the process, then run "
    ...
    try:
        ...
        if background:
            ...
            try:
                ...
                # anakot-agent dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in anakot-agent dev work (PRs #31329, "
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    _gw_platform = _gse("ANAKOT_SESSION_PLATFORM", "")
                    if _gw_platform:
                        _gw_chat_id = _gse("ANAKOT_SESSION_CHAT_ID", "")
                        _gw_thread_id = _gse("ANAKOT_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("ANAKOT_SESSION_USER_ID", "")
                        _gw_user_name = _gse("ANAKOT_SESSION_USER_NAME", "")
                        _gw_message_id = _gse("ANAKOT_SESSION_MESSAGE_ID", "")
            ...
            try:
                from anakot_cli.plugins import invoke_hook
