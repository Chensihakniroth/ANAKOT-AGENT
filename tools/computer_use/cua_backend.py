...
can break on OS updates. Pin the installed version via `ANAKOT_CUA_DRIVER_
...
PINNED_CUA_DRIVER_VERSION = os.environ.get("ANAKOT_CUA_DRIVER_VERSION", "0.5.0")

_CUA_DRIVER_CMD = os.environ.get("ANAKOT_CUA_DRIVER_CMD", "cua-driver")
...
def cua_driver_binary_available() -> bool:
    """True if `cua-driver` is on $PATH or ANAKOT_CUA_DRIVER_CMD resolves."""
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def capture(self, mode: str = "som", app: Optional[str] = None) -> CaptureResult:
        ...
        Maps anakot `capture(mode, app)` → cua-driver `list_windows` +
