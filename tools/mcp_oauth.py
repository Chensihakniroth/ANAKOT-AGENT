...
This module provides the glue:
    - ``AnakotTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Anakot Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses ANAKOT_HOME so each profile gets its own OAuth tokens.
    Layout: ``ANAKOT_HOME/mcp-tokens/``
    """
    try:
        from anakot_constants import get_anakot_home
        base = Path(get_anakot_home())
    except ImportError:
        base = Path(os.environ.get("ANAKOT_HOME", str(Path.home() / ".anakot")))
...
# ---------------------------------------------------------------------------
# AnakotTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------
...
class AnakotTokenStorage:
    ...
    File layout::
        ...
        ANAKOT_HOME/mcp-tokens/<server_name>.json         -- tokens
        ANAKOT_HOME/mcp-tokens/<server_name>.client.json   -- client info
        ANAKOT_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Anakot records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Anakot.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://github.com/Chensihakniroth/ANAKOT-AGENT/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `anakot mcp login <server>` later to "
