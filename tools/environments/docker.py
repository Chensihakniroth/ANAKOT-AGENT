...
from tools.environments.local import _ANAKOT_PROVIDER_ENV_BLOCKLIST
...
def _load_anakot_env_vars() -> dict[str, str]:
    """Load ~/.anakot/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active Anakot profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale anakot-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=anakot-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Anakot process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=anakot-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a anakot process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Anakot
    process leaves its container behind permanently because there's no
    subsequent Anakot process scheduled to reuse that exact (task, profile)
    pair.
    """
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``ANAKOT_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
...
#   SETUID/SETGID - the image's init drops from root to the 'anakot'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``anakot-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.anakot/sandboxes/). Non-persistent
        ...
        # s6-overlay images (e.g. anakot-agent:latest) already use /init as PID 1
        ...
        # Labels make anakot-created containers identifiable to:
        #   * the orphan reaper (`anakot-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`anakot-task-id`, `anakot-profile`)
        #   * operators running `docker ps --filter label=anakot-agent=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Anakot profile is captured at
        ...
        label_args = [
            "--label", "anakot-agent=1",
            "--label", f"anakot-task-id={task_label}",
            "--label", f"anakot-profile={profile_name}",
        ]
        ...
        # container shared across sessions").  If a prior Anakot process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Anakot secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _ANAKOT_PROVIDER_ENV_BLOCKLIST)
        anakot_env = _load_anakot_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if not value:
                value = anakot_env.get(key)
            if value:
                exec_env[key] = value
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("anakot-task-id", "")
        profile_label = self._labels.get("anakot-profile", "")
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"anakot-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``anakot-*`` but were
        ...
        # container) but can happen if a previous Anakot process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Anakot exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Anakot process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Anakot startup. That covers the
        ...
        # outlives Anakot processes, processes inside it stay alive, and
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``anakot /quit``
