...
def iter_sync_files(container_base: str = "/root/.anakot") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.anakot to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``ANAKOT_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, anakot_home: Path | None = None) -> None:
        ...
        Downloads the remote ``.anakot/`` directory as a tar archive,
        ...
        # against an uninitialized remote .anakot/ directory.
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="anakot-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.anakot/skills/a.md`` →
        ``~/.anakot/skills/a.md``, a new remote file at
        ``/root/.anakot/skills/b.md`` maps to ``~/.anakot/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
