...
# Anakot-internal env vars that should NOT leak into terminal subprocesses.
_ANAKOT_PROVIDER_ENV_FORCE_PREFIX = "_ANAKOT_FORCE_"

# Anakot-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Anakot inference secret exactly
...
def _inject_context_anakot_home(env: dict) -> None:
    """Bridge the context-local Anakot home override into subprocess env."""
    try:
        from anakot_constants import get_anakot_home_override

        value = get_anakot_home_override()
        if value:
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Anakot-managed secrets from a subprocess environment."""
    try:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_ANAKOT_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if key not in _ANAKOT_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_ANAKOT_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_ANAKOT_PROVIDER_ENV_FORCE_PREFIX):]
            sanitized[real_key] = value
        elif key not in _ANAKOT_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    _inject_context_anakot_home(sanitized)
...
def _find_bash() -> str:
    ...
    #   PortableGit: %LOCALAPPDATA%\anakot\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\anakot\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _anakot_portable_git = os.path.join(_local_appdata, "anakot", "git") if _local_appdata else ""
    if _anakot_portable_git:
        for candidate in (
            os.path.join(_anakot_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_anakot_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_ANAKOT_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_ANAKOT_PROVIDER_ENV_FORCE_PREFIX):]
            run_env[real_key] = v
        elif k not in _ANAKOT_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    ...
    # Anakot is driving via _find_bash (Git Bash), and Git Bash itself
    ...
    _inject_context_anakot_home(run_env)

    # Per-profile HOME isolation: redirect system tool configs (git, ssh, gh,
    # npm …) into {ANAKOT_HOME}/home/ when that directory exists.  Only the
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Anakot trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``ANAKOT_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under ANAKOT_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_anakot_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "anakot_terminal"
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_anakot_pgid", None)
                    if pgid is None:
                        raise
