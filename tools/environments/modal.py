...
wrapper, while preserving Anakot' persistent snapshot behavior across sessions.
"""
...
_SNAPSHOT_STORE = get_anakot_home() / "modal_snapshots.json"
...
class ModalEnvironment(BaseEnvironment):
    ...
    def _modal_bulk_download(self, dest: Path) -> None:
        ...
        Modal sandboxes always run as root, so /root/.anakot is hardcoded
        (consistent with iter_sync_files call on line 269).
        """
        async def _download():
            proc = await self._sandbox.exec.aio(
                "bash", "-c", "tar cf - -C / root/.anakot"
            )
