...
`mcp_tool.py`, and `anakot_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# AnakotMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_anakot_provider_class() -> Optional[type]:
    ...
    class AnakotMCPOAuthProvider(OAuthClientProvider):
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`AnakotTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, AnakotTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._anakot_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import AnakotTokenStorage
                        if isinstance(storage, AnakotTokenStorage):
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import AnakotTokenStorage
            if not isinstance(storage, AnakotTokenStorage):
                return
    ...
    return AnakotMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_ANAKOT_PROVIDER_CLS: Optional[type] = _make_anakot_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`AnakotMCPOAuthProvider` directly using the helpers
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``anakot mcp remove <name>`` and (indirectly) by
        ``anakot mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
