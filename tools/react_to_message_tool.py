#!/usr/bin/env python3
"""Let the agent react to a message with an emoji in the anakot desktop app.
...
Gated on the desktop emitter being wired (``desktop_ui.available()``) plus the
``display.message_reactions`` config flag, so it costs nothing on every other
surface — the platform adapters already expose reactions through
...
def react_to_message_tool(emoji: str, message_row_id=None, messages_back=None) -> str:
    ...
    session_key = get_session_env("ANAKOT_SESSION_KEY", "") or get_session_env(
        "ANAKOT_SESSION_ID", ""
    )
...
def check_react_requirements() -> bool:
    ...
    The emitter is wired only on the gateway the app spawns; the feature itself
    is off by default and enabled from Settings → Appearance (the desktop
    mirrors the toggle into ``display.message_reactions``).
    """
