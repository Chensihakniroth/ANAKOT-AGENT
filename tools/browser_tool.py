...
supports multiple backends — **Browser Use** (cloud, default for callmemo
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    anakot_node_bin = str(anakot_home / "node" / "bin")
    anakot_node_root = str(anakot_home / "node")
    anakot_nm_bin = str(anakot_home / "node_modules" / ".bin")
    return [anakot_node_bin, anakot_node_root, anakot_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
# plugins drop in under ``~/.anakot/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    fallback. If unset, fall back to Browser Use (managed callmemo gateway or
    ...
    browser plugins (``~/.anakot/plugins/browser/<vendor>/``) participate
    ...
    if resolved is None:
        # Auto-detect path: Browser Use first (managed callmemo gateway or
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Anakot silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Anakot CLI where sys.stdout and sys.stderr both report
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-anakot_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed anakot processes — this way every clean anakot exit sweeps
    ...
    # Sweep orphans from other crashed anakot processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live anakot processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current anakot PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live anakot process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning anakot process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced anakot PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent anakot instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-anakot_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live anakot process.
            continue
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Anakot-managed
    ...
    # Build an extended search PATH including Anakot-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            ...
            if not recheck:
                anakot_nm = str(get_anakot_home() / "node_modules" / ".bin")
                recheck = shutil.which("agent-browser", path=anakot_nm)
            if not recheck:
                anakot_node_bin = str(get_anakot_home() / "node" / "bin")
                recheck = shutil.which("agent-browser", path=anakot_node_bin)
            if not recheck:
                anakot_node_root = str(get_anakot_home() / "node")
                recheck = shutil.which("agent-browser", path=anakot_node_root)
            if recheck:
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this anakot PID as the session owner (cross-process safe
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Anakot
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/nousresearch/anakot-agent:latest")
                else:
