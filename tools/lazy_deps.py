"""
Lazy dependency installer for opt-in Anakot Agent backends.

Many Anakot features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``anakot-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``anakot tools`` or the manual pip command.
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # Dashboard (`anakot dashboard`)
    "tool.dashboard": (
        "fastapi==0.133.1",
        "uvicorn[standard]==0.41.0",
        "starlette==1.0.1",  # CVE-2026-48710 (BadHost) — keep lazy-install in sync with pyproject [web]
        "python-multipart>=0.0.20",  # FastAPI UploadFile/Form support — required by kanban plugin_api.py
    ),
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``anakot update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    Mirrors the strategy in ``anakot_cli.tools_config._pip_install`` but
...
def active_features() -> list[str]:
    ...
    Used by ``anakot update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``anakot update``. Never raises; lazy-install failures
