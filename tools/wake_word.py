"""Wake-word ("hey casca") detection — hands-free session trigger.

A lightweight, always-on hotword listener that fires a callback when a wake
phrase is spoken — the "Hey Siri" / "Alexa" pattern. Say the wake word, Anakot
opens a fresh session and captures voice via the existing pipeline, then
answers.

Engine: **openwakeword** (free, no API key, fully on-device) — loads an ONNX
model. Defaults to the bundled "hey casca" model (``tools/wakewords/``) so
the wake word works out of the box; or point ``wake_word.openwakeword.model``
at a built-in name (``hey_jarvis``, ``alexa``, ...) or a custom ``.onnx`` for
another phrase. No audio leaves the machine for detection.

Audio capture reuses the same 16 kHz mono int16 ``sounddevice`` path as voice
mode (``tools.voice_mode``). The detector runs on its own daemon thread;
callers ``pause()`` it while a voice turn holds the microphone and ``resume()``
it once the system is idle again (two input streams on one device is
unreliable cross-platform).

Nothing here mutates agent context or the prompt cache — on wake we hand a
plain string to the caller, exactly like a voice transcript.
"""
...
# 16 kHz mono int16 — Whisper-native and what the engine expects.
SAMPLE_RATE = 16000

# Minimum gap between two consecutive wake fires, so one "hey casca" can't
...
# Bundled models (tools/wakewords/) — hey_casca is the default, so the wake
# word works out of the box; hey_hermes is kept as an optional alternative.
# Config names in _BUNDLED_MODEL_ALIASES resolve to their bundled file, not an
# openwakeword built-in (the catalog has no hey_casca/hey_hermes).
_BUNDLED_MODEL_NAME = "hey_casca"
_BUNDLED_MODEL_FILES = {
    "hey_casca": "hey_casca",    # default — "hey casca" hotword
    "hey_hermes": "hey_hermes",  # optional — "hey hermes" hotword
}
...
def _bundled_wakeword_path(framework: str = "onnx", stem: str | None = None) -> str:
    """Path to a bundled model (.onnx/.tflite) for ``framework``."""
    ext = "tflite" if str(framework).strip().lower() == "tflite" else "onnx"
    stem = stem or _BUNDLED_MODEL_NAME
    return os.path.join(os.path.dirname(__file__), "wakewords", f"{stem}.{ext}")
...
def default_inference_framework() -> str:
    ...
    shared *embedding* model is the broken stage. Prefer the tflite backend
    there; ONNX stays the default everywhere else.
...
def resolve_inference_framework(cfg: Dict[str, Any]) -> str:
    ...
    (upstream #336), which is coerced to tflite so we never ship a dead ear.
...
def wake_phrase(cfg: Optional[Dict[str, Any]] = None) -> str:
    ...
    return str(_get(cfg, "phrase")) or "hey casca"
...
def silent_audio_hint(details: Dict[str, Any]) -> str:
    ...
    if sys.platform == "darwin":
        return (
            "Microphone delivers only silence. Grant the Anakot backend "
...
class _Engine:
    ...
    #: Multi-phrase engines set this for profile routing; the single-phrase
    #: engine leaves it None (callers fall back to the configured phrase).
...
class _OpenWakeWordEngine(_Engine):
    ...
    def __init__(self, cfg: Dict[str, Any]):
        ...
        # openWakeWord silently downgrades tflite -> onnx when no tflite
        # runtime is present; the platform gate lives in
        # ``default_inference_framework`` so Windows/ONNX is the shipped path.

        # Default (or an explicit bundled alias like "hey_casca"/"hey casca"/
        # "casca", or the optional "hey_hermes"/"hermes") → the bundled model;
        # a built-in name or custom path is used as-is.
        alias_key = model_ref.lower()
        if alias_key in _BUNDLED_MODEL_ALIASES:
            model_ref = _bundled_wakeword_path(framework, stem=_BUNDLED_MODEL_ALIASES[alias_key])
...
def _stt_ready() -> bool:
    ...
    try:
        ...
        return bool(reqs.get("stt_available"))
    except Exception:
        return False
...
def _tts_ready() -> bool:
    ...
    PROBE, not an installer: ``check_tts_requirements`` only inspects installed
    packages/keys — running it inside a status poll is safe.
    """
    try:
...
def check_wake_word_requirements(cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if not deps_ok and not lazy_ok:
        ...
        hint = f"Wake word needs {missing} configured — see the voice-mode docs."
    ...
    return {
        "available": stt_ok and tts_ok and mic_ok,
        "provider": provider,
...
class WakeWordDetector:
    ...
    def __init__(self, engine: _Engine, on_wake: Callable[[], None],
        ...
        # Surfaced via wake.status so users can tell "armed" from "deaf".
...
def _lock_path() -> Path:
    ...
    return get_anakot_home() / "runtime" / "wake-word.lock"
...
def start_listening(
    ...
    *,
    ...
    Raises if engine construction fails (missing deps / model); callers should
    probe :func:`check_wake_word_requirements` first. A different owner,
...
def get_last_match() -> Optional[tuple[str, str]]:
    ...
    reports per-phrase matches. None otherwise."""
    with _detector_lock:
        det = _detector
    if det is None:
        return None
