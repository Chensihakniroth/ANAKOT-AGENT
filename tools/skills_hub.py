#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Anakot Skills Hub.
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by callmemo) but not activated
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                anakot_meta = meta_block.get("anakot", {})
                if isinstance(anakot_meta, dict):
                    tags = anakot_meta.get("tags", [])
...
def _load_anakot_index() -> Optional[dict]:
    ...
    We cache it locally for ANAKOT_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if ANAKOT_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - ANAKOT_INDEX_CACHE_FILE.stat().st_mtime
            if age < ANAKOT_INDEX_TTL:
                return json.loads(ANAKOT_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass
    ...
    # Cache locally
    try:
        ANAKOT_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        ANAKOT_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class AnakotIndexSource(SkillSource):
    """Skill source backed by the centralized Anakot Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_anakot_index()
            self._loaded = True
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "anakot-index"),
            identifier=entry.get("identifier", ""),
