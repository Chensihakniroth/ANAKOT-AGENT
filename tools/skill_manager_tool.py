...
~/.anakot/skills/. Existing skills (bundled, hub-installed, or user-created)
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `anakot config set skills.guard_agent_created true`.
    """
    try:
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`anakot curator unpin {name}` if they want to delete it. "
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.anakot/skills/) first, then any
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER Anakot profiles.
    ...
    # Default profile (~/.anakot/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.anakot/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"another profile, switch profiles (`anakot -p "
            ...
            base += (
                ...
                f"Switch profiles (`anakot -p <name>`) to edit there, or "
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {display_anakot_home()}/skills/; existing skills can be modified wherever they live.\n\n"
        ...
        "will refuse with a message pointing the user to `anakot curator unpin <name>`. "
