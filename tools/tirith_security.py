...
it is automatically downloaded from GitHub releases to $ANAKOT_HOME/bin/tirith.
...
def _get_anakot_home() -> str:
    """Return the Anakot home directory, respecting ANAKOT_HOME env var."""
    return str(get_anakot_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_anakot_home(), ".tirith-install-failed")
...
def _anakot_bin_dir() -> str:
    """Return $ANAKOT_HOME/bin, creating it if needed."""
    d = os.path.join(_get_anakot_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ANAKOT_HOME/bin/tirith.
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ANAKOT_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ANAKOT_HOME/bin/tirith
    ...
    if os.path.isfile(anakot_bin) and os.access(anakot_bin, os.X_OK):
        _resolved_path = anakot_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return anakot_bin
