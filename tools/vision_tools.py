...
callmemo, Codex, native Anthropic, or a custom OpenAI-compatible endpoint.
...
def _resolve_download_timeout() -> float:
    env_val = os.getenv("ANAKOT_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
...
async def vision_analyze_tool(
    ...
    Note:
        - For URLs, temporary images are stored under $ANAKOT_HOME/cache/vision/ and cleaned up
...
if __name__ == "__main__":
    ...
    if not api_available:
        ...
        print("Configure a supported multimodal backend (OpenRouter, callmemo, Codex, Anthropic, or a custom OpenAI-compatible endpoint).")
        sys.exit(1)
    else:
