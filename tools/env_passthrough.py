...
def _is_anakot_provider_credential(name: str) -> bool:
    """True if ``name`` is a Anakot-managed provider credential (API key,
    token, or similar) per ``_ANAKOT_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Anakot API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    try:
        ...
        return False
    return name in _ANAKOT_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Anakot-managed provider credentials (from
    ``_ANAKOT_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Anakot-managed
    ...
    Non-Anakot third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Anakot-managed provider credentials must not be passed
