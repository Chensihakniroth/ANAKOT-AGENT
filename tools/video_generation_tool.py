...
provider** — turn on a backend by enabling a plugin (``anakot plugins
enable video_gen/<name>``) and selecting it in ``anakot tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/anakot-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `anakot tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `anakot tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `anakot tools` → "
...
# `anakot tools` or `/skills`, the schema rebuilds automatically.
