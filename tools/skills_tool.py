...
# All skills live in ~/.anakot/skills/ (seeded from bundled skills/ on install).
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from ANAKOT_HOME/.env."""
    env_path = get_anakot_home() / ".env"
...
def _capture_required_environment_variables(
    ...
    # — the desktop app / TUI — set ANAKOT_INTERACTIVE and register a
    # secret-capture callback that routes to a secure secret.request overlay, so
    # they fall through and actually prompt. (ANAKOT_INTERACTIVE is the same flag
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_anakot_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.anakot/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``ANAKOT_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
        return get_session_env("ANAKOT_SESSION_PLATFORM") or ""
    except Exception:
        return ""
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``ANAKOT_PLATFORM`` environment variable
    3. ``ANAKOT_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from anakot_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("ANAKOT_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.anakot/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``anakot skills`` config UI). Default False
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": f"No skills found. Skills directory created at {display_anakot_home()}/skills/",
                },
                ensure_ascii=False,
            )
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: anakot plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.anakot/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `anakot skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.anakot.* first (agentskills.io convention), fall back to top-level
        anakot_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            anakot_meta = metadata.get("anakot", {}) or {}

        tags = _parse_tags(anakot_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            anakot_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
