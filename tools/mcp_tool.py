...
transport, discovers their tools, and registers them into the anakot-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.anakot/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.anakot/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            anakot_home = os.path.expanduser(
                os.getenv(
                    "ANAKOT_HOME", os.path.join(os.path.expanduser("~"), ".anakot")
                )
            )
            candidates = [
                os.path.join(anakot_home, "node", "bin", resolved_command),
                ...
                # image (which the Anakot Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Anakot MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Anakot gateways know how to render.
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "  pip install 'anakot-agent[mcp]'\n"
                "or (full install):\n"
                "  pip install 'anakot-agent[all]'"
            )
    ...
    async def run(self, config: dict):
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Anakot is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `anakot mcp login {server_name}` (or delete the tokens "
            f"file under ~/.anakot/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Anakot config file.
    ...
    ``os.environ`` (which includes ``~/.anakot/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Anakot' MEDIA: tag convention and out to messaging
            ...
            # Anakot' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Anakot's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Anakot registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``anakot tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Anakot registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple anakot
