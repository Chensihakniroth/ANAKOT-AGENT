...
def managed_callmemo_tools_enabled(*, force_fresh: bool = False) -> bool:
    """Return True when the user is entitled to the callmemo Tool Gateway.

    Entitlement is paid callmemo Portal service access OR a live free tool pool
...
def nous_tool_gateway_unavailable_message(
    ...
    *,
    ...
    """Return account-aware guidance for an unavailable callmemo Tool Gateway path."""
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `anakot model` to refresh your "
        "callmemo Portal login and billing status."
    )
...
def resolve_modal_backend_state(
    ...
    *,
    ...
    if managed_enabled is None:
        managed_enabled = managed_callmemo_tools_enabled()
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.anakot/.env`` (via
    ``anakot_cli.config.get_env_value`` when available) so tool-side
