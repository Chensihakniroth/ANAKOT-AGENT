...
selectable via ``anakot tools`` → Image Generation; the active model is
...
FAL_MODELS: Dict[str, Dict[str, Any]] = {
    ...
    "fal-ai/nano-banana-pro": {
        ...
        "defaults": {
            ...
            # Users on callmemo Subscription should stay at 1K for predictable billing.
            "resolution": "1K",
        },
        "supports": {
    ...
    "fal-ai/gpt-image-2": {
        ...
        "defaults": {
            # Same quality pinning as gpt-image-1.5: medium keeps callmemo
    ...
    # existing FAL key / callmemo Portal subscription rather than register
...
# ---------------------------------------------------------------------------
# Managed FAL gateway (callmemo Subscription)
# ---------------------------------------------------------------------------
...
def _submit_fal_request(model: str, arguments: Dict[str, Any]):
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                f"callmemo Subscription gateway rejected model '{model}' "
                f"(HTTP {status}). This model may not yet be enabled on "
                f"the callmemo Portal's FAL proxy. Either:\n"
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    ...
    # Anakot cache roots can be translated without side effects. SSH can still
...
def _build_no_backend_setup_message() -> str:
    ...
    Used by the in-tree FAL path. Mentions:
      - FAL_KEY signup link
      - managed-gateway status (if callmemo tools are enabled)
    ...
    if managed_callmemo_tools_enabled():
        lines.append(
            "  2. Sign in to a callmemo account that has the managed FAL "
            "gateway enabled (`anakot setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `anakot tools` "
        "→ Image Generation (run `anakot plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
...
def check_image_generation_requirements() -> bool:
    ...
    the historical behavior: shipping anakot with a FAL key configured
    ...
    if provider is None:
        return json.dumps({
            ...
            "error": (
                ...
                f"registered that name. Run `anakot plugins list` to see "
