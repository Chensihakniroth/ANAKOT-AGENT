...
def _get_anakot_config_resolved() -> str | None:
    """Return the resolved absolute path of the Anakot config file (cached)."""
    global _anakot_config_resolved, _anakot_config_resolved_loaded
    if _anakot_config_resolved_loaded:
        return _anakot_config_resolved
    _anakot_config_resolved_loaded = True
    try:
        from anakot_cli.config import get_config_path
        _anakot_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _anakot_config_resolved = str(Path("~/.anakot/config.yaml").expanduser().resolve())
        except Exception:
            _anakot_config_resolved = None
    return _anakot_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the Anakot config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    anakot_config = _get_anakot_config_resolved()
    if anakot_config and (resolved == anakot_config or normalized == anakot_config):
        return (
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Anakot mirror prefix for Docker file tools."""
    try:
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    """Return a soft-guard warning when ``filepath`` lands in another Anakot
    profile's scoped area, a host-side sandbox-mirror of authoritative profile
    state, or the Docker container's sandbox mirror of Anakot state.
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.anakot/…`` mirror created by a
      non-local terminal backend (Docker, Daytona, etc.), where the host
      Anakot process never reads the mirror and the authoritative file is
      left untouched.
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.anakot/…`` path.

    Returns ``None`` when the write is in-scope or outside Anakot scope.
    ...
    # in a session that cd'd into ``~/.anakot/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Anakot internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under ANAKOT_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == ANAKOT_HOME (e.g. "auth.json") still hits the
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-Anakot-profile guard. The
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-Anakot-profile guard for
...
WRITE_FILE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Anakot profile's skills/plugins/cron/memories — by default these writes are blocked with a warning because they affect a different profile than the one this session is running under.",
                "default": False,
            },
        },
...
PATCH_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Anakot profile's skills/plugins/cron/memories.",
                "default": False,
            },
        },
        "required": ["mode"],
