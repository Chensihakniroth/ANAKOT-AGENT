...
Backend is selected during ``anakot tools`` setup (web.backend in config.yaml).
When available, Anakot can route Firecrawl calls through a callmemo-hosted tool-gateway
for callmemo Subscribers only.
...
- Firecrawl: https://docs.firecrawl.dev/introduction (search, extract; direct or derived firecrawl-gateway.<domain> for callmemo Subscribers)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.anakot/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``anakot tools``).
    ...
    # tool gateway is configured for callmemo subscribers.
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `anakot tools` repaint.
        try:
...
def _web_requires_env() -> list[str]:
    ...
    set.  Gating them on ``managed_callmemo_tools_enabled()`` only saved
    string noise in the metadata list, but cost a synchronous HTTP
    refresh against the callmemo portal on every CLI startup (invoked at
...
def _is_nous_auxiliary_client(client: Any) -> bool:
    """Return True when the resolved auxiliary backend is callmemo Portal."""
    from urllib.parse import urlparse

    base_url = str(getattr(client, "base_url", "") or "")
    host = (urlparse(base_url).hostname or "").lower()
    return host == "callmemo.ai" or host.endswith(".callmemo.ai")
...
def _resolve_web_extract_auxiliary(model: Optional[str] = None) -> tuple[Optional[Any], Optional[str], Dict[str, Any]]:
    ...
    if client is not None and _is_nous_auxiliary_client(client):
        ...
        extra_body = get_auxiliary_extra_body() or {"tags": callmemo_portal_tags()}
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `anakot tools` to set one up."
                ),
            }
        else:
            logger.info(
...
if __name__ == "__main__":
    ...
    if not nous_available:
        ...
        print("Set OPENROUTER_API_KEY, configure callmemo Portal, or set OPENAI_BASE_URL + OPENAI_API_KEY")
