...
Lets the LLM write a Python script that calls Anakot tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `anakot_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `anakot_tools.py` with file-based RPC stubs
...
# available on every platform Anakot itself runs on.
...
# match a safe prefix, the operational ANAKOT_ allowlist, or (on Windows) an
# OS-essential name.
#
# NB: the broad "ANAKOT_" prefix was deliberately removed (#27303) — it leaked
# ANAKOT_*-named config that lacks a secret substring (e.g. ANAKOT_BASE_URL,
# ANAKOT_KANBAN_DB, ANAKOT_*_WEBHOOK).  The child only needs the few
# location/profile vars in _ANAKOT_CHILD_ALLOWED below; ANAKOT_RPC_SOCKET /
# ANAKOT_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational ANAKOT_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set anakot_cli treats as the
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational ANAKOT_* vars (_ANAKOT_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret ANAKOT_* vars dropped by the tightened allowlist (#27303). The
    # broad "ANAKOT_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like ANAKOT_KANBAN_DB / ANAKOT_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("ANAKOT_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped ANAKOT_* var.
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Anakot-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Anakot-profile soft guard."""',
...
def generate_anakot_tools_module(enabled_tools: List[str],
    ...
    """
    Build the source code for the anakot_tools.py stub module.
...
def _execute_remote(
    ...
    The script and the generated anakot_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/anakot_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_anakot_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/anakot_tools.py", tools_src)
        ...
        env_prefix = (
            f"ANAKOT_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("ANAKOT_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    # --- Set up temp directory with anakot_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="anakot_sandbox_")
    ...
    # selector from ANAKOT_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"anakot_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated anakot_tools module.
        ...
        tools_src = generate_anakot_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "anakot_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  ANAKOT_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        # Ensure the anakot-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from anakot_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _anakot_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _anakot_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # ANAKOT_TIMEZONE is an internal Anakot setting and must not leak
        # into child processes.
        _tz_name = os.getenv("ANAKOT_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("ANAKOT_TIMEZONE", None)

        # Per-profile HOME isolation: redirect system tool configs into
        # {ANAKOT_HOME}/home/ when that directory exists.
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.anakot/.env')).
...
def _load_config() -> dict:
    ...
    ``anakot --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (anakot-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``anakot tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and anakot-agent's own python.
    if mode == "strict":
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Anakot tools programmatically. "
        ...
        "Also available (no import needed — built into anakot_tools):\n"
