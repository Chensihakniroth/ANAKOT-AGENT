...
# Must match anakot_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
# (anakot-* prefixed), and scenario toolsets.
#
...
_SUBAGENT_TOOLSETS = sorted(
    ...
    and not name.startswith("anakot-")
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``anakot-cli`` (which bundles
...
def _build_child_agent(
    ...
    model on OpenRouter while the parent runs on callmemo Portal).
    """
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. anakot-cli) so that individual
    ...
    # Extract parent's API key so subagents inherit auth (e.g. callmemo Portal).
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.anakot/logs/subagent-<sid>-<ts>.log``
...
def _load_config() -> dict:
    ...
    to the persistent config (anakot_cli/config.py load_config()) so that
        ...
        "properties": {
            ...
            "acp_command": {
                ...
                "description": (
                    ...
                    "Leave empty to use the parent's default transport (Anakot subagents)."
                ),
            },
            "acp_args": {
                "type": "array",
