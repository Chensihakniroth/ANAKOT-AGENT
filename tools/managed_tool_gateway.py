"""Generic managed-tool gateway helpers for callmemo-hosted vendor passthroughs."""
...
_DEFAULT_TOOL_GATEWAY_DOMAIN = "callmemo.ai"
...
def peek_nous_access_token() -> Optional[str]:
    """Cheap probe for a callmemo gateway token without triggering refresh.

    Availability scans (`anakot tools`, banner/status paint, provider
...
def read_nous_access_token() -> Optional[str]:
    """Read a callmemo Subscriber OAuth access token from auth store or env override."""
    explicit = os.getenv("TOOL_GATEWAY_USER_TOKEN")
    if isinstance(explicit, str) and explicit.strip():
        return explicit.strip()
    nous_provider = _read_nous_provider_state() or {}
    ...
    try:
        ...
        logger.debug("callmemo access token refresh failed: %s", exc)
...
def is_managed_tool_gateway_ready(
    ...
    """Return True when gateway URL and a likely-usable callmemo token are present.
