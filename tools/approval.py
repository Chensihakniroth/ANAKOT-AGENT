...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return os.getenv("ANAKOT_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set ANAKOT_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind ANAKOT_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds ANAKOT_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("ANAKOT_CRON_SESSION"):
        return False
...
# via shell expansions like $HOME or $ANAKOT_HOME.
...
# ~/.anakot/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _ANAKOT_ENV_PATH; matches the ANAKOT_HOME override form as
# well as ~/.anakot/.
_ANAKOT_CONFIG_PATH = (
    r'(?:~\/\.anakot/|'
    r'(?:\$home|\$\{home\})/\.anakot/|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\banakot\s+gateway\s+(stop|restart)\b', "stop/restart anakot gateway (kills running agents)"),
    ...
    # that should always require user consent, just like `anakot gateway restart`
    ...
    # The name-based pattern above catches `pkill anakot` but not
    # `kill -9 $(pgrep -f anakot)` because the substitution is opaque
    ...
    # In-place edit of a Anakot-managed security file (~/.anakot/config.yaml or
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("ANAKOT_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set ANAKOT_INTERACTIVE or ANAKOT_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("ANAKOT_EXEC_ASK"):
