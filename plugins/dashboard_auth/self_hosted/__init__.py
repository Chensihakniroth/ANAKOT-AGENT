...
A standards-compliant OpenID Connect Relying Party for the ``anakot dashboard``
OAuth gate. Unlike the bundled ``nous`` provider (which encodes callmemo Portal's
...
:class:`~anakot_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``anakot_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~anakot_cli.dashboard_auth.Session`.
...
provider verifies its *access* token because callmemo Portal mints a custom JWT
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Anakot does not call
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``ANAKOT_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "ANAKOT_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(ANAKOT_DASHBOARD_OIDC_ISSUER + ANAKOT_DASHBOARD_OIDC_CLIENT_ID) "
