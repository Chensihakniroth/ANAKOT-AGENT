...
into the same ``DashboardAuthProvider`` framework as the callmemo OAuth
...
mirroring the callmemo provider's precedence convention:
  ...
  Environment overrides::
      ...
      ANAKOT_DASHBOARD_BASIC_AUTH_PASSWORD_HASH   # preferred
      ANAKOT_DASHBOARD_BASIC_AUTH_PASSWORD        # plaintext fallback
...
Skip reasons:
  Like the callmemo provider, this exposes a module-level ``LAST_SKIP_REASON``
...
def _resolve_secret(cfg_section: dict) -> bytes:
    ...
    """
    raw = _resolve(
        "ANAKOT_DASHBOARD_BASIC_AUTH_SECRET", cfg_section, "secret"
    )
    if not raw:
        logger.info(
            ...
            "secret (or ANAKOT_DASHBOARD_BASIC_AUTH_SECRET) for stable "
            "sessions."
        )
...
def register(ctx) -> None:
    ...
    username = _resolve(
        "ANAKOT_DASHBOARD_BASIC_AUTH_USERNAME", section, "username"
    )
    password_hash = _resolve(
        "ANAKOT_DASHBOARD_BASIC_AUTH_PASSWORD_HASH", section, "password_hash"
    )
    plaintext = _resolve(
        "ANAKOT_DASHBOARD_BASIC_AUTH_PASSWORD", section, "password"
    )
    ttl_raw = _resolve(
        "ANAKOT_DASHBOARD_BASIC_AUTH_TTL_SECONDS", section, "session_ttl_seconds"
    )

    if not username:
        LAST_SKIP_REASON = (
            "dashboard.basic_auth.username is not set (and "
            "ANAKOT_DASHBOARD_BASIC_AUTH_USERNAME is empty). Set a username "
    ...
    # ANAKOT_DASHBOARD_BASIC_AUTH_PASSWORD env var overrides a config.yaml
