"""NousDashboardAuthProvider — callmemo Portal OAuth (authorization-code + PKCE).
...
Configuration surfaces (env wins over config.yaml when set non-empty):
  ...
  per-deploy values don't need to bake into ``config.yaml``:
      ...
      ANAKOT_DASHBOARD_OAUTH_CLIENT_ID  — shape ``agent:{agent_instance_id}``
      ANAKOT_DASHBOARD_PORTAL_URL       — defaults to
                                          ``https://portal.callmemo.ai``
...
Key contract points encoded here:
  ...
  - audience claim is the bare ``client_id`` (no ``anakot-cli:`` prefix).
...
"state": …}`` and the route serializes those into the ``anakot_session_pkce``
cookie.
...
Skip reasons:
  ...
  ("Set ANAKOT_DASHBOARD_OAUTH_CLIENT_ID …") instead of the bare "no
...
class NousDashboardAuthProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        # The auth-route layer expects ``cookie_payload[\"anakot_session_pkce\"]``
        ...
        cookie_payload = {
            "anakot_session_pkce": f"state={state};verifier={code_verifier}",
        }
    ...
    def complete_login(
        ...
        *,
        ...
        # we just receive it for symmetry with the protocol. callmemo Portal
    ...
    def _verify_jwt(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # between ANAKOT_DASHBOARD_PORTAL_URL / ANAKOT_DASHBOARD_OAUTH_CLIENT_ID
    ...
    def _check_contract_version(self, claims: Dict[str, Any]) -> None:
        ...
        if contract_version is None:
            logger.warning(
                "callmemo Portal token missing oauth_contract_version claim "
...
def _resolve_client_id() -> str:
    ...
    Order:
      1. ``ANAKOT_DASHBOARD_OAUTH_CLIENT_ID`` env var (when non-empty
    ...
    """
    env = os.environ.get("ANAKOT_DASHBOARD_OAUTH_CLIENT_ID", "").strip()
    if env:
        return env
...
def _resolve_portal_url() -> str:
    ...
    Order:
      1. ``ANAKOT_DASHBOARD_PORTAL_URL`` env var (non-empty after strip).
      2. ``dashboard.oauth.portal_url`` in ``config.yaml``.
      3. :data:`_DEFAULT_PORTAL_URL` (production Portal).
    """
    env = os.environ.get("ANAKOT_DASHBOARD_PORTAL_URL", "").strip()
    if env:
        return env
    cfg_value = str(
...
def register(ctx) -> None:
    ...
    configured (either via ``ANAKOT_DASHBOARD_OAUTH_CLIENT_ID`` env var
    ...
    can surface "Set ANAKOT_DASHBOARD_OAUTH_CLIENT_ID …" instead of the
    ...
    engagement layer (``anakot_cli.web_server.should_require_auth`` +
    ...
    if not client_id:
        LAST_SKIP_REASON = (
            "ANAKOT_DASHBOARD_OAUTH_CLIENT_ID is not set (and "
            "dashboard.oauth.client_id in config.yaml is empty). The "
            "callmemo Portal provisions this env var (shape "
            "'agent:{instance_id}') when it deploys a Anakot Agent "
    ...
    if not client_id.startswith("agent:"):
        LAST_SKIP_REASON = (
            f"ANAKOT_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match "
            f"the contract shape 'agent:{{instance_id}}'. The callmemo Portal "
