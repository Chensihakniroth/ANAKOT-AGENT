...
def _xai_user_agent() -> str:
    try:
        ...
        return anakot_xai_user_agent()
    except Exception:
        return "anakot-agent/video_gen"
...
class XAIVideoGenProvider(VideoGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        # hook (``anakot_cli/tools_config.py``) so the picker doesn't blindly
    ...
    async def _generate_async(
        ...
        *,
        ...
        if not api_key:
            return error_response(
                error=(
                    "No xAI credentials found. Sign in via `anakot auth add xai-oauth` "
