...
Authentication via ``FAL_KEY`` or the managed callmemo gateway. Output is an
...
def _submit_fal_video_request(endpoint: str, arguments: Dict[str, Any]):
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            raise ValueError(
                f"callmemo Subscription gateway rejected endpoint '{endpoint}' "
                f"(HTTP {status}). This model may not yet be enabled on "
                f"the callmemo Portal's FAL proxy. Either:\n"
                f"  • Set FAL_KEY in your environment to use FAL.ai directly, or\n"
                f"  • Pick a different model via `anakot tools` → Video Generation."
            ) from exc
        raise
        ...
        if not _check_fal_video_available():
            return error_response(
                error=(
                    ...
                    "(run `anakot tools` → Video Generation → FAL to configure) "
                    "or sign in to callmemo (`anakot setup`) for managed gateway access."
                ),
            ...
            if not endpoint:
                return error_response(
                    error=(
                        ...
                        f"via `anakot tools` → Video Generation."
                    ),
