# Anakot Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: anakot-achievements plugin
...
### `GET /api/plugins/anakot-achievements/achievements`
...
### `POST /api/plugins/anakot-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/anakot-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/anakot-achievements/overview`
...
- Persist plugin data under:
  - `~/.anakot/plugins/anakot-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.anakot/plugins/anakot-achievements/scan_checkpoint.json`
...
- `~/.anakot/plugins/anakot-achievements/`
