/**
 * Anakot Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.anakot/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__ANAKOT_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const FALLBACK_COLUMN_HELP = {
    ...
    scheduled: "Time-based follow-ups",
    ready: "Dependencies satisfied; assign a profile to dispatch",
    running: "Claimed by a worker — in-flight",
    blocked: "Worker asked for human input",
    review: "Awaiting human review before done",
    done: "Completed",
    archived: "Archived",
  };
  ...
  const COLUMN_DOT = {
    triage: "anakot-kanban-dot-triage",
    todo: "anakot-kanban-dot-todo",
    scheduled: "anakot-kanban-dot-scheduled",
    ready: "anakot-kanban-dot-ready",
    running: "anakot-kanban-dot-running",
    blocked: "anakot-kanban-dot-blocked",
    review: "anakot-kanban-dot-review",
    done: "anakot-kanban-dot-done",
    archived: "anakot-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-anakot-task";
  ...
  const DOCS_URL = "https://anakot-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://anakot-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "anakot.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="anakot-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "anakot-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "anakot-kanban-md",
  ...
  // standard `drop` event and our `anakot-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("anakot-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("anakot-kanban-column--drop");
          if (target) target.classList.add("anakot-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("anakot-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("anakot-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("anakot-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__ANAKOT_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "anakot-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "anakot-kanban-attention",
        "anakot-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "anakot-kanban-attention-bar" },
        h("span", { className: "anakot-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "anakot-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "anakot-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "anakot-kanban-attention-row",
                  "anakot-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "anakot-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "anakot-kanban-attention-row-id" }, task.id),
                h("span", { className: "anakot-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "anakot-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "anakot-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "anakot-kanban-diag-action-btn",
      action.suggested ? "anakot-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " anakot-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".anakot-kanban-drawer-comment-row input, .anakot-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "anakot-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("anakot-kanban-diag", sevClass) },
      h("div", { className: "anakot-kanban-diag-header" },
        h("span", { className: "anakot-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "anakot-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "anakot-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "anakot-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "anakot-kanban-diag-data-row" },
                  h("span", { className: "anakot-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "anakot-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "anakot-kanban-diag-data-row" },
                h("span", { className: "anakot-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "anakot-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "anakot-kanban-diag-reassign-row" },
            h("span", { className: "anakot-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "anakot-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "anakot-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "anakot-kanban-diag-msg",
              msg.ok ? "anakot-kanban-diag-msg--ok" : "anakot-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "anakot-kanban-section" },
      h("div", { className: "anakot-kanban-section-head-row" },
        h("span", { className: "anakot-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "anakot-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "anakot-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "anakot-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "anakot-kanban-docs-link",
      title: "Open Anakot Kanban docs in a new tab",
      "aria-label": "Anakot Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `anakot kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "anakot-kanban-msg-ok" : "anakot-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "anakot-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "anakot-kanban-boardswitcher" },
      h("div", { className: "anakot-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "anakot-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "anakot-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "anakot-kanban-dialog-title" },
        ...
        h("div", { className: "anakot-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Anakot profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "anakot-kanban-bulk" },
      h("span", { className: "anakot-kanban-bulk-count" },
      ...
      h("div", { className: "anakot-kanban-bulk-priority",
      ...
      h("div", { className: "anakot-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Anakot profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "anakot-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("anakot-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("anakot-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "anakot-kanban-trash",
        dragOver ? "anakot-kanban-trash--drop" : "",
        props.draggingTaskId ? "anakot-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "anakot-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "anakot-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".anakot-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "anakot-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("anakot-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("anakot-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "anakot-kanban-column",
        dragOver ? "anakot-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "anakot-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "anakot-kanban-col-check",
        ...
        h("span", { className: cn("anakot-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "anakot-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "anakot-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "anakot-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "anakot-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "anakot-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "anakot-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "anakot-kanban-lane" },
                  h("div", { className: "anakot-kanban-lane-head" },
                    h("span", { className: "anakot-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "anakot-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "anakot-kanban-card--stale-red";
    if (age >= tier.amber) return "anakot-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".anakot-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "anakot-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "anakot-kanban-card",
        props.selected ? "anakot-kanban-card--selected" : "",
        props.failed ? "anakot-kanban-card--failed" : "",
        props.draggingSource ? "anakot-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "anakot-kanban-card-content" },
          h("div", { className: "anakot-kanban-card-row" },
            h("label", {
              className: "anakot-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "anakot-kanban-card-check",
            ...
            h("span", { className: "anakot-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or anakot kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "anakot-kanban-warning-badge",
                    "anakot-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "anakot-kanban-needs-assignee",
          ...
          h("div", { className: "anakot-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "anakot-kanban-card-row anakot-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "anakot-kanban-assignee",
                            title: `Assigned to Anakot profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "anakot-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "anakot-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "anakot-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "anakot-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Anakot profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Anakot profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "anakot-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "anakot-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "anakot-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "anakot-kanban-drawer-close",
        ...
        data ? h("div", { className: "anakot-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "anakot-kanban-section" },
      h("div", { className: "anakot-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "anakot-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "anakot-kanban-drawer-body" },
      h("div", { className: "anakot-kanban-drawer-title" },
        h("span", { className: cn("anakot-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "anakot-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "anakot-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "anakot-kanban-section" },
        h("div", { className: "anakot-kanban-section-head" }, tx(i18n, "result", "Result")),
      ...
      h("div", { className: "anakot-kanban-section" },
        h("div", { className: "anakot-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "anakot-kanban-comment" },
                h("div", { className: "anakot-kanban-comment-head" },
                  h("span", { className: "anakot-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "anakot-kanban-comment-ago" },
      ...
      h("div", { className: "anakot-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "anakot-kanban-event",
              isDiag ? "anakot-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "anakot-kanban-event-header" },
                  h("span", { className: "anakot-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "anakot-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "anakot-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "anakot-kanban-event-header-plain" },
                  h("span", { className: "anakot-kanban-event-kind" }, e.kind),
                  h("span", { className: "anakot-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "anakot-kanban-event-phantom-row" },
                  h("span", { className: "anakot-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "anakot-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "anakot-kanban-event-payload" },
    ...
    return h("div", { className: "anakot-kanban-section" },
      h("div", { className: "anakot-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "anakot-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `anakot-kanban-run--${r.outcome || r.status || "ended"}`
          : "anakot-kanban-run--active";
        return h("div", { key: r.id, className: cn("anakot-kanban-run", outcomeClass) },
          h("div", { className: "anakot-kanban-run-head" },
            h("span", { className: "anakot-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "anakot-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "anakot-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "anakot-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "anakot-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "anakot-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "anakot-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "anakot-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "anakot-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "anakot-kanban-pre anakot-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "anakot-kanban-meta-row" },
      h("span", { className: "anakot-kanban-meta-label" }, props.label),
      h("span", { className: "anakot-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "anakot-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "anakot-kanban-meta-row" },
        h("span", { className: "anakot-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "anakot-kanban-meta-value anakot-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "anakot-kanban-meta-row" },
        h("span", { className: "anakot-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "anakot-kanban-meta-value anakot-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "anakot-kanban-section" },
      h("div", { className: "anakot-kanban-section-head-row" },
        h("span", { className: "anakot-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "anakot-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "anakot-kanban-section" },
      h("div", { className: "anakot-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "anakot-kanban-deps-row" },
        h("span", { className: "anakot-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "anakot-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "anakot-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "anakot-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "anakot-kanban-dep-chip-x",
      ...
      h("div", { className: "anakot-kanban-deps-row" },
        h("span", { className: "anakot-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "anakot-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "anakot-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "anakot-kanban-msg-ok"
          : "anakot-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "anakot-kanban-section" },
      ...
      h("div", { className: "anakot-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "anakot-kanban-home-sub anakot-kanban-home-sub--on"
              : "anakot-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__ANAKOT_PLUGINS__ && typeof window.__ANAKOT_PLUGINS__.register === "function") {
    window.__ANAKOT_PLUGINS__.register("kanban", KanbanPage);
  }
})();
