"""SimpleX Chat platform adapter (Anakot plugin).
...
This adapter ships as a Anakot platform plugin under
``plugins/platforms/simplex/``. The Anakot plugin loader scans the
...
discoverable and `anakot setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Anakot — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"anakot-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``anakot setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.anakot/.env`` via ``anakot_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("anakot_cli.config not available; set SIMPLEX_* vars manually in ~/.anakot/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `anakot gateway status` without
