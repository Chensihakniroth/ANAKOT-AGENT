"""
Microsoft Teams platform adapter for Anakot Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def _on_card_action(
        ...
        choice = choice_map.get(anakot_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="anakot_approve",
                    data={**btn_data_base, "anakot_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="anakot_approve",
                    data={**btn_data_base, "anakot_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="anakot_approve",
                    data={**btn_data_base, "anakot_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="anakot_approve",
                    data={**btn_data_base, "anakot_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Anakot\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
