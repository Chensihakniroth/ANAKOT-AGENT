...
# ``model_tools`` import time, so ``anakot status``, ``anakot chat``, etc.
...
# ``anakot_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Anakot is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a anakot MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${ANAKOT_HOME}/google_chat_thread_counts.json so
        ...
        # they don't sit in the chat forever as "Anakot is thinking…".
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${ANAKOT_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Anakot is thinking…' marker message.
        ...
        "Anakot is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Anakot is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Anakot is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Anakot is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Anakot is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``anakot setup``.

    The setup wizard at ``anakot_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.anakot/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. anakot-chat-events) and a Pull subscription.")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``anakot cron`` running as a
    separate process from ``anakot gateway``).  Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'anakot-agent[google_chat]'",
        ...
        # the "Anakot is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Anakot is thinking…' marker message "
