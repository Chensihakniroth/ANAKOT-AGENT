...
- Per-user tokens (keyed by sender email):
    ``${ANAKOT_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${ANAKOT_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${ANAKOT_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${ANAKOT_HOME}/google_chat_user_oauth_pending.json``
- OAuth client secret (profile-scoped — each profile registers its own):
    ``${ANAKOT_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's ANAKOT_HOME helper so the token follows the user's
# profile (e.g. tests can override via ANAKOT_HOME=/tmp/...).
try:
    from anakot_constants import display_anakot_home, get_anakot_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where anakot_constants isn't importable
    # (mirrors the same fallback used by the google-workspace skill's
    # _anakot_home.py shim).
...
def _anakot_home() -> Path:
    """Resolve ANAKOT_HOME at call time (NOT module import).

    Tests and ``ANAKOT_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.anakot/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _anakot_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _anakot_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _anakot_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _anakot_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _anakot_home() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install anakot-agent[google_chat]."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'anakot-agent[google_chat]'")
        return False
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_anakot_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_anakot_home()}/google_chat_user_token.json"
    )
