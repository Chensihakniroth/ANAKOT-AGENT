"""
IRC Platform Adapter for Anakot Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Anakot agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "anakot-bot")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: anakot_, anakot_1, anakot_2...
...
def check_requirements() -> bool:
    ...
    # anakot setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `anakot gateway setup` flow for the IRC platform.

    Lazy-imports ``anakot_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Anakot to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #anakot — comma-separate for multiple)",
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "anakot-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "anakot-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Anakot Agent (cron)")
