"""
LINE Messaging API platform adapter for Anakot Agent.
...
to Anakot Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``ANAKOT_HOME``. PR #8398.
        """
        from aiohttp import web
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``anakot status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``anakot setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.anakot/.env`` via ``anakot_cli.config``.
    """
    print()
    ...
    try:
        from anakot_cli.config import get_env_var, set_env_var
    except ImportError:
        print("anakot_cli.config not available; set LINE_* vars manually in ~/.anakot/.env")
        return
