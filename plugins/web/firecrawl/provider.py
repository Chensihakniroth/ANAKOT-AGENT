...
Env vars::
    ...
    FIRECRAWL_GATEWAY_URL=...        # callmemo tool-gateway (subscribers)
...
def _is_tool_gateway_ready() -> bool:
    """Return True when gateway URL + callmemo Subscriber token are available.
...
def check_firecrawl_api_key() -> bool:
    ...
    existing tests and the ``anakot tools`` setup flow.
    """
...
def _firecrawl_backend_help_suffix() -> str:
    ...
    if not _wt.managed_callmemo_tools_enabled():
        return ""
    return (
        ", or use the callmemo Tool Gateway via your subscription "
...
def _raise_web_backend_configuration_error() -> None:
    ...
    if _wt.managed_callmemo_tools_enabled():
        message += (
            " With your callmemo subscription you can also use the Tool Gateway. "
            "run `anakot tools` and select callmemo Subscription as the web provider."
        )
    else:
