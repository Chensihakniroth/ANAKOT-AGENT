...
``{title, url, description, position}`` rows every other Anakot web
provider produces.
...
prefers Anakot-managed xAI Grok OAuth (via ``anakot auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.anakot/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``anakot tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `anakot auth` to sign in with "
