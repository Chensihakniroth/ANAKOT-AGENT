"""langfuse — Anakot plugin for Langfuse observability.

Traces Anakot conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Anakot plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``anakot plugins enable
observability/langfuse`` or ``anakot tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``anakot tools`` or ~/.anakot/.env):
  ANAKOT_LANGFUSE_PUBLIC_KEY  - Langfuse project public key (pk-lf-...)
  ANAKOT_LANGFUSE_SECRET_KEY  - Langfuse project secret key (sk-lf-...)
  ANAKOT_LANGFUSE_BASE_URL    - Langfuse server URL (default: https://cloud.langfuse.com)

Optional env vars:
  ANAKOT_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  ANAKOT_LANGFUSE_RELEASE     - release/version tag
  ANAKOT_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  ANAKOT_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  ANAKOT_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
def _debug_enabled() -> bool:
    return _env_bool("ANAKOT_LANGFUSE_DEBUG")
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Anakot plugin system —
    ...
    public_key = _env("ANAKOT_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
    secret_key = _env("ANAKOT_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
    ...
    base_url = _env("ANAKOT_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
    environment = _env("ANAKOT_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
    release = _env("ANAKOT_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
    sample_rate = _env("ANAKOT_LANGFUSE_SAMPLE_RATE")
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Anakot branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Anakot also has a turn-scoped
    ...
    # messages is a list only for legacy Anakot branches that fired
    # pre_llm_call with API messages directly. Current Anakot fires
...
def register(ctx) -> None:
    ...
    # Anakot versions.  pre_api_request / post_api_request fire per API
