...
Optional Anakot observability plugin that maps Anakot observer hooks to
...
not replace Anakot Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Anakot emit NeMo Relay lifecycle events for
the work Anakot already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Anakot Agent can:

- Preserve Anakot execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
anakot plugins enable observability/nemo_relay
```

The `ANAKOT_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `ANAKOT_HOME` that the
agent run will use:

```bash
env ANAKOT_HOME=/tmp/anakot-nemo-relay-test \
  anakot plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`ANAKOT_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`ANAKOT_HOME` is the Anakot profile/config home used by both
`anakot plugins enable ...` and the later `anakot chat ...` run. If unset,
Anakot uses the user's default home, usually `~/.anakot`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export ANAKOT_HOME=/tmp/anakot-nemo-relay-test
anakot plugins enable observability/nemo_relay
anakot chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `anakot` command you run is built from the
...
uv run anakot plugins enable observability/nemo_relay
uv run anakot chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/anakot_agent-*.whl
...
The plugin can configure exporters directly from `ANAKOT_NEMO_RELAY_*`
...
```bash
export ANAKOT_NEMO_RELAY_ATOF_ENABLED=1
export ANAKOT_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export ANAKOT_NEMO_RELAY_ATIF_ENABLED=1
export ANAKOT_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `ANAKOT_NEMO_RELAY_ATOF_FILENAME`
- `ANAKOT_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
...
- `ANAKOT_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Anakot at it:

```bash
export ANAKOT_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `ANAKOT_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`ANAKOT_NEMO_RELAY_ATOF_*` fallback setup is skipped.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Anakot routes LLM and tool
...
For the full generic Anakot middleware contract, see
...
export ANAKOT_HOME=/tmp/anakot-nemo-relay-docs/anakot-home
mkdir -p "$ANAKOT_HOME"

cat > "$ANAKOT_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Anakot session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export ANAKOT_NEMO_RELAY_ATOF_ENABLED=1
export ANAKOT_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/anakot-nemo-relay-docs/subagent/atof
export ANAKOT_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export ANAKOT_NEMO_RELAY_ATOF_MODE=overwrite
export ANAKOT_NEMO_RELAY_ATIF_ENABLED=1
export ANAKOT_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/anakot-nemo-relay-docs/subagent/atif
export ANAKOT_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export ANAKOT_NEMO_RELAY_ATIF_AGENT_NAME='Anakot Agent E2E'
export ANAKOT_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export ANAKOT_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"anakot.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Anakot Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Anakot dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/anakot-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/anakot-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/anakot-nemo-relay-docs/workdir/beta.txt
cd /tmp/anakot-nemo-relay-docs/workdir

export ANAKOT_NEMO_RELAY_ATOF_ENABLED=1
export ANAKOT_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/anakot-nemo-relay-docs/parallel/atof
export ANAKOT_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export ANAKOT_NEMO_RELAY_ATOF_MODE=overwrite
export ANAKOT_NEMO_RELAY_ATIF_ENABLED=1
export ANAKOT_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/anakot-nemo-relay-docs/parallel/atif
export ANAKOT_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export ANAKOT_NEMO_RELAY_ATIF_AGENT_NAME='Anakot Agent E2E'
export ANAKOT_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Anakot sessions map to `agent` scopes.
- Anakot API request hooks map to `llm` scope start/end events.
- Anakot tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Anakot execution middleware to hand
...
Enable it for Anakot:

```bash
export ANAKOT_NEMO_RELAY_PLUGINS_TOML=/tmp/anakot-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Anakot routes execution
through these boundaries:

```text
Anakot provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Anakot provider adapter next_call(...)

Anakot tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Anakot tool dispatcher next_call(...)
```
...
middleware for a local Anakot run.

```bash
pip install "nemo-relay==0.3"

export ANAKOT_HOME=/tmp/anakot-middleware-test/anakot-home
mkdir -p "$ANAKOT_HOME" /tmp/anakot-middleware-test/nemo-relay
...
cat > /tmp/anakot-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/anakot-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/anakot-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Anakot Middleware E2E"
agent_version = "local"
...
export ANAKOT_NEMO_RELAY_PLUGINS_TOML=/tmp/anakot-middleware-test/nemo-relay/plugins.toml
