...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: anakot)
...
Or via $ANAKOT_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Anakot can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $ANAKOT_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_anakot_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "anakot": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "anakot"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Anakot config."""
    profile = config.get("profile", "anakot")
    return str(profile or "anakot")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Anakot profile name (from agent_identity)
      {workspace} — Anakot workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``anakot-{user}`` with no user becomes ``anakot``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Anakot Agent and the User"
    ...
    def save_config(self, values, anakot_home):
        """Write config to $ANAKOT_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(anakot_home) / "hindsight"
    ...
    def post_setup(self, anakot_home: str, config: dict) -> None:
        ...
        from anakot_cli.memory_setup import _curses_select
        ...
        self.save_config(provider_config, anakot_home)
        ...
        if mode == "local_embedded":
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(anakot_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "anakot"), llm_provider)
                kwargs = dict(
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "anakot", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "anakot")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Anakot Agent and the User")
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Anakot created/used that client on the shared async loop,
