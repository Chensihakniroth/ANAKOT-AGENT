...
Handles: anakot honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "anakot" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Anakot profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'anakot honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'anakot honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `anakot update` -- no output, no exceptions.
    """
    try:
        from anakot_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Anakot profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $ANAKOT_HOME/honcho.json so each profile/instance gets
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        anakot_host.get("pinUserPeer"),
        anakot_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in anakot_host:
        aliases_src = anakot_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in anakot_host:
        prefix_src = anakot_host.get("runtimePeerPrefix")
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Anakot persistent cross-session memory.")
    ...
    anakot_host = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'anakot honcho setup' again once you have a key.\n")
            return
    ...
    if new_peer:
        anakot_host["peerName"] = new_peer

    current_ai = anakot_host.get("aiPeer") or cfg.get("aiPeer", "anakot")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        anakot_host["aiPeer"] = new_ai

    current_workspace = anakot_host.get("workspace") or cfg.get("workspace", "anakot")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        anakot_host["workspace"] = new_workspace
    ...
    if new_shape == "single":
        ...
        anakot_host["pinPeerName"] = True
        print(f"  pinPeerName=true → all gateway users route to '{anakot_host.get('peerName', '?')}'.")
        ...
        if prior_aliases:
            anakot_host["userPeerAliases"] = prior_aliases
        ...
        if _new_prefix and not (prefix_from_root and _new_prefix == current_prefix):
            anakot_host["runtimePeerPrefix"] = _new_prefix
        print("  Multi-user mode: each runtime ID → own peer. Use 'anakot honcho status' to inspect.")
        ...
        if existing_aliases:
            anakot_host["userPeerAliases"] = existing_aliases
        _prefix_default = current_prefix or ""
        _new_prefix = _prompt(
    ...
    # --- 4. Observation mode ---
    current_obs = anakot_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        anakot_host["observationMode"] = new_obs
    else:
        anakot_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(anakot_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        anakot_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        anakot_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = anakot_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        anakot_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = anakot_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    current_dialectic = str(anakot_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    current_reasoning = (
        anakot_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        anakot_host["dialecticReasoningLevel"] = new_reasoning
    else:
        anakot_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = anakot_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        anakot_host["sessionStrategy"] = new_strat
    ...
    try:
        ...
        print("  Run: anakot config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    anakot honcho status     -- show full config")
    print("    anakot honcho mode       -- change recall/observation mode")
    print("    anakot honcho tokens     -- tune context and dialectic budgets")
    print("    anakot honcho peer       -- update peer names")
    print("    anakot honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Anakot profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    if not cfg:
        ...
        print("  Run 'anakot honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: anakot honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        anakot = hosts.get(_host_key(), {})
        user = anakot.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = anakot.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = anakot.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = anakot.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Anakot' identity in Honcho. Seed with 'anakot honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: anakot honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: anakot honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = anakot.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = anakot.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = anakot.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Anakot asks Honcho's AI peer a question")
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'anakot honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'anakot honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    anakot honcho identity --show        — show both peer representations")
        print("    anakot honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    ...
    if has_key:
        ...
        print("  2. Run:  anakot honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'anakot honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'anakot honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  anakot honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'anakot'")
        print("  with Honcho configured and no prior session history.")
        print("  (Anakot calls migrate_memory_files() on first session init.)")
        print()
    ...
    print()
    print("  In Anakot, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            for f in agent_files:
                print(f"    anakot honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  anakot honcho identity <path/to/SOUL.md>")
    ...
    print("    Anakot:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Anakot:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Anakot:   Honcho observes every message and updates representations")
    ...
    print("    Anakot:   per-session by default — each run gets its own session")
    print("              Map a custom name:  anakot honcho map <session-name>")
    ...
    if not has_key:
        ...
        print("  2. anakot honcho migrate            — re-run this walkthrough")
    else:
        print("  1. anakot honcho status             — verify Honcho connection")
        print("  2. anakot                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. anakot honcho identity --show    — verify AI peer representation")
        print("  4. anakot honcho tokens             — tune context and dialectic budgets")
        print("  5. anakot honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'anakot memory setup'...\n")
...
def register_cli(subparser) -> None:
    """Build the ``anakot honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``anakot honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to anakot memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Anakot Honcho",
    )
