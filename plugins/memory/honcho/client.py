...
Resolution order for config file:
  1. $ANAKOT_HOME/honcho.json  (instance-local, enables isolated Anakot instances)
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Anakot profile.

    Resolution order:
      1. ANAKOT_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``anakot.<profile>``
      3. Fallback: ``"anakot"`` (default profile)
    """
    explicit = os.environ.get("ANAKOT_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $ANAKOT_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.anakot/honcho.json          (default profile — shared host blocks live here)
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Anakot system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.anakot
    ...
    def from_global_config(
        ...
        Resolution: $ANAKOT_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Anakot profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.anakot block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Anakot session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — Anakot session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Anakot session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    # the "go run anakot honcho setup" hint they used to.
    try:
    ...
    if not resolved_base_url or resolved_timeout is None:
        try:
            ...
            anakot_cfg = load_config()
            honcho_cfg = anakot_cfg.get("honcho", {})
