...
**Deployment shapes** (`anakot memory setup honcho` asks one prompt to set these):

- **Single-operator** — `pinUserPeer: true`. All gateway users → `peerName`. Recommended for personal use where you connect Anakot to your own Telegram/Discord/etc.
...
| 4 | `per-session` strategy | Anakot session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`anakot-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`anakot`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-anakot-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening anakot in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
