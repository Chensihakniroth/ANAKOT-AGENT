...
``_check_spotify_available()`` — when the user has not run ``anakot auth
spotify``, the tools remain registered (so they appear in ``anakot
...
The Spotify auth flow (``anakot auth spotify``), CLI plumbing, and docs
