...
Launched by ``anakot meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$ANAKOT_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``anakot meet node approve <name> <url> <token>``.
...
def _default_token_path() -> Path:
    return Path(get_anakot_home()) / "workspace" / "meetings" / "node_token.json"
...
class NodeServer:
    ...
    def __init__(
        ...
        display_name: str = "anakot-meet-node",
    ) -> None:
        self.host = host
        self.port = port
