...
``$ANAKOT_HOME/workspace/meetings/<meeting-id>/``. The main anakot process
...
Run standalone for debugging::
    ...
    ANAKOT_MEET_URL=https://meet.google.com/abc-defg-hij \\
    ANAKOT_MEET_OUT_DIR=/tmp/meet-debug \\
    ANAKOT_MEET_HEADED=1 \\
...
# Filenames the bot reads/writes in ``ANAKOT_MEET_OUT_DIR``.
...
# and exposes ``window.__anakotMeetDrain()`` to pull new entries. This
...
def _start_realtime_speaker(
    *,
    ...
    if platform_tag == "linux":
        ...
        sink = (bridge_info or {}).get("write_target") or "anakot_meet_sink"
        try:
            proc = _sp.Popen(
                [
                    "paplay",
...
def run_bot() -> int:  # noqa: C901 — orchestration, explicit branches
    url = os.environ.get("ANAKOT_MEET_URL", "").strip()
    out_dir_env = os.environ.get("ANAKOT_MEET_OUT_DIR", "").strip()
    headed = os.environ.get("ANAKOT_MEET_HEADED", "").lower() in {"1", "true", "yes"}
    auth_state = os.environ.get("ANAKOT_MEET_AUTH_STATE", "").strip()
    guest_name = os.environ.get("ANAKOT_MEET_GUEST_NAME", "Anakot Agent")
    duration_s = _parse_duration(os.environ.get("ANAKOT_MEET_DURATION", ""))
    # v2: optional realtime mode. Enabled when ANAKOT_MEET_MODE=realtime.
    mode = os.environ.get("ANAKOT_MEET_MODE", "transcribe").strip().lower()
    realtime_model = os.environ.get("ANAKOT_MEET_REALTIME_MODEL", "gpt-realtime")
    realtime_voice = os.environ.get("ANAKOT_MEET_REALTIME_VOICE", "alloy")
    realtime_instructions = os.environ.get("ANAKOT_MEET_REALTIME_INSTRUCTIONS", "")
    realtime_api_key = os.environ.get("ANAKOT_MEET_REALTIME_KEY") or os.environ.get("OPENAI_API_KEY", "")

    if not url or not _is_safe_meet_url(url):
        sys.stderr.write(
            "google_meet bot: refusing to launch — ANAKOT_MEET_URL must be a "
    ...
    try:
        with sync_playwright() as pw:
            ...
            lobby_deadline = time.time() + float(
                os.environ.get("ANAKOT_MEET_LOBBY_TIMEOUT", "300")
            )
...
def _detect_admission(page) -> bool:
    ...
    (() => {
      ...
      if (window.__anakotMeetInstalled) {
