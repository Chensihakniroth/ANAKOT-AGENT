...
session-scoped state file under ``$ANAKOT_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $ANAKOT_HOME):
#
#   workspace/meetings/
...
def start(
    ...
    *,
    ...
    If a bot is already running for this anakot install, leave it first —
    ...
    env = os.environ.copy()
    env["ANAKOT_MEET_URL"] = url
    env["ANAKOT_MEET_OUT_DIR"] = str(out)
    env["ANAKOT_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["ANAKOT_MEET_HEADED"] = "1"
    if auth_state:
        env["ANAKOT_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["ANAKOT_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if ANAKOT_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["ANAKOT_MEET_MODE"] = mode
    if realtime_model:
        env["ANAKOT_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["ANAKOT_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["ANAKOT_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
