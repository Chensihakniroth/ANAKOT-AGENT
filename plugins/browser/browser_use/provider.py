...
``BROWSER_USE_API_KEY`` for self-billed users, or the managed callmemo tool
gateway (which Anakot uses to bill Browser Use sessions to a callmemo
...
Auth env vars (one of)::
    ...
    # OR a managed callmemo gateway entry (configured via 'anakot setup')
"""
...
# Idempotency tracking for managed-mode session creation. The managed callmemo
...
class BrowserUseBrowserProvider(BrowserProvider):
    ...
    to the managed callmemo tool gateway when ``tool_gateway.browser`` config
    ...
    # ------------------------------------------------------------------
    # Config resolution (direct API key OR managed callmemo gateway)
    # ------------------------------------------------------------------

    def _get_config_or_none(self, *, refresh_token: bool = True) -> Optional[Dict[str, Any]]:
        # Import here to avoid a hard dependency at module-import time —
        # managed_tool_gateway pulls in the callmemo auth stack which can be
        ...
        # managed callmemo gateway via ``tool_gateway.browser: gateway``.
    ...
    def create_session(self, task_id: str) -> Dict[str, object]:
        ...
        # default to a long Browser-Use timeout when Anakot only needs a task-
        ...
        if managed_mode:
            _clear_pending_create_key(task_id)
        session_name = f"anakot_{task_id}_{uuid.uuid4().hex[:8]}"
        external_call_id = (
