...
function Test-Node {
    ...
    try {
        ...
        if ($zipName) {
            ...
            if ($extractedDir) {
                if (Test-Path "$AnakotHome\node") { Remove-Item -Recurse -Force "$AnakotHome\node" }
                Move-Item $extractedDir.FullName "$AnakotHome\node"
...
function Set-PathVariable {
    ...
    if ($NoVenv) {
        ...
        $anakotBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so anakot is globally available
    # On Windows, the anakot.exe in venv\Scripts\ has the venv Python baked in
...
function Copy-ConfigTemplates {
    ...
    # Create ~/.anakot directory structure
    ...
    if (-not (Test-Path $envPath)) {
        ...
        if (Test-Path $examplePath) {
            ...
            Write-Success "Created ~/.anakot/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.anakot/.env"
        }
    } else {
        Write-Info "~/.anakot/.env already exists, keeping it"
    }
    ...
    if (-not (Test-Path $configPath)) {
        ...
        if (Test-Path $examplePath) {
            ...
            Write-Success "Created ~/.anakot/config.yaml from template"
        }
    } else {
        Write-Info "~/.anakot/config.yaml already exists, keeping it"
    }
...
"@
        ...
        Write-Success "Created ~/.anakot/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.anakot/"
    
    # Seed bundled skills into ~/.anakot/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.anakot/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.anakot/skills/"
        } catch {
            ...
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                ...
                Write-Success "Skills copied to ~/.anakot/skills/"
            }
        }
    }
}
...
function Install-Desktop {
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Anakot-managed
    ...
    try {
        ...
        if ($code -ne 0) {
            ...
            # ENOENT on the final `electron` -> `Anakot` rename: app-builder's
