...
# Bootstrap Open WebUI against Anakot Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.anakot/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-anakot.sh
...
#   ANAKOT_API_PORT=8642
#   ANAKOT_API_HOST=127.0.0.1
#   ANAKOT_API_MODEL_NAME='Anakot Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Anakot Agent WebUI}"
...
ANAKOT_ENV_FILE="${ANAKOT_ENV_FILE:-$HOME/.anakot/.env}"
ANAKOT_API_PORT="${ANAKOT_API_PORT:-8642}"
ANAKOT_API_HOST="${ANAKOT_API_HOST:-127.0.0.1}"
ANAKOT_API_CONNECT_HOST="${ANAKOT_API_CONNECT_HOST:-127.0.0.1}"
ANAKOT_API_MODEL_NAME="${ANAKOT_API_MODEL_NAME:-Anakot Agent}"
ANAKOT_API_BASE_URL="http://${ANAKOT_API_CONNECT_HOST}:${ANAKOT_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-anakot.sh"
LOG_DIR="$HOME/.anakot/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$ANAKOT_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$ANAKOT_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.anakot.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.anakot</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.anakot"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.anakot"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-anakot.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Anakot Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-anakot.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.anakot/logs/openwebui.log
StandardError=append:%h/.anakot/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-anakot.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$ANAKOT_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Anakot API server is configured...'
  upsert_env API_SERVER_ENABLED true "$ANAKOT_ENV_FILE"
  upsert_env API_SERVER_HOST "$ANAKOT_API_HOST" "$ANAKOT_ENV_FILE"
  upsert_env API_SERVER_PORT "$ANAKOT_API_PORT" "$ANAKOT_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$ANAKOT_API_MODEL_NAME" "$ANAKOT_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$ANAKOT_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Anakot gateway so API server settings take effect...'
  anakot gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${ANAKOT_API_CONNECT_HOST}:${ANAKOT_API_PORT}/health" >/dev/null; then
    log 'Anakot API server did not answer on the first check. Trying to start gateway in the background...'
    nohup anakot gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${ANAKOT_API_CONNECT_HOST}:${ANAKOT_API_PORT}/health" >/dev/null
  ...
  log "Anakot API endpoint: ${ANAKOT_API_BASE_URL}"
