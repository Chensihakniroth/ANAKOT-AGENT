...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``anakot -c`` / ``--resume`` (relaunch
...
_loaded_env_paths = load_anakot_dotenv(anakot_home=_anakot_home, project_env=_project_env)
...
class AIAgent:
    ...
    def _ensure_db_session(self) -> None:
        ...
        if self._session_db_created or not self._session_db:
            return
        source = self.platform or os.environ.get("ANAKOT_SESSION_SOURCE", "cli")
        try:
        ...
        **extra_context,
        ...
        if should_start and target_session_id and hasattr(engine, "on_session_start"):
            start_context = {
                ...
                "platform": getattr(self, "platform", None) or os.environ.get("ANAKOT_SESSION_SOURCE", "cli"),
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        """
        Preload the LM Studio model with at least Anakot' minimum context.
        """
        if (self.provider or "").strip().lower() != "lmstudio":
            return
        try:
            from agent.model_metadata import MINIMUM_CONTEXT_LENGTH
            from anakot_cli.models import ensure_lmstudio_model_loaded
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``anakot chat -q``. In that mode,
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``ANAKOT_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``ANAKOT_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return float(os.getenv("ANAKOT_API_TIMEOUT", 1800.0))
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``ANAKOT_API_CALL_STALE_TIMEOUT`` env var
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        anakot-agent #21444 for the symptom history.  The upstream backend
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        # callmemo serves GPT-5.x models via its OpenAI-compatible chat
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use Anakot against xai-oauth.  Lots of
    ...
    def _invoke_api_request_error_hook(
        ...
        *,
        ...
        # ``monkeypatch.setattr("anakot_cli.plugins.has_hook", ...)`` still
    ...
    def _redact_message_content(content):
        ...
        Respects ``ANAKOT_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.anakot/sessions/session_{sid}.json``
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects ANAKOT_REDACT_SECRETS via
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.anakot/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``ANAKOT_TURN_COMPLETION_EXPLAINER`` env var overrides
        config.  Exposed as a method so tests can patch a single seam,
        mirroring ``_file_mutation_verifier_enabled``.
        """
        try:
            import os as _os
            env = _os.environ.get("ANAKOT_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
            # Read from the persisted config.yaml so gateway and CLI share
            # the same setting.  Import lazily to avoid a startup-time cycle.
    ...
    def _touch_activity(self, desc: str) -> None:
        ...
        process is a dispatcher-spawned worker (ANAKOT_KANBAN_TASK set),
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        """
        # Dev test fixture (ANAKOT_DEV_CREDITS_FIXTURE): inject a chosen notice state
        ...
        if not headers:
            return
        _dev = is_truthy_value(os.environ.get("ANAKOT_DEV_CREDITS"))
        ...
        if state is None:
            if _dev:
                logger.info(
                    ...
                    "(miss — producer off / non-callmemo path / >TTL stale)"
                )
            return
        ...
        if _dev:
            # ANAKOT_DEV_CREDITS: stream each capture to agent.log — watch live with
            # `anakot logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``anakot auth add xai-oauth``) whose tokens belong to
    ...
    def _try_refresh_nous_client_credentials(
        ...
        *,
        ...
        try:
            ...
            logger.debug("callmemo credential refresh failed: %s", exc)
            return False
        ...
        self._client_kwargs["api_key"] = self.api_key
        self._client_kwargs["base_url"] = self.base_url
        # callmemo requests should not inherit OpenRouter-only attribution headers.
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        known reasoning-capable model families and direct callmemo Portal.
        """
        if base_url_host_matches(self._base_url_lower, "callmemo.ai"):
            return True
        if (
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from anakot_cli.models import lmstudio_model_reasoning_options
