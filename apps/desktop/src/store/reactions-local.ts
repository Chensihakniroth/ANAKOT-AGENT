...
import type { MessageReaction } from '@/lib/chat-messages'
 ...
 * reactions) still arrive through the message's `reactions` field; this layer
 * sits on top of it so the interaction never waits on the backend to feel
 * alive.
 */
 ...
 * resume regenerates. The resume also rebuilds from the gateway's in-memory
 * history, which doesn't carry a reaction written to the DB mid-turn — this
 * overlay outlives that clobber and a real reload hydrates the same reaction
 * from disk.
 */
