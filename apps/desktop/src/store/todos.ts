...
/**
 * Live todo list per session, rendered by the composer todo panel. Fed from
 * two places:
 *
 ...
 *
 * Keyed by the gateway session id (the same id tool events carry and the
 * sidebar rows use — Anakot resumes sessions under their stored id).
 */
...
/** Live "X/Y" per session id, for the sidebar's session rows. Cancelled items
 *  don't count toward either side of the fraction. Values are the rendered
 *  "X/Y" string — primitives, so stableRecord can suppress no-op emits. */
export const $todoProgressBySession = computed([$todosBySession], (todosMap) => {
  const next: Record<string, string> = {}

  for (const [sessionId, todos] of Object.entries(todosMap)) {
    ...
    next[sessionId] = `${counted.filter(t => t.status === 'completed').length}/${counted.length}`
  }
...
/** Clear a session's live list (turn end, session switch) and forget its
 *  revision watermark so a later replay can re-pin. */
