import type { ChatMessage, MessageReaction } from '@/lib/chat-messages'
  ...
  setMessages(messages =>
    messages.map(message =>
      message.id === messageId
        ? { ...message, reactions, ...(rowId === undefined ? {} : { rowId }) }
        : message
    )
  )
}
 ...
 * A failed write rolls back to the snapshot.
 */
export async function toggleMessageReaction(
  ...
  // rowId. Rather than disable the affordance, let the backend resolve the
  // newest row of this role — which is exactly the message being reacted to.
  ...
  try {
    ...
    // why, instead of the reaction quietly vanishing.
