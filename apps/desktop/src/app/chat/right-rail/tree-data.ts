// Accept both GitFile (store/git.ts) and GitStatusEntry (store/git-commit.ts)
// which differ only in status: union vs string.
...
// descendants' +/- so a collapsed folder still shows its total churn.
...
export function buildReviewFlatList(files: TreeFile[]): ReviewTreeNode[] {
  return [...files]
...
export function buildReviewTree(files: TreeFile[], compact = true): ReviewTreeNode[] {
  const root = makeDir('', '')
