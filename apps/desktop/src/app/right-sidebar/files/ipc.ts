...
async function gitRootFor(start: string) {
  if (!window.anakotDesktop?.gitRoot) {
    return null
  }
  ...
  if (!cached) {
    cached = window.anakotDesktop.gitRoot(key)
...
async function readGitignore(dir: string): Promise<GitignoreRule | null> {
  if (!window.anakotDesktop?.readDir || !window.anakotDesktop.readFileDataUrl) {
    return null
  }

  try {
    const listing = await window.anakotDesktop.readDir(dir)

    if (!listing.entries.some(e => e.name === '.gitignore' && !e.isDirectory)) {
      return null
    }

    const text = decodeDataUrl(await window.anakotDesktop.readFileDataUrl(`${dir}/.gitignore`))
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<AnakotReadDirResult> {
  ...
  const result = await window.anakotDesktop.readDir(dirPath)
