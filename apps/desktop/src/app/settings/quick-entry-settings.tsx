 ...
 *
 * Ported from Hermes Agent (apps/desktop/src/app/settings/quick-entry-settings.tsx),
 * hermes→anakot renames applied; Hermes' `ToggleRow` primitive doesn't exist in
 * Anakot, so the enable switch renders as a `ListRow` with a `Switch` action.
 */
  ...
  return (
    <>
      <ListRow
        ...
        title={q.enabledTitle}
        action={
          <Switch
            aria-label={q.enabledTitle}
            checked={state.enabled}
            onCheckedChange={enabled => void saveQuickEntrySettings({ enabled: enabled === true })}
          />
        }
      />
      <ListRow
