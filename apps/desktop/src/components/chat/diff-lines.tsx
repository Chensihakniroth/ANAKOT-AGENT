 ...
 *  - `DiffLines` is the color-only fallback (no language, or while Shiki loads).
 * Both drop git file-headers + `@@` hunk noise and the `+/-` gutter so changes
 * read by color + a 2px gutter accent, the way Cursor / GitHub PR diffs do.
 */
...
// `index …`, `--- a/path`, `+++ b/path`, and arrow lines. That preamble just
// repeats the path (which the tool row already shows) and reads especially
// badly for absolute paths (`a//Users/…`). Strip the leading header zone up to
// the first hunk.
...
function useThemeName() {
  const current = () => (document.documentElement.classList.contains('dark') ? 'github-dark-default' : 'github-light-default')
...
function SyntaxDiff({ language, lines }: { language: string; lines: DiffLine[] }) {
  ...
  // app.asar.unpacked) throws past Suspense, which only covers the pending
  // state. Without a local boundary that throw reaches the workspace
...
/**
 * PR-style diff panel with optional Shiki syntax highlighting. Use this when
 * you want the full diff rendering (line numbers, syntax highlighting) — for
 * example in a tool-result card where the diff is the primary content.
 *
 * `language` is optional; when omitted the diff renders as color-only (no Shiki).
 */
export function DiffLinesWithSyntax({
  className,
  language,
  text,
  ...props
}: DiffLinesProps & { language?: string }) {
  const lines = React.useMemo(() => parseDiff(text), [text])
...
// ── Full-file diff (expand hunks against current file text) ────────────────
...
// ── Preview rows (windowed, no tokens) ─────────────────────────────────────
function PreviewDiffRows({
  ...
  afterLines?: number
  beforeLines?: number
  chunks: Array<{ lines: DiffLine[]; start: number }>
...
// ── Tokenized diff body (windowed + Shiki) ─────────────────────────────────
function TokenizedDiffBody({
  ...
  React.useEffect(() => {
    ...
    setTokens(null)
    void import('shiki')
      .then(({ codeToTokens }) => codeToTokens(code, { lang: language as any, theme }))
      .then(result => {
        if (!cancelled) {
...
// ── Overview ruler (VS Code-style minimap of changes) ─────────────────────
...
// ── FileDiffPanel (full-featured diff with line numbers + virtualization) ──
interface FileDiffPanelProps {
  className?: string
  diff: string
  /** Current file text. When provided, expands hunks into a full-file view. */
  fullText?: string
  path?: string
  /** Show old/new line-number gutter (full preview diff). */
  showLineNumbers?: boolean
  /** Virtualize rows for large diffs (review panel). */
  virtualized?: boolean
}
      ...
      >
        {showLineNumbers ? (
          <div className="grid min-w-max grid-cols-[auto_minmax(0,1fr)]">
            <div
              className="sticky left-0 z-1 select-none bg-(--ui-bg-chrome) py-3 text-muted-foreground/55"
              data-glass-opaque=""
            >
