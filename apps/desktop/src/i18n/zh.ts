...
export const zh: Translations = {
  ...
  settings: {
    ...
    appearance: {
      ...
      pet: {
        ...
        restartHint: '后端已更新，请关闭并重新打开宠物面板以应用更改。',
        searchPlaceholder: '按名称或 slug 搜索…',
        unreachable: '无法通过本地后端访问宠物图库。',
        noMatch: (query: string) => `没有与"${query}"匹配的宠物。`,
        turnOnFailed: '无法开启宠物显示。',
        turnOffFailed: '无法关闭宠物显示。',
        ...
        generatedTag: '已生成',
        installedTag: '已安装',
        rename: (name: string) => `重命名 ${name}…`,
        exportPet: (name: string) => `导出 ${name}…`,
        delete: (name: string) => `删除 ${name}…`,
        uninstall: (name: string) => `卸载 ${name}…`,
        countCapped: (cap: number, total: number) => `显示 ${cap} / ${total} 个宠物。`,
        count: (n: number) => `${n} 个宠物`,
        chooseDesc: '选择你的活跃宠物或切换宠物显示开关。',
        chooseTitle: '宠物',
        off: '关闭',
        on: '开启',
        scaleTitle: '宠物大小',
        scaleDesc: '调整浮动宠物的尺寸。',
        roamTitle: '宠物漫游',
        roamDesc: '让宠物在屏幕上自主移动。',
        deleteConfirm: '删除',
        deleteBody: '删除是永久操作，无法撤销。',
        deleteTitle: (name: string) => `删除 ${name}？`,
        renameTitle: '重命名宠物',
        renamePlaceholder: '输入新名称…',
        renameSave: '保存',
        popOutTitle: '弹出窗口',
        popOutDesc: '在独立置顶窗口中显示宠物。',
        opacityTitle: '宠物透明度',
        opacityDesc: '调整浮动宠物的透明程度。',
        anchorTitle: '宠物位置',
    ...
    quickEntry: {
      enabledTitle: '快捷输入',
      enabledDesc: '使用全局快捷键随时随地召唤迷你输入框。',
      shortcutTitle: '快捷键',
      shortcutDesc: '输入 Electron 快捷键格式（例如 CommandOrControl+Shift+Space）。',
      takenBy: '此快捷键已被其他应用程序占用。',
      invalidShortcut: '快捷键无效 — 至少需要一个修饰键加一个按键（例如 CommandOrControl+Shift+Space）。',
      active: '已启用'
    },
    credentials: {
      pasteKey: '粘贴密钥',
    ...
    gateway: {
      ...
      envOverride: '环境变量覆盖',
      intro:
        'Anakot Desktop 默认会启动自己的本地网关。当你希望此应用控制另一台机器上或可信代理后的现有 Anakot 后端时，可以使用远程网关。下面可按 profile 指定各自的远程主机。',
      appliesTo: '应用于',
      ...
      remoteTitle: '远程网关',
      remoteDesc:
        '将此桌面外壳连接到远程 Anakot 后端。托管网关使用 OAuth 或用户名密码；自托管网关也可能使用会话 token。',
      ...
      savedTitle: '网关设置已保存',
      restartingMessage: 'Anakot Desktop 将使用已保存设置重新连接。',
    ...
    toolsets: {
      ...
      ready: '就绪',
      nousIncluded: '包含在 callmemo 订阅中；登录 callmemo Portal 即可激活。',
  ...
  commandCenter: {
    ...
    gitUntracked: '未跟踪',
    gitNoChanges: '无更改',
    gitNotAGitRepo: '不是 Git 仓库',
    gitLoadingGit: '正在加载 Git 状态...',
    gitCommit: '提交',
    gitCommitPlaceholder: '提交信息',
    gitStageFile: '暂存',
    gitUnstageFile: '取消暂存',
    gitDiscardChanges: '放弃更改',
    gitStageAll: '全部暂存',
    gitCommitHint: 'Ctrl+Enter 提交',
    commandCenter: '命令中心',
    appearance: '外观',
    settings: '设置',
    changeTheme: '更改主题...',
    ...
    nav: {
      newChat: { title: '新会话', detail: '开始一个新的会话' },
      settings: { title: '设置', detail: '配置 Anakot 桌面' },
  ...
  messaging: {
    ...
    platformIntro: {
      ...
      bluebubbles: '在装有 iMessage 的 Mac 上运行 BlueBubbles Server，暴露其 API，然后用服务器密码把 Anakot 指向该 URL。',
      ...
      weixin: '登录微信公众平台，复制 AppID 和 Token，并把消息回调 URL 指向 Anakot。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      reveal: '在文件夹中显示',
      copyPath: '复制路径',
      removeFromSidebar: '从侧边栏隐藏',
      createFailed: '无法创建项目',
      staleBackend: '更新 Anakot 后端以创建项目——你的后端版本低于此桌面应用（设置 → 更新 → 后端）。',
      deleteConfirm: '这将从 Anakot 中移除已保存的项目。文件、git 仓库和工作树保持不变。',
      startWork: '新建工作树',
      newWorktreeTitle: '新建工作树',
      newWorktreeDesc: '为此工作树命名分支。',
      branchPlaceholder: '例如 my-feature',
      branchOff: () => ({ after: '', before: '分支基于 ' }),
      baseBranchPlaceholder: '搜索分支…',
      baseBranchNone: '未找到分支',
      startWorkFailed: '无法创建工作树',
      convertBranch: '转换分支…',
      convertBranchTitle: '转换分支',
      convertBranchDesc: '打开已检出的分支，或为未占用的分支创建工作树。',
      convertBranchPlaceholder: '搜索分支…',
      convertBranchInstead: '转换现有分支',
      branchOpenExisting: '打开',
      branchSwitchHome: '切换主页',
      branchCreateWorktree: '新建工作树',
      branchesLoading: '加载分支中…',
      noBranches: '未找到分支',
      removeWorktree: '移除工作树',
      removeWorktreeFailed: '无法移除工作树（有未提交更改？）',
      removeWorktreeConfirm: '从 git 中移除（删除工作树目录；分支保留），或仅从侧边栏隐藏此通道并将工作树保留在磁盘上。',
      removeWorktreeDirty: '此工作树有未提交的更改。强制移除（放弃这些更改），或仅隐藏此通道并将其保留在磁盘上。',
      forceRemove: '强制移除',
      enter: label => `打开 ${label}`,
      reorder: label => `重新排序 ${label}`,
      toggle: (label, open) => `${open ? '显示' : '隐藏'} ${label} 的会话`,
      back: '全部项目'
    },
    row: {
      pin: '置顶',
  ...
  updates: {
    ...
    applyingBody: 'Anakot 更新器会在自己的窗口中接管，并在完成后重新打开 Anakot。',
    applyingClose: 'Anakot 将关闭以应用更新。',
    errorTitle: '更新未完成',
  ...
  modelPicker: {
    ...
    pro: 'Pro',
    proNeedsSubscription: 'Pro 模型需要付费 callmemo 订阅。',
    free: '免费',
    freeTier: '免费层',
  ...
  preview: {
    ...
    largeTitle: '此文件较大',
    largeBody: (label, size) => `${label} 大小为 ${size}。Anakot 只会显示前 2 MB。`,
    oversizedTitle: '文件过大，无法预览',
    oversizedBody: (label, size) => `${label} 大小为 ${size}。超过 10 MB 的文件无法在编辑器中预览。`,
    largeFileWarning: (size, lines) => `此文件为 ${size}，约 ${lines} 行。渲染可能较慢。`,
    previewAnyway: '仍然预览',
    truncated: '显示前 2 MB。',
  ...
  prompts: {
    ...
    sudoTitle: '管理员密码',
    sudoDesc: 'Anakot 需要你的 sudo 密码来运行特权命令。它只会发送给你的本地 agent。',
