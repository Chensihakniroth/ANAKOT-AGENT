 ...
 * against the Anakot DOM; surface='preview' is currently unsupported (no
 * preview-tour equivalent exists in Anakot yet).
 *
 * Dynamic-imported by gateway-event.ts, so driver.js stays off the boot path
 * until a tour actually runs.
 */
...
export async function runTour(action: TourAction, surface: TourSurface = 'app'): Promise<TourResult> {
  try {
    if (surface === 'preview') {
      // TODO: Preview-tour support is not yet ported to Anakot.
      //       Hermes ran the engine source inside the preview pane's guest
      //       page via runPreviewTour(); Anakot has no equivalent yet.
      return { error: 'preview surface is not supported in Anakot', success: false }
    }
