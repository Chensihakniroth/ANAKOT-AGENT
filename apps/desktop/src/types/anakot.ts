...
export interface ModelOptionProvider {
   ...
   *  the `anakot model` CLI / onboarding OAuth flow. */
  auth_type?: string
  ...
  /** callmemo only: whether the current account is on the free tier. */
  free_tier?: boolean
  /** callmemo only: paid models a free-tier user cannot select (shown disabled). */
...
export interface ModelAssignmentResponse {
  ...
  base_url?: string
  /** Toolset keys auto-routed through the callmemo Tool Gateway as a result of
   *  switching the main provider to callmemo. Empty unless provider === 'nous'
