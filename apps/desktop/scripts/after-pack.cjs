 ...
 *
 * Stamps the Anakot icon + identity onto the packed Windows Anakot.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `anakot desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Anakot')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Anakot'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Anakot.exe keeps the stock Electron icon`)
  }
}
