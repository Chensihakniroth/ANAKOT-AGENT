...
function resolveAnakotHome() {
  if (process.env.ANAKOT_HOME) return path.resolve(process.env.ANAKOT_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'anakot-home')
...
const MEDIA_PROTOCOL = 'anakot-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(anakot-media://…)`.
...
// (the desktop's launch profile) stays managed by anakotProcess +
// connectionPromise + startAnakot(); this pool only holds EXTRA profile
...
// findGitBash — locate bash.exe on Windows. Anakot' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\anakot\git\... — checked
  // first so users who installed via install.ps1 are detected before we
  // start probing system-wide locations.
  const localAppData = process.env.LOCALAPPDATA || ''
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'anakot', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'anakot', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// apps/bootstrap-installer paths::copy_self_to_anakot_home). That binary owns
// ALL repo mutation — running `anakot update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'anakot-setup.exe' : 'anakot-setup'
  const candidate = path.join(ANAKOT_HOME, name)
...
function venvAnakotShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'anakot.exe')
    : path.join(updateRoot, 'venv', 'bin', 'anakot')
}
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (anakotProcess && Number.isInteger(anakotProcess.pid)) pids.push(anakotProcess.pid)
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri anakot-setup. Unlike Windows
      ...
      // whole update itself: `anakot update` (backend) + `anakot desktop
    ...
    if (!updater) {
      ...
      // anakot-setup.exe into ANAKOT_HOME). They DO have a working `anakot`
      ...
      // checkout they're on — bare `anakot update` defaults to main and would
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') command = `anakot update --branch ${branch}`
        }
      } catch {
    ...
    emitUpdateProgress({ stage: 'restart', message: 'Handing off to the Anakot updater…', percent: 100 })
    ...
    const child = spawn(updater, updaterArgs, {
      ...
      env: {
        ...process.env,
        ANAKOT_HOME,
        PATH: [path.join(ANAKOT_HOME, 'node', 'bin'), venvBin, process.env.PATH].filter(Boolean).join(path.delimiter)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
...
// Resolve the anakot CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `anakot` on PATH.
function resolveAnakotCliBinary(updateRoot) {
  const venvAnakot = path.join(updateRoot, 'venv', 'bin', 'anakot')
  if (fileExists(venvAnakot)) return venvAnakot
  return findOnPath('anakot') || null
}
...
// macOS/Linux in-app update: backend (`anakot update`) + OS-aware GUI rebuild
// (`anakot desktop --build-only`), then atomically swap the running .app bundle
// with the freshly built one and relaunch. Degrades to "backend updated,
// restart to load the new GUI" if the swap can't be performed.
async function applyUpdatesPosixInApp() {
  const updateRoot = resolveUpdateRoot()
  const anakot = resolveAnakotCliBinary(updateRoot)
  if (!anakot) {
  ...
  // Put the Anakot-managed Node and the venv on PATH so `anakot desktop`'s
  // npm build can find them on a machine with no system Node.
  const extraPath = [path.join(ANAKOT_HOME, 'node', 'bin'), path.join(updateRoot, 'venv', 'bin')]
    .filter(Boolean)
    .join(path.delimiter)
  const env = {
    ANAKOT_HOME,
  ...
  // `anakot update` reaps stale `anakot dashboard` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads ANAKOT_DESKTOP_CHILD_PID and excludes
  ...
  if (anakotProcess && Number.isInteger(anakotProcess.pid)) {
    desktopChildPids.push(anakotProcess.pid)
  }
  for (const entry of backendPool.values()) {
  ...
  if (desktopChildPids.length) {
    env.ANAKOT_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Anakot (git + dependencies)…', percent: 10 })
  const updated = await runStreamedUpdate(anakot, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'anakot update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'anakot update failed' }
  }

  emitUpdateProgress({ stage: 'rebuild', message: 'Rebuilding the desktop app…', percent: 60 })
  const rebuilt = await runStreamedUpdate(anakot, ['desktop', '--build-only'], {
    cwd: updateRoot,
    env,
    stage: 'rebuild'
  })
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Anakot to retry.',
  ...
  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Anakot.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Anakot.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Anakot to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.anakot-update-new"; then
    rm -rf "$DST.anakot-update-old" 2>/dev/null || true
    mv "$DST" "$DST.anakot-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.anakot-update-new" "$DST"
    rm -rf "$DST.anakot-update-old" 2>/dev/null || true
  fi
fi
...
/usr/bin/open "$DST"
`
  const scriptPath = path.join(app.getPath('temp'), `anakot-desktop-update-${Date.now()}.sh`)
  try {
    fs.writeFileSync(scriptPath, swapScript, { mode: 0o755 })
  } catch (err) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Anakot to load the new version.',
      percent: 100
    })
...
function createPythonBackend(root, label, dashboardArgs, options = {}) {
  ...
  return {
    ...
    label,
    command: python,
    args: ['-m', 'anakot_cli.main', ...dashboardArgs],
    env: {
...
function resolveAnakotBackend(dashboardArgs) {
  ...
  if (overrideRoot && isAnakotSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Anakot source at ${overrideRoot}`, dashboardArgs)
  ...
  if (!IS_PACKAGED && isAnakotSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Anakot source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
    if (backend) return backend
  }

  // 3. Bootstrap-complete ACTIVE_ANAKOT_ROOT -- the canonical install at
  ...
  //    to spawning anakot. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `anakot update` from the CLI.
  ...
  if (process.env.ANAKOT_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    if (anakotCommand) {
      ...
      if (verifyAnakotCli(anakotCommand, { shell: shellForProbe })) {
        return {
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Anakot install was cancelled.')
    ...
    return ensureRuntime(resolveAnakotBackend(backend.args))
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // isAnakotSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Anakot venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }
...
async function waitForAnakot(baseUrl, token) {
  ...
  throw new Error(`Anakot backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
const OAUTH_SESSION_PARTITION = 'persist:anakot-remote-oauth'
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Anakot gateway',
...
// a valid stored value (pins the root ANAKOT_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  return {
    ...
    envOverride: key ? false : Boolean(process.env.ANAKOT_DESKTOP_REMOTE_URL)
  }
}
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (anakot #37247), and the
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.ANAKOT_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.ANAKOT_DESKTOP_REMOTE_TOKEN
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
...
function resetAnakotConnection() {
...
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetAnakotConnection() nulls anakotProcess.
...
// profile to startAnakot() (the window backend: boot UI, bootstrap, remote
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForAnakot(remote.baseUrl, remote.token)
    return {
      ...remote,
      profile,
  ...
  const backend = await ensureRuntime(resolveAnakotBackend(dashboardArgs))
  ...
  child.once('exit', (code, signal) => {
    rememberLog(`Anakot backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(new Error(`Anakot backend for profile "${profile}" exited before it became ready (${signal || code}).`))
    }
  })
...
async function startAnakot() {
  ...
  connectionPromise = (async () => {
    ...
    anakotProcess = spawn(backend.command, backend.args, {
      cwd: anakotCwd,
      env: {
        ...process.env,
    ...
    anakotProcess.once('exit', (code, signal) => {
      rememberLog(`Anakot backend exited (${signal || code})`)
      anakotProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })
      if (!backendReady) {
        const message = `Anakot backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
...
ipcMain.handle('anakot:connection', async (_event, profile) => ensureBackend(profile))
...
ipcMain.handle('anakot:bootstrap:repair', async () => {
  ...
  // startAnakot() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('anakot:setting:defaultProjectDir:get', async () => ({
  ...
  defaultLabel: path.join(app.getPath('home'), 'anakot-projects')
}))
...
async function getUninstallSummary() {
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(py, ['-m', 'anakot_cli.main', 'uninstall', '--gui-summary'], {
        cwd: agentRoot,
