...
function downloadInstallScript(commit, destPath) {
  ...
  const repo = process.env.ANAKOT_REPO_RAW || 'Chensihakniroth/ANAKOT-AGENT'
  const url = `https://raw.githubusercontent.com/${repo}/${commit}/scripts/${scriptName}`
...
async function resolveInstallScript({ installStamp, sourceRepoRoot, anakotHome, emit }) {
  ...
  const cached = cachedScriptPath(anakotHome, installStamp.commit)
  try {
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, anakotHome } = {}) {
  return new Promise((resolve, reject) => {
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, anakotHome } = {}) {
  return new Promise((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, anakotHome }) {
...
async function fetchManifest({ scriptPath, installerKind, emit, anakotHome, activeRoot, installStamp }) {
  const isPosix = installerKind === 'posix'
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, anakotHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, anakotHome, activeRoot, abortSignal, installStamp }) {
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, anakotHome })
      ]
