 ...
 * duration of an `--update` run. The marker body is two lines: the updater's
 ...
 * (taskkill /IM anakot.exe) kills it, the launch fails with the 45s "backend
 * didn't come up" timeout, and the user relaunches into the same trap — an
 * infinite respawn/kill loop. The desktop gates local backend startup on this
 * marker and parks until the update finishes.
 *
...
function markerPath(anakotHome) {
 ...
 * clock for tests.
 */
function readLiveUpdateMarker(anakotHome, { kill, now = Date.now, maxAgeMs = UPDATE_MARKER_MAX_AGE_MS } = {}) {
 ...
 */
function writeUpdateMarker(anakotHome, pid, { now = Date.now } = {}) {
  const file = markerPath(anakotHome)
