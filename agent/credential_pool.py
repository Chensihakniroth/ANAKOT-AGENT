...
class PooledCredential:
    ...
    def runtime_api_key(self) -> str:
        if self.provider == "nous":
            # callmemo stores the runtime inference credential in agent_key for
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``anakot model`` / ``anakot auth`` which
        ...
        Mirrors the callmemo/Anthropic resync paths above.  Only applies to
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Anakot process
    ...
    def _sync_nous_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        """Sync a callmemo pool entry from auth.json if tokens differ.

        callmemo OAuth refresh tokens are single-use.  When another process
        (e.g. a concurrent cron) refreshes the token via
        ``resolve_callmemo_runtime_credentials``, it writes fresh tokens to
        auth.json under ``_auth_store_lock``.  The pool entry's tokens
        become stale.  This method detects that and adopts the newer pair,
        avoiding a "refresh token reuse" revocation on the callmemo Portal.
        """
        ...
        try:
            ...
            if should_sync:
                logger.debug(
                    "Pool entry %s: syncing callmemo state from auth.json",
                    entry.id,
                )
                field_updates: Dict[str, Any] = {
                    "last_status": None,
            ...
            logger.debug("Failed to sync callmemo entry from auth.json: %s", exc)
        return entry
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (currently callmemo, OpenAI Codex, and xAI Grok OAuth).
        ...
        here would mean every callmemo/Codex/xAI refresh in a multi-provider
        setup silently flips the ``active_provider`` flag — the next
        ``anakot`` invocation that defaults to the active provider
        (e.g. setup wizard, ``anakot auth status``) would land on
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("anakot_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another Anakot
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("anakot_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
            ...
            if self.provider == "xai-oauth":
                ...
                # in-memory pool.  Mirrors the callmemo quarantine path above.
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    self._persist()
                    return None
            # For openai-codex: same race as xAI/nous — another Anakot process
            ...
            if self.provider == "openai-codex":
                ...
                # in-memory pool.  Mirrors the xAI and callmemo quarantine paths.
            ...
            if self.provider == "nous":
                synced = self._sync_nous_entry_from_auth_store(entry)
                if synced.refresh_token != entry.refresh_token:
                    logger.debug("callmemo refresh failed but auth.json has newer tokens — adopting")
                    updated = replace(
                        synced,
                        last_status=STATUS_OK,
                        last_status_at=None,
                ...
                if auth_mod._is_terminal_nous_refresh_error(exc):
                    logger.debug("callmemo refresh token is terminally invalid; clearing local token state")
                    try:
                        with _auth_store_lock():
                            auth_store = _load_auth_store()
                        ...
                        logger.debug("Failed to clear terminal callmemo OAuth state: %s", clear_exc)
    ...
    def _entry_needs_refresh(self, entry: PooledCredential) -> bool:
        ...
        if self.provider == "nous":
            # callmemo refresh can require network access and should happen when
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Anakot profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # resolve_callmemo_runtime_credentials(), making this entry's
            ...
            # re-authed via `anakot model` / `anakot auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `anakot model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``anakot auth add``.  Singleton-
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `anakot auth remove <provider> <N>` is stable across all source types.
    try:
        from anakot_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Anakot PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `anakot setup` ("Claude
        ...
        # Claude Code CLI, anakot_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.anakot/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `anakot setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"anakot_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_anakot_oauth_credentials

        for source_name, creds in (
            ("anakot_pkce", read_anakot_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `anakot auth add nous --label <name>`).  Fall back to the
        ...
        # Anakot auth store or env vars, so resolve them here.
        ...
        # MiniMax OAuth tokens live in ~/.anakot/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `anakot auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `anakot auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Anakot auth store.  Without this gate the removal is instantly
        ...
        # Anakot owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``anakot model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``anakot auth list`` reflects the logged-in state and so the pool
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.anakot/.env over os.environ — the user's config file is the
    # authoritative source for Anakot credentials. Stale env vars from parent
    ...
    # Honour user suppression — `anakot auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.anakot/.env.
    ...
    try:
        from anakot_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    if provider == "openrouter":
        # Prefer ~/.anakot/.env over os.environ
