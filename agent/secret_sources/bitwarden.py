...
Anakot pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.anakot/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<anakot_home>/bin/bws`` on
  first use.  Anakot pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.anakot/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``anakot`` invocations don't
  hammer the API.
* Failures NEVER block Anakot startup.  Missing binary, no network,
...
# In-process cache so repeated load_anakot_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, "_CachedFetch"] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `anakot chat -q ...`
...
# <anakot_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.anakot/.env (which
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<anakot_home>/bin/bws``  (our managed copy — preferred)
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``anakot secrets bitwarden setup``
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<anakot_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
...
# ---------------------------------------------------------------------------
# Public entry point — called from anakot_cli.env_loader
# ---------------------------------------------------------------------------
