...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    anakot_home = _anakot_home_path()
    anakot_root = _anakot_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(anakot_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(anakot_root / ".anthropic_oauth.json"),
...
def get_safe_write_root() -> Optional[str]:
    """Return the resolved ANAKOT_WRITE_SAFE_ROOT path, or None if unset."""
    root = os.getenv("ANAKOT_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    # Anakot control-plane files: block both the ACTIVE profile's view
    # (anakot_home) AND the global root view. Without the root pass, a
    ...
    anakot_dirs = []
    for base in (_anakot_home_path(), _anakot_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in anakot_dirs:
                anakot_dirs.append(real)
        except Exception:
            continue

    for base_real in anakot_dirs:
...
def get_read_block_error(path: str) -> Optional[str]:
    ...
    Three categories are blocked:
      ...
      * Internal Anakot cache files under ``ANAKOT_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under ANAKOT_HOME and the global Anakot
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.anakot/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active ANAKOT_HOME (profile-aware) AND the global
    # Anakot root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (ANAKOT_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    anakot_dirs: list[Path] = []
    for base in (_anakot_home_path(), _anakot_root_path()):
        try:
            real = base.resolve()
            if real not in anakot_dirs:
    ...
    for hd in anakot_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Anakot cache file "
    ...
    for hd in anakot_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Anakot credential store "
    ...
    for hd in anakot_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Anakot MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Anakot MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Anakot profiles are separate ANAKOT_HOME dirs under
...
#
# Reference: May 2026 incident where a anakot-security profile session
# edited skills under both ``~/.anakot/profiles/anakot-security/skills/``
# AND ``~/.anakot/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under ANAKOT_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from ANAKOT_HOME.
    ...
    """
    try:
        home_real = _anakot_home_path().resolve()
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Anakot entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Anakot profile {info['target_profile']!r}, but the "
...
# ``…/sandboxes/<backend>/<task>/home/.anakot/…`` segment and warns
# regardless of which Anakot profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.anakot/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.anakot`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.anakot/…`` and returns the
    index where the inner Anakot-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .anakot / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".anakot":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Anakot state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.anakot``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.anakot`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Anakot resolver to
    succeed, so it works correctly even when called from contexts where
    ANAKOT_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Anakot process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real ANAKOT_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.anakot/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.anakot/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host ANAKOT_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Anakot state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Anakot "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real ANAKOT_HOME. Use the "
