...
def _compression_threshold_for_model(model: Optional[str]) -> Optional[float]:
    ...
    consumed before Anakot triggers summarization.  Higher values delay
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://github.com/Chensihakniroth/ANAKOT-AGENT",
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``ANAKOT_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``ANAKOT_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "AnakotAgent",
}
...
# callmemo Portal extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed by callmemo Portal.
#
...
# ``_nous_extra_body()`` or import ``callmemo_portal_tags`` directly.
NOUS_EXTRA_BODY = _nous_extra_body()

# Set at resolve time — True if the auxiliary client points to callmemo Portal
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Anakot Agent)",
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``anakot_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _nous_base_url() -> str:
    """Resolve the callmemo inference base URL from env or default."""
    return os.getenv("NOUS_INFERENCE_BASE_URL", _NOUS_DEFAULT_BASE_URL)


def _resolve_nous_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Return fresh callmemo runtime credentials when available.
    ...
    try:
        ...
        creds = resolve_callmemo_runtime_credentials(
            timeout_seconds=float(os.getenv("ANAKOT_NOUS_TIMEOUT_SECONDS", "15")),
            force_refresh=force_refresh,
        )
    except Exception as exc:
        logger.debug("Auxiliary callmemo runtime credential resolution failed: %s", exc)
        return None
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``anakot auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``anakot_cli.auth``'s singleton runtime resolver for older
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Anakot auth store.
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    if runtime is not None:
        ...
        if not api_key:
            logger.warning(
                "Auxiliary callmemo client unavailable: no usable inference JWT found "
...
def _refresh_nous_recommended_model(
    *, vision: bool, stale_model: Optional[str]
    ...
    """Re-fetch the callmemo Portal's recommended model after a stale-model 404.
    ...
    the callmemo → OpenRouter catalog, every auxiliary call 404s with
...
def _try_azure_foundry(
    *,
    ...
    :func:`anakot_cli.runtime_provider._resolve_azure_foundry_runtime` —
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second anakot
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``anakot config aux reset``)."""
...
def _callmemo_portal_account_has_fresh_paid_access() -> bool:
    ...
    try:
        ...
        logger.debug("Auxiliary callmemo paid-entitlement refresh check failed: %s", exc)
        return False
...
def _is_model_not_found_error(exc: Exception) -> bool:
    ...
    that has since been dropped from the callmemo → OpenRouter catalog. The callmemo
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    Priority:
      1. User's main provider + main model, regardless of provider type.
         ...
         chat.  Users on OpenRouter/callmemo get their chosen chat model; users
      ...
      2. OpenRouter → callmemo → custom → Codex → API-key providers (fallback
    ...
    #    scenario where a user switches providers via `anakot model` but the
    #    old OPENAI_BASE_URL lingers in ~/.anakot/.env. ──
    ...
    # on aggregators (OpenRouter, callmemo) who previously got routed to a
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but callmemo Portal not configured (run: anakot auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: anakot model)")
                return None, None
    ...
    # OpenRouter / callmemo bills for side tasks they thought were running on
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: anakot doctor for "
                "diagnostics)"
            )
            return None, None
...
def get_available_vision_backends() -> List[str]:
    ...
    Order: active provider → OpenRouter → callmemo → stop.  This is the single
    ...
    if main_provider and main_provider not in {"auto", ""}:
        if main_provider in _VISION_AUTO_PROVIDER_ORDER:
            ...
            if client is not None:
                available.append(main_provider)
    # 2. OpenRouter, 3. callmemo — skip if already covered by main provider.
...
def get_auxiliary_extra_body() -> dict:
    ...
    Includes callmemo Portal product tags when the auxiliary client is backed
    by callmemo Portal. Returns empty dict otherwise.
    """
...
def _refresh_nous_auxiliary_client(
    *,
    ...
    """Refresh callmemo runtime creds, rebuild the client, and replace the cache entry."""
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`anakot_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def _build_call_kwargs(
    ...
    if provider == "nous" or auxiliary_is_nous:
        merged_extra.setdefault("tags", []).extend(_callmemo_portal_tags())
    if merged_extra:
...
def call_llm(
    ...
    *,
        ...
        if client is None:
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                raise RuntimeError(
                    ...
                    f"variable, or switch to a different provider with `anakot model`."
                )
    ...
    try:
        ...
        # ── Stale-model self-heal (callmemo Portal recommendation drift) ───
        # A long-lived process can pin a Portal-recommended model that has
        # since been dropped from the callmemo → OpenRouter catalog, so every
        ...
        if _is_model_not_found_error(first_err) and _heal_is_nous:
            ...
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s: model %r no longer in callmemo catalog; "
        ...
        if (
            ...
            and client_is_nous
            and _callmemo_portal_account_has_fresh_paid_access()
        ):
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s: refreshed callmemo runtime credentials after paid account check, retrying",
                    task or "call",
                )
        ...
        if _is_auth_error(first_err) and client_is_nous:
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s: refreshed callmemo runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
...
async def async_call_llm(
    ...
    *,
    ...
    try:
        ...
        # the callmemo → OpenRouter catalog, 404'ing every auxiliary call. Force a
        ...
        if _is_model_not_found_error(first_err) and _heal_is_nous:
            ...
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s (async): model %r no longer in callmemo catalog; "
                    "retrying with refreshed recommendation %r",
                    task or "call", kwargs.get("model"), healed_model,
                )
                kwargs["model"] = healed_model
        ...
        if (
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s (async): refreshed callmemo runtime credentials after paid account check, retrying",
                    task or "call",
                )
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
        ...
        if _is_auth_error(first_err) and client_is_nous:
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s (async): refreshed callmemo runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
