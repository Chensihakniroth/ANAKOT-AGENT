...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Anakot starts and finds ``~/.openclaw/``.

    Points users at ``anakot claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``anakot claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Anakot, run "
        "`anakot claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `anakot claw cleanup` (renames it to ~/.openclaw.pre-migration — "
