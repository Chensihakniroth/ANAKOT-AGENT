...
as ``kind: backend``) or ``~/.anakot/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``anakot tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$ANAKOT_HOME/cache/videos/``, creating parents as needed."""
    from anakot_constants import get_anakot_home

    path = get_anakot_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$ANAKOT_HOME/cache/videos/``.
