...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Anakot with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.anakot/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Anakot
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``anakot tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``anakot setup`` for
