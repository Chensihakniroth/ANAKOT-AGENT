...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.anakot/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `anakot update` re-seeds.
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``anakot update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``anakot curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `anakot curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    ...
    "~/.anakot/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.anakot/logs/curator/``)
    ...
    authored skill data in ``~/.anakot/skills/``.

    ``ensure_anakot_home()`` pre-creates this dir on every CLI launch and
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every anakot install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: anakot curator status
        keep an umbrella stable: anakot curator pin document-tools
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: anakot curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.anakot/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.anakot/skills/.archive/` for "
            "safety and can be restored via `anakot curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.anakot/skills/.archive/`. "
            "Restore any via `anakot curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `anakot curator restore <name>`")
    lines.append("- All archives live under `~/.anakot/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `anakot curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `anakot update` only if "
        ...
        # recorded in state so `anakot curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``anakot model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `anakot model` → auxiliary
