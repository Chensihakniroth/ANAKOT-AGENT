...
# `export ANAKOT_REDACT_SECRETS=false`) cannot disable redaction
# mid-session.  ON by default — secure default per issue #17691. Users who
# need raw credential values in tool output (e.g. working on the redactor
# itself) can opt out via `security.redact_secrets: false` in config.yaml
# (bridged to this env var in anakot_cli/main.py, gateway/run.py, and
# cli.py) or `ANAKOT_REDACT_SECRETS=false` in ~/.anakot/.env. An opt-out
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across Anakot — used by
    ``anakot config``, ``anakot status``, ``anakot dump``, and anywhere
...
def redact_sensitive_text(text: str, *, force: bool = False, code_file: bool = False) -> str:
    ...
    for URLs, ``"eyJ" in text`` for JWTs). On a typical anakot log line
