"""Projects codex app-server events into Anakot' messages list.

The translator that lets Anakot' memory/skill review keep working under the
...
Anakot' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape Anakot
...
class CodexEventProjector:
    ...
    items but Anakot stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Anakot already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Anakot' messages store text only.
        text_parts: list[str] = []
