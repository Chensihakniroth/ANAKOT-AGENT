...
from anakot_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``ANAKOT_KANBAN_TASK`` / ``ANAKOT_KANBAN_BOARD`` in the
        ...
        # The Anakot Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``ANAKOT_PLATFORM`` or
            ``ANAKOT_SESSION_PLATFORM`` env vars.  Falls back to the
...
# which becomes the dominant cost of ``anakot`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.anakot/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``anakot`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against ANAKOT_HOME, not cwd
        if not p.is_absolute():
            p = (anakot_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.anakot/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    anakot = metadata.get("anakot") or {}
    if not isinstance(anakot, dict):
        anakot = {}
    return {
        "fallback_for_toolsets": anakot.get("fallback_for_toolsets", []),
        "requires_toolsets": anakot.get("requires_toolsets", []),
        "fallback_for_tools": anakot.get("fallback_for_tools", []),
        "requires_tools": anakot.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    anakot = metadata.get("anakot")
    if not isinstance(anakot, dict):
        return []
    raw = anakot.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Anakot metadata, VCS, virtualenv/dependency, and cache
