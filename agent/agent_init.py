...
def init_agent(
    ...
    Args:
        ...
        load_soul_identity (bool): If True, still use ~/.anakot/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from anakot_cli.config import load_config as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind ANAKOT_DEV_CREDITS.
    ...
    # both live under ~/.anakot/logs/.  Idempotent, so gateway mode
    ...
    if agent.verbose_logging:
        ...
        # on the fact that anakot_logging.setup_logging() does not
        ...
        # handler level inside anakot_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        try:
            from anakot_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `anakot model` to "
                        "select a provider, or run `anakot setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff ANAKOT_KANBAN_TASK is set).
    ...
    try:
        ...
        os.environ["ANAKOT_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.anakot/sessions/ alongside gateway sessions
    anakot_home = get_anakot_home()
    agent.logs_dir = anakot_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.anakot/sessions/session_{sid}.json)
    ...
    try:
        from anakot_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from anakot_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "anakot"
                    except Exception:
                        pass
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Anakot Agent.  Choose a model with at least "
