...
def compute_session_context_breakdown(agent: Any, messages: Optional[List[dict]] = None) -> Dict[str, Any]:
    """Return a context usage breakdown for one live agent."""
...
def compute_context_details(agent: Any) -> Dict[str, Any]:
    ...
    toolsets_map: Dict[str, Dict[str, Any]] = {}
    for tool in tools:
        fn = tool.get("function") if isinstance(tool, dict) else None
        name = str((fn if isinstance(fn, dict) else tool).get("name") or "")
        toolset = getattr(tool, "_toolset", None) or ("mcp" if name.startswith("mcp_") else "core")
        if toolset not in toolsets_map:
            toolsets_map[toolset] = {"toolset": toolset, "tool_count": 0, "json_bytes": 0}
        toolsets_map[toolset]["tool_count"] += 1
        toolsets_map[toolset]["json_bytes"] += len(json.dumps(tool, ensure_ascii=False))

    toolsets = sorted(toolsets_map.values(), key=lambda x: x["json_bytes"], reverse=True)
    return {
        "skills": [],
        "toolsets": [
            {
...
def render_context_grid(payload: Dict[str, Any]) -> List[str]:
    """Glyph grid: 100 cells, one per percent of the context window."""
...
def _table(lines: List[str], title: str, rows: List[Dict[str, Any]], fmt) -> None:
    """Append a titled, display-capped table."""
    if not rows:
        return
    if lines:
        lines.append("")
...
def render_context_details_lines(details: Dict[str, Any]) -> List[str]:
    """Render the expanded ``/context all`` per-toolset tables."""
    lines: List[str] = []
...
def render_context_breakdown_lines(
    ...
    *,
    ...
    if details is None:
        lines.extend(["", "Use /context all for per-toolset costs."])
