"""Credits tracking for callmemo inference API responses.
...
def parse_credits_headers(
    ...
    try:
        ...
        # sentinel header is absent (the common case for non-callmemo providers, on
        ...
        if version_val != 1:
            if version_val > 1 and not _version_warning_emitted:
                ...
                logger.warning(
                    "credits header version %d unsupported, ignoring — update Anakot",
                    version_val,
                )
            return None
...
# ── Dev test fixtures (ANAKOT_DEV_CREDITS_FIXTURE) ───────────────────────────
# Throwaway dev scaffolding: trigger any notice state on demand for testing,
# without real spend or Redis seeding. Set ANAKOT_DEV_CREDITS_FIXTURE to either a
...
# ANAKOT_DEV_CREDITS scaffolding.
...
def dev_fixture_credits_state() -> Optional[CreditsState]:
    """Return a fixture CreditsState for ANAKOT_DEV_CREDITS_FIXTURE, or None.
    ...
    Hard prod-leak guard: a fixture applies ONLY when the dev flag ANAKOT_DEV_CREDITS
    is also on, so a stray ANAKOT_DEV_CREDITS_FIXTURE (leaked into a shell profile, a
    container env, a launch plist, …) can never surface fabricated balances/notices
    on a real account.
    """
    if not is_truthy_value(os.environ.get("ANAKOT_DEV_CREDITS")):
        return None
    raw = os.environ.get("ANAKOT_DEV_CREDITS_FIXTURE", "").strip()
    if not raw:
        return None
    name = raw
