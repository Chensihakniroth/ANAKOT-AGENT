...
appropriate.  All installs go to a Anakot-owned bin staging dir,
``<ANAKOT_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``anakot lsp
  status``.
...
# ``<ANAKOT_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def anakot_lsp_bin_dir() -> Path:
    ...
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".anakot")
    p = Path(home) / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(anakot_lsp_bin_dir() / name):
...
def _install_npm(
    ...
    for c in _native_binary_candidates(nm_bin):
        if c.exists():
            ...
            link = anakot_lsp_bin_dir() / c.name
            if not link.exists():
                try:
                    link.symlink_to(c)
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a anakot-owned target dir.
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = anakot_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``anakot lsp status`` CLI to give users a quick
