...
A pre-run snapshot of ``~/.anakot/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.anakot/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.anakot/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _backups_dir() -> Path:
    return get_anakot_home() / "skills" / ".curator_backups"
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.anakot/cron/jobs.json``)."""
    return get_anakot_home() / "cron" / "jobs.json"
...
def snapshot_skills(reason: str = "manual") -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.anakot/skills/`` and prune old ones.
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.anakot/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.anakot/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `anakot curator rollback --list` to see available snapshots)",
            None,
        )
