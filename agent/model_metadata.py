...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Anakot also honours ANAKOT_CA_BUNDLE (its own convention)
...
# Minimum context length required to run Anakot Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # (native MiniMax-M3, OpenRouter/callmemo minimax/minimax-m3).
    ...
    # so these hardcoded fallbacks prevent Anakot from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    "inference-api.callmemo.ai": "nous",
    ...
    # (often 8K) makes it unusable for Anakot' system prompt, but mapping
...
def _model_name_suggests_minimax_m3(model: str) -> bool:
    ...
    across surfaces (native MiniMax-M3, OpenRouter/callmemo minimax/minimax-m3).
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Anakot grow conversations past the runtime limit and Ollama
...
def _normalize_model_version(model: str) -> str:
    ...
    callmemo uses dashes: claude-opus-4-6, claude-sonnet-4-5
...
def _resolve_nous_context_length(
    ...
    """Resolve callmemo Portal model context length.

    Tries the live callmemo inference endpoint first (authoritative), then falls
    back to OpenRouter metadata with suffix/version matching.

    callmemo model IDs are bare after prefix-stripping (e.g. 'qwen3.6-plus',
    ...
    Returns ``(context_length, source)`` where ``source`` is one of:
      ...
      - ``""``           — could not resolve
    """
    # Portal first — the callmemo /models endpoint is authoritative for what our
    ...
    def _safe_ctx(or_id: str, entry: dict) -> Optional[int]:
        ...
        if ctx <= 32768 and _model_name_suggests_kimi(or_id):
            logger.info(
                ...
                "(Kimi-family underreport, callmemo path); falling through to hardcoded defaults",
                ctx, or_id,
            )
            return None
        return ctx
...
def get_model_context_length(
    ...
    1. Persistent cache (previously discovered via probing).  callmemo URLs
    ...
    5. Provider-aware lookups (before generic OpenRouter cache):
       ...
       b. callmemo: live /v1/models probe first (authoritative), then OR
    ...
    if base_url and provider != "lmstudio":
        ...
        if cached is not None:
            ...
            # leftover — drop it and fall through to the hardcoded default.
                ...
                _invalidate_cached_context_length(model, base_url)
            # callmemo Portal: the portal /v1/models endpoint is authoritative.
            ...
            # cost amortise to ~0 within a process.
                ...
                logger.debug(
                    "Bypassing persistent cache for %s@%s (callmemo portal authoritative)",
                    model, base_url,
                )
    ...
    if effective_provider == "nous":
        ...
        if ctx:
            ...
            # want it leaking into the persistent cache for callmemo URLs.
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Anakot sends to providers:
