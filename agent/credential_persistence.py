...
``auth.json``.  They intentionally have no dependency on ``anakot_cli.auth`` so
...
# Sources Anakot owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Anakot-owned OAuth/device-code state)
