...
Anakot infers the row taxonomy from the sheet and maps agent activity onto
...
- :mod:`agent.pet.store`     — install / list / resolve pets on disk
                               (profile-aware via ``get_anakot_home()``).
- :mod:`agent.pet.render`    — decode a spritesheet (thumbnail, frame-count
                               helpers for the desktop canvas).
