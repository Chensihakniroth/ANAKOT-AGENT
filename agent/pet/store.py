...
Pets live under ``get_anakot_home()/pets/<slug>/`` so every profile gets its
own set.  Each installed pet directory holds:
...
def install_pet(slug: str, *, force: bool = False, timeout: float = _DOWNLOAD_TIMEOUT) -> InstalledPet:
    ...
    # Host-pin every asset URL to petdex.
...
def register_local_pet(
    ...
    *spritesheet* may be a PIL image, raw WebP/PNG bytes, or a path. The pet
    appears in :func:`installed_pets` immediately.
    """
    slug = slugify(slug)
    directory = pets_dir() / slug
    directory.mkdir(parents=True, exist_ok=True)
    ...
    if sheet_bytes is None and source_url and _is_petdex_host(source_url):
        try:
            ...
            resp = httpx.get(
                ...
                headers={"User-Agent": "anakot-agent-petdex"},
            )
...
def remove_pet(slug: str) -> bool:
    ...
    # dir, so rmtree won't catch it. Drop it too.
    try:
...
def rename_pet(slug: str, display_name: str) -> str | None:
    ...
    Returns the resulting slug on success, or ``None`` on failure.
    """
    slug = _safe_slug(slug)
