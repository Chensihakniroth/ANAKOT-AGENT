...
as ``kind: backend``) or ``~/.anakot/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``anakot tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``anakot tools`` picker.
...
def _images_cache_dir() -> Path:
    """Return ``$ANAKOT_HOME/cache/images/``, creating parents as needed."""
    from anakot_constants import get_anakot_home

    path = get_anakot_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$ANAKOT_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$ANAKOT_HOME/cache/images/``.
