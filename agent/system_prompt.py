...
upstream prefix cache warm.  See ``anakot-agent-dev``'s
...
def _ra():
    ...
    ``build_context_files_prompt``, ``build_callmemo_subscription_prompt``,
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Anakot never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want ANAKOT_HOME persona while keeping
    ...
    # ANAKOT_KANBAN_TASK env var). Normal chat sessions never see
    ...
    callmemo_subscription_prompt = _r.build_callmemo_subscription_prompt(agent.valid_tool_names)
    if callmemo_subscription_prompt:
        stable_parts.append(callmemo_subscription_prompt)
    ...
    # Active-profile hint — names the Anakot profile the agent is running
    # under so it doesn't conflate ~/.anakot/skills/ (default profile) with
    # ~/.anakot/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            ...
            "under ~/.anakot/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            ...
            f"and writes ~/.anakot/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.anakot/skills/, ~/.anakot/plugins/, "
            f"~/.anakot/cron/, ~/.anakot/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Anakot never rebuilds or reinjects parts of it
