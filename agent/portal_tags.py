"""Centralized callmemo Portal request tags.

Every Anakot request that hits the callmemo Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
callmemo can attribute usage to Anakot Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=anakot-agent",
        "client=anakot-client-v<__version__>",
    ]

The version is sourced live from ``anakot_cli.__version__`` so it auto-aligns
...
``anakot_cli.__version__`` is the canonical source of truth.
"""
...
def _anakot_version() -> str:
    """Return the current Anakot release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``anakot_cli`` cannot be imported (should
...
def anakot_client_tag() -> str:
    """Return the ``client=...`` tag for callmemo Portal requests.

    Format: ``client=anakot-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
