...
Plugins built on Anakot Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Anakot
...
:class:`~anakot_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Anakot' main config
...
handles every provider, fallback chain, and per-task override Anakot
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`anakot_cli.plugins.PluginContext`
