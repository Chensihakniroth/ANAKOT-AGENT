"""Anthropic Messages API adapter for Anakot Agent.

Translates between Anakot's internal OpenAI-style message format and
...
# Anakot effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Anakot historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Anakot)
    ...
    # 1. Anakot-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Anakot configs.
...
# ── Anakot-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.anakot/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_ANAKOT_OAUTH_FILE = get_anakot_home() / ".anthropic_oauth.json"
...
def run_anakot_oauth_login_pure() -> Optional[Dict[str, Any]]:
    ...
    print()
    print("Authorize Anakot with your Claude Pro/Max subscription.")
    print()
...
def read_anakot_oauth_credentials() -> Optional[Dict[str, Any]]:
    ...
    if _ANAKOT_OAUTH_FILE.exists():
        try:
            data = json.loads(_ANAKOT_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Anakot OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See anakot-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for m in result:
        if m["role"] == "assistant" and isinstance(m["content"], list):
            ...
            # replaying it verbatim.  See anakot-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See anakot-agent#13848 (Kimi) and
    anakot-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Anakot Agent", "Claude Code")
                text = text.replace("Anakot agent", "Claude Code")
                text = text.replace("anakot-agent", "claude-code")
                text = text.replace("callmemo", "Anthropic")
                block["text"] = text
    ...
    # silently hides reasoning text that Anakot surfaces in its CLI. We
