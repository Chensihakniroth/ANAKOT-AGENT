...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Anakot tool use: "
    ...
    return (
        ...
        f"context, but Anakot needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Anakot config, set `model.ollama_num_ctx: 65536` "
...
def _is_nous_inference_route(provider: str, base_url: str) -> bool:
    ...
    return (
        base_url_host_matches(base, "inference-api.callmemo.ai")
        or base_url_host_matches(base, "inference.callmemo.ai")
    )
...
def _try_refresh_nous_paid_entitlement_credentials(agent) -> bool:
    """Refresh callmemo runtime credentials after a fresh paid-entitlement check."""
    try:
...
def run_conversation(
    ...
    # ``anakot logs --session <id>`` can filter a single conversation.
    ...
    # are reused.  The system prompt is Anakot's territory; plugins
    ...
    # all run inside Codex). Default Anakot path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Anakot internals.
        #
        # Anakot invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Anakot tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            # ── callmemo Portal rate limit guard ──────────────────────
            # If another session already recorded that callmemo is rate-
            # limited, skip the API call entirely.  Each attempt
            # (including SDK-level retries) counts against RPH and
            # deepens the rate limit hole.
            if agent.provider == "nous":
                try:
                    from agent.callmemo_rate_guard import (
                    ...
                    if _nous_remaining is not None and _nous_remaining > 0:
                        _nous_msg = (
                            f"callmemo Portal rate limit active — "
            ...
            try:
                ...
                # Clear callmemo rate limit state on successful request —
                # proves the limit has reset and other sessions can
                # resume hitting callmemo.
                if agent.provider == "nous":
                    try:
                        from agent.callmemo_rate_guard import clear_nous_rate_limit
                ...
                if (
                    ...
                    if _try_refresh_nous_paid_entitlement_credentials(agent):
                        agent._vprint(
                            f"{agent.log_prefix}🔐 callmemo paid access verified — "
                ...
                if (
                    ...
                    if agent._try_refresh_nous_client_credentials(force=True):
                        print(f"{agent.log_prefix}🔐 callmemo agent key refreshed after 401. Retrying request...")
                        continue
                    ...
                    try:
                        ...
                        pass
                    print(f"{agent.log_prefix}🔐 callmemo 401 — Portal authentication failed.")
                    if _body_text:
                        print(f"{agent.log_prefix}   Response: {_body_text}")
                    if not _print_nous_entitlement_guidance(agent, "callmemo model access"):
                        print(f"{agent.log_prefix}   Most likely: Portal OAuth expired, account out of credits, or agent key revoked.")
                    print(f"{agent.log_prefix}   Troubleshooting:")
                    print(f"{agent.log_prefix}     • Re-authenticate: anakot auth add nous")
                    print(f"{agent.log_prefix}     • Check credits / billing: https://portal.callmemo.ai")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `anakot doctor` for credential-chain diagnostics, or")
                    ...
                    from anakot_constants import display_anakot_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Anakot-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: anakot config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: anakot config set ANTHROPIC_API_KEY \"\"")
                ...
                # ── callmemo Portal: record rate limit & skip retries ─────
                # When callmemo returns a 429 that is a genuine account-
                ...
                #
                # IMPORTANT: callmemo Portal multiplexes multiple upstream
                # providers (DeepSeek, Kimi, MiMo, Anakot).  A 429 can
                # also mean an UPSTREAM provider is out of capacity
                # for one specific model -- transient, clears in
                # seconds, nothing to do with the caller's quota.
                # Tripping the cross-session breaker on that would
                # block every callmemo model for minutes.  We use
                ...
                if (
                    ...
                    try:
                        ...
                        if _genuine_nous_rate_limit:
                            ...
                            logger.info(
                                "callmemo 429 looks like upstream capacity "
                ...
                # which Anakot' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Anakot' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`anakot",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `anakot auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `anakot model`.", force=True)
                            else:  # nous
                                agent._vprint(f"{agent.log_prefix}   💡 callmemo Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: anakot portal", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Check your portal account: https://portal.callmemo.ai", force=True)
                                # ``:free`` is OpenRouter slug syntax; callmemo Portal will reject
                                ...
                                if isinstance(_model, str) and _model.endswith(":free"):
                                    ...
                                    agent._vprint(f"{agent.log_prefix}         callmemo Portal won't recognize that model name. Either switch to a", force=True)
                                    agent._vprint(f"{agent.log_prefix}         callmemo catalog model, or run `/model openrouter:{_model}` to use OpenRouter.", force=True)
                        else:
                            agent._vprint(f"{agent.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: anakot setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        anakot fallback add   (interactive picker — same as `anakot model`)",
                            force=True,
                        )
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        _policy_response = (
                            ...
                            f"(not a Anakot/gateway failure).\n\n"
                            f"Provider message: {_summary}\n\n"
                            f"Try rephrasing the request, narrowing the context, or "
                            f"adding a fallback provider with `anakot fallback add`."
                        )
                        return {
    ...
    if final_response is None and (
        ...
        _kanban_task = os.environ.get("ANAKOT_KANBAN_TASK")
        if _kanban_task:
            try:
