...
Storage
-------
Bundles live in ``~/.anakot/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``ANAKOT_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``anakot bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under ANAKOT_HOME.

    Honors ``ANAKOT_BUNDLES_DIR`` for tests; falls back to
    ``<ANAKOT_HOME>/skill-bundles``.
    """
    override = os.environ.get("ANAKOT_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_anakot_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `anakot bundles` CLI subcommand.
# ---------------------------------------------------------------------------
