...
Anakot seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.anakot/.env
    claude_code   — ~/.claude/.credentials.json
    anakot_pkce   — ~/.anakot/.anthropic_oauth.json
    ...
    manual        — user ran `anakot auth add`
...
unify here is **removal**:
    ...
    ``anakot auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), anakot_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.anakot/.env  → clear it
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Anakot stops reading it.
    """
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    """callmemo OAuth lives in auth.json providers.nous — clear it and suppress.

    We suppress in addition to clearing because nothing else stops a future
    `anakot auth add nous` (or any other path that writes providers.nous)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `anakot auth add nous` to re-engage, which is the
...
def _remove_minimax_oauth(provider: str, removed) -> RemovalResult:
    ...
    Same pattern as callmemo: single-source OAuth state with refresh tokens.
...
def _remove_xai_oauth_loopback_pkce(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``anakot auth remove xai-oauth <N>`` silently undoes
    ...
    Belt-and-braces against the manual entry path: ``anakot auth add
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Anakot auth store is not enough — _seed_from_singletons() would
    ...
    via ``anakot auth add openai-codex``), but in both cases the re-seed
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Anakot stops picking the token up.
    """
