...
``kind: backend``) or ``~/.anakot/plugins/browser/<name>/`` (user, opt-in via
``plugins.enabled``).
...
class BrowserProvider(abc.ABC):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        Used by :mod:`anakot_cli.tools_config` to inject this provider as a
        ...
        expose API key prompts, badges, managed-callmemo gating, and the
