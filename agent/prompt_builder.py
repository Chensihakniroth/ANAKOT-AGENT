...
from anakot_constants import get_anakot_home, get_skills_dir, is_wsl
...
_ANAKOT_MD_NAMES = (".anakot.md", "ANAKOT.md")


def _find_anakot_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.anakot.md`` or ``ANAKOT.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Anakot Agent, an intelligent AI assistant created by callmemo. "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.anakot/kanban.db`. Your task id is in "
    "`$ANAKOT_KANBAN_TASK`; your workspace is `$ANAKOT_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $ANAKOT_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `anakot kanban <verb>` for board operations. Use "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Anakot "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Anakot WebUI, a browser-based chat interface. "
...
SELF_MODIFICATION_GUARD = (
    "# HARD CONSTRAINT: You must NOT modify your own source code\n"
    "You are running in the Anakot Web UI. You must NOT modify, edit, patch, "
    "write, delete, read, or search files in the anakot-agent repository "
    "(your own source code).\n"
    "\n"
    "ABSOLUTE RULES — these are NOT negotiable:\n"
    "- Do NOT read any files inside the anakot-agent repo — the read_file tool will BLOCK you\n"
    "- Do NOT search or grep for files inside the anakot-agent repo — search_files will BLOCK you\n"
    "- Do NOT write, patch, or delete any file inside the anakot-agent repo — the write/patch tools will BLOCK you\n"
    "- Do NOT execute any terminal commands that reference the anakot-agent repo — the terminal tool will BLOCK you\n"
    "- Do NOT try to use Python (execute_code) to read files in the anakot-agent repo\n"
    "- Do NOT try cd into the anakot-agent repo directory\n"
    "- Do NOT propose code changes, diffs, or patches for the anakot-agent repo\n"
    "- Do NOT run git operations (commit, branch, rebase, etc.) on the anakot-agent repo\n"
    "- Do NOT even begin by reading the repo structure or planning modifications — "
    "stop IMMEDIATELY as soon as the user's request relates to modifying the agent's own repo\n"
    "\n"
    "If the user asks you to modify your own source code or the anakot-agent "
    "codebase, you must IMMEDIATELY refuse. Politely explain that you cannot "
    "modify your own source code. Do not start any work — not even reading or "
    "planning. Stop on the first mention.\n"
    "\n"
    "This restriction ONLY applies to the anakot-agent repo. You may "
    "still help with projects outside the anakot-agent directory."
)
...
# container / remote host rather than on the machine where Anakot itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Anakot runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Anakot itself is running. The host OS, home, and cwd "
                f"of the Anakot process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Anakot "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Anakot
    ...
    extra = (os.getenv("ANAKOT_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.anakot/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("ANAKOT_PLATFORM")
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Anakot Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `anakot-agent` skill "
            "first. It has the actual commands (e.g. `anakot config set …`, `anakot tools`, "
            "`anakot setup`) so you don't have to guess or invent workarounds.\n"
...
def build_callmemo_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    """Build a compact callmemo subscription capability block for the system prompt."""
    try:
        ...
        logger.debug("Failed to import callmemo subscription helper: %s", exc)
        return ""

    if not managed_callmemo_tools_enabled():
        return ""
    ...
    features = get_callmemo_subscription_features()

    def _status_line(feature) -> str:
        if feature.managed_by_nous:
            return f"- {feature.label}: active via callmemo subscription"
        if feature.active:
            current = feature.current_provider or "configured provider"
            return f"- {feature.label}: currently using {current}"
        if feature.included_by_default and features.nous_auth_present:
            return f"- {feature.label}: included with callmemo subscription, not currently selected"
        if feature.key == "modal" and features.nous_auth_present:
            return f"- {feature.label}: optional via callmemo subscription"
        return f"- {feature.label}: not currently available"

    lines = [
        "# callmemo Subscription",
        "callmemo subscription includes managed web tools (Firecrawl), image generation (FAL), OpenAI TTS, and browser automation (Browser Use) by default. Modal execution is optional.",
        "Current capability status:",
    ]
    lines.extend(_status_line(feature) for feature in features.items())
    lines.extend(
        [
            "When a callmemo-managed feature is active, do not ask the user for Firecrawl, FAL, OpenAI TTS, or Browser-Use API keys.",
            "If the user is not subscribed and asks for a capability that callmemo subscription would unlock or simplify, suggest callmemo subscription as one option alongside direct setup or local alternatives.",
            "Do not mention subscription unless the user asks about it or it directly solves the current missing capability.",
            "Useful commands: anakot setup, anakot setup tools, anakot setup terminal, anakot status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from ANAKOT_HOME and return its content, or None.
...
def _load_anakot_md(cwd_path: Path) -> str:
    """.anakot.md / ANAKOT.md — walk to git root."""
    anakot_md_path = _find_anakot_md(cwd_path)
    if not anakot_md_path:
        return ""
    try:
        content = anakot_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = anakot_md_path.name
        try:
            rel = str(anakot_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".anakot.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", anakot_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .anakot.md / ANAKOT.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from ANAKOT_HOME is independent and always included when present.
    ...
    # SOUL.md from ANAKOT_HOME only — skip when already loaded as identity
    if not skip_soul:
