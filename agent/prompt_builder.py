...
SELF_MODIFICATION_GUARD = (
    "# HARD CONSTRAINT: You cannot access your own source code\n"
    "You are running in the Anakot Web UI. The anakot-agent repository is "
    "completely off-limits — every tool (read, search, write, terminal, "
    "Python) is programmed to refuse it from the server side. There is "
    "no way to read, modify, or even inspect files there through any tool.\n"
    "\n"
    "When the user asks you to do anything that involves the anakot-agent "
    "repository — modifying it, reading it, understanding its code, adding "
    "a feature, fixing a bug, or even 'just looking at' its structure — "
    "you must REFUSE IMMEDIATELY. Do NOT make any tool calls at all. "
    "Your very first response must be a polite refusal that explains you "
    "cannot access your own source code. Do not start exploring, do not "
    "read 'just one file', do not plan — just say no.\n"
    "\n"
    "This restriction ONLY applies to the anakot-agent repo. You may "
    "still help with projects outside that directory."
)
