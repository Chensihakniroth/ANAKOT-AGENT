...
ANAKOT_HOME_DIR="${ANAKOT_HOME:-$HOME/.anakot}"
ANAKOT_CFG="${ANAKOT_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Anakot config has twozero_td MCP entry ──
if [[ ! -f "$ANAKOT_CFG" ]]; then
    echo -e " ${FAIL} Anakot config not found at ${ANAKOT_CFG}"
    manual_steps+=("Create ${ANAKOT_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ANAKOT_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Anakot config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Anakot config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ANAKOT_CFG} manually"); }
    manual_steps+=("Restart Anakot session to pick up config change")
fi
