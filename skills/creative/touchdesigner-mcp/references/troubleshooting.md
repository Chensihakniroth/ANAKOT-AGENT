...
## 5. Anakot Configuration

### Config location

`$ANAKOT_HOME/config.yaml` (defaults to `~/.anakot/config.yaml` when `ANAKOT_HOME` is unset)
...
Restart the Anakot session for changes to take effect. The MCP connection is
