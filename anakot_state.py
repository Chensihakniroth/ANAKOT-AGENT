#!/usr/bin/env python3
"""
SQLite State Store for Anakot Agent.
...
DEFAULT_DB_PATH = get_anakot_home() / "state.db"
...
# anakot_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``anakot_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see anakot_cli/kanban_db.py) would
...
class SessionDB:
    ...
    # With multiple anakot processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            # ``anakot_state._set_last_init_error(None)`` explicitly.
    ...
    def get_messages_as_conversation(
        ...
        for row in rows:
            ...
            # that replay reasoning (OpenRouter, OpenAI, callmemo) receive
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another Anakot surface and jump
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Anakot, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Anakot session.

        A Anakot session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Anakot session is already bound to any Telegram DM topic.
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``ANAKOT_DISABLE_FTS_TRIGRAM`` or not yet created), so
