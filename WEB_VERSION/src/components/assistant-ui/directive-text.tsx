...
const ANAKOT_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
...
const ICON_PATHS: Record<AnakotRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as AnakotRefType] ?? ICON_FALLBACK
...
const ANAKOT_DIRECTIVE_RE = new RegExp(
  ...
  const matches = [
    ...
    ...Array.from(text.matchAll(ANAKOT_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as AnakotRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: AnakotRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
export function DirectiveContent({ text }: { text: string }) {
  ...
  const segments = useMemo(() => anakotDirectiveFormatter.parse(cleanedText), [cleanedText])
