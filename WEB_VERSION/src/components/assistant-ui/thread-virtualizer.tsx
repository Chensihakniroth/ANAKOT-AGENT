...
function useThreadScrollAnchor({
  ...
  useEffect(() => {
    ...
    // When session changes, reset the group-count tracker so the subsequent
    // message load triggers a scroll — sessionChanged fires before messages
    // load, so jumpToBottom sees the old groupCount and the becameNonEmpty
    // check is missed.
    prevGroupCountRef.current = sessionChanged ? 0 : groupCount
