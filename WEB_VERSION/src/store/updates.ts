...
export function reportBackendContract(contract: number | undefined): void {
  ...
  notify({
    action: { label: translateNow('notifications.updateAnakot'), onClick: () => void applyUpdates() },
    durationMs: 0,
    id: SKEW_TOAST_ID,
    kind: 'warning',
...
export async function checkUpdates(): Promise<DesktopUpdateStatus | null> {
  ...
  try {
    ...
    // The update check pulls the latest anakot_cli + bundled package metadata
...
function onFocus() {
  ...
  // updated Anakot from another window/CLI between focuses, and About should
