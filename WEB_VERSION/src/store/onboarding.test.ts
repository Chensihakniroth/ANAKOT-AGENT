...
describe('OAuth onboarding', () => {
  ...
  it('clears stale readiness errors after OAuth succeeds and model confirmation is shown', async () => {
    ...
    $desktopOnboarding.set(
      baseState({
        flow: {
          ...
          provider: provider('nous', 'callmemo Portal'),
          start: {
        ...
        reason:
          'No access token found for callmemo Portal login. setup.status reports configured credentials, but runtime resolution still failed.',
        requested: true
      })
    )
    ...
    if (state.flow.status === 'confirming_model') {
      expect(state.flow.label).toBe('callmemo Portal')
