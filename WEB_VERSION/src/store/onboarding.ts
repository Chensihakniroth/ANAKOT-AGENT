...
import type { ModelOptionProvider, OAuthProvider, OAuthStartResponse } from '@/types/anakot'
...
// Human-friendly labels for tools auto-routed through the callmemo Tool Gateway,
// mirroring anakot_cli/callmemo_subscription._GATEWAY_TOOL_LABELS so the GUI and
...
// When switching to callmemo auto-routes unconfigured tools through the Tool
...
function notifyGatewayTools(tools: string[] | undefined) {
  ...
  notify({
    durationMs: 8000,
    kind: 'info',
    message: `${list} now run through your callmemo subscription — no separate API keys needed.`,
...
async function fetchProviderDefaultModel(
  ...
  // `anakot model` does (for callmemo it honors the user's free/paid tier, so a
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Anakot still cannot reach ${url}.` }
    }
