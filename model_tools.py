...
#   - cli.py, anakot_cli/*      -> inline on startup (no event loop)
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if os.environ.get("ANAKOT_KANBAN_TASK") and "kanban" not in effective_enabled_toolsets:
            # Dispatcher-spawned workers are scoped by ANAKOT_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like anakot-cli)
    ...
    # window). Core Anakot tools (toolsets._ANAKOT_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from anakot_cli.config import load_config as _load
        cfg = _load() or {}
...
def handle_function_call(
    ...
    try:
        ...
        if not skip_pre_tool_call_hook:
            ...
            try:
                from anakot_cli.plugins import get_pre_tool_call_block_message
