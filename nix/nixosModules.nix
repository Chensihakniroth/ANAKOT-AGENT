# nix/nixosModules.nix — NixOS module for anakot-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: anakot runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $ANAKOT_HOME/.env
# and read by anakot at startup — no container recreation needed for env changes.
#
# Tool resolution: the anakot wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.anakot-agent;
    effectivePackage =
    ...
    anakot-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-anakot-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "anakot-config-attrs";
      description = "Anakot YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "anakot-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "anakot-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'ANAKOT_DOC_EOF'\n${value}\nANAKOT_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "anakot-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/anakot";
    ...
    # anakot user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "anakot-container-entrypoint" ''
      set -eu

      ANAKOT_UID="''${ANAKOT_UID:?ANAKOT_UID must be set}"
      ANAKOT_GID="''${ANAKOT_GID:?ANAKOT_GID must be set}"

      # ── Group: ensure a group with GID=$ANAKOT_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$ANAKOT_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="anakot"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$ANAKOT_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$ANAKOT_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$ANAKOT_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$ANAKOT_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="anakot"
        TARGET_HOME="/home/anakot"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$ANAKOT_UID" -g "$ANAKOT_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$ANAKOT_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$ANAKOT_UID:$ANAKOT_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure ANAKOT_HOME is owned by the target user.
      ...
      if [ -n "''${ANAKOT_HOME:-}" ] && [ -d "$ANAKOT_HOME" ]; then
        find "$ANAKOT_HOME" \! -user "$ANAKOT_UID" -exec chown "$ANAKOT_UID:$ANAKOT_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/anakot-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/anakot-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/anakot ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/anakot
        chmod 0440 /etc/sudoers.d/anakot
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$ANAKOT_UID" --regid="$ANAKOT_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $ANAKOT_HOME/.env.
    ...
    # Default: /var/lib/anakot/workspace → /data/workspace.
  ...
  in {
    options.services.anakot-agent = with lib; {
      enable = mkEnableOption "Anakot Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = anakot-agent;
        description = "The anakot-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/anakot";
        description = "State directory. Contains .anakot/ subdir (ANAKOT_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Anakot config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $ANAKOT_HOME/.env at activation time.
          Anakot reads this file on every startup via load_anakot_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $ANAKOT_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $ANAKOT_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `anakot gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the anakot user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the anakot plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Anakot discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          anakot_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as anakot (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in anakot-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the anakot CLI to environment.systemPackages and export
          ANAKOT_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.anakot symlink to the service
            stateDir. These users are automatically added to the anakot group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.anakot-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the anakot CLI to system PATH and export ANAKOT_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.anakot/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.ANAKOT_HOME = "${cfg.stateDir}/.anakot";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.anakot-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the anakot user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.anakot-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed anakot
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure anakot is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.anakot        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.anakot/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.anakot/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.anakot/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.anakot/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.anakot/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."anakot-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.anakot
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.anakot ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.anakot ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.anakot -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.anakot/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.anakot/$_subdir"
            chmod 2770 "${cfg.stateDir}/.anakot/$_subdir"
            find "${cfg.stateDir}/.anakot/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.anakot/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.anakot/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.anakot/config.yaml
            chmod 0640 ${cfg.stateDir}/.anakot/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.anakot/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.anakot/.managed
          chmod 0644 ${cfg.stateDir}/.anakot/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.anakot/.container-mode <<'ANAKOT_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    anakot_bin=${containerDataDir}/current-package/bin/anakot
    ANAKOT_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.anakot/.container-mode
            chmod 0644 ${cfg.stateDir}/.anakot/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.anakot/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.anakot";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.anakot" ]; then
                  rm -f "${symlinkPath}"
                  echo "anakot-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.anakot -> stateDir/.anakot for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.anakot";
                target = "${cfg.stateDir}/.anakot";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "anakot-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.anakot/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.anakot/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.anakot/auth.json
              fi
            ''}
          ''}
          ...
          # Anakot reads $ANAKOT_HOME/.env at startup via load_anakot_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.anakot/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'ANAKOT_NIX_ENV_EOF'
    ${envFileContent}
    ANAKOT_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.anakot/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.anakot/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.anakot/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.anakot-agent = {
          description = "Anakot Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            ANAKOT_HOME = "${cfg.stateDir}/.anakot";
            ANAKOT_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $ANAKOT_HOME/.env by the activation script. load_anakot_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/anakot"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the anakot group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.anakot-agent = {
          description = "Anakot Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ANAKOT_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              ANAKOT_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env ANAKOT_UID="$ANAKOT_UID" \
                --env ANAKOT_GID="$ANAKOT_GID" \
                --env ANAKOT_HOME=${containerDataDir}/.anakot \
                --env ANAKOT_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/anakot gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
