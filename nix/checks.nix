...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      anakot-agent = self'.packages.default;
      anakotVenv = anakot-agent.anakotVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "anakot-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${anakotVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "anakot-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "anakot-build-package" { } ''
          echo "PASS: package built at ${anakot-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "anakot-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "anakot-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${anakot-agent}/bin/anakot || (echo "FAIL: anakot binary missing"; exit 1)
          test -x ${anakot-agent}/bin/anakot-agent || (echo "FAIL: anakot-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${anakot-agent}/bin/anakot version 2>&1 | grep -qi "anakot" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "anakot-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in anakot anakot-agent anakot-acp; do
            test -x ${anakot-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "anakot-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking anakot --help ==="
          ${anakot-agent}/bin/anakot --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${anakot-agent}/bin/anakot --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "anakot-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${anakot-agent}/share/anakot-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${anakot-agent}/share/anakot-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "ANAKOT_BUNDLED_SKILLS" ${anakot-agent}/bin/anakot || \
            (echo "FAIL: ANAKOT_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: ANAKOT_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "anakot-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${anakot-agent}/share/anakot-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${anakot-agent}/share/anakot-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "ANAKOT_BUNDLED_PLUGINS" ${anakot-agent}/bin/anakot || \
            (echo "FAIL: ANAKOT_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: ANAKOT_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "anakot-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${anakot-agent}/share/anakot-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${anakot-agent}/share/anakot-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${anakot-agent}/share/anakot-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "ANAKOT_BUNDLED_LOCALES" ${anakot-agent}/bin/anakot || \
            (echo "FAIL: ANAKOT_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: ANAKOT_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (ANAKOT_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && ANAKOT_BUNDLED_LOCALES=${anakot-agent}/share/anakot-agent/locales \
            ${anakotVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with ANAKOT_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${anakotVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${anakotVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without ANAKOT_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "anakot-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${anakot-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${anakot-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "ANAKOT_TUI_DIR" ${anakot-agent}/bin/anakot || \
            (echo "FAIL: ANAKOT_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: ANAKOT_TUI_DIR set in wrapper"
        ...
        # Verify ANAKOT_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        anakot-node = pkgs.runCommand "anakot-node-version" { } ''
          set -e
          echo "=== Checking ANAKOT_NODE in wrapper ==="
          grep -q "ANAKOT_NODE" ${anakot-agent}/bin/anakot || \
            (echo "FAIL: ANAKOT_NODE not set in wrapper"; exit 1)
          echo "PASS: ANAKOT_NODE present in wrapper"

          ANAKOT_NODE=$(sed -n "s/^export ANAKOT_NODE='\(.*\)'/\1/p" ${anakot-agent}/bin/anakot)
          test -x "$ANAKOT_NODE" || (echo "FAIL: ANAKOT_NODE=$ANAKOT_NODE not executable"; exit 1)
          echo "PASS: ANAKOT_NODE executable at $ANAKOT_NODE"

          NODE_MAJOR=$("$ANAKOT_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All ANAKOT_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify ANAKOT_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "anakot-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(ANAKOT_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking ANAKOT_MANAGED guards ==="
          check_blocked "config set" ${anakot-agent}/bin/anakot config set model foo
          check_blocked "config edit" ${anakot-agent}/bin/anakot config edit
        ...
        extra-python-packages = let
          ...
          anakotWithExtra = anakot-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "anakot-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${anakotWithExtra}/bin/anakot || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${anakotWithExtra}/bin/anakot || \
          ...
          if grep -q "PYTHONPATH" ${anakot-agent}/bin/anakot; then
        ...
        extra-dependency-groups = let
          anakotWithGroups = anakot-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "anakot-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${anakotWithGroups}"
          echo "venv: ${anakotWithGroups.anakotVenv}"
        ...
        messaging-variant = pkgs.runCommand "anakot-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.anakotVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "anakot-config-roundtrip" {
          ...
          merge_and_load() {
            ...
            ${configMergeScript} ${nixSettings} "$anakot_home/config.yaml"
            ${anakotVenv}/bin/python3 -c '
import json, sys
