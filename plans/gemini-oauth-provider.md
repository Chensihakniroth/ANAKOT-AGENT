...
## Client ID
- Need to register a "Desktop app" OAuth client on a callmemo GCP project
...
## Token Lifecycle
- Store at `~/.anakot/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `anakot_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `anakot_cli/models.py` — Add Gemini model catalog
4. `anakot_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `anakot_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `anakot_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
## Prerequisites
- callmemo GCP project with Desktop OAuth client registered
- OR: accept user-provided client_id via ANAKOT_GEMINI_CLIENT_ID env var
...
- anakot-agent Copilot OAuth: `anakot_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
