...
Uses a file-based lock (~/.anakot/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `anakot update` reloads
# the module) fail with ModuleNotFoundError for anakot_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from anakot_constants import get_anakot_home
from anakot_cli._subprocess_compat import windows_hide_flags
from anakot_cli.config import load_config, _expand_env_vars
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``anakot tools`` config for the ``cron`` platform.
...
def _get_anakot_home() -> Path:
    """Resolve Anakot home dynamically while preserving test monkeypatch hooks."""
...
def _job_profile_context(job_id: str, profile: Optional[str]):
    """Temporarily run a job under a specific Anakot profile.
    ...
    the scheduler's test/override hook and a context-local Anakot home override
    both point at the resolved profile directory so _get_anakot_home(),
    .env/config loading, script resolution, AIAgent construction, and downstream
    get_anakot_home() callers agree on the same home.
    ...
    override_token = None
    try:
        override_token = set_anakot_home_override(profile_home)
        _anakot_home = profile_home
        logger.info(
            "Job '%s': using Anakot profile '%s' (%s)",
            job_id,
            normalized_profile,
            profile_home,
        )
        ...
        _anakot_home = prior_override
        if override_token is not None:
            reset_anakot_home_override(override_token)
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("ANAKOT_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid ANAKOT_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within ANAKOT_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    Args:
        script_path: Path to the script.  Relative paths are resolved
            against ANAKOT_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_anakot_home() / "scripts"
    ...
    # escape — scripts MUST reside within ANAKOT_HOME/scripts/.
    try:
    ...
    run_env["ANAKOT_HOME"] = str(_get_anakot_home())
    try:
...
def _run_job_impl(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    os.environ["ANAKOT_CRON_SESSION"] = "1"
    ...
    # gateway message. Do not seed ANAKOT_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads ANAKOT_SESSION_PLATFORM
    #     and ANAKOT_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via ANAKOT_CRON_AUTO_DELIVER_* below.
    ...
    #     send_message gate read ANAKOT_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the ANAKOT_CRON_AUTO_DELIVER_* vars set
    # below, so clearing ANAKOT_SESSION_* here does not affect delivery.
    ...
    try:
        ...
        try:
            load_dotenv(str(_get_anakot_home() / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_get_anakot_home() / ".env"), override=True, encoding="latin-1")
        ...
        model = job.get("model") or os.getenv("ANAKOT_MODEL") or ""
        ...
        prefill_file = (
            os.getenv("ANAKOT_PREFILL_MESSAGES_FILE", "")
        ...
        try:
            # Do not inject ANAKOT_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # ANAKOT_HOME. When a workdir is configured, also inject project
        ...
        # override via ANAKOT_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("ANAKOT_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid ANAKOT_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
**Job ID:** {job_id}
**Run Time:** {_anakot_now().strftime('%Y-%m-%d %H:%M:%S')}
...
def tick(verbose: bool = True, adapters=None, loop=None, sync: bool = True) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _anakot_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _anakot_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set ANAKOT_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("ANAKOT_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
        ...
        # Anakot home override, scheduler _anakot_home hook, and temporary
