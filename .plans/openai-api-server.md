# OpenAI-Compatible API Server for Anakot Agent
...
SSE streaming. By exposing this endpoint, anakot-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  anakot-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.anakot/.env`
2. Run `anakot gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with anakot-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns anakot-agent as a model) |
...
Authorization: Bearer anakot-api-key-here
...
But anakot-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "anakot-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured anakot-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of anakot-agent's core prompt
...
Once implemented, anakot-agent works as a drop-in backend for:
