...
export async function fetchJSON<T>(
  ...
  if (res.status === 401) {
    ...
    if (!window.__ANAKOT_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      ...
      try {
        alreadyReloaded =
          sessionStorage.getItem("anakot.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("anakot.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    try {
      sessionStorage.removeItem("anakot.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__ANAKOT_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the Anakot dashboard server");
}
 ...
 * callmemo Portal contract V1 (the access token has no email/name claims —
