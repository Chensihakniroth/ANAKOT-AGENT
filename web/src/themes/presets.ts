...
/**
 * callmemo Blue — the inverted "light mode" Anakot look, ported from the
 ...
 * the orange midground reads as #0053FD callmemo-blue on screen, against a
 * cream `#E8F2FD` canvas.
 *
 * Note on bg blend mode: the DS Lens uses `multiply` for LENS_5I because
 * nousnet-web's <body> is white; anakot-agent's App root is `bg-black`,
...
export const nousBlueTheme: DashboardTheme = {
  name: "nous-blue",
  label: "callmemo Blue",
  description: "Light mode — vivid callmemo-blue accents on cream canvas",
  palette: {
  ...
  // bright callmemo-blue canvas — the coral is the "red" users see for
  // Output values without these overrides. Source → on-screen:
  //   Input:  #ffe6cb → #001934 (dark navy)        ← unchanged
  //   Output: #ffac02 → #0053fd (vivid callmemo-blue)  ← brand accent
  ...
  // callmemo-blue so the "primary" series in token-flow charts reads as
  ...
  //   white → vivid callmemo-blue → cream/light-blue
