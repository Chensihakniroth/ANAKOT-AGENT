#!/usr/bin/env python3
"""
Anakot Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Anakot Agent, inspired by Claude Code.
...
os.environ["ANAKOT_QUIET"] = "1"  # Our own modules
...
from anakot_cli.banner import _format_context_length, format_banner_version_label
...
def _resolve_prefill_messages_file(config: Dict[str, Any]) -> str:
    ...
    """
    env_path = os.getenv("ANAKOT_PREFILL_MESSAGES_FILE", "").strip()
    if env_path:
        return env_path
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.anakot/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If ANAKOT_IGNORE_USER_CONFIG=1 is set (via ``anakot chat --ignore-user-config``),
    the user config at ``~/.anakot/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({ANAKOT_HOME}/config.yaml)
    user_config_path = _anakot_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("ANAKOT_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see anakot_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
        ...
        try:
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and AnakotCLI.__init__ checks "default" first.
    ...
    # Normalize config key: the new config system (anakot_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && anakot` to control it.
    ...
    # UNLESS we're inside a gateway process (detected by _ANAKOT_GATEWAY marker)
...
# Initialize centralized logging early — agent.log + errors.log in ~/.anakot/logs/.
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``anakot --help`` paths inside the chat command flow).  See
...
from anakot_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
def _prepare_deferred_agent_startup() -> None:
    ...
    _accept_hooks = os.environ.get("ANAKOT_ACCEPT_HOOKS", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`anakot_cli.config.load_config` (the authoritative loader that
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`anakot_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``anakot/*`` and ``pr-*`` local branches that
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``anakot/anakot-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``anakot -w`` sessions and PR review
...
#
# Detection priority:
#   1. ANAKOT_LIGHT / ANAKOT_TUI_LIGHT env (true/false) — explicit override
#   2. ANAKOT_TUI_THEME=light|dark — explicit theme
#   3. ANAKOT_TUI_BACKGROUND=#RRGGBB — explicit bg hint
#   4. COLORFGBG env (set by xterm/Konsole/urxvt) — bg slot 7/15 = light
#   5. OSC 11 query (\x1b]11;?\x1b\\) — ask the terminal directly
#   6. Default: assume dark (matches the legacy Anakot assumption)
#
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("ANAKOT_TUI_BACKGROUND") or ""
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in Anakot output it's common to display cron schedules verbatim.
    #
...
def _apply_bracketed_paste_timeout_patch() -> None:
    ...
    ``_anakot_bp_timeout_patched`` sentinel on the module.
    """
    try:
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``AnakotCLI._busy_command()``.
        """
        yield self

# ASCII Art - ANAKOT-AGENT logo (full width, single line - requires ~95 char terminal)
...
# ASCII Art - Anakot Caduceus (compact, fits in left panel)
...
def _build_compact_banner() -> str:
    ...
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- callmemo[/]\n"
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.anakot/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.anakot/config.yaml on first use)
...
class AnakotCLI:
    """
    Interactive CLI for the Anakot Agent.
    ...
    def __init__(
        ...
        # Read max_tokens from config (env var override: ANAKOT_MAX_TOKENS)
        ...
        self.requested_provider = (
            ...
            or os.getenv("ANAKOT_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("ANAKOT_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("ANAKOT_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        # by `anakot chat --ignore-rules` in anakot_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("ANAKOT_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("ANAKOT_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        # it's shared across all Anakot processes for this ANAKOT_HOME.
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if not _is_callable_provider and (not isinstance(api_key, str) or not api_key):
            ...
            if _has_custom_base:
                ...
                print("\n⚠️  Provider resolver returned an empty API key. "
                      "Set OPENROUTER_API_KEY or run: anakot setup")
                return False
        ...
        # `anakot chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `anakot auth add openai-codex`
        # without `anakot model`), fall back to the provider's first catalog
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                if _quiet_mode:
                    ...
                    print(
                        "Use a session ID from a previous CLI run (anakot sessions list).",
                        file=sys.stderr,
                    )
                else:
                    _cprint(f"\033[1;31mSession not found: {self.session_id}{_RST}")
                    _cprint(f"{_DIM}Use a session ID from a previous CLI run (anakot sessions list).{_RST}")
                return False
        ...
        try:
            ...
            # + fail-open inside the helper; harmless for non-callmemo providers.
            try:
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``anakot doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``anakot doctor`` so the banner stays
        small.
        """
        try:
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ Anakot: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ Anakot: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.anakot/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from anakot_cli.clipboard import save_clipboard_image

        img_dir = get_anakot_home() / "images"
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: anakot --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Anakot config/state.
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run anakot chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'anakot setup' to configure[/]")
        except Exception:
    ...
    def show_help(self):
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Anakot!{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in anakot_cli/colors.py
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from anakot_cli.main import _relative_time
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `anakot gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            _cprint("  Tip:   Use /history or `anakot sessions list` to find sessions.")
            return
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `anakot sessions list` to see available sessions.")
            return
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.anakot/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``anakot --resume <id>``
        ...
        try:
            ...
            return
        path = saved_dir / f"anakot_conversation_{timestamp}.json"
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `anakot model` and gateway pickers).
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from anakot_cli.model_switch import switch_model, parse_model_flags
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to anakot_cli.curator so the CLI and the `anakot curator`
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to anakot_cli.skills_hub."""
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in anakot_cli/commands.py instead of touching every dispatch site.
        ...
        if canonical in {"quit", "exit"}:
            ...
            try:
                ...
                if not plugins:
                    ...
                    print(f"Drop plugin directories into {display_anakot_home()}/plugins/ to get started.")
                else:
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``anakot bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: anakot bundles create "
        ...
        _cprint(
            ...
            f"Manage with `anakot bundles`.{_RST}"
        )
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Anakot keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _is_session_yolo_active(self) -> bool:
        ...
        ``ANAKOT_YOLO_MODE`` into ``_YOLO_MODE_FROZEN`` before tool imports
        happen.
        """
        try:
            from tools.approval import (
    ...
    def _toggle_yolo(self):
        ...
        ``ANAKOT_YOLO_MODE`` here — that env var is read once at module import
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Anakot is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Anakot is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while Anakot is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from anakot_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
            model = getattr(agent, "model", None) or getattr(self, "model", None)
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Anakot Agent to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``anakot update`` so the user sees update output directly and gets
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Anakot Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title=" Update Anakot Agent",
            detail="This will exit the current session and run `anakot update`.",
            choices=choices,
        )
        if raw is None:
    ...
    def _show_usage(self):
        """Rate limits + session token usage (when a live agent exists) + callmemo credits.

        The callmemo credits block is agent-independent (a portal fetch), so it runs even
        ...
        # callmemo credits magnitudes + monthly-grant gauge (agent-independent — also
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by anakot_logging not
            # installing a console StreamHandler in non-verbose mode.

    def _print_nous_credits_block(self) -> bool:
        """Print the callmemo credits magnitudes + monthly-grant gauge when a callmemo account
        ...
        It's agent-independent (a portal fetch gated on "a callmemo account is logged in",
        ...
        wall-clock-bounded inside the helper; also honors ANAKOT_DEV_CREDITS_FIXTURE
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.anakot/skills/ and queue a note for the
        next user turn.
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _anakot_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard ANAKOT_HOME, so no -p needed.
            try:
                from anakot_cli.profiles import get_active_profile_name
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Anakot Agent! Type your message or /help for commands.")
        ...
        try:
            from anakot_cli.model_switch import prewarm_picker_cache_async
        ...
        # after an OpenClaw→Anakot migration (especially migrations done by
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in anakot_cli.pt_input_extras
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Anakot binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        # Dynamic prompt: shows Anakot symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Anakot needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Anakot needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Anakot needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``ANAKOT_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("ANAKOT_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the anakot process when
...
def _run_kanban_goal_loop_q(cli: "AnakotCLI", first_response: str) -> None:
    """Drive a kanban goal_mode worker through the Ralph-style goal loop.

    Called from the quiet single-query path AFTER the worker's first turn,
    only when ``ANAKOT_KANBAN_GOAL_MODE`` is set (dispatcher-spawned
    ...
    task_id = (_os.environ.get("ANAKOT_KANBAN_TASK") or "").strip()
    if not task_id:
        return
...
def main(
    ...
    # Default to anakot-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
    ...
    # mode registers its own inside AnakotCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  ANAKOT_SIGTERM_GRACE overrides the 1.5 s
    ...
    if query or image:
        ...
        # Kanban workers spawn with ``anakot chat -q "work kanban task <id>"``;
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if cli._init_agent(
                    ...
                    # machine-readable (no styled "Anakot" box, no tool-gen
                    ...
                    if isinstance(result, dict) and result.get("failed"):
                        _exit_code = 1
                        if os.environ.get("ANAKOT_KANBAN_TASK") and result.get(
                            "failure_reason"
            ...
            # Single-query mode (`anakot chat -q "…"`): skip the welcome
