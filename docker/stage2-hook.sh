...
# the container starts non-root. The baked image trees (/opt/data, /opt/anakot/
# .venv, ui-tui, node_modules) stay owned by the anakot build UID (10000), so an
...
# into $ANAKOT_HOME). See #34837 for the supervision-tree side of this.
#
...
and the baked image directories are owned by the anakot user (UID $(id -u anakot)),
...
if [ "$needs_chown" = true ]; then
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing; do
        if [ -e "$ANAKOT_HOME/$sub" ]; then
            chown -R anakot:anakot "$ANAKOT_HOME/$sub" 2>/dev/null || \
                echo "[stage2] Warning: chown $ANAKOT_HOME/$sub failed (rootless container?) — continuing"
        fi
    done
fi

# --- Fix ownership of build trees under $INSTALL_DIR ---
# Anakot-owned trees under $INSTALL_DIR must be re-chowned whenever the
# runtime anakot UID no longer owns them — otherwise:
...
#     the source mtime is newer than dist/ or when ANAKOT_TUI_FORCE_BUILD
#     is set) and writes to ui-tui/dist/. Without this chown the new
#     anakot UID can't write the build output (#28851).
...
# The set mirrors the build-time `chown -R anakot:anakot` line in the
# Dockerfile — keep them in sync if the Dockerfile chown set changes.
# These are under $INSTALL_DIR (not $ANAKOT_HOME), so the bind-mount
# concern doesn't apply — recursive is fine.
#
# This MUST be gated independently of the $ANAKOT_HOME ownership check
# above. `usermod -u <new> anakot` re-chowns the anakot home dir
# ($ANAKOT_HOME == /opt/data) to the new UID as a side effect, so after a
# ANAKOT_UID/PUID remap `stat $ANAKOT_HOME` always already matches the new
# UID and `needs_chown` is false — but the build trees under /opt/anakot
# are NOT touched by usermod and remain owned by the build-time UID
# (10000). Gating them on $ANAKOT_HOME ownership (as #35027 did) silently
...
# when the venv is not already owned by the runtime anakot UID. Idempotent
...
if [ -n "$venv_owner" ] && [ "$venv_owner" != "$actual_anakot_uid" ]; then
    echo "[stage2] Fixing ownership of build trees under $INSTALL_DIR to anakot ($actual_anakot_uid)"
    chown -R anakot:anakot \
        "$INSTALL_DIR/.venv" \
        "$INSTALL_DIR/ui-tui" \
        "$INSTALL_DIR/gateway" \
...
# chown would fail.
if [ -d "$ANAKOT_HOME/profiles" ]; then
    chown -R anakot:anakot "$ANAKOT_HOME/profiles" 2>/dev/null || true
fi
...
# --- Install-method stamp (read by detect_install_method() in anakot status) ---
# Preserved from the tini-era entrypoint (PR #27843). Must be written as
# the anakot user so ownership matches the file's documented owner.
# tee is invoked directly via s6-setuidgid (no `sh -c` wrapper) for the
# same shell-metacharacter safety described above.
printf 'docker\n' | as_anakot tee "$ANAKOT_HOME/.install_method" >/dev/null \
    || true
