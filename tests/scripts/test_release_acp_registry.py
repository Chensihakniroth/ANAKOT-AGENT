        ...
        json.dumps(
            {
                ...
                "distribution": {
                    "uvx": {
                        "package": f"anakot-agent[acp]=={version}",
                        "args": ["anakot-acp"],
                    }
                },
            },
...
def test_update_acp_registry_versions_bumps_manifest_and_pin(monkeypatch, tmp_path):
    ...
    assert manifest["distribution"]["uvx"]["package"] == "anakot-agent[acp]==0.14.0"
    # args stay untouched so we don't accidentally rewrite them.
    assert manifest["distribution"]["uvx"]["args"] == ["anakot-acp"]
...
def test_update_version_files_bumps_manifest_alongside_pyproject(
    ...
    version_dir = tmp_path / "anakot_cli"
    version_dir.mkdir()
