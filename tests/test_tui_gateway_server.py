...
def test_voice_toggle_tts_branch_also_carries_record_key(monkeypatch):
    ...
    monkeypatch.setenv("ANAKOT_VOICE", "1")
    monkeypatch.delenv("ANAKOT_VOICE_TTS", raising=False)
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    ...
    # whose tools live in anakot-cli's universe (see toolsets.py).
...
def test_setup_status_reports_provider_config(monkeypatch):
    monkeypatch.setattr("anakot_cli.main._has_any_provider_configured", lambda: False)
...
def test_setup_runtime_check_rejects_empty_runtime_key(monkeypatch):
    monkeypatch.setattr("anakot_cli.main._has_any_provider_configured", lambda: True)
    monkeypatch.setattr(
...
def test_config_set_model_syncs_inference_provider_env(monkeypatch):
    """After an explicit provider switch, ANAKOT_INFERENCE_PROVIDER must
    ...
    assert os.environ["ANAKOT_INFERENCE_PROVIDER"] == "anthropic"


def test_config_set_model_syncs_tui_provider_unconditionally(monkeypatch):
    """Regression for #16857: /model must set ANAKOT_TUI_PROVIDER even when
    ...
    # Both env vars must reflect the user's choice. ANAKOT_TUI_PROVIDER is
    # the canonical explicit-this-process carrier consumed by
    # _resolve_startup_runtime() on /new.
    assert os.environ["ANAKOT_TUI_PROVIDER"] == "custom:xuanji"
    assert os.environ["ANAKOT_INFERENCE_PROVIDER"] == "custom:xuanji"
...
def test_config_set_model_syncs_tui_provider_env(monkeypatch):
    ...
    try:
        ...
        assert resp["result"]["value"] == "anthropic/claude-sonnet-4.6"
        assert os.environ["ANAKOT_TUI_PROVIDER"] == "anthropic"
        assert os.environ["ANAKOT_MODEL"] == "anthropic/claude-sonnet-4.6"
        assert os.environ["ANAKOT_INFERENCE_MODEL"] == "anthropic/claude-sonnet-4.6"
    finally:
        server._sessions.clear()
...
def test_session_compress_syncs_session_key_after_rotation(monkeypatch):
    ...
    continuation session — mirrors AnakotCLI._manual_compress's
...
def test_complete_slash_surfaces_completer_error(monkeypatch):
    with patch(
        "anakot_cli.commands.SlashCommandCompleter",
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    for providers like callmemo whose /models endpoint returns image/video
...
def test_browser_manage_connect_default_local_reports_launch_hint(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        ...
        with (
            patch(
                "anakot_cli.browser_connect.try_launch_chrome_debug", return_value=False
            ),
            patch(
...
def test_browser_manage_connect_default_local_retries_after_launch(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        with patch(
            "anakot_cli.browser_connect.try_launch_chrome_debug", return_value=True
        ):
            resp = server.handle_request(
                {"id": "1", "method": "browser.manage", "params": {"action": "connect"}}
            )
