...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `anakot cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # anakot_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.anakot/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "anakot_config_mod"}
...
def test_rebrand_text_replaces_legacy_bot_names():
    ...
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Anakot remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "anakot prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Anakot was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "anakot uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Anakot said to ask Anakot about Anakot settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.anakot`` (the real Anakot home), not the broken
    ``.Anakot``.
    ...
    rewritten to ``~/.Anakot/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.anakot/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .anakot directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.anakot'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "anakot config path: ~/.anakot/"
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are Anakot" in result
