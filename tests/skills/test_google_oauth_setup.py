...
class TestAnakotConstantsFallback:
    """Tests for _anakot_home.py fallback when anakot_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_anakot_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _anakot_home.py with anakot_constants blocked."""
        monkeypatch.setitem(sys.modules, "anakot_constants", None)
        spec = importlib.util.spec_from_file_location("_anakot_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_anakot_home_env_var(self, monkeypatch, tmp_path):
        """When anakot_constants is missing, ANAKOT_HOME comes from env var."""
        monkeypatch.setenv("ANAKOT_HOME", str(tmp_path / "custom-anakot"))
        module = self._load_helper(monkeypatch)
        assert module.get_anakot_home() == tmp_path / "custom-anakot"

    def test_fallback_defaults_to_dot_anakot(self, monkeypatch):
        """When anakot_constants is missing and ANAKOT_HOME unset, default to ~/.anakot."""
        monkeypatch.delenv("ANAKOT_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_anakot_home() == Path.home() / ".anakot"

    def test_fallback_ignores_empty_anakot_home(self, monkeypatch):
        """Empty/whitespace ANAKOT_HOME is treated as unset."""
        monkeypatch.setenv("ANAKOT_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_anakot_home() == Path.home() / ".anakot"

    def test_fallback_display_anakot_home_shortens_path(self, monkeypatch):
        """Fallback display_anakot_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("ANAKOT_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_anakot_home() == "~/.anakot"

    def test_fallback_display_anakot_home_profile_path(self, monkeypatch):
        """Fallback display_anakot_home() handles profile paths under ~/."""
        monkeypatch.setenv("ANAKOT_HOME", str(Path.home() / ".anakot/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_anakot_home() == "~/.anakot/profiles/coder"

    def test_fallback_display_anakot_home_custom_path(self, monkeypatch):
        """Fallback display_anakot_home() returns full path for non-home locations."""
        monkeypatch.setenv("ANAKOT_HOME", "/opt/anakot-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_anakot_home() == "/opt/anakot-custom"

    def test_delegates_to_anakot_constants_when_available(self):
        """When anakot_constants IS importable, _anakot_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_anakot_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import anakot_constants
        assert module.get_anakot_home is anakot_constants.get_anakot_home
        assert module.display_anakot_home is anakot_constants.display_anakot_home
...
class TestInstallDeps:
    ...
    Regression coverage for the Anakot Docker image, whose venv is built with
    ...
    def test_returns_false_when_no_pip_and_no_uv(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "anakot-agent[google]" in out
