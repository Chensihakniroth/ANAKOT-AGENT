"""Tests for agent/callmemo_rate_guard.py — cross-session callmemo Portal rate limit guard."""
...
class TestRecordNousRateLimit:
    ...
    def test_records_with_header_reset(self, rate_guard_env):
        from agent.callmemo_rate_guard import record_nous_rate_limit, _state_path
...
class TestNousRateLimitRemaining:
    ...
    def test_returns_remaining_seconds_when_active(self, rate_guard_env):
        from agent.callmemo_rate_guard import record_nous_rate_limit, nous_rate_limit_remaining
    ...
    def test_returns_none_when_expired(self, rate_guard_env):
        from agent.callmemo_rate_guard import nous_rate_limit_remaining, _state_path
...
class TestFormatRemaining:
    ...
    def test_seconds(self):
        from agent.callmemo_rate_guard import format_remaining
...
class TestParseResetSeconds:
    ...
    def test_case_insensitive_headers(self, rate_guard_env):
        from agent.callmemo_rate_guard import _parse_reset_seconds
...
class TestAuxiliaryClientIntegration:
    ...
    def test_try_nous_skips_when_rate_limited(self, rate_guard_env, monkeypatch):
        from agent.callmemo_rate_guard import record_nous_rate_limit
...
class TestIsGenuineNousRateLimit:
    ...
    callmemo Portal multiplexes upstreams (DeepSeek, Kimi, MiMo, Anakot).
    ...
    def test_exhausted_hourly_bucket_in_429_headers_is_genuine(self):
        from agent.callmemo_rate_guard import is_genuine_nous_rate_limit
    ...
    def test_healthy_headers_on_429_are_upstream_capacity(self):
        # Classic upstream-capacity symptom: callmemo edge reports plenty of
