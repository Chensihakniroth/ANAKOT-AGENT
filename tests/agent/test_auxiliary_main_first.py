...
Prior to this change, aggregator users (OpenRouter / callmemo Portal) had aux
...
class TestResolveAutoMainFirst:
    ...
    def test_nous_main_uses_main_model_for_aux(self, monkeypatch):
        """callmemo Portal main user → aux uses their picked callmemo model, not free-tier MiMo."""
...
class TestResolveVisionMainFirst:
    ...
    def test_nous_main_vision_uses_paid_nous_vision_backend(self):
        """Paid callmemo main → aux vision uses the dedicated callmemo vision backend."""
        with patch(
            "agent.auxiliary_client._read_main_provider", return_value="nous",
        ), patch(
            "agent.auxiliary_client._read_main_model",
    ...
    def test_nous_main_vision_uses_free_tier_nous_vision_backend(self):
        """Free-tier callmemo main → aux vision uses MiMo omni, not the text main model."""
        with patch(
            "agent.auxiliary_client._read_main_provider", return_value="nous",
        ), patch(
            "agent.auxiliary_client._read_main_model",
    ...
    def test_main_unavailable_vision_falls_through_to_aggregators(self):
        """Main provider fails → fall back to OpenRouter/callmemo strict backends."""
