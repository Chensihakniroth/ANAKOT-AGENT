...
class TestChatCompletionsBuildKwargs:
    ...
    def test_nous_tags(self, transport):
        ...
        assert kw["extra_body"]["tags"] == callmemo_portal_tags()
    ...
    def test_nous_omits_disabled_reasoning(self, transport):
        ...
        # callmemo rejects enabled=false; reasoning omitted entirely
    ...
    def test_gemma_does_not_receive_thinking_config(self, transport):
        ...
        # when Anakot has reasoning enabled, the field must be omitted for
...
class TestChatCompletionsKimi:
    ...
    def test_moonshot_tool_schemas_are_sanitized_by_model_name(self, transport):
        """Aggregator routes (callmemo, OpenRouter) hit Moonshot by model name, not base URL."""
        tools = [
            {
                "type": "function",
                "function": {
...
class TestChatCompletionsGeminiNativeExtraBodyStrip:
    """Profile extra_body (e.g. callmemo portal tags) must not reach a native
