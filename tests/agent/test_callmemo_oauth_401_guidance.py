"""Tests for the callmemo OAuth 401 actionable-guidance branch in
...
Regression context: ashh hit a callmemo 401 (OAuth token expired / portal said
account out of credits) plus a model slug ``deepseek/deepseek-v4-flash:free``
that's OpenRouter syntax, not a callmemo catalog name. The previous guidance
...
def test_nous_401_guidance_strings_present():
    """User-facing remediation strings for callmemo OAuth 401s must exist."""
    source = inspect.getsource(conversation_loop.run_conversation)

    # Must tell the user it's an OAuth token problem, NOT an API key problem
    # (callmemo Portal has no API key path — auth_type=oauth_device_code only).
    assert "callmemo Portal OAuth token was rejected" in source
    ...
    assert "portal.callmemo.ai" in source
...
def test_free_slug_hint_for_nous_provider():
    ...
    on the next message because callmemo Portal doesn't carry the OpenRouter
    free-tier slug.
    """
    source = inspect.getsource(conversation_loop.run_conversation)
