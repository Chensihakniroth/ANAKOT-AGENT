...
class TestDefaultContextLengths:
    ...
    def test_deepseek_v4_models_1m_context(self):
        ...
        # / callmemo Portal) to 1M without probing down to the legacy 128K
...
# =========================================================================
# callmemo Portal context-window resolution (provider="nous")
# =========================================================================

class TestNousPortalContextResolution:
    """callmemo Portal /v1/models is authoritative for what callmemo infra enforces
    ...
    def test_portal_value_wins_over_openrouter_catalog(
        ...
        the callmemo portal correctly enforces 262144.  Portal must win."""
        import agent.model_metadata as mm
        cache_file = tmp_path / "context_length_cache.yaml"
        monkeypatch.setattr(mm, "_get_context_cache_path", lambda: cache_file)
    ...
    def test_bypass_keyed_on_url_not_provider_string(
        ...
        when the user is really on callmemo Portal (e.g. cred-pool fallback).
        The callmemo-URL bypass must trigger off the URL host, not the provider
        ...
        for provider_arg in ("", "openrouter", "custom"):
            ...
            assert ctx == 262_144, (
                f"URL-based callmemo detection must fire for provider={provider_arg!r}; "
                f"got {ctx}"
            )
