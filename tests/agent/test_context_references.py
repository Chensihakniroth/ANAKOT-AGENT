...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_anakot_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("ANAKOT_HOME", str(tmp_path / ".anakot"))

    anakot_env = tmp_path / ".anakot" / ".env"
    anakot_env.parent.mkdir(parents=True)
    anakot_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.anakot/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
