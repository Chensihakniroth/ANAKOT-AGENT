...
class TestBuildCallKwargsMaxTokens:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("nous", "anakot-4", "https://inference-api.callmemo.ai/v1"),
...
class TestReadCodexAccessToken:
    ...
    def test_pool_without_selected_entry_falls_back_to_auth_store(self, tmp_path, monkeypatch):
        ...
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(True, None)), \
             patch("anakot_cli.auth._read_codex_tokens", return_value={
...
class TestResolveProviderClientUniversalModelFallback:
    ...
    fallback chain (OpenRouter / callmemo / etc.), so aux tasks billed
...
class TestExpiredCodexFallback:
    ...
    def test_anakot_oauth_file_sets_oauth_flag(self, monkeypatch):
        """OAuth-style tokens should get is_oauth=*** (token is not sk-ant-api-*)."""
        # Mock resolve_anthropic_token to return an OAuth-style token
        with patch("agent.anthropic_adapter.resolve_anthropic_token", return_value="sk-ant-oat-anakot-token"), \
             patch("agent.anthropic_adapter.build_anthropic_client") as mock_build, \
             patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)):
            mock_build.return_value = MagicMock()
            from agent.auxiliary_client import _try_anthropic
...
class TestAuxiliaryPoolAwareness:
    def test_try_nous_uses_pool_entry(self):
        ...
        with (
            patch("agent.auxiliary_client.load_pool", return_value=_Pool()),
            patch("agent.auxiliary_client.OpenAI") as mock_openai,
            patch("anakot_cli.models.get_callmemo_recommended_aux_model", return_value=None),
        ):
    ...
    def test_try_nous_uses_portal_recommendation_for_text(self):
        ...
        fresh_base = "https://inference-api.callmemo.ai/v1"
        with (
            ...
            patch("anakot_cli.models.get_callmemo_recommended_aux_model", return_value="minimax/minimax-m2.7") as mock_rec,
            patch("agent.auxiliary_client.OpenAI") as mock_openai,
        ):
            from agent.auxiliary_client import _try_nous
    ...
    def test_try_nous_uses_portal_recommendation_for_vision(self):
        ...
        with (
            patch("agent.auxiliary_client._read_nous_auth", return_value={"access_token": "***"}),
            patch("agent.auxiliary_client._resolve_nous_runtime_api", return_value=("fresh-agent-key", fresh_base)),
            patch("anakot_cli.models.get_callmemo_recommended_aux_model", return_value="google/gemini-3-flash-preview") as mock_rec,
            patch("agent.auxiliary_client.OpenAI"),
        ):
            from agent.auxiliary_client import _try_nous
    ...
    def test_try_nous_falls_back_when_recommendation_lookup_raises(self):
        ...
        with (
            patch("agent.auxiliary_client._read_nous_auth", return_value={"access_token": "***"}),
            patch("agent.auxiliary_client._resolve_nous_runtime_api", return_value=("fresh-agent-key", fresh_base)),
            patch("anakot_cli.models.get_callmemo_recommended_aux_model", side_effect=RuntimeError("portal down")),
            patch("agent.auxiliary_client.OpenAI"),
        ):
            from agent.auxiliary_client import _try_nous
            client, model = _try_nous()
    ...
    def test_call_llm_retries_nous_after_401(self):
        ...
        stale_client.base_url = "https://inference-api.callmemo.ai/v1"
        stale_client.chat.completions.create.side_effect = _Auth401("stale nous key")

        fresh_client = MagicMock()
        fresh_client.base_url = "https://inference-api.callmemo.ai/v1"
        ...
        with (
            ...
            patch("agent.auxiliary_client._resolve_nous_runtime_api", return_value=("fresh-agent-key", "https://inference-api.callmemo.ai/v1")),
        ):
            result = call_llm(
                task="compression",
                messages=[{"role": "user", "content": "hi"}],
    ...
    def test_call_llm_refreshes_nous_after_free_tier_block_when_account_paid(self):
        ...
        with (
            ...
            patch("agent.auxiliary_client._resolve_nous_runtime_api", return_value=("fresh-agent-key", "https://inference-api.callmemo.ai/v1")),
            patch(
                "anakot_cli.callmemo_account.get_callmemo_portal_account_info",
    ...
    async def test_async_call_llm_retries_nous_after_401(self):
        ...
        fresh_async_client.base_url = "https://inference-api.callmemo.ai/v1"
...
class TestIsPaymentError:
    ...
    def test_404_free_tier_model_block_is_payment(self):
        exc = Exception(
            ...
            "Upgrade at https://portal.callmemo.ai or pick a free model."
        )
        exc.status_code = 404
        assert _is_payment_error(exc) is True
...
class TestIsModelNotFoundError:
    ...
    def test_nous_openrouter_catalog_404(self):
        ...
        the callmemo → OpenRouter catalog."""
        exc = Exception(
...
class TestRefreshNousRecommendedModel:
    ...
    def test_returns_fresh_portal_recommendation(self, monkeypatch):
        monkeypatch.setattr(
            "anakot_cli.models.get_callmemo_recommended_aux_model",
    ...
    def test_falls_back_to_default_when_portal_unavailable(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "anakot_cli.models.get_callmemo_recommended_aux_model", _boom)
        out = _refresh_nous_recommended_model(
...
class TestIsRateLimitError:
    ...
    def test_429_with_resets_in_message(self):
        """callmemo-style 429: 'resets in 3508s'."""
...
class TestTryPaymentFallback:
    ...
    def test_codex_not_in_fallback_chain(self):
        ...
        When OR/callmemo/custom/api-key all fail, payment-fallback returns None —
...
class TestAuxiliaryClientPoisonedCacheEviction:
    ...
    See https://github.com/callmemo/anakot-agent/issues/23432.
    """
...
class TestBuildCallKwargsToolDedup:
    ...
    regresses.  See: https://github.com/callmemo/anakot-agent/issues/18478
    """
