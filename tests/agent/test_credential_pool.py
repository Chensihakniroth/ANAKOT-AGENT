...
def test_nous_pool_terminal_refresh_removes_device_code_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(auth_mod, "resolve_callmemo_runtime_credentials", _terminal_refresh_failure)
...
def test_load_pool_removes_stale_file_backed_singleton_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.read_anakot_oauth_credentials",
        lambda: None,
    )
    monkeypatch.setattr(
...
def test_singleton_seed_does_not_clobber_manual_oauth_entry(tmp_path, monkeypatch):
    ...
    assert {entry.source for entry in entries} == {"manual:anakot_pkce", "anakot_pkce"}
...
def test_load_pool_api_key_path_skips_oauth_autodiscovery(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("agent.anthropic_adapter.read_anakot_oauth_credentials", _fake_pkce)
...
def test_load_pool_api_key_path_prunes_stale_oauth_entries(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("anakot_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    monkeypatch.setattr("agent.anthropic_adapter.read_anakot_oauth_credentials", lambda: None)
        ...
        tmp_path,
        {
            ...
            "providers": {
                "nous": {
                    ...
                    "inference_base_url": "https://inference.callmemo.ai/v1",
                    "token_type": "Bearer",
...
# ── PR #10160 salvage: callmemo OAuth cross-process sync tests ─────────────────
...
def test_nous_exhausted_entry_recovers_via_auth_store_sync(tmp_path, monkeypatch):
    """An exhausted callmemo entry should recover when auth.json has newer tokens."""
    monkeypatch.setenv("ANAKOT_HOME", str(tmp_path / "anakot"))
    from agent.credential_pool import load_pool, STATUS_EXHAUSTED
...
def test_sync_codex_entry_from_auth_store_adopts_newer_tokens(tmp_path, monkeypatch):
    ...
    # Simulate `anakot auth openai-codex` replacing the token pair on disk.
...
def test_codex_exhausted_entry_recovers_via_auth_store_sync(tmp_path, monkeypatch):
    ...
    rate-limit reset the user ran `anakot model` to reauth, but the pool
    ...
    # Simulate `anakot model` / `anakot auth` refreshing the tokens.
...
def test_is_terminal_xai_oauth_refresh_error():
    from anakot_cli.auth import AuthError, _is_terminal_xai_oauth_refresh_error
    ...
    # callmemo error does not trigger xAI check
    assert not _is_terminal_xai_oauth_refresh_error(
...
def test_is_terminal_codex_oauth_refresh_error():
    from anakot_cli.auth import AuthError, _is_terminal_codex_oauth_refresh_error
