...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_anakot_env(self):
        """Anakot' own .env inside ANAKOT_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: anakot internal .env is in ~/.anakot/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.anakot/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.anakot/.env"))
        assert error is not None
