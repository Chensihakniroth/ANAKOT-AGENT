...
Tests cover:
- managed_callmemo_tools_enabled() subscription-based gate
...
class TestManagedNousToolsEnabled:
    """Subscription-based gate: True for paid callmemo subscribers."""
    ...
    def test_disabled_when_not_logged_in(self, monkeypatch):
        ...
        assert managed_callmemo_tools_enabled() is False
    ...
    def test_enabled_for_paid_subscriber(self, monkeypatch):
        ...
        assert managed_callmemo_tools_enabled() is True
    ...
    def test_force_fresh_is_forwarded(self, monkeypatch):
        ...
        assert managed_callmemo_tools_enabled(force_fresh=True) is True
        assert calls == [True]
...
class TestResolveModalBackendState:
    ...
    def _resolve(monkeypatch, mode, *, has_direct, managed_ready, nous_enabled=False):
        ...
        monkeypatch.setattr(
            "tools.tool_backend_helpers.managed_callmemo_tools_enabled",
            lambda: nous_enabled,
        )
