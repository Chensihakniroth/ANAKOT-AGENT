...
class TestYoloMode:
    ...
    def test_yolo_mode_empty_string_does_not_bypass(self, monkeypatch):
        """Empty string for ANAKOT_YOLO_MODE should not trigger bypass."""
        monkeypatch.setenv("ANAKOT_YOLO_MODE", "")
        monkeypatch.setenv("ANAKOT_INTERACTIVE", "1")
        monkeypatch.setenv("ANAKOT_SESSION_KEY", "test-session")

        # Empty string is falsy in Python, so getenv("ANAKOT_YOLO_MODE") returns ""
