...
$INSTALL_DIR (/opt/anakot/.venv, ui-tui, node_modules) to the runtime anakot
UID whenever they are not already anakot-owned — INDEPENDENTLY of whether
$ANAKOT_HOME ownership already matches.

Regression guard for the ANAKOT_UID/PUID remap path broken by #35027.

`usermod -u <new> anakot` re-chowns the anakot home dir ($ANAKOT_HOME ==
/opt/data) to the new UID as a side effect. #35027 gated the build-tree chown
behind `stat $ANAKOT_HOME != anakot_uid`, so after any remap that stat is
...
The fix probes the build trees directly (stat .venv) rather than $ANAKOT_HOME.
...
def test_build_tree_chown_not_gated_on_anakot_home(stage2_text: str) -> None:
    """The build-tree chown must NOT live inside the `if [ "$needs_chown" = true ]`
    block keyed on $ANAKOT_HOME ownership — that is exactly the #35027 bug."""
    block = _build_tree_block(stage2_text)
    # The block probes the venv owner, not $ANAKOT_HOME.
...
def _run_build_tree_block(
    text: str, *, venv_owner: int, anakot_uid: int
) -> bool:
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        #   id -u anakot       -> handled via actual_anakot_uid var below
        #   chown ...          -> record that it fired
        script = (
            "set -eu\n"
            f'INSTALL_DIR="/opt/anakot"\n'
            f'actual_anakot_uid={anakot_uid}\n'
...
def test_chown_fires_when_venv_owner_differs(stage2_text: str) -> None:
    """The #35027 regression scenario: after a remap $ANAKOT_HOME already
    matches the new UID, but the venv is still owned by the build-time UID
    (10000). The build-tree chown MUST still fire."""
    fired = _run_build_tree_block(stage2_text, venv_owner=10000, anakot_uid=4242)
    assert fired, (
        "build-tree chown must fire when the venv is not owned by the runtime "
        "anakot UID, regardless of $ANAKOT_HOME ownership (#35027 regression)"
    )


def test_chown_skipped_when_venv_already_owned(stage2_text: str) -> None:
    """Idempotency: once the venv is anakot-owned, the recursive chown is
    skipped on subsequent boots."""
    fired = _run_build_tree_block(stage2_text, venv_owner=4242, anakot_uid=4242)
    assert not fired, (
        "build-tree chown must be skipped when the venv already matches the "
        "runtime anakot UID (avoid expensive recursive chown on every restart)"
    )


def test_chown_skipped_for_default_uid(stage2_text: str) -> None:
    """No remap: venv owned by the default build UID (10000) and anakot is
    still 10000 — nothing to do."""
    fired = _run_build_tree_block(stage2_text, venv_owner=10000, anakot_uid=10000)
    assert not fired
