...
class TestWriteDenyExactPaths:
    ...
    def test_anakot_env(self):
        # ``.env`` under the active ANAKOT_HOME (profile-aware, not just
        # ``~/.anakot``) must be write-denied. The hermetic test conftest
        # points ANAKOT_HOME at a tempdir — resolve via get_anakot_home()
        # to match the denylist.
        from anakot_constants import get_anakot_home
        path = str(get_anakot_home() / ".env")
        assert _is_write_denied(path) is True
...
class TestWriteAllowed:
    ...
    def test_anakot_config_not_env(self):
        path = os.path.join(str(Path.home()), ".anakot", "config.yaml")
