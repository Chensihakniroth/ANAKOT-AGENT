...
class TestModalBulkUpload:
    ...
    def test_mkdir_includes_all_parents(self, monkeypatch, tmp_path):
        ...
        files = [
            (str(src), "/root/.anakot/credentials/f.txt"),
            (str(src), "/root/.anakot/skills/deep/nested/f.txt"),
        ]
        ...
        cmd = exec_calls[0][2]
        assert "/root/.anakot/credentials" in cmd
        assert "/root/.anakot/skills/deep/nested" in cmd
    ...
    def test_single_exec_call(self, monkeypatch, tmp_path):
        ...
        for i in range(20):
            ...
            files.append((str(src), f"/root/.anakot/cache/file_{i}.txt"))
    ...
    def test_timeout_set_to_120(self, monkeypatch, tmp_path):
        ...
        src = tmp_path / "f.txt"
        src.write_text("data")
        files = [(str(src), "/root/.anakot/f.txt")]
