...
class TestRegisterCredentialFiles:
    ...
    def test_string_entry(self, tmp_path):
        anakot_home = tmp_path / ".anakot"
        anakot_home.mkdir()
        (anakot_home / "secret.key").write_text("key")
...
class TestPathTraversalSecurity:
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside ANAKOT_HOME must still be registered."""
        anakot_home = tmp_path / ".anakot"
        anakot_home.mkdir()
        monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
        (anakot_home / "token.json").write_text('{"token": "abc"}')
...
class TestCacheDirectoryMounts:
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.anakot/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        anakot_home = tmp_path / ".anakot"
        anakot_home.mkdir()
        # Use legacy dir name — get_anakot_dir prefers old if it exists
        (anakot_home / "document_cache").mkdir()
        (anakot_home / "image_cache").mkdir()
        monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
...
class TestMapCachePathToContainer:
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.anakot")
            == "/home/agent/.anakot/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        anakot_home = tmp_path / ".anakot"
        (anakot_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(anakot_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        anakot_home = tmp_path / ".anakot"
        ss_dir = anakot_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.anakot/cache/screenshots/session_abc/screen1.png"
