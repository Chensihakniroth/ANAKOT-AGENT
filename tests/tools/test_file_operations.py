...
class TestIsWriteDenied:
    ...
    def test_anakot_control_files_oauth_and_mcp_tokens_denied(self, path):
        """Anakot control files, PKCE creds, mcp-tokens, and pairing entries must be write-denied."""
    ...
    def test_anakot_control_files_and_oauth_traversal_denied(self, path):
        """Path traversal attempts to protected Anakot files must be blocked."""
        from anakot_constants import get_anakot_home
        anakot_home = get_anakot_home()
        full_path = str(anakot_home / path)
        assert _is_write_denied(full_path) is True
    ...
    def test_control_files_and_oauth_protected_in_profile_mode(self, tmp_path, monkeypatch, name):
        ...
        global ~/.anakot/{auth.json,config.yaml,webhook_subscriptions.json,
        .anthropic_oauth.json} writable — the same gap PR #15981 fixed
        for .env.
        """
        # Simulate a profile-mode ANAKOT_HOME layout:
