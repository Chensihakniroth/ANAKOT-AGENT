...
When ANAKOT_UID is remapped at container boot, ``usermod -u`` only rewrites
files under the anakot user's home directory ($ANAKOT_HOME == /opt/data).
Runtime-writable trees under ``/opt/anakot`` must be explicitly chowned to the
new UID before services drop privileges. ``/opt/anakot/gateway`` is one such
