...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad ANAKOT_ prefix dropped,
...
def test_env_scrub_anakot_allowlist_and_secret_blocks():
    ...
    env = {
        ...
        "ANAKOT_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("ANAKOT_HOME", "ANAKOT_PROFILE", "ANAKOT_CONFIG", "ANAKOT_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "ANAKOT_BASE_URL", "ANAKOT_INTERACTIVE", "ANAKOT_KANBAN_DB",
...
def test_env_scrub_logs_dropped_anakot_vars(caplog):
    """Dropping a non-allowlisted, non-secret ANAKOT_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset ANAKOT_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "ANAKOT_HOME": "/h",          # allowlisted → kept, not logged
        "ANAKOT_BASE_URL": "https://x",   # dropped → logged
        "ANAKOT_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "ANAKOT_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "ANAKOT_HOME" in out and "PATH" in out
    assert "ANAKOT_BASE_URL" not in out and "ANAKOT_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "ANAKOT_BASE_URL" in msgs and "ANAKOT_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "ANAKOT_API_KEY" not in msgs
