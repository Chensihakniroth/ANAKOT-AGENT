...
def test_manager_builds_anakot_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns AnakotMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _ANAKOT_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("ANAKOT_HOME", str(tmp_path))
    ...
    assert _ANAKOT_PROVIDER_CLS is not None
    assert isinstance(provider, _ANAKOT_PROVIDER_CLS)
    assert provider._anakot_server_name == "srv"
