...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_anakot_bin(self):
        """After _INSTALL_FAILED, manual install in ANAKOT_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        anakot_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(anakot_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(anakot_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._anakot_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == anakot_bin
            assert _tirith_mod._resolved_path == anakot_bin
            mock_clear.assert_called_once()
...
class TestAnakotHomeIsolation:
    ...
    def test_failure_marker_respects_anakot_home(self):
        """_failure_marker_path must use ANAKOT_HOME, not hardcoded ~/.anakot."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"ANAKOT_HOME": "/custom/anakot"}):
            result = _failure_marker_path()
        assert result == "/custom/anakot/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets ANAKOT_HOME; verify it's active."""
        anakot_home = os.getenv("ANAKOT_HOME")
        assert anakot_home is not None, "ANAKOT_HOME should be set by conftest"
        assert "anakot_test" in anakot_home, "Should point to test temp dir"

    def test_get_anakot_home_fallback(self):
        """Without ANAKOT_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_anakot_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove ANAKOT_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".anakot")
            result = _get_anakot_home()
