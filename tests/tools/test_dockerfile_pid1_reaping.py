...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    directly, the shell becomes PID 1 and will ``exec`` into anakot,
    ...
    assert routes_through_init, (
        ...
        "not wired into ENTRYPOINT, anakot still runs as PID 1 and zombies "
...
def test_dockerfile_installs_tui_dependencies(dockerfile_text):
    ...
    # resolve dependencies. The bundled ``anakot-ink`` workspace package is
    ...
    assert "ui-tui/packages/anakot-ink/" in dockerfile_text
...
def test_dockerfile_preinstalls_hindsight_memory_dependency(dockerfile_text):
    ...
    assert any("--extra hindsight" in step for step in sync_steps), (
        ...
        "into /opt/anakot/.venv. It lazy-installs into the image layer (not the "
...
def test_dockerfile_materializes_local_tui_ink_package(dockerfile_text):
    # ``anakot-ink`` is a bundled workspace package referenced from
    # ``ui-tui/package.json`` via ``file:`` — not pulled from the npm
    # registry. The contract this test pins is just that the image
    # actually carries the package source so ``await import('@anakot/ink')``
    # can resolve at runtime; the previous, much pickier assertion (manual
    # ``rm -rf`` + ``npm install --omit=dev --prefix node_modules/@anakot/ink``)
    ...
    assert "ui-tui/packages/anakot-ink/" in dockerfile_text, (
        "Dockerfile must COPY the bundled anakot-ink workspace package "
        "so ``await import('@anakot/ink')`` resolves at runtime."
    )
