...
class TestFirecrawlClientConfig:
    ...
    def setup_method(self):
        ...
        self._managed_patchers = [
            patch("tools.web_tools.managed_callmemo_tools_enabled", return_value=True),
            patch("tools.managed_tool_gateway.managed_callmemo_tools_enabled", return_value=True),
        ]
    ...
    def test_tool_gateway_domain_builds_firecrawl_gateway_origin(self):
        ...
        with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "callmemo.ai"}):
            with patch("tools.web_tools._read_nous_access_token", return_value="nous-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    ...
                    mock_fc.assert_called_once_with(
                        api_key="nous-token",
                        api_url="https://firecrawl-gateway.callmemo.ai",
                    )
    ...
    def test_tool_gateway_scheme_can_switch_derived_gateway_origin_to_http(self):
        ...
        with patch.dict(os.environ, {
            "TOOL_GATEWAY_DOMAIN": "callmemo.ai",
            ...
            with patch("tools.web_tools._read_nous_access_token", return_value="nous-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    ...
                    mock_fc.assert_called_once_with(
                        api_key="nous-token",
                        api_url="http://firecrawl-gateway.callmemo.ai",
                    )
                    assert result is mock_fc.return_value
    ...
    def test_nous_auth_token_respects_anakot_home_override(self, tmp_path):
        """Auth lookup should read from ANAKOT_HOME/auth.json, not ~/.anakot/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".anakot").mkdir(parents=True)
