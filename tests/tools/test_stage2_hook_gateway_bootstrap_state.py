...
ANAKOT_GATEWAY_BOOTSTRAP_STATE on first boot, so a freshly-provisioned
...
ANAKOT_GATEWAY_BOOTSTRAP_STATE=running; stage2-hook.sh (installed as
/etc/cont-init.d/01-anakot-setup, which runs lexicographically BEFORE
...
This mirrors the existing ANAKOT_AUTH_JSON_BOOTSTRAP env-seed pattern: it seeds
...
def test_seed_block_present_and_guarded(stage2_text: str) -> None:
    ...
    assert '[ ! -f "$ANAKOT_HOME/gateway_state.json" ]' in block, (
        "seed must be guarded by [ ! -f ] so persisted state wins on restart"
    )
    assert "ANAKOT_GATEWAY_BOOTSTRAP_STATE" in block
...
def _run_seed(
    ...
    """Run the extracted seed block in a sandbox $ANAKOT_HOME.

    ``env_value`` is the ANAKOT_GATEWAY_BOOTSTRAP_STATE value (None = unset).
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        env_line = (
            f'export ANAKOT_GATEWAY_BOOTSTRAP_STATE="{env_value}"\n'
            if env_value is not None
            else "unset ANAKOT_GATEWAY_BOOTSTRAP_STATE\n"
        )
        script = (
            "set -e\n"
            f'ANAKOT_HOME="{home}"\n'
...
def test_no_seed_when_env_unset(stage2_text: str) -> None:
    ...
    assert out is None, "seed must not run when ANAKOT_GATEWAY_BOOTSTRAP_STATE is unset"
