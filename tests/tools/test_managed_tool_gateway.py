...
def test_resolve_managed_tool_gateway_derives_vendor_origin_from_shared_domain():
    with patch.dict(
        ...
        clear=False,
    ), patch.object(managed_tool_gateway, "managed_callmemo_tools_enabled", return_value=True):
    ...
    assert result.gateway_origin == "https://firecrawl-gateway.callmemo.ai"
...
def test_resolve_managed_tool_gateway_is_disabled_without_subscription():
    with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "callmemo.ai"}, clear=False), \
         patch.object(managed_tool_gateway, "managed_callmemo_tools_enabled", return_value=False):
        result = resolve_managed_tool_gateway(
            "firecrawl",
            token_reader=lambda: "nous-token",
        )
