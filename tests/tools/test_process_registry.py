...
from tools.environments.local import _ANAKOT_PROVIDER_ENV_FORCE_PREFIX
...
class TestSpawnEnvSanitization:
    ...
    def test_spawn_via_env_uses_backend_temp_dir_for_artifacts(self, registry):
        ...
        assert "/data/data/com.termux/files/usr/tmp/anakot_bg_" in bg_command
        assert ".exit" in bg_command
        assert "rc=$?;" in bg_command
        assert " > /tmp/anakot_bg_" not in bg_command
        assert "cat /tmp/anakot_bg_" not in bg_command
    ...
    def test_env_poller_quotes_temp_paths_with_spaces(self, registry):
        ...
        assert env.commands[0][0] == "cat '/path with spaces/anakot_bg.log' 2>/dev/null"
