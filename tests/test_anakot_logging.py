...
class TestSetupLogging:
    ...
    def test_creates_errors_log_handler(self, anakot_home):
        anakot_logging.setup_logging(anakot_home=anakot_home)
        root = logging.getLogger()
    ...
    def test_force_reinitializes(self, anakot_home):
        anakot_logging.setup_logging(anakot_home=anakot_home)
        # Force still won't add duplicate handlers because _add_rotating_handler
        # checks by resolved path.
        anakot_logging.setup_logging(anakot_home=anakot_home, force=True)
    ...
    def test_custom_log_level(self, anakot_home):
        anakot_logging.setup_logging(anakot_home=anakot_home, log_level="DEBUG")
    ...
    def test_custom_max_size_and_backup(self, anakot_home):
        anakot_logging.setup_logging(
            anakot_home=anakot_home, max_size_mb=10, backup_count=5
        )
    ...
    def test_suppresses_noisy_loggers(self, anakot_home):
        anakot_logging.setup_logging(anakot_home=anakot_home)
    ...
    def test_warnings_appear_in_both_logs(self, anakot_home):
        anakot_logging.setup_logging(anakot_home=anakot_home)

        test_logger = logging.getLogger("test_anakot_logging.warning_test")
        ...
        agent_log = anakot_home / "logs" / "agent.log"
        errors_log = anakot_home / "logs" / "errors.log"
        assert "this is a warning" in agent_log.read_text()
        assert "this is a warning" in errors_log.read_text()

    def test_info_not_in_errors_log(self, anakot_home):
        anakot_logging.setup_logging(anakot_home=anakot_home)

        test_logger = logging.getLogger("test_anakot_logging.info_test")
    ...
    def test_reads_config_yaml(self, anakot_home):
    ...
    def test_record_factory_installed(self, anakot_home):
        """The custom record factory injects session_tag on all records."""
        anakot_logging.setup_logging(anakot_home=anakot_home)
        factory = logging.getLogRecordFactory()
        assert getattr(factory, "_anakot_session_injector", False), (
            "Record factory should have _anakot_session_injector marker"
        )
...
class TestGatewayMode:
    ...
    def test_gateway_log_created_after_cli_init(self, anakot_home):
    ...
    def test_gateway_log_created_after_cli_init_without_duplicate_handlers(self, anakot_home):
    ...
    def test_agent_log_still_receives_all(self, anakot_home):
...
class TestGuiMode:
    ...
    def test_gui_log_created_after_cli_init(self, anakot_home):
        anakot_logging.setup_logging(anakot_home=anakot_home, mode="cli")
        anakot_logging.setup_logging(anakot_home=anakot_home, mode="gui")
...
class TestSessionContext:
    ...
    def test_no_session_tag_without_context(self, anakot_home):
    ...
    def test_clear_session_context(self, anakot_home):
        """After clearing, session tag disappears."""
        anakot_logging.setup_logging(anakot_home=anakot_home)
        anakot_logging.set_session_context("xyz789")
        anakot_logging.clear_session_context()
    ...
    def test_session_context_thread_isolated(self, anakot_home):
        ...
        def thread_a():
            anakot_logging.set_session_context("thread_a_session")
            logging.getLogger("test.thread_a").info("from thread A")
            for h in logging.getLogger().handlers:
                h.flush()

        def thread_b():
            anakot_logging.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        anakot_logging.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        anakot_logging.set_session_context("any_handler_test")
...
class TestComponentFilter:
    ...
    def test_multiple_prefixes(self):
        f = anakot_logging._ComponentFilter(("agent", "run_agent", "model_tools"))
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        assert "gateway" in anakot_logging.COMPONENT_PREFIXES
        # The gateway component captures both core gateway logs and the
        # anakot_plugins facility (plugin-installed gateway adapters log
        # under that prefix).
        assert ("gateway", "anakot_plugins") == anakot_logging.COMPONENT_PREFIXES["gateway"]

    def test_agent_prefix(self):
        prefixes = anakot_logging.COMPONENT_PREFIXES["agent"]
        assert "agent" in prefixes
        assert "run_agent" in prefixes
        assert "model_tools" in prefixes

    def test_tools_prefix(self):
        assert ("tools",) == anakot_logging.COMPONENT_PREFIXES["tools"]

    def test_cli_prefix(self):
        prefixes = anakot_logging.COMPONENT_PREFIXES["cli"]
        assert "anakot_cli" in prefixes
        assert "cli" in prefixes

    def test_cron_prefix(self):
        assert ("cron",) == anakot_logging.COMPONENT_PREFIXES["cron"]

    def test_gui_prefix(self):
        prefixes = anakot_logging.COMPONENT_PREFIXES["gui"]
        assert "anakot_cli.web_server" in prefixes
...
class TestSetupVerboseLogging:
    ...
    def test_idempotent(self, anakot_home):
        anakot_logging.setup_logging(anakot_home=anakot_home)
        anakot_logging.setup_verbose_logging()
        anakot_logging.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        formatter = logging.Formatter("%(message)s")
        component_filter = anakot_logging._ComponentFilter(("test",))
...
class TestReadLoggingConfig:
    ...
    def test_handles_missing_logging_section(self, anakot_home):
        import yaml
        config = {"model": "test"}
        (anakot_home / "config.yaml").write_text(yaml.dump(config))
