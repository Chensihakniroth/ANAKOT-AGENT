...
class TestMaxTurnsResolution:
    ...
    def test_env_var_max_turns(self):
        ...
        cli_obj = _make_cli(env_overrides={"ANAKOT_MAX_ITERATIONS": "42"})
        assert cli_obj.max_turns == 42

    def test_invalid_env_var_max_turns_falls_back_to_default(self):
        """Invalid env values should not crash CLI init."""
        cli_obj = _make_cli(env_overrides={"ANAKOT_MAX_ITERATIONS": "not-a-number"})
...
class TestHistoryDisplay:
    ...
    def test_resume_updates_anakot_session_id_env_and_context(self, tmp_path):
        ...
        os.environ["ANAKOT_SESSION_ID"] = "current_session"
        _VAR_MAP["ANAKOT_SESSION_ID"].set("current_session")

        try:
            cli._handle_resume_command("/resume target_session")

            assert cli.session_id == "target_session"
            assert os.environ["ANAKOT_SESSION_ID"] == "target_session"
            assert get_session_env("ANAKOT_SESSION_ID") == "target_session"
        finally:
