...
def test_cli_prefers_config_provider_over_stale_env_override(monkeypatch):
    ...
    shell = cli.AnakotCLI(model="fireworks/minimax-m2p5", compact=True, max_turns=1)
...
def test_model_flow_callmemo_prints_subscription_guidance_without_mutating_explicit_tts(monkeypatch, capsys):
    monkeypatch.setattr(
        "anakot_cli.callmemo_subscription.managed_callmemo_tools_enabled",
    ...
    monkeypatch.setattr(
        "anakot_cli.auth.fetch_nous_models",
        lambda *args, **kwargs: ["claude-opus-4-6"],
    )
    monkeypatch.setattr("anakot_cli.auth._prompt_model_selection", lambda model_ids, current_model="", pricing=None, **kw: "claude-opus-4-6")
    monkeypatch.setattr("anakot_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("anakot_cli.auth._update_config_for_provider", lambda provider, url: None)

    anakot_main._model_flow_callmemo(config, current_model="claude-opus-4-6")
...
def test_model_flow_callmemo_offers_tool_gateway_prompt_when_unconfigured(monkeypatch, capsys):
    ...
    monkeypatch.setattr("anakot_cli.setup.prompt_checklist", _fake_checklist, raising=False)
...
def test_codex_provider_preserves_explicit_codex_model(monkeypatch):
    ...
    shell = cli.AnakotCLI(model="gpt-5.1-codex-mini", compact=True, max_turns=1)
...
def test_codex_provider_strips_provider_prefix_from_model(monkeypatch):
    ...
    shell = cli.AnakotCLI(model="openai/gpt-5.3-codex", compact=True, max_turns=1)
...
def test_cmd_model_falls_back_to_auto_on_invalid_provider(monkeypatch, capsys):
    ...
    monkeypatch.setattr("anakot_cli.auth.resolve_provider", _resolve_provider)
    monkeypatch.setattr(anakot_main, "_prompt_provider_choice", lambda choices, **kwargs: len(choices) - 1)
    monkeypatch.setattr("sys.stdin", type("FakeTTY", (), {"isatty": lambda self: True})())

    anakot_main.cmd_model(SimpleNamespace())
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    ...
    monkeypatch.setattr("anakot_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr(
        "anakot_cli.main._save_custom_provider",
...
def test_cmd_model_forwards_nous_login_tls_options(monkeypatch):
    ...
    monkeypatch.setattr("anakot_cli.auth._login_callmemo", _fake_login)

    anakot_main.cmd_model(
        SimpleNamespace(
            portal_url="https://portal.callmemo.ai",
            inference_url="https://inference.callmemo.ai/v1",
...
def test_auto_provider_name_localhost():
    from anakot_cli.main import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from anakot_cli.main import _save_custom_provider
