...
def test_install_bws_rejects_malicious_member(anakot_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(anakot_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(anakot_home, monkeypatch):
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under anakot_home/cache/."""
    home = tmp_path / ".anakot"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
