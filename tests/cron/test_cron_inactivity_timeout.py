...
class TestInactivityTimeout:
    ...
    def test_timeout_zero_means_unlimited(self, monkeypatch):
        """ANAKOT_CRON_TIMEOUT=0 yields None (unlimited)."""
    ...
    def test_timeout_invalid_value_falls_back_to_default(self, monkeypatch):
        """ANAKOT_CRON_TIMEOUT=abc should fall back to 600s, not raise ValueError."""
        monkeypatch.setenv("ANAKOT_CRON_TIMEOUT", "abc")
        raw = os.getenv("ANAKOT_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
    ...
    def test_timeout_empty_string_uses_default(self, monkeypatch):
        """ANAKOT_CRON_TIMEOUT='' (empty) should use the 600s default."""
        monkeypatch.setenv("ANAKOT_CRON_TIMEOUT", "")
        raw = os.getenv("ANAKOT_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
        assert _cron_timeout == 600.0
...
class TestSysPathOrdering:
    ...
    def test_anakot_constants_importable(self):
        """anakot_constants should be importable from cron context."""
        from anakot_constants import get_anakot_home
        assert callable(get_anakot_home)
