...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves anakot_cli.kanban_db on its own
  ...
  - worker logs are captured to ANAKOT_HOME/kanban/logs/<task>.log
    ...
    def _spawn(task, workspace):
        ...
        env = {
            **os.environ,
            ...
            "HOME": home,
            "PYTHONPATH": WT,
            "ANAKOT_KANBAN_TASK": task.id,
            "ANAKOT_KANBAN_WORKSPACE": workspace,
...
def main():
    home = tempfile.mkdtemp(prefix="anakot_e2e_")
    os.environ["ANAKOT_HOME"] = home
    os.environ["HOME"] = home
    sys.path.insert(0, WT)
    from anakot_cli import kanban_db as kb

    # Point the `anakot` CLI child processes will run at the worktree
    # anakot_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "anakot")
    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh
exec {PY} -m anakot_cli.main "$@"
""")
    ...
    print("=" * 60)
    # Scenario A workers wrote to /tmp/anakot_e2e_*/worker_*.log
    import glob
