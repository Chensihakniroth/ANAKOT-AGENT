...
def test_show_status_reports_nous_auth_error(monkeypatch, capsys, tmp_path):
    ...
    output = capsys.readouterr().out
    assert "callmemo Portal   ✗ not logged in (run: anakot portal)" in output
...
def test_show_status_reports_nous_inference_key_without_portal_login(monkeypatch, capsys, tmp_path):
    ...
    monkeypatch.setattr(status_mod, "managed_callmemo_tools_enabled", lambda: False, raising=False)
    monkeypatch.setattr(auth_mod, "get_codex_auth_status", lambda: {}, raising=False)
    monkeypatch.setattr(auth_mod, "get_qwen_auth_status", lambda: {}, raising=False)
    monkeypatch.setattr(auth_mod, "get_xai_oauth_auth_status", lambda: {}, raising=False)
    monkeypatch.setattr(gateway_mod, "find_gateway_pids", lambda exclude_pids=None: [], raising=False)
    ...
    output = capsys.readouterr().out
    assert "callmemo Portal   ✗ not logged in (callmemo inference key configured)" in output
    assert "Inference:  https://inference.example.com/v1" in output
    assert "callmemo inference credentials are configured" in output
...
class TestShowStatusXaiOAuth:
    ...
    def test_import_failure_does_not_break_other_oauth_providers(self, monkeypatch, capsys, tmp_path):
        """callmemo/Codex/MiniMax rows must still appear when xAI import fails."""
        import anakot_cli.auth as auth_mod
        status_mod = _base_xai_mocks(monkeypatch, tmp_path)
