...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_anakot_auth(anakot_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
    ...
    monkeypatch.setattr("anakot_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    anakot_home = tmp_path / "anakot"
    _setup_anakot_auth(anakot_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    would have run ``anakot auth add openai-codex`` as a workaround, leaving
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr("anakot_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("anakot_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
