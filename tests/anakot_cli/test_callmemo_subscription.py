"""Tests for callmemo subscription feature detection."""

from anakot_cli.callmemo_account import NousPortalAccountInfo, NousToolAccessInfo
from anakot_cli import callmemo_subscription as ns
...
def test_get_callmemo_subscription_features_recognizes_direct_exa_backend(monkeypatch):
    env = {"EXA_API_KEY": "exa-test"}

    monkeypatch.setattr(ns, "get_env_value", lambda name: env.get(name, ""))
    monkeypatch.setattr(
        ns, "get_callmemo_portal_account_info", lambda: _account(logged_in=False)
    )
    ...
    features = ns.get_callmemo_subscription_features({"web": {"backend": "exa"}})
...
def test_get_callmemo_subscription_features_force_fresh_forwards_account_request(monkeypatch):
    calls = []
    ...
    monkeypatch.setattr(ns, "get_callmemo_portal_account_info", fake_account_info)
    ...
    features = ns.get_callmemo_subscription_features({}, force_fresh=True)
...
def test_get_callmemo_subscription_features_prefers_managed_modal_in_auto_mode(monkeypatch):
    monkeypatch.setattr("tools.tool_backend_helpers.managed_callmemo_tools_enabled", lambda: True)
    monkeypatch.setattr(ns, "get_env_value", lambda name: "")
    monkeypatch.setattr(
        ns, "get_callmemo_portal_account_info", lambda: _account(logged_in=True, paid=True)
    )
    ...
    features = ns.get_callmemo_subscription_features(
...
def test_get_callmemo_subscription_features_marks_browser_use_as_managed_when_gateway_ready(monkeypatch):
    monkeypatch.setattr(ns, "get_env_value", lambda name: "")
    monkeypatch.setattr(
        ns, "get_callmemo_portal_account_info", lambda: _account(logged_in=True, paid=True)
    )
...
def test_get_callmemo_subscription_features_uses_direct_browserbase_when_no_managed_gateway(monkeypatch):
...
def test_get_callmemo_subscription_features_prefers_camofox_over_managed_browser_use(monkeypatch):
...
def test_get_callmemo_subscription_features_requires_agent_browser_for_browserbase(monkeypatch):
    env = {
        "BROWSERBASE_API_KEY": "bb-key",
        "BROWSERBASE_PROJECT_ID": "bb-project",
    }
...
def test_get_callmemo_subscription_features_does_not_treat_quoted_false_as_gateway_opt_in(monkeypatch):
    env = {"EXA_API_KEY": "exa-test"}
...
def test_get_gateway_eligible_tools_ignores_quoted_false_opt_in(monkeypatch):
    ...
    monkeypatch.setattr(
        ns, "get_callmemo_portal_account_info", lambda **kw: _account(logged_in=True, paid=True)
    )
    monkeypatch.setattr(
        ns,
...
def test_get_gateway_eligible_tools_pool_excludes_video(monkeypatch):
    ...
    monkeypatch.setattr(ns, "get_callmemo_portal_account_info", lambda **kw: _pool_account())
    monkeypatch.setattr(
        ns,
        "_get_gateway_direct_credentials",
...
def test_get_gateway_eligible_tools_empty_when_not_entitled(monkeypatch):
    ...
    monkeypatch.setattr(
        ns, "get_callmemo_portal_account_info", lambda **kw: _account(logged_in=True, paid=False)
    )
...
def test_apply_nous_managed_defaults_writes_video_gen_config(monkeypatch):
    ...
    video_gen.use_gateway when a callmemo subscriber selects video_gen
    without a direct FAL_KEY."""
    monkeypatch.setattr(ns, "managed_callmemo_tools_enabled", lambda **kw: True)
    monkeypatch.delenv("FAL_KEY", raising=False)
    monkeypatch.setattr(ns, "fal_key_is_configured", lambda: False)
    monkeypatch.setattr(
        ns, "get_callmemo_portal_account_info",
...
def test_apply_nous_managed_defaults_writes_image_gen_config(monkeypatch):
    ...
    when a callmemo subscriber selects image_gen without a direct FAL_KEY."""
    monkeypatch.setattr(ns, "managed_callmemo_tools_enabled", lambda **kw: True)
    monkeypatch.delenv("FAL_KEY", raising=False)
    monkeypatch.setattr(ns, "fal_key_is_configured", lambda: False)
    monkeypatch.setattr(
...
# ---------------------------------------------------------------------------
# ensure_callmemo_portal_access — inline login gate for `anakot tools`
# ---------------------------------------------------------------------------


def test_ensure_callmemo_portal_access_fast_path_when_already_paid(monkeypatch):
    ...
    monkeypatch.setattr(ns, "_run_callmemo_portal_login_only", _login)

    assert ns.ensure_callmemo_portal_access() is True
    assert login_called["v"] is False


def test_ensure_callmemo_portal_access_logs_in_then_grants(monkeypatch):
    ...
    monkeypatch.setattr(
        ns, "get_callmemo_portal_account_info", lambda **kw: next(states),
    )
    monkeypatch.setattr(ns, "_run_callmemo_portal_login_only", lambda **kw: True)

    assert ns.ensure_callmemo_portal_access() is True


def test_ensure_callmemo_portal_access_returns_false_when_login_declined(monkeypatch):
    monkeypatch.setattr(
        ns, "get_callmemo_portal_account_info",
        lambda **kw: _account(logged_in=False, paid=None),
    )
    monkeypatch.setattr(ns, "_run_callmemo_portal_login_only", lambda **kw: False)

    assert ns.ensure_callmemo_portal_access() is False


def test_ensure_callmemo_portal_access_false_when_logged_in_but_unpaid(monkeypatch):
