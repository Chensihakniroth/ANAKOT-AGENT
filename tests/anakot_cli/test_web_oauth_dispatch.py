...
def _fake_nous_device_data():
    return {
        ...
        "user_code": "NOUS-1234",
        "verification_uri": "https://portal.callmemo.ai/device",
        "verification_uri_complete": (
            "https://portal.callmemo.ai/device?user_code=NOUS-1234"
        ),
        "expires_in": 600,
        "interval": 5,
    }
...
def _invoke_scope_refusal():
    request = httpx.Request("POST", "https://portal.callmemo.ai/oauth/device/code")
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        ...
        "anakot_cli.web_server._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_nous_dashboard_device_flow_ignores_legacy_scope_override(monkeypatch):
    ...
    try:
        ...
        assert (
            ...
            == auth_mod.DEFAULT_CALLMEMO_SCOPE
        )
    finally:
...
def test_nous_dashboard_device_flow_does_not_retry_legacy_scope_on_invoke_refusal(monkeypatch):
    ...
    monkeypatch.delenv("ANAKOT_AGENT_USE_LEGACY_SESSION_KEYS", raising=False)
    monkeypatch.setattr(auth_mod, "_request_device_code", fake_request_device_code)
    monkeypatch.setattr(ws, "_nous_poller", lambda sid: None)
...
def test_nous_dashboard_poller_preserves_effective_scope_when_token_omits_scope(monkeypatch):
    ...
    try:
        ...
        assert captured_state["scope"] == auth_mod.DEFAULT_CALLMEMO_SCOPE
...
def test_anthropic_pkce_branch_still_works():
    ...
    with patch(
        "anakot_cli.web_server._start_anthropic_pkce",
...
def test_unknown_pkce_provider_rejected_cleanly():
    ...
    fake_entry = {
        ...
        "flow": "pkce",
        "cli_command": "anakot auth add hypothetical-pkce-provider",
