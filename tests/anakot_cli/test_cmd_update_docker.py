...
from anakot_cli.main import _cmd_update_check, cmd_update
...
def test_cmd_update_in_docker_prints_guidance_and_exits(
    ...
    """``anakot update`` inside Docker → friendly message + exit 1, no git calls."""
...
def test_cmd_update_check_in_docker_prints_guidance_and_exits(
    ...
    """``anakot update --check`` inside Docker → same message + exit 1, no fetch."""
    with pytest.raises(SystemExit) as excinfo:
