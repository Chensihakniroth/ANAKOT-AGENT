...
class TestGetAnakotHome:
    ...
    def test_env_override(self):
        with patch.dict(os.environ, {"ANAKOT_HOME": "/custom/path"}):
            home = get_anakot_home()
...
class TestLoadConfigParseFailure:
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        with patch.dict(os.environ, {"ANAKOT_HOME": str(tmp_path)}):
            ...
            # stands out at anakot startup before logging is configured)
            captured = capsys.readouterr()
            assert "anakot config:" in captured.err
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"ANAKOT_HOME": str(tmp_path)}):
            ...
            assert "anakot config:" in after_edit, "edited file should re-warn"
...
class TestRemoveEnvValue:
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ANAKOT_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"ANAKOT_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or any ``ANAKOT_*`` runtime flag.
    ...
    def test_anakot_integration_keys_still_writable(self, allowed_key):
        ...
        credentials following the ``ANAKOT_*`` convention must keep
    ...
    def test_arbitrary_user_key_still_works(self):
        ...
        denylist and outside ``ANAKOT_*``) keep working. The denylist
