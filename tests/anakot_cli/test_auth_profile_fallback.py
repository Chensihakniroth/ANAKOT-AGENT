...
When ``ANAKOT_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.anakot/profiles/coder.
...
#
# Several runtime helpers (notably ``resolve_callmemo_runtime_credentials`` and
...
# ``_load_provider_state`` itself, those helpers raise ``"Anakot is not
# logged into callmemo Portal"`` even though the user has a valid global callmemo
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    _write(anakot_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (ANAKOT_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.anakot as the "real home" — which is NOT equal
    # to the ANAKOT_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    anakot_home = tmp_path / "classic"
    ...
    _write(anakot_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from anakot_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: ANAKOT_HOME is set to a custom path that is NOT under
    # ~/.anakot/profiles/ — get_default_anakot_root() returns ANAKOT_HOME
