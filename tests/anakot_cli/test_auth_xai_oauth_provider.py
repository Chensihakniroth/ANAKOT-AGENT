...
def test_xai_oauth_authorize_url_includes_referrer_anakot_agent():
    """Attribution: xAI's OAuth server can identify Anakot-originated logins
    ...
    params = _parse_authorize_url(url)
    assert params["referrer"] == "anakot-agent"
...
def test_xai_callback_handler_returns_400_when_callback_url_lacks_code_and_error():
    ...
    try:
        ...
        assert "anakot auth add xai-oauth" in body
...
def test_read_xai_oauth_tokens_missing_refresh_token(tmp_path, monkeypatch):
    anakot_home = tmp_path / "anakot"
    _setup_anakot_auth(anakot_home, refresh_token="")
    monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
...
def test_resolve_xai_runtime_credentials_honours_env_base_url(tmp_path, monkeypatch):
    ...
    _setup_anakot_auth(anakot_home, access_token=fresh)
    monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
    monkeypatch.setenv("ANAKOT_XAI_BASE_URL", "https://custom.x.ai/v1/")
...
def test_resolve_credentials_does_not_quarantine_on_transient_refresh_failure(
    ...
    """
    anakot_home = tmp_path / "anakot"
    _seed_xai_oauth_state(anakot_home, dict(_STALE_XAI_OAUTH_STATE))
    monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
    ...
    monkeypatch.setattr("anakot_cli.auth._refresh_xai_oauth_tokens", _transient_refresh)
...
def test_credential_pool_seed_respects_suppression(tmp_path, monkeypatch):
    """`anakot auth remove xai-oauth <N>` for the seeded entry suppresses
    ...
    # Suppress the source — mimic `anakot auth remove`.
...
def test_pool_entry_needs_refresh_when_jwt_within_skew(tmp_path, monkeypatch):
    ...
    from agent.credential_pool import load_pool, AUTH_TYPE_OAUTH, PooledCredential
    from anakot_cli.auth import XAI_ACCESS_TOKEN_REFRESH_SKEW_SECONDS
    import uuid
...
def test_pool_refresh_marks_entry_exhausted_on_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("anakot_cli.auth.refresh_xai_oauth_pure", _fake_refresh_fail)
...
def test_pool_seeded_entry_sync_back_after_refresh(tmp_path, monkeypatch):
    ...
    anakot_home = tmp_path / "anakot"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_anakot_auth(anakot_home, access_token=near_expiry, refresh_token="rt-singleton")
    monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
...
def test_pool_refresh_adopts_singleton_tokens_when_consumed_elsewhere(tmp_path, monkeypatch):
    """Multi-process race: another Anakot process refreshed the singleton
    ...
    profiles + Anakot processes."""
    from agent.credential_pool import load_pool

    anakot_home = tmp_path / "anakot"
    in_memory_at = _jwt_with_exp(int(time.time()) + 30)  # near-expiry
    _setup_anakot_auth(anakot_home, access_token=in_memory_at, refresh_token="rt-stale")
    monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
...
def test_pool_exhausted_xai_entry_recovers_after_singleton_refresh(tmp_path, monkeypatch):
    ...
    user runs ``anakot model`` -> xAI Grok OAuth (or another process
    ...
    _setup_anakot_auth(anakot_home, access_token=stale_at, refresh_token="rt-stale")
    monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
    ...
    # Simulate the user re-running `anakot model` -> xAI Grok OAuth: the
...
def test_pool_manual_xai_entry_not_synced_from_singleton(tmp_path, monkeypatch):
    ...
    ``anakot auth add xai-oauth``) own their own refresh-token lifecycle
...
def test_auxiliary_client_routes_xai_oauth_through_responses_api(tmp_path, monkeypatch):
    ...
    callmemo, etc.).  Users on xAI Grok OAuth would then see surprise charges
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    using it on the sync-back path means every xAI/Codex/callmemo refresh in a
    ...
    assert raw_after["active_provider"] == "openrouter", (
        ...
        "callmemo token rotations silently take over multi-provider users' "
