...
def _valid_manifest() -> dict:
    return {
        ...
        "providers": {
            ...
            "nous": {
                "metadata": {"display_name": "callmemo Portal"},
                "models": [
...
class TestFallbackChain:
    ...
    PRIMARY = "https://github.com/Chensihakniroth/ANAKOT-AGENT/docs/api/model-catalog.json"
    FALLBACK = (
        "https://raw.githubusercontent.com/callmemo/anakot-agent"
...
class TestIntegrationWithModelsModule:
    ...
    def test_curated_nous_ids_falls_back_to_hardcoded_on_empty_catalog(
        ...
        from anakot_cli import model_catalog
        from anakot_cli.models import get_curated_callmemo_model_ids, _PROVIDER_MODELS

        with patch.object(model_catalog, "_fetch_manifest", return_value=None):
            result = get_curated_callmemo_model_ids()

        assert result == list(_PROVIDER_MODELS["nous"])

    def test_curated_nous_ids_prefers_manifest(self, isolated_home):
        from anakot_cli import model_catalog
        from anakot_cli.models import get_curated_callmemo_model_ids
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        importlib.reload(model_catalog)
        try:
            from anakot_cli.model_switch import list_picker_providers
            ...
            # (``curated["nous"] = get_curated_callmemo_model_ids()``), so the test
            # stays an invariant — it can't rot as the curated/manifest list grows.
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("anakot_cli.models.check_callmemo_free_tier", return_value=False), patch(
                ...
                expected = get_curated_callmemo_model_ids()
