...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    with patch("anakot_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_invalidates_on_version_change(tmp_path, monkeypatch):
    ...
    fake_banner = tmp_path / "anakot_cli" / "banner.py"
    ...
    with patch("anakot_cli.banner.subprocess.run") as mock_run, \
         patch("anakot_cli.banner.check_via_pypi", return_value=0) as mock_pypi:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ANAKOT_HOME", str(tmp_path))
    with patch("anakot_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ANAKOT_HOME", str(tmp_path))
    with patch("anakot_cli.banner.subprocess.run") as mock_run:
        with patch("anakot_cli.banner.check_via_pypi", return_value=0):
            result = banner.check_for_updates()
    assert result == 0
    mock_run.assert_not_called()
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when ANAKOT_HOME has no git repo."""
    import anakot_cli.banner as banner
    ...
    # Point ANAKOT_HOME at a temp dir with no anakot-agent/.git
    monkeypatch.setenv("ANAKOT_HOME", str(tmp_path))
    with patch("anakot_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_docker_returns_none(tmp_path, monkeypatch):
    ...
    ANAKOT_REVISION (nix-only), so without a docker guard check_for_updates()
    ...
    container, and `anakot update` correctly refusing to run there. The guard
    ...
    with patch("anakot_cli.config.detect_install_method", return_value="docker"), \
         patch("anakot_cli.banner.subprocess.run") as mock_run, \
         patch("anakot_cli.banner.check_via_pypi") as mock_pypi:
        result = banner.check_for_updates()
...
def test_check_for_updates_non_docker_still_checks(tmp_path, monkeypatch):
    ...
    with patch("anakot_cli.config.detect_install_method", return_value="pip"), \
         patch("anakot_cli.banner.subprocess.run") as mock_run, \
         patch("anakot_cli.banner.check_via_pypi", return_value=1) as mock_pypi:
        result = banner.check_for_updates()
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from anakot_cli.main import _invalidate_update_cache

    # Build a fake ~/.anakot with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"ANAKOT_HOME": str(default_home)}):
