...
class TestValidateProfileName:
    ...
    @pytest.mark.parametrize("name", ["anakot", "test", "tmp", "root", "sudo"])
    def test_reserved_names_rejected(self, name):
        """Reserved names collide with the Anakot install itself or with
...
class TestGetProfileDir:
    ...
    def test_named_profile_returns_profiles_subdir(self, profile_env):
        ...
        assert result == tmp_path / ".anakot" / "profiles" / "coder"

    def test_named_profile_matching_is_case_insensitive(self, profile_env):
        tmp_path = profile_env
        assert get_profile_dir("Coder") == tmp_path / ".anakot" / "profiles" / "coder"
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.anakot must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".anakot"
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes anakot-agent, .worktrees,
        ...
        (default_home / "anakot-agent" / ".git").mkdir(parents=True)
        (default_home / "anakot-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "anakot-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "anakot-agent").exists()
...
class TestNoSkillsOptOut:
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `anakot update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestGetActiveProfileName:
    ...
    def test_default_anakot_home_returns_default(self, profile_env):
        # ANAKOT_HOME points to tmp_path/.anakot which is the default
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("anakot")
        assert result is not None
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        ...
        assert "anakot -p mybot" in content
    ...
    def test_remove_returns_false_when_absent(self, profile_env):
        from anakot_cli.profiles import remove_wrapper_script
    ...
    def test_custom_alias_target_on_posix(self, profile_env, monkeypatch):
        ...
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "anakot -p redqueen" in content
...
class TestFindAliasForProfile:
    ...
    def test_no_alias_returns_none(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from anakot_cli.profiles import find_alias_for_profile
...
class TestRenameProfile:
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "anakot.ssi_health": {"workspace": "anakot", "enabled": True}
            }
        }))
        ...
        assert "anakot.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["anakot_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["anakot_heimdall"]["workspace"] == "anakot"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "anakot.ssi_health": {"aiPeer": "ssi_health"},
                "anakot_heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["anakot.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["anakot_heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("anakot-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".anakot_history",
        ...
        excluded_prefixes = [
            "default/anakot-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.anakot_history",
...
class TestInternalHelpers:
    ...
    def test_profiles_root_under_home(self, profile_env):
        ...
        assert root == tmp_path / ".anakot" / "profiles"

    def test_default_anakot_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_anakot_home()
        assert home == tmp_path / ".anakot"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (ANAKOT_HOME outside ~/.anakot), profiles go under ANAKOT_HOME."""
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (ANAKOT_HOME under ~/.anakot), profiles root is still ~/.anakot/profiles."""
        native = tmp_path / ".anakot"
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under ANAKOT_HOME."""
        from anakot_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ANAKOT_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".anakot" / "profiles" / "mybot"
        assert result == expected
