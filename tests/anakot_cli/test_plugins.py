...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    *base* is expected to be ``<anakot_home>/plugins/``; we derive
    ``<anakot_home>`` from it by walking one level up.
    """
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.anakot/plugins/ are discovered."""
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ANAKOT_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".anakot" / "plugins"
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("anakot_cli.plugins._plugin_manager", mgr):
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "anakot_test" / "plugins"
        # Set ANAKOT_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginDebugLogging:
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With ANAKOT_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("ANAKOT_PLUGINS_DEBUG", "1")
        from anakot_cli import plugins as plugins_mod
