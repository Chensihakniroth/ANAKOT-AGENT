...
def test_get_platform_tools_x_search_auto_enabled_when_xai_oauth_present(monkeypatch):
    ...
    to also click through `anakot tools` → X (Twitter) Search to flip the
    toolset on. Tool's check_fn still gates schema registration if creds
    later go missing.
    """
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "anakot_cli.tools_config._xai_credentials_present", lambda: True
    )
...
def test_get_platform_tools_x_search_off_when_no_xai_credentials(monkeypatch):
    ...
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "anakot_cli.tools_config._xai_credentials_present", lambda: False
    )
...
def test_get_platform_tools_x_search_respects_explicit_config(monkeypatch):
    """Once the user has saved an explicit toolset list via `anakot tools`,
    ...
    # User explicitly opted into spotify but not x_search via `anakot tools`.
    config = {"platform_toolsets": {"cli": ["anakot-cli", "spotify"]}}
    enabled = _get_platform_tools(config, "cli")
...
def test_get_platform_tools_expands_composite_when_mixed_with_configurable():
    """``[anakot-cli, spotify]`` (composite + configurable) must keep the full
    ``anakot-cli`` toolset alongside the explicit Spotify opt-in. The
    has_explicit_config branch used to drop ``anakot-cli`` on the floor,
    ...
    for ts in ("terminal", "file", "web", "browser", "memory", "delegation",
        ...
        assert ts in enabled, f"{ts} should be enabled when anakot-cli is listed"
...
def test_get_platform_tools_mixed_does_not_resurrect_default_off():
    ...
    pull-in. Without this, ``anakot-cli`` expansion would re-enable
    ``moa`` / ``rl`` / ``homeassistant`` for users who never opted in."""
    config = {"platform_toolsets": {"cli": ["anakot-cli", "terminal"]}}
...
def test_get_platform_tools_preserves_explicit_empty_selection():
    ...
    # live in _ANAKOT_CORE_TOOLS but aren't user-toggleable) are still
    # auto-recovered by _get_platform_tools so saving via `anakot tools`
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/callmemo/anakot-agent/issues/1247
    """
    config = {
        "platform_toolsets": {
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    """Platform default toolsets (anakot-cli, anakot-telegram, etc.) must NOT
    ...
    ``anakot tools``, but anakot-cli stays in the config and re-enables
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                ...
                "delegation", "file", "anakot-cli",  # <-- the culprit
    ...
    # anakot-cli must NOT survive — it's a platform default, not an MCP server
    assert "anakot-cli" not in saved
...
def test_save_platform_tools_does_not_preserve_anakot_telegram():
    """Same bug for Telegram — anakot-telegram must not be preserved."""
    config = {
        "platform_toolsets": {
            "telegram": [
                "browser", "file", "anakot-telegram", "terminal", "web",
            ]
        }
    }
    ...
    assert "anakot-telegram" not in saved
    assert "web" in saved
...
def test_save_platform_tools_still_preserves_mcp_with_platform_default_present():
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                "web", "terminal", "anakot-cli", "my-mcp-server", "github-tools",
            ]
        }
    }
...
def test_visible_providers_include_callmemo_subscription_when_logged_in(monkeypatch):
    config = {"model": {"provider": "nous"}}

    monkeypatch.setattr(
        "anakot_cli.callmemo_subscription.get_callmemo_portal_account_info",
    ...
    # The managed callmemo row is listed (not necessarily first — "Local Browser"
    # sorts first so a fresh-install Enter lands on the free local backend).
    assert any(p["name"].startswith("callmemo Subscription") for p in providers)
    # "Local Browser" must be the index-0 default so pressing Enter never
    # walks a user into a paid callmemo Portal login.
    assert providers[0]["name"] == "Local Browser"


def test_visible_providers_show_callmemo_subscription_when_logged_out(monkeypatch):
    """callmemo-managed Tool Gateway rows are always listed, even logged out.
...
def test_visible_providers_show_callmemo_subscription_when_paid_access_is_false(monkeypatch):
    ...
    shown; ``ensure_callmemo_portal_access`` blocks activation if still unpaid.
    """
    config = {"model": {"provider": "nous"}}
...
def test_visible_providers_force_fresh_shows_callmemo_subscription_after_upgrade(monkeypatch):
    calls = []
    ...
    monkeypatch.setattr(
        "anakot_cli.tools_config.get_callmemo_subscription_features",
    ...
    # The managed callmemo row reappears after the entitlement upgrade. It is no
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("anakot_cli.tools_config._run_post_setup", lambda key: None)
...
def test_fresh_install_browser_default_is_free_local_not_paid_nous():
    ...
    backend, never the paid callmemo Subscription gateway.

    Regression: the callmemo row used to sort first, so the menu cursor defaulted
    to index 0 (callmemo) and pressing Enter walked users straight into a callmemo
    Portal login for a paid offering (Javier's bug, June 2026).
    """
    from anakot_cli.tools_config import _detect_active_provider_index
...
def test_reconfigure_lists_enabled_web_without_existing_provider_config(monkeypatch):
    ...
    monkeypatch.setattr("anakot_cli.tools_config._prompt_choice", fake_prompt_choice)
    monkeypatch.setattr(
        "anakot_cli.tools_config._configure_tool_category_for_reconfig",
...
def test_first_install_nous_auto_configures_managed_defaults(monkeypatch):
    monkeypatch.setattr("anakot_cli.callmemo_subscription.managed_callmemo_tools_enabled", lambda: True)
    config = {
        "model": {"provider": "nous"},
        "platform_toolsets": {"cli": []},
    }
...
def test_first_install_nous_auto_configures_video_gen(monkeypatch):
    """When a callmemo subscriber checks video_gen in the toolset checklist,
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/callmemo/anakot-agent/issues/6901
    """
    config = {
...
def test_computer_use_needs_configuration_when_cua_driver_post_setup_pending():
    ...
    Returning users enabling Computer Use through `anakot tools` must reach the
...
def test_computer_use_respects_custom_cua_driver_command():
    """The setup gate should match runtime's ANAKOT_CUA_DRIVER_CMD override."""
    def fake_which(name: str):
        return "/opt/bin/custom-cua" if name == "custom-cua" else None

    with patch.dict("os.environ", {"ANAKOT_CUA_DRIVER_CMD": "custom-cua"}), \
...
def test_computer_use_blank_custom_driver_command_falls_back_to_default():
    ...
    with patch.dict("os.environ", {"ANAKOT_CUA_DRIVER_CMD": "   "}), \
         patch("shutil.which", side_effect=fake_which):
        assert _toolset_needs_configuration_prompt("computer_use", {}) is False
...
class TestImagegenBackendRegistry:
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        """Both callmemo Subscription and FAL.ai providers must carry the
...
def test_save_platform_tools_clears_no_mcp_sentinel():
    """`anakot tools` has no UI for no_mcp, so saving from the picker clears
...
def test_get_platform_tools_recovers_non_configurable_toolsets_from_composite():
    ...
    fake_toolsets["anakot-_test_platform"] = {
    ...
    test_platforms = {
        "_test_platform": {"label": "Test", "default_toolset": "anakot-_test_platform"},
    }

    with mock_patch("anakot_cli.tools_config.PLATFORMS", {**PLATFORMS, **test_platforms}):
...
def test_get_platform_tools_second_pass_skips_fully_claimed_toolsets():
    ...
    be added by the second pass (prevents 'search', 'anakot-acp' noise).
    """
...
def test_get_platform_tools_discord_both_off_by_default():
    """Both `discord` and `discord_admin` are opt-in via `anakot tools`,
...
def test_discord_toolsets_not_available_on_other_platforms():
    ...
    from anakot_cli.tools_config import _toolset_allowed_for_platform
...
def test_discord_toolsets_user_enabled_are_honored():
    """When the user opts in via `anakot tools`, the toolset appears."""
...
def test_save_platform_tools_strips_restricted_toolsets():
    ...
    from anakot_cli.tools_config import _save_platform_tools
    config = {}
...
def test_reconfigure_provider_syncs_use_gateway(monkeypatch, provider, config_key, expected):
    ...
    monkeypatch.setattr(
        "anakot_cli.callmemo_subscription.ensure_callmemo_portal_access",
        lambda **kwargs: True,
    )
    config = {}
...
def test_reconfigure_provider_runs_post_setup_for_env_var_providers(
    ...
    called = []
    monkeypatch.setattr("anakot_cli.tools_config._run_post_setup", lambda key: called.append(key))
    monkeypatch.setattr("anakot_cli.tools_config.get_env_value", lambda k: None)
    monkeypatch.setattr("anakot_cli.tools_config._prompt", lambda *a, **kw: "")
    monkeypatch.setattr("anakot_cli.tools_config.save_env_value", lambda k, v: None)
...
# ---------------------------------------------------------------------------
# Inline callmemo Portal login gate on managed-provider selection
# ---------------------------------------------------------------------------


def test_configure_managed_provider_blocks_when_not_entitled(monkeypatch):
    """Selecting a callmemo-managed backend without paid access writes no config."""
    monkeypatch.setattr(
        "anakot_cli.callmemo_subscription.ensure_callmemo_portal_access",
        lambda **kwargs: False,
    )
    provider = {
        "name": "callmemo Subscription (Firecrawl)",
...
def test_configure_non_managed_provider_skips_portal_gate(monkeypatch):
    """A self-hosted provider must never trigger the callmemo Portal login gate."""
    called = {"gate": False}
    ...
    monkeypatch.setattr(
        "anakot_cli.callmemo_subscription.ensure_callmemo_portal_access", _boom
    )
...
def test_apply_provider_selection_web_sets_backend():
    ...
    from anakot_cli.tools_config import apply_provider_selection
...
#    reported as added/removed by `anakot tools` ──────────────────────────
...
def test_kanban_not_reported_as_removed_in_diff():
    """Reproduces the false-signal bug: `anakot tools` printed ``- kanban``
