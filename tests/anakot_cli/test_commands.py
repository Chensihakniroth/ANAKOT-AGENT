...
class TestSlackNativeSlashes:
    ...
    def test_anakot_catchall_is_first(self):
        """``/anakot`` must be reserved as the first slot so the legacy
        ``/anakot <subcommand>`` form keeps working after we add new
        commands and hit the 50-slash cap."""
        slashes = slack_native_slashes()
        assert slashes[0][0] == "anakot"
    ...
    def test_excludes_slack_reserved_commands(self):
        ...
        Users can still reach them via /anakot <command>."""
        names = {n for n, _d, _h in slack_native_slashes()}
    ...
    def test_includes_aliases_as_first_class_slashes(self):
        ...
        anything dropped remains reachable via ``/anakot <command>``."""
        names = {n for n, _d, _h in slack_native_slashes()}
        assert "btw" in names
        assert "bg" in names
...
from anakot_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def test_plugin_command_appears_in_slack_subcommand_map(self, monkeypatch):
        """/anakot metricas must route through the Slack subcommand map."""
        self._patch_plugin_commands(monkeypatch, {
            "metricas": {
                "handler": lambda _a: "ok",
                "description": "Metrics",
