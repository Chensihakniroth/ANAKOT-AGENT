...
class TestReloadEnv:
    ...
    def test_does_not_remove_unknown_vars(self, tmp_path):
        """reload_env() preserves non-Anakot env vars even when absent from .env."""
        env_file = tmp_path / ".env"
        env_file.write_text("")
        with patch.dict(reload_env.__globals__, {"get_env_path": lambda: env_file}):
...
class TestWebServerEndpoints:
    ...
    def test_update_anakot_returns_docker_guidance_without_spawning(self, monkeypatch):
        ...
        monkeypatch.setattr(web_server, "_spawn_anakot_action", fail_spawn)
        web_server._ACTION_PROCS.pop("anakot-update", None)
        web_server._ACTION_RESULTS.pop("anakot-update", None)
    ...
    def test_update_anakot_spawns_on_non_docker_install(self, monkeypatch):
        ...
        monkeypatch.setattr(web_server, "_spawn_anakot_action", fake_spawn)
        web_server._ACTION_PROCS.pop("anakot-update", None)
        web_server._ACTION_RESULTS.pop("anakot-update", None)
        ...
        assert resp.status_code == 200
        assert resp.json() == {"ok": True, "pid": 12345, "name": "anakot-update"}
        assert calls == [(["update"], "anakot-update")]
    ...
    def test_get_env_vars_marks_channel_managed_keys(self):
        from anakot_cli.web_server import _channel_managed_env_keys
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        from anakot_cli.web_server import (
    ...
    def test_telegram_onboarding_start_strips_poll_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            ...
            return {
                ...
                "suggested_username": "anakot_pair123_bot",
                "deep_link": "https://t.me/newbot/AnakotSetupBot/anakot_pair123_bot",
                "qr_payload": "https://t.me/newbot/AnakotSetupBot/anakot_pair123_bot",
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "anakot_pair_ready_bot",
                    "deep_link": "https://t.me/newbot/AnakotSetupBot/anakot_pair_ready_bot",
                    "qr_payload": "https://t.me/newbot/AnakotSetupBot/anakot_pair_ready_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "anakot_pair_ready_bot",
    ...
    def test_telegram_onboarding_apply_requires_ready_pairing(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "anakot_pair_waiting_bot",
                "deep_link": "https://t.me/newbot/AnakotSetupBot/anakot_pair_waiting_bot",
                "qr_payload": "https://t.me/newbot/AnakotSetupBot/anakot_pair_waiting_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_telegram_onboarding_cancel_clears_local_session(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "anakot_pair_cancel_bot",
                "deep_link": "https://t.me/newbot/AnakotSetupBot/anakot_pair_cancel_bot",
                "qr_payload": "https://t.me/newbot/AnakotSetupBot/anakot_pair_cancel_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/anakot_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
            assert "FastAPI" not in resp.text  # Should not serve the actual source

    def test_set_model_main_nous_applies_gateway_defaults(self, monkeypatch):
        """Switching the main provider to callmemo calls apply_nous_managed_defaults
        ...
        resp = self.client.post(
            "/api/model/set",
            json={"scope": "main", "provider": "nous", "model": "anakot-4"},
        )
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_set_model_main_non_nous_skips_gateway_defaults(self, monkeypatch):
        """Non-callmemo providers must NOT trigger Tool Gateway auto-routing."""
        import anakot_cli.callmemo_subscription as ns
    ...
    def test_apply_main_model_assignment_base_url_and_context_reconcile(self):
        ...
        from anakot_cli.web_server import _apply_main_model_assignment
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from anakot_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_reports_stale_auxiliary_pins(self):
        ...
        cfg = load_config()
        cfg["model"] = {"provider": "nous", "default": "anakot-4"}
        cfg["auxiliary"] = {
    ...
    def test_recommended_default_nous_honors_free_tier(self, monkeypatch):
        """For a free-tier callmemo user, the recommended default must be a free
        model (mirroring `anakot model`), not the first curated paid entry."""
        import anakot_cli.models as models_mod

        monkeypatch.setattr(models_mod, "get_curated_callmemo_model_ids", lambda: ["paid/expensive", "free/cheap"])
        monkeypatch.setattr(
            models_mod, "get_pricing_for_provider",
            lambda provider: {"paid/expensive": {"input": "1"}, "free/cheap": {"input": "0"}},
        )
        monkeypatch.setattr(models_mod, "check_callmemo_free_tier", lambda *, force_fresh=False: True)
        monkeypatch.setattr(
    ...
    def test_recommended_default_nous_paid_uses_curated_default(self, monkeypatch):
        """A paid callmemo user gets the first curated/paid-augmented model."""
        import anakot_cli.models as models_mod

        monkeypatch.setattr(models_mod, "get_curated_callmemo_model_ids", lambda: ["top/model", "other/model"])
        monkeypatch.setattr(models_mod, "get_pricing_for_provider", lambda provider: {})
        monkeypatch.setattr(models_mod, "check_callmemo_free_tier", lambda *, force_fresh=False: False)
        monkeypatch.setattr(
    ...
    def test_recommended_default_handles_failure_gracefully(self, monkeypatch):
        ...
        monkeypatch.setattr(models_mod, "get_curated_callmemo_model_ids", boom)
...
class TestBuildSchemaFromConfig:
    def test_produces_expected_field_count(self):
        from anakot_cli.web_server import CONFIG_SCHEMA
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from anakot_cli.web_server import _build_schema_from_config
...
class TestNewEndpoints:
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        ...
        named = anakot_home / "profiles" / "multi-agent"
    ...
    def test_profile_setup_command_uses_named_profile_wrapper(self):
        ...
        (get_anakot_home() / "profiles" / "coder").mkdir(parents=True)
    ...
    def test_profile_setup_command_uses_anakot_for_default_profile(self):
        from anakot_constants import get_anakot_home
        ...
        assert resp.status_code == 200
        assert resp.json()["command"] == "anakot setup"
    ...
    def test_profiles_create_with_clone_from_default_copies_default_skills(self, monkeypatch):
        ...
        default_skill = get_anakot_home() / "skills" / "custom" / "new-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = get_anakot_home() / "profiles" / "cloned" / "skills" / "custom" / "new-skill" / "SKILL.md"
    ...
    def test_profiles_create_with_clone_from_duplicates_source(self, monkeypatch):
        ...
        source_skill = get_anakot_home() / "profiles" / "source-prof" / "skills" / "custom" / "src-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = (
            get_anakot_home() / "profiles" / "source-prof-copy" / "skills" / "custom" / "src-skill" / "SKILL.md"
        )
        assert cloned_skill.exists()
    ...
    def test_profiles_create_without_clone_seeds_bundled_skills(self, monkeypatch):
        ...
        assert resp.status_code == 200
        seeded_skill = get_anakot_home() / "profiles" / "fresh" / "skills" / "software-development" / "plan" / "SKILL.md"
    ...
    def test_profile_model_round_trip(self, monkeypatch):
        ...
        import yaml
        cfg_path = get_anakot_home() / "profiles" / "model-prof" / "config.yaml"
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import anakot_cli.skills_config as skills_config
        import anakot_cli.web_server as web_server
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from anakot_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from anakot_cli.web_server import _denormalize_config_from_web
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_missing_name(self):
        from anakot_cli.web_server import _normalise_theme_definition
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        assert env["ANAKOT_TUI_INLINE"] == "1"
        assert env["ANAKOT_TUI_DISABLE_MOUSE"] == "1"
    ...
    def test_streams_child_stdout_to_client(self, monkeypatch):
        ...
        with self.client.websocket_connect(self._url()) as conn:
            ...
            while time.monotonic() < deadline:
                ...
                if frame:
                    buf += frame
                if b"anakot-ws-ok" in buf:
                    break
            assert b"anakot-ws-ok" in buf
    ...
    def test_channel_param_propagates_sidecar_url(self, monkeypatch):
        ...
        ANAKOT_TUI_SIDECAR_URL env var pointing back at /api/pub on the
