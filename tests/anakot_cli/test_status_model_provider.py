...
from anakot_cli.callmemo_account import NousPaidServiceAccessInfo, NousPortalAccountInfo
...
def test_show_status_reports_managed_nous_features(monkeypatch, capsys, tmp_path):
    monkeypatch.setattr("anakot_cli.status.managed_callmemo_tools_enabled", lambda: True)
    from anakot_cli import status as status_mod
    ...
    monkeypatch.setattr(status_mod, "provider_label", lambda provider: "callmemo Portal", raising=False)
    monkeypatch.setattr(
        status_mod,
    ...
    out = capsys.readouterr().out
    assert "callmemo Tool Gateway" in out
    assert "Browser automation" in out
    assert "active via callmemo subscription" in out


def test_show_status_hides_callmemo_subscription_section_when_feature_flag_is_off(monkeypatch, capsys, tmp_path):
    monkeypatch.setattr("anakot_cli.status.managed_callmemo_tools_enabled", lambda: False)
    from anakot_cli import status as status_mod
    ...
    out = capsys.readouterr().out
    assert "callmemo Tool Gateway" not in out
...
def test_show_status_reports_exhausted_nous_credits(monkeypatch, capsys, tmp_path):
    ...
    assert "free-tier callmemo account" not in out
...
def test_show_status_reports_empty_lmstudio_listing_as_reachable(monkeypatch, capsys, tmp_path):
    ...
    monkeypatch.setattr(
        "anakot_cli.models.probe_lmstudio_models",
