...
class TestResolveVerifyFallback:
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("ANAKOT_CA_BUNDLE", raising=False)
        result = _resolve_verify(auth_state={"tls": {}})
        assert result is True

    def test_missing_anakot_ca_bundle_env_falls_back(self, monkeypatch):
        from anakot_cli.auth import _resolve_verify

        monkeypatch.setenv("ANAKOT_CA_BUNDLE", "/nonexistent/anakot-ca.pem")
...
def test_resolve_callmemo_runtime_credentials_prefers_invoke_jwt_and_mirrors(
    tmp_path,
    monkeypatch,
):
    ...
    _setup_nous_auth(
        anakot_home,
        access_token=token,
        scope=auth_mod.DEFAULT_CALLMEMO_SCOPE,
        expires_at=_future_iso(3600),
        expires_in=3600,
    )
    monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))

    creds = auth_mod.resolve_callmemo_runtime_credentials()
...
def test_resolve_callmemo_runtime_credentials_invoke_jwt_is_idempotent(
    tmp_path,
    monkeypatch,
):
    import anakot_cli.auth as auth_mod
...
def test_resolve_callmemo_runtime_credentials_trusts_invoke_jwt_exp_over_stale_metadata(
    tmp_path,
    monkeypatch,
):
    import anakot_cli.auth as auth_mod
...
def test_resolve_callmemo_runtime_credentials_does_not_apply_agent_key_ttl_to_invoke_jwt(
    tmp_path,
    monkeypatch,
):
    import anakot_cli.auth as auth_mod
...
def test_resolve_callmemo_runtime_credentials_refreshes_legacy_agent_key_to_invoke_jwt(
    tmp_path,
    monkeypatch,
):
    import anakot_cli.auth as auth_mod
...
def test_resolve_callmemo_runtime_credentials_reauths_when_invoke_scope_missing(
    tmp_path,
    monkeypatch,
):
    import anakot_cli.auth as auth_mod
    ...
    with pytest.raises(AuthError) as exc:
        auth_mod.resolve_callmemo_runtime_credentials()
...
def test_nous_device_code_login_does_not_retry_legacy_scope_when_invoke_refused(monkeypatch):
    ...
    assert scopes == [auth_mod.DEFAULT_CALLMEMO_SCOPE]
...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    assert requested_scopes == [auth_mod.DEFAULT_CALLMEMO_SCOPE]
...
def test_get_nous_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    """get_nous_auth_status() should find callmemo credentials in the pool
    even when the auth store has no callmemo provider entry — this is the
    ...
    anakot_home = tmp_path / "anakot"
    anakot_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store — no callmemo provider entry
    (anakot_home / "auth.json").write_text(json.dumps({
        "version": 1, "providers": {},
    }))
    monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))

    # Seed the credential pool with a callmemo entry
...
def test_get_nous_auth_status_pool_opaque_key_is_not_inference_credential(tmp_path, monkeypatch):
    from anakot_cli.auth import get_nous_auth_status, invalidate_nous_auth_status_cache
...
def test_get_nous_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    ...
    anakot_home = tmp_path / "anakot"
    _setup_nous_auth(anakot_home, access_token="at-123")
    monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
    monkeypatch.setattr(
        "anakot_cli.auth.resolve_callmemo_runtime_credentials",
        lambda **kwargs: {
            "base_url": "https://inference.example.com/v1",
...
def test_get_nous_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    ...
    anakot_home = tmp_path / "anakot"
    _setup_nous_auth(anakot_home, access_token="at-fresh")
    monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
...
def test_get_nous_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("anakot_cli.auth.resolve_callmemo_runtime_credentials", _boom)
...
def test_refresh_token_persisted_when_refreshed_jwt_lacks_invoke_scope(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("anakot_cli.auth._refresh_access_token", _fake_refresh_access_token)
...
def test_terminal_refresh_failure_quarantines_tokens(
    ...
    """A revoked/invalid callmemo refresh token must not be replayed forever."""
...
def test_managed_access_token_refresh_failure_quarantines_tokens(
    ...
    anakot_home = tmp_path / "anakot"
    _setup_nous_auth(anakot_home, refresh_token="refresh-old")
    monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
    from agent.credential_pool import load_pool
...
# =============================================================================
# _login_callmemo: "Skip (keep current)" must preserve prior provider + model
# =============================================================================


class TestLoginNousSkipKeepsCurrent:
    """When a user runs `anakot model` → callmemo Portal → Skip (keep current) after
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        """Patch OAuth + model-list + prompt so _login_callmemo doesn't hit network."""
        import anakot_cli.auth as auth_mod
        import anakot_cli.models as models_mod
        import anakot_cli.callmemo_subscription as ns

        fake_auth_state = {
            "access_token": "fake-nous-token",
            "agent_key": "fake-agent-key",
            "inference_base_url": "https://inference-api.callmemo.ai",
            "portal_base_url": "https://portal.callmemo.ai",
        ...
        def _check_callmemo_free_tier(**kwargs):
            free_tier_calls.append(kwargs)
            return None

        monkeypatch.setattr(models_mod, "check_callmemo_free_tier", _check_callmemo_free_tier)
        monkeypatch.setattr(
            models_mod, "partition_callmemo_models_by_tier",
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        """User picks Skip → config.yaml untouched, callmemo creds still saved."""
        import argparse
        import yaml
        from anakot_cli.auth import PROVIDER_REGISTRY, _login_callmemo
        ...
        _login_callmemo(args, PROVIDER_REGISTRY["nous"])
        ...
        # auth.json: active_provider restored to openrouter, but callmemo creds saved
    ...
    def test_picking_model_switches_to_nous(self, tmp_path, monkeypatch):
        """User picks a callmemo model → provider flips to nous with that model."""
        import argparse
        import yaml
        from anakot_cli.auth import PROVIDER_REGISTRY, _login_callmemo
    ...
    def test_skip_with_no_prior_active_provider_clears_it(self, tmp_path, monkeypatch):
        ...
        # But callmemo creds are still saved
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    wrote only the pool. After the callmemo agent_key's 24h TTL expired, the
    401-recovery path in run_agent.py called resolve_callmemo_runtime_credentials
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    """End-to-end: after persisting via the helper, resolve_callmemo_runtime_credentials
    must succeed (not raise "Anakot is not logged into callmemo Portal").

    This is the exact path that run_agent.py's `_try_refresh_nous_client_credentials`
    calls after a callmemo 401 — before the fix it would raise AuthError because
    ...
    # doesn't raise "Anakot is not logged into callmemo Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `anakot auth add nous --type oauth --label <name>` silently
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    When the callmemo Portal server returns ``invalid_grant`` with
...
def test_refresh_token_reuse_error_code_is_terminal():
    """callmemo may return refresh_token_reused as the OAuth error code itself."""
    from anakot_cli import auth as auth_mod
...
def test_refresh_token_exchange_sends_refresh_token_header():
    """callmemo refresh tokens must be sent in a header so sandbox proxies can
...
# =============================================================================
# Shared callmemo token store — cross-profile persistence (Codex-style auto-import)
# =============================================================================
...
def shared_store_env(tmp_path, monkeypatch):
    ...
    Required for every test that exercises the shared callmemo store — the
...
def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="shared callmemo auth store"):
        _nous_shared_store_path()


def test_shared_store_honors_env_override(tmp_path, monkeypatch):
    """ANAKOT_SHARED_AUTH_DIR must redirect the path."""
    from anakot_cli.auth import _nous_shared_store_path, NOUS_SHARED_STORE_FILENAME

    custom_dir = tmp_path / "custom_shared"
    monkeypatch.setenv("ANAKOT_SHARED_AUTH_DIR", str(custom_dir))
...
def test_shared_store_read_missing_returns_none(shared_store_env):
    ...
    from anakot_cli.auth import _read_shared_nous_state
...
def test_shared_store_read_malformed_returns_none(shared_store_env):
    ...
    from anakot_cli.auth import _nous_shared_store_path, _read_shared_nous_state
...
def test_shared_store_write_skips_when_refresh_token_missing(shared_store_env):
    ...
    from anakot_cli.auth import _nous_shared_store_path, _write_shared_nous_state
...
def test_try_import_shared_returns_none_when_store_missing(shared_store_env):
    ...
    from anakot_cli.auth import _try_import_shared_nous_state
...
def test_shared_store_survives_across_profile_switch(
    ...
    (different ANAKOT_HOME) sees the same shared state and can rehydrate
    ...
    monkeypatch.setenv("ANAKOT_HOME", str(profile_a))
    ...
    # Profile B: fresh ANAKOT_HOME, no auth yet, but the shared store
    ...
    monkeypatch.setenv("ANAKOT_HOME", str(profile_b))
...
def test_runtime_refresh_uses_newer_shared_token_before_local_stale_token(
    ...
    """A sibling profile may rotate the single-use callmemo refresh token.
