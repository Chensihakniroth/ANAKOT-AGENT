...
def test_get_container_exec_info_none_inside_container(container_env):
    ...
    with patch("anakot_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_anakot_dev(container_env, monkeypatch):
    """Returns None when ANAKOT_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("ANAKOT_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_anakot_dev_zero(container_env, monkeypatch):
    """ANAKOT_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("ANAKOT_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        anakot_home = Path(tmpdir) / ".anakot"
        anakot_home.mkdir()
        (anakot_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("anakot_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_anakot_home": lambda: anakot_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ANAKOT_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=anakot-custom\n"
        "exec_user=myuser\n"
        "anakot_bin=/opt/anakot/bin/anakot\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "anakot-custom"
    assert info["exec_user"] == "myuser"
    assert info["anakot_bin"] == "/opt/anakot/bin/anakot"
