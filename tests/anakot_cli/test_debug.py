...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from anakot_cli.debug import _upload_dpaste_com
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, anakot_home):
    ...
    def test_empty_primary_reports_file_empty(self, anakot_home):
    ...
    def test_truncates_large_file(self, anakot_home):
        """Files larger than max_bytes get tail-truncated."""
        from anakot_cli.debug import _capture_log_snapshot

        # Write a file larger than 1KB
        big_content = "x" * 100 + "\n"
        (anakot_home / "logs" / "agent.log").write_text(big_content * 200)
    ...
    def test_drops_partial_when_truncation_mid_line(self, anakot_home):
    ...
    def test_unknown_log_returns_none(self, anakot_home):
        from anakot_cli.debug import _capture_log_snapshot
        snap = _capture_log_snapshot("nonexistent", tail_lines=10)
        assert snap.full_text is None

    def test_falls_back_to_rotated_file(self, anakot_home):
        """When gateway.log doesn't exist, falls back to gateway.log.1."""
        from anakot_cli.debug import _capture_log_snapshot

        logs_dir = anakot_home / "logs"
    ...
    def test_prefers_primary_over_rotated(self, anakot_home):
    ...
    def test_falls_back_when_primary_empty(self, anakot_home):
...
class TestCollectDebugReport:
    ...
    def test_report_includes_agent_log(self, anakot_home):
        from anakot_cli.debug import collect_debug_report
    ...
    def test_report_includes_errors_log(self, anakot_home):
        from anakot_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, anakot_home):
        from anakot_cli.debug import collect_debug_report
    ...
    def test_report_includes_desktop_log(self, anakot_home):
        from anakot_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_share_survives_sweep_failure(self, anakot_home, capsys):
        ...
        with patch("anakot_cli.dump.run_dump"), \
             patch(
                 "anakot_cli.debug._sweep_expired_pastes",
    ...
    def test_local_flag_prints_full_logs(self, anakot_home, capsys):
    ...
    def test_share_uploads_four_pastes(self, anakot_home, capsys):
    ...
    def test_share_keeps_report_and_full_log_on_same_snapshot(self, anakot_home, capsys):
        """A mid-run rotation must not make full agent.log older than the report."""
        from anakot_cli.debug import run_debug_share, collect_debug_report as real_collect_debug_report
        ...
        with patch("anakot_cli.dump.run_dump"), \
             patch("anakot_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("anakot_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
    ...
    def test_share_continues_on_log_upload_failure(self, anakot_home, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, anakot_home, capsys):
...
class TestScheduleAutoDelete:
    ...
    def test_does_not_spawn_subprocess(self, anakot_home):
        ...
        import ast
        import inspect
        from anakot_cli.debug import _schedule_auto_delete
    ...
    def test_skips_non_paste_rs_urls(self, anakot_home):
    ...
    def test_merges_with_existing_pending(self, anakot_home):
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, anakot_home):
    ...
    def test_sweep_drops_entries_past_grace_window(self, anakot_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_survives_sweep_failure(self, anakot_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "Usage: anakot debug" in out
...
class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        ...
        with patch("anakot_cli.debug.delete_paste", return_value=True):
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, anakot_home, capsys):
        from anakot_cli.debug import run_debug_share
        ...
        with patch("anakot_cli.dump.run_dump"), \
             ...
             patch("anakot_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_local_no_privacy_notice(self, anakot_home, capsys):
        from anakot_cli.debug import run_debug_share
...
class TestBuildDebugShare:
    ...
    def test_returns_structured_urls(self, anakot_home):
        from anakot_cli.debug import build_debug_share, DebugShareResult
    ...
    def test_skips_missing_logs_without_failure(self, anakot_home):
        ...
        with patch("anakot_cli.dump.run_dump"), patch(
            "anakot_cli.debug.upload_to_pastebin",
    ...
    def test_required_report_failure_raises(self, anakot_home):
        from anakot_cli.debug import build_debug_share
