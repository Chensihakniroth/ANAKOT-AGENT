...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["anakot", "-w", "--tui"], None),
        (["anakot", "version"], "version"),
        (["anakot", "--tui", "chat"], "chat"),
        (["anakot", "-w", "logs"], "logs"),
        (["anakot", "chat", "hello world"], "chat"),
        (["anakot", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["anakot", "-m", "gpt5", "chat"], "chat"),
        (["anakot", "--model", "gpt5", "chat", "hi"], "chat"),
        (["anakot", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["anakot", "-z", "hello world"], None),
        (["anakot", "-z", "hello", "chat"], "chat"),
        (["anakot", "--model=gpt5", "chat"], "chat"),     # inline form
        (["anakot", "--", "chat"], "chat"),               # -- terminator
        (["anakot", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["anakot", "some-plugin-cmd"], "some-plugin-cmd"),
        (["anakot", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["anakot"],                          # bare → chat
        ["anakot", "--help"],                # top-level help
        ["anakot", "-h"],
        ["anakot", "version"],               # known built-in
        ["anakot", "logs"],
        ["anakot", "gateway", "run"],
        ["anakot", "--tui"],
        ["anakot", "-w", "--tui"],
        ["anakot", "chat", "hi"],
        ["anakot", "help"],                  # accepted built-in-ish
        ["anakot", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["anakot", "meet", "join"],          # potential google_meet plugin
        ["anakot", "honcho", "status"],      # potential memory plugin
        ["anakot", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``anakot help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"anakot_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
