...
The portal HTTP call and the callmemo token resolution are both mocked — this
...
class TestFastFails:
    def test_not_logged_in_exits_1_with_setup_hint(self, capsys):
        ...
        assert "not logged into callmemo Portal" in out
...
class TestHappyPath:
    def _run(self, *, args, account_token="tok_abc", portal="https://portal.callmemo.ai",
    ...
    def test_custom_redirect_uri_is_forwarded(self, capsys):
        captured: dict = {}
        self._run(
            args=_ns(redirect_uri="https://anakot.example.com/auth/callback"),
            captured=captured,
        )
        assert (
            captured["body"]["custom_redirect_uri"]
            == "https://anakot.example.com/auth/callback"
        )
    ...
    def test_non_default_portal_is_persisted(self, capsys):
        ...
        assert (
            saved["ANAKOT_DASHBOARD_PORTAL_URL"]
...
class TestPortalResolution:
    ...
    def test_falls_back_to_stored_login_portal(self):
        with patch(
            ...
            return_value={"portal_base_url": "https://portal.staging-callmemo.ai"},
        ):
            assert (
                dr._resolve_portal_base_url(None)
                == "https://portal.staging-callmemo.ai"
            )
...
class TestPortalErrors:
    def _run_http_error(self, code, body):
        err = urllib.error.HTTPError(
            url="https://portal.callmemo.ai/api/oauth/self-hosted-client",
            code=code,
            msg="err",
            hdrs=None,
        ...
        with patch(
            ...
            dr, "_resolve_portal_base_url", return_value="https://portal.callmemo.ai"
