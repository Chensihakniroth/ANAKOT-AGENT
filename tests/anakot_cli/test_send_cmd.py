...
def test_load_anakot_env_does_not_override_existing(tmp_path, monkeypatch):
    """Existing env vars must not be clobbered by config.yaml values."""
    import os

    anakot_home = tmp_path / ".anakot"
    anakot_home.mkdir()
    (anakot_home / "config.yaml").write_text("TELEGRAM_HOME_CHANNEL: yaml_value\n")
...
def test_load_anakot_env_handles_missing_files(tmp_path, monkeypatch):
