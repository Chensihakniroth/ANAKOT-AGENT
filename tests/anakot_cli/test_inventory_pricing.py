...
columns + Free/Pro badges and gate paid models on free callmemo accounts, the
...
def _patch_pricing(monkeypatch, *, free_tier, pricing, unavailable=None):
    ...
    monkeypatch.setattr(models_mod, "check_callmemo_free_tier", lambda *, force_fresh=False: free_tier)
    monkeypatch.setattr(
...
def test_apply_pricing_nous_free_tier_gates_paid_models(monkeypatch):
    """A free-tier callmemo account marks paid models unavailable and sets the flag."""
    _patch_pricing(
        monkeypatch,
        free_tier=True,
        pricing={
...
def test_apply_pricing_nous_paid_tier_no_gating(monkeypatch):
    """A paid callmemo account gates nothing."""
    _patch_pricing(
        monkeypatch,
        free_tier=False,
