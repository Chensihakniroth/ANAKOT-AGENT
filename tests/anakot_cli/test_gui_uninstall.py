...
def test_agent_is_installed_detects_source_and_venv(tmp_path):
    ...
    anakot_home.mkdir()
    assert gu.agent_is_installed(anakot_home) is False
    _make_agent(anakot_home)
    assert gu.agent_is_installed(anakot_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    anakot_home = tmp_path / ".anakot"
    (anakot_home / "anakot-agent" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(anakot_home) is True
...
def test_source_built_artifacts_lists_known_paths(tmp_path):
    ...
    artifacts = gu.source_built_gui_artifacts(anakot_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(anakot_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(anakot_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    anakot_home = tmp_path / ".anakot"
    agent_root = _make_agent(anakot_home)
    _make_gui_build(anakot_home)
    _make_user_data(anakot_home)
    ...
    assert not (anakot_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "anakot_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (anakot_home / "config.yaml").exists()
    assert (anakot_home / ".env").exists()
    assert (anakot_home / "sessions").exists()
...
def test_uninstall_gui_removes_userdata(tmp_path, monkeypatch):
    anakot_home = tmp_path / ".anakot"
    _make_agent(anakot_home)
    userdata = tmp_path / "Anakot-userdata"
    userdata.mkdir()
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(anakot_home, remove_userdata=False)
...
def test_userdata_dir_per_platform(monkeypatch):
    """userData path matches Electron's app.getPath('userData') for "Anakot"."""
    home = Path("/home/tester")
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))

    monkeypatch.setattr(gu.sys, "platform", "darwin")
    assert gu.desktop_userdata_dir() == home / "Library" / "Application Support" / "Anakot"

    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    assert gu.desktop_userdata_dir() == home / ".config" / "Anakot"
...
def test_userdata_dir_windows(monkeypatch):
    ...
    assert gu.desktop_userdata_dir() == Path(r"C:\Users\tester\AppData\Roaming") / "Anakot"
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    ANAKOT_HOME with all the destructive externals stubbed out.
    """
    ...
    from anakot_cli import gui_uninstall as gu_mod
...
def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    """``--yes --full`` removes the whole ANAKOT_HOME non-interactively."""
    import anakot_cli.uninstall as uninstall

    anakot_home = tmp_path / ".anakot"
    (anakot_home / "anakot-agent" / "anakot_cli").mkdir(parents=True)
    (anakot_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
    ...
    assert not anakot_home.exists()


def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m anakot_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_anakot_home", lambda: anakot_home)
    ...
    assert not (desktop / "release").exists()
    assert not (anakot_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "anakot_cli").exists()
    assert (anakot_home / "config.yaml").exists()
