...
from anakot_cli.proxy.adapters.callmemo_portal import NousPortalAdapter
...
def test_get_adapter_case_insensitive():
    ...
    assert isinstance(get_adapter("  callmemo  "), NousPortalAdapter)
...
def test_nous_adapter_metadata():
    ...
    assert adapter.display_name == "callmemo Portal"
...
def test_nous_adapter_not_authenticated_when_no_auth_file(tmp_path, monkeypatch):
    # ANAKOT_HOME is already set by conftest, but make doubly sure
...
def test_nous_adapter_authenticated_with_agent_key(tmp_path, monkeypatch):
    ...
    _write_auth_store(tmp_path, {
        ...
        "inference_base_url": "https://inference-api.callmemo.ai/v1",
    })
...
def test_nous_adapter_get_credential_uses_runtime_resolver(tmp_path, monkeypatch):
    ...
    with patch(
        "anakot_cli.proxy.adapters.callmemo_portal.resolve_callmemo_runtime_credentials",
    ...
    assert cred.base_url == "https://inference-api.callmemo.ai/v1"
...
def test_nous_adapter_get_credential_raises_when_not_logged_in(tmp_path, monkeypatch):
    monkeypatch.setenv("ANAKOT_HOME", str(tmp_path))
    adapter = NousPortalAdapter()
    with pytest.raises(RuntimeError, match="anakot auth add nous"):
...
def test_xai_adapter_retry_refreshes_current_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("anakot_cli.auth.refresh_xai_oauth_pure", fake_refresh)
...
def test_server_forwards_chat_completions():
    async def run():
        ...
        try:
            ...
            assert "Anakot-4-70B" in req["body"]
        finally:
...
def test_cmd_proxy_status_runs(capsys, tmp_path, monkeypatch):
    monkeypatch.setenv("ANAKOT_HOME", str(tmp_path))
    from anakot_cli.proxy.cli import cmd_proxy_status
    ...
    assert "nous" in out
    assert "callmemo Portal" in out
    assert "not logged in" in out


def test_cmd_proxy_providers_runs(capsys):
    from anakot_cli.proxy.cli import cmd_proxy_list_providers
...
def test_cmd_proxy_start_refuses_unknown_provider(capsys):
    from anakot_cli.proxy.cli import cmd_proxy_start
...
def test_cmd_proxy_start_refuses_when_unauthenticated(capsys, tmp_path, monkeypatch):
    ...
    assert rc == 2
    err = capsys.readouterr().err
    assert "anakot auth add nous" in err
