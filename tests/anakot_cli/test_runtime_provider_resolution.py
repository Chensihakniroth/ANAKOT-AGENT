...
def test_named_custom_provider_does_not_shadow_builtin_provider(monkeypatch):
    ...
    assert resolved["base_url"] == "https://inference-api.callmemo.ai/v1"
...
def test_auto_detected_nous_auth_failure_falls_through_to_openrouter(monkeypatch):
    """When auto-detect picks callmemo but credentials are revoked, fall through to OpenRouter."""
    from anakot_cli.auth import AuthError
    ...
    # callmemo credential resolution fails with revoked token
    monkeypatch.setattr(
        rp, "resolve_callmemo_runtime_credentials",
...
def test_explicit_nous_auth_failure_still_raises(monkeypatch):
    """When user explicitly requests callmemo and auth fails, the error should propagate."""
    from anakot_cli.auth import AuthError
    import pytest
...
class TestAzureFoundryResolution:
    ...
    def test_azure_foundry_missing_api_key_raises(self, monkeypatch):
        ...
        # `get_env_value` reads from ~/.anakot/.env — mock it to return None
...
def test_minimax_oauth_runtime_uses_inference_base_url(monkeypatch):
    ...
    from anakot_cli.auth import MINIMAX_OAUTH_CN_INFERENCE
