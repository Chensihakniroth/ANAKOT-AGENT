...
class TestSystemdServiceRefresh:
    ...
    def test_run_gateway_refreshes_outdated_unit_on_boot(self, tmp_path, monkeypatch):
        ...
        via exit-code-75 (bypassing `anakot gateway restart`)."""
        unit_path = tmp_path / "anakot-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
...
class TestGeneratedSystemdUnits:
    ...
    def test_system_unit_includes_wsl_windows_interop_paths(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "_anakot_home_for_target_user", lambda home: "/home/alice/.anakot")
        monkeypatch.setenv("PATH", "/usr/local/bin:/mnt/c/WINDOWS/system32")
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
...
class TestLaunchdServiceRecovery:
    def test_get_restart_drain_timeout_prefers_env_then_config_then_default(self, monkeypatch):
        ...
        monkeypatch.setenv("ANAKOT_RESTART_DRAIN_TIMEOUT", "9")
        assert gateway_cli._get_restart_drain_timeout() == 9.0

        monkeypatch.setenv("ANAKOT_RESTART_DRAIN_TIMEOUT", "invalid")
        assert (
            gateway_cli._get_restart_drain_timeout()
            == DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT
        )
    ...
    def test_launchd_fallback_exits_when_spawn_fails(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "nohup anakot gateway run" in out
...
class TestGatewaySystemServiceRouting:
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: anakot gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup anakot gateway" in out
...
class TestSystemUnitAnakotHome:
    ...
    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: ANAKOT_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("ANAKOT_HOME", "/root/.anakot/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'ANAKOT_HOME=/home/alice/.anakot/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_anakot_home(self, monkeypatch):
        # Custom ANAKOT_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("ANAKOT_HOME", "/opt/anakot-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'ANAKOT_HOME=/opt/anakot-shared' in unit
...
class TestAnakotHomeForTargetUser:
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._anakot_home_for_target_user("/home/alice")
        assert result == "/home/alice/.anakot/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("ANAKOT_HOME", "/opt/anakot")

        result = gateway_cli._anakot_home_for_target_user("/home/alice")
        assert result == "/opt/anakot"
...
class TestProfileArg:
    ...
    def test_default_anakot_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.anakot should not produce a --profile flag."""
        anakot_home = tmp_path / ".anakot"
        anakot_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
        result = gateway_cli._profile_arg(str(anakot_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.anakot/profiles/mybot should return '--profile mybot'."""
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile ANAKOT_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "anakot"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.anakot/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".anakot" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".anakot" / "profiles" / "My Bot!"
...
class TestRemapPathForUser:
    ...
    def test_keeps_system_path_unchanged(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "root")
        (tmp_path / "root").mkdir()
        result = gateway_cli._remap_path_for_user("/opt/anakot", str(tmp_path / "alice"))
        assert result == "/opt/anakot"

    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        (tmp_path / "alice").mkdir()
        original = str(tmp_path / "alice" / ".anakot" / "anakot-agent")
...
class TestDockerAwareGateway:
    ...
    def test_uninstall_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'anakot gateway uninstall' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'anakot gateway start' inside Docker exits 0 with container guidance."""
        import pytest
...
class TestLegacyAnakotUnitDetection:
    ...
    def test_detects_legacy_anakot_service_in_user_scope(self, tmp_path, monkeypatch):
        ...
        assert name == "anakot.service"
        assert path == legacy
        assert is_system is False
        assert gateway_cli.has_legacy_anakot_units() is True

    def test_detects_legacy_anakot_service_in_system_scope(self, tmp_path, monkeypatch):
        _, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = system_dir / "anakot.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_ignores_profile_unit_anakot_gateway_coder(self, tmp_path, monkeypatch):
        """CRITICAL: profile units must NOT be flagged as legacy.

        Teknium's concern — ``anakot-gateway-coder.service`` is our standard
        ...
        for base in (user_dir, system_dir):
            (base / "anakot-gateway-coder.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "anakot-gateway-orcha.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "anakot-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
        ...
        assert results == []
        assert gateway_cli.has_legacy_anakot_units() is False

    def test_ignores_unrelated_anakot_service(self, tmp_path, monkeypatch):
        """Third-party ``anakot.service`` that isn't ours stays untouched.

        If a user has some other package named ``anakot`` installed as a
        service, we must not flag it.
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "anakot.service").write_text(
            "[Unit]\nDescription=Some Other Anakot\n[Service]\n"
            "ExecStart=/opt/other-anakot/bin/daemon --foreground\n",
            encoding="utf-8",
        )
    ...
    def test_returns_empty_when_no_legacy_files_exist(self, tmp_path, monkeypatch):
        ...
        assert gateway_cli._find_legacy_anakot_units() == []
        assert gateway_cli.has_legacy_anakot_units() is False
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        for i, execstart in enumerate(variants):
            name = f"anakot.service" if i == 0 else f"anakot.service"  # same name
    ...
    def test_handles_unreadable_unit_file_gracefully(self, tmp_path, monkeypatch):
        ...
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        unreadable = user_dir / "anakot.service"
...
class TestRemoveLegacyAnakotUnits:
    ...
    def test_dry_run_lists_without_removing(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_anakot_units(
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo anakot gateway migrate-legacy" in out
    ...
    def test_system_scope_with_root_removes(self, tmp_path, monkeypatch, capsys):
        ...
        assert any(
            c.startswith("systemctl stop anakot.service") for c in cmds_joined
        )
        assert any(
            c.startswith("systemctl disable anakot.service") for c in cmds_joined
        )
    ...
    def test_removes_both_scopes_with_root(self, tmp_path, monkeypatch, capsys):
        ...
        user_legacy = user_dir / "anakot.service"
        system_legacy = system_dir / "anakot.service"
    ...
    def test_interactive_prompt_no_skips_removal(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_anakot_units(interactive=True)
...
class TestSystemScopeRemediationOutput:
    ...
    def test_restart_remediation_uses_systemctl_restart(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl restart anakot-gateway" in out
    ...
    def test_stop_remediation_uses_systemctl_stop(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl stop anakot-gateway" in out
...
class TestServiceWorkingDirIsStable:
    ...
    def test_stable_working_dir_uses_anakot_home(self, tmp_path, monkeypatch):
    ...
    def test_stable_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        # ANAKOT_HOME points somewhere that does not exist -> fall back.
        missing = tmp_path / "does-not-exist" / ".anakot"
        monkeypatch.setattr(gateway_cli, "get_anakot_home", lambda: missing)
