...
def test_connect_honors_kanban_busy_timeout_env(kanban_home, monkeypatch):
    ...
    """
    monkeypatch.setenv("ANAKOT_KANBAN_BUSY_TIMEOUT_MS", "123456")
...
def test_claim_uses_env_default_ttl(kanban_home, monkeypatch):
    monkeypatch.setenv("ANAKOT_KANBAN_CLAIM_TTL_SECONDS", "3600")
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x", assignee="a")
...
def test_detect_crashed_workers_skips_freshly_claimed_tasks(
    ...
    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.delenv("ANAKOT_KANBAN_CRASH_GRACE_SECONDS", raising=False)
...
def test_detect_crashed_workers_grace_period_env_override(
    ...
    """ANAKOT_KANBAN_CRASH_GRACE_SECONDS env var adjusts the window."""
    import anakot_cli.kanban_db as _kb

    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("ANAKOT_KANBAN_CRASH_GRACE_SECONDS", "5")
...
def test_resolve_crash_grace_seconds_handles_bad_env(monkeypatch):
    ...
    for bad_val in ("notanumber", "-5", ""):
        monkeypatch.setenv("ANAKOT_KANBAN_CRASH_GRACE_SECONDS", bad_val)
...
def test_resolve_rate_limit_cooldown_handles_bad_env(monkeypatch):
    ...
    for bad_val in ("notanumber", "-5", ""):
        monkeypatch.setenv(
            "ANAKOT_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", bad_val
        )
        assert (
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Anakot profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="anakot-task", assignee="daily")
...
def test_scratch_workspace_created_under_anakot_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
def test_cleanup_workspace_removes_managed_scratch_dir(kanban_home):
    ...
    with kb.connect() as conn:
        ...
        assert ws.is_dir()
        kb.complete_task(conn, t, result="ok")
    assert not ws.exists(), "Anakot-managed scratch dir should be cleaned up"
...
def test_cleanup_workspace_honors_workspaces_root_env_override(tmp_path, monkeypatch):
    """``ANAKOT_KANBAN_WORKSPACES_ROOT`` extends the managed-scratch set.
    ...
    monkeypatch.setenv("ANAKOT_KANBAN_WORKSPACES_ROOT", str(workspaces_override))
    kb.init_db()
...
class TestSharedBoardPaths:
    ...
    def test_default_install_anchors_at_home_dot_anakot(
        self, tmp_path, monkeypatch
    ):
        # Standard install: ANAKOT_HOME == ~/.anakot, no profile active.
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # ANAKOT_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `anakot -p coder`).
    ...
    def test_docker_custom_anakot_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: ANAKOT_HOME points outside ~/.anakot.
        # `get_default_anakot_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".anakot"`.
        custom_root = tmp_path / "opt" / "anakot"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: ANAKOT_HOME=/opt/anakot/profiles/coder;
        # `get_default_anakot_root()` walks up to /opt/anakot because
    ...
    def test_explicit_override_via_anakot_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Explicit override: ANAKOT_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".anakot"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ANAKOT_HOME", str(profile_home))
        monkeypatch.setenv("ANAKOT_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ANAKOT_KANBAN_HOME", "   ")
    ...
    def test_anakot_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # ANAKOT_KANBAN_DB pins the file path directly and beats both
        # ANAKOT_KANBAN_HOME and the `get_default_anakot_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ANAKOT_HOME", str(default_home))
        monkeypatch.setenv("ANAKOT_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("ANAKOT_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows ANAKOT_KANBAN_HOME -- the pins
        # are independent.
        assert kb.workspaces_root() == umbrella / "kanban" / "workspaces"

    def test_anakot_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # ANAKOT_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".anakot"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("ANAKOT_HOME", str(default_home))
        monkeypatch.setenv("ANAKOT_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("ANAKOT_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows ANAKOT_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ANAKOT_HOME", str(default_home))
        monkeypatch.setenv("ANAKOT_KANBAN_DB", "   ")
        monkeypatch.setenv("ANAKOT_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject ANAKOT_KANBAN_DB
        # and ANAKOT_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites ANAKOT_HOME.
        default_home = tmp_path / ".anakot"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see anakot_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_resolve_anakot_argv_prefers_path_shim(monkeypatch):
    """When `anakot` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import anakot_cli.kanban_db as kb

    monkeypatch.delenv("ANAKOT_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/anakot")
    argv = kb._resolve_anakot_argv()
    assert argv == ["/usr/local/bin/anakot"]


def test_resolve_anakot_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    """A relative executable override must not remain workspace-cwd-dependent."""
    import anakot_cli.kanban_db as kb

    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("ANAKOT_BIN", ".\\anakot.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_anakot_argv() == [os.path.abspath(".\\anakot.exe")]


def test_resolve_anakot_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "anakot.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("ANAKOT_BIN", raising=False)
    monkeypatch.setenv("PATH", str(bin_dir))
    monkeypatch.setenv("PATHEXT", ".CMD")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_anakot_argv() == [sys.executable, "-m", "anakot_cli.main"]


def test_resolve_anakot_argv_honors_anakot_bin_path_override(monkeypatch, tmp_path):
    """An explicit path-like ANAKOT_BIN lets service managers pin the executable."""
    import shutil
    import anakot_cli.kanban_db as kb

    shim = tmp_path / "bin" / "anakot"
    shim.parent.mkdir()
    shim.write_text("#!/bin/sh\n", encoding="utf-8")
    monkeypatch.setenv("ANAKOT_BIN", str(shim))
    monkeypatch.setattr(shutil, "which", lambda name: None)

    assert kb._resolve_anakot_argv() == [str(shim)]


def test_resolve_anakot_argv_anakot_bin_bare_name_uses_path(monkeypatch, tmp_path):
    """Bare ANAKOT_BIN values keep PATH semantics instead of cwd shadowing."""
    import stat
    import anakot_cli.kanban_db as kb

    cwd_anakot = tmp_path / "anakot"
    cwd_anakot.write_text("wrong\n", encoding="utf-8")
    cwd_anakot.chmod(cwd_anakot.stat().st_mode | stat.S_IXUSR)
    path_anakot = tmp_path / "bin" / "anakot"
    path_anakot.parent.mkdir()
    path_anakot.write_text("right\n", encoding="utf-8")
    path_anakot.chmod(path_anakot.stat().st_mode | stat.S_IXUSR)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("PATH", str(path_anakot.parent))
    monkeypatch.setenv("ANAKOT_BIN", "anakot")

    assert kb._resolve_anakot_argv() == [str(path_anakot)]


def test_resolve_anakot_argv_anakot_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    """Bare ANAKOT_BIN does not accept current-directory shadow executables."""
    import sys
    import anakot_cli.kanban_db as kb

    (tmp_path / "anakot.exe").write_text("wrong\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
...
def test_resolve_anakot_argv_anakot_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
    """A PATH-resolved ANAKOT_BIN batch shim is not used as worker argv[0]."""
    import sys
    import anakot_cli.kanban_db as kb
...
def test_resolve_anakot_argv_anakot_bin_unresolved_bare_name_falls_back(monkeypatch):
    """Unresolved ANAKOT_BIN command names do not delegate cwd search to Popen."""
    import sys
    import anakot_cli.kanban_db as kb
...
def test_write_txn_healthy_commit_no_exception(tmp_path):
    ...
    from anakot_cli.kanban_db import connect, write_txn
    db = tmp_path / "test.db"
...
def test_write_txn_raises_on_truncated_file(tmp_path):
    ...
    with pytest.raises(sqlite3.DatabaseError, match="torn-extend|page count mismatch"):
        with unittest.mock.patch("anakot_cli.kanban_db.os.path.getsize", side_effect=fake_getsize):
            with write_txn(conn) as c:
                c.execute(
                    "INSERT INTO tasks (id, title, assignee, status, priority, created_at) "
...
def test_write_txn_post_commit_check_fires_every_call(tmp_path):
    ...
    from anakot_cli.kanban_db import connect, write_txn
    import anakot_cli.kanban_db as kanban_db_module
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
    call_count = 0
...
def test_connect_sets_wal_autocheckpoint_100(tmp_path):
    ...
    from anakot_cli.kanban_db import connect
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    import struct
    from anakot_cli.kanban_db import connect, _check_file_length_invariant
...
def test_reap_worker_zombies_returns_count():
    ...
    with patch("anakot_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("anakot_cli.kanban_db._record_worker_exit"):
...
def test_reap_worker_zombies_noop_on_windows(monkeypatch):
    ...
    monkeypatch.setattr("anakot_cli.kanban_db.os.name", "nt")
    with patch("anakot_cli.kanban_db.os.waitpid") as mock_waitpid:
...
def test_reap_worker_zombies_noop_no_children():
    ...
    with patch("anakot_cli.kanban_db.os.waitpid", side_effect=ChildProcessError):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_reap_worker_zombies_records_exit_status():
    ...
    with patch("anakot_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch(
            "anakot_cli.kanban_db._record_worker_exit",
...
def test_reap_worker_zombies_handles_waitpid_os_error():
    ...
    with patch("anakot_cli.kanban_db.os.waitpid", side_effect=OSError("test error")):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_zombie_reaper_survives_all_boards_failing():
    ...
    for tick in range(5):
        ...
        with patch(
            "anakot_cli.kanban_db.os.waitpid", side_effect=make_fake_waitpid(pids)
        ):
            with patch("anakot_cli.kanban_db._record_worker_exit"):
                pids = kb.reap_worker_zombies()
...
def test_dispatch_once_still_reaps_via_extracted_fn(kanban_home):
    ...
    with patch("anakot_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("anakot_cli.kanban_db._record_worker_exit"):
            with patch("anakot_cli.kanban_db.os.name", "posix"):
                pids = kb.reap_worker_zombies()
