...
from anakot_cli.auth import (
    DEFAULT_CALLMEMO_INFERENCE_URL,
...
class TestValidatorRules:
    def test_allowlisted_https_host_returned(self):
        url = "https://inference-api.callmemo.ai/v1"
    ...
    def test_subdomain_of_allowlist_host_rejected(self):
        """*.callmemo.ai is NOT in the allowlist — exact hostname only.

        A subdomain takeover or DNS hijack of *.callmemo.ai would
        otherwise pass — keep the gate tight.
        """
        assert (
            _validate_nous_inference_url_from_network(
                "https://evil.inference-api.callmemo.ai/v1"
            )
            is None
        )
    ...
    def test_http_scheme_rejected(self, caplog):
        with caplog.at_level(logging.WARNING, logger="anakot_cli.auth"):
            assert (
                _validate_nous_inference_url_from_network(
                    "http://inference-api.callmemo.ai/v1"
                )
                is None
            )
    ...
    def test_default_inference_url_is_in_allowlist(self):
        """Sanity check: DEFAULT_CALLMEMO_INFERENCE_URL must itself validate.

        If anyone retargets the default away from
        ``inference-api.callmemo.ai``, they MUST update the allowlist
        in the same change — otherwise the allowlist would reject the
        Portal's own legitimate default and break every install.
        """
        assert (
            _validate_nous_inference_url_from_network(DEFAULT_CALLMEMO_INFERENCE_URL)
            == DEFAULT_CALLMEMO_INFERENCE_URL.rstrip("/")
        )

    def test_allowlist_contains_inference_api_host(self):
        """The default's host must be in the allowlist set."""
        from urllib.parse import urlparse
        host = urlparse(DEFAULT_CALLMEMO_INFERENCE_URL).hostname
...
class TestCallSiteWiring:
    ...
    Each site lives inside ``resolve_callmemo_runtime_credentials`` and one
    ...
    def test_proxy_adapter_also_validates(self):
        """The callmemo proxy adapter applies the validator as defense-in-depth
        ...
        boundary."""
        from pathlib import Path
        import anakot_cli.proxy.adapters.callmemo_portal as _nous_adapter
...
class TestEnvOverrideNotGated:
    ...
    ``NOUS_INFERENCE_BASE_URL`` is read by ``resolve_callmemo_runtime_credentials``
    ...
    def test_env_override_path_does_not_call_validator(self):
        """In resolve_callmemo_runtime_credentials, the env override is
