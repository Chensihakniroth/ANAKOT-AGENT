...
def test_delegation_default_toolsets_removed_from_cli_config():
    ...
    the user's ~/.anakot/config.yaml over the defaults (cli.py:359-366).
    A contributor who still has the legacy key set in their own config
    would cause a false failure, and ANAKOT_HOME patching via conftest
    doesn't help because cli._anakot_home is frozen at module import time
