...
# ---------------------------------------------------------------------------
# callmemo shared-credential store write (inside _write_shared_nous_state)
# ---------------------------------------------------------------------------


def test_shared_nous_store_writes_0o600_with_0o700_parent(tmp_path, monkeypatch):
    """The callmemo shared-credential store must land at 0o600 / parent 0o700."""
    monkeypatch.setenv("ANAKOT_HOME", str(tmp_path))
    ...
    assert path.exists(), "shared callmemo store was not written"
    mode = stat.S_IMODE(path.stat().st_mode)
    parent_mode = stat.S_IMODE(path.parent.stat().st_mode)

    assert mode == 0o600, (
        f"callmemo shared store mode 0o{mode:o} != 0o600 — TOCTOU race regressed"
    )
    assert parent_mode == 0o700, (
        f"callmemo shared store parent dir mode 0o{parent_mode:o} != 0o700"
    )
