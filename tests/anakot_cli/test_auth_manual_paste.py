...
Regression coverage for [#26923](https://github.com/callmemo/anakot-agent/issues/26923):
...
def test_xai_loopback_login_manual_paste_skips_http_server(monkeypatch):
    ...
    def _fake_prompt(_redirect_uri):
        # Anakot generates state internally; we won't know it ahead of
        # time, so capture the state Anakot baked into the authorize
