...
def test_dry_run_makes_no_filesystem_changes(tmp_path: Path) -> None:
    ...
    actions = reconcile_profile_gateways(
        anakot_home=tmp_path, scandir=scandir, dry_run=True,
    )
...
def test_missing_profiles_root_still_registers_default_slot(
    ...
    """When $ANAKOT_HOME/profiles doesn't exist (fresh install), the
    ...
    so `anakot gateway start` (no -p) has somewhere to land."""
    scandir = tmp_path / "run-service"; scandir.mkdir()
    actions = reconcile_profile_gateways(
        anakot_home=tmp_path, scandir=scandir, dry_run=False,
    )
...
def test_default_slot_always_registered_on_empty_home(tmp_path: Path) -> None:
    """Bare ANAKOT_HOME with nothing under it still produces a
...
def test_default_slot_run_script_omits_profile_flag(tmp_path: Path) -> None:
    ...
    that would resolve to $ANAKOT_HOME/profiles/default/ instead of
    the root profile. It must call `anakot gateway run` directly."""
    scandir = tmp_path / "run-service"; scandir.mkdir()
...
@pytest.mark.parametrize(
    ...
    [
        ("gateway", "run"),
        ("/init", "/opt/anakot/docker/main-wrapper.sh", "gateway", "run"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("/init", "/opt/anakot/docker/main-wrapper.sh", "gateway", "run", "--no-supervise"),
    ],
)
...
def test_legacy_gateway_run_env_no_supervise_does_not_seed_s6_state(
    ...
    scandir = tmp_path / "run-service"; scandir.mkdir()
    monkeypatch.setenv("ANAKOT_GATEWAY_NO_SUPERVISE", "1")
...
def test_default_slot_cleans_up_stale_runtime_files_at_root(
    ...
    """gateway.pid and processes.json at the ANAKOT_HOME root (left
