...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.run_anakot_oauth_login_pure",
        lambda: {
            "access_token": token,
    ...
    entry = next(item for item in entries if item["source"] == "manual:anakot_pkce")
    assert entry["label"] == "claude@example.com"
    assert entry["source"] == "manual:anakot_pkce"
...
def test_auth_add_google_gemini_cli_sets_active_provider(tmp_path, monkeypatch):
    """anakot auth add google-gemini-cli must set active_provider in auth.json.
    ...
    # pool entry from pool.add_entry() still present for anakot auth list
...
def test_auth_add_qwen_oauth_sets_active_provider(tmp_path, monkeypatch):
    """anakot auth add qwen-oauth must set active_provider in auth.json.
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    # 401-recovery path (resolve_callmemo_runtime_credentials) can refresh an
    # invoke JWT when the token expires. If this mirror is missing, recovery
    # raises "Anakot is not logged into callmemo Portal" and the agent dies.
...
def test_auth_add_minimax_oauth_starts_login_and_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "anakot_cli.auth._minimax_oauth_login",
        lambda **kwargs: {
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "anakot_cli.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """anakot auth add xai-oauth must write providers singleton and set active_provider.
    ...
    monkeypatch.setattr(
        "anakot_cli.auth._xai_oauth_loopback_login",
        lambda **kwargs: {
            "tokens": {
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from anakot_cli.auth_commands import auth_reset_command
...
def test_logout_defaults_to_configured_codex_when_no_active_provider(tmp_path, monkeypatch, capsys):
    """Bare `anakot logout` should target configured Codex if auth has no active provider."""
    anakot_home = tmp_path / "anakot"
    monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}, "credential_pool": {}})
    (anakot_home / "config.yaml").write_text(
...
def test_logout_clears_stale_active_codex_without_provider_credentials(tmp_path, monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "Logged out of OpenAI Codex." in out
    auth_payload = json.loads((anakot_home / "auth.json").read_text())
...
def test_reset_config_provider_uses_atomic_yaml_write(tmp_path, monkeypatch):
    ...
    from anakot_cli.auth import _reset_config_provider
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        "anakot_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("anakot_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("anakot_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("anakot_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "anakot_cli.auth_commands.time.time",
...
def test_auth_remove_claude_code_suppresses_reseed(tmp_path, monkeypatch):
    ...
    updated = json.loads((anakot_home / "auth.json").read_text())
...
def test_unsuppress_credential_source_returns_false_when_absent(tmp_path, monkeypatch):
    ...
    from anakot_cli.auth import unsuppress_credential_source
...
def test_auth_add_codex_clears_suppression_marker(tmp_path, monkeypatch):
    """Re-linking codex via `anakot auth add openai-codex` must clear any suppression marker."""
    monkeypatch.setenv("ANAKOT_HOME", str(tmp_path / "anakot"))
    anakot_home = tmp_path / "anakot"
    anakot_home.mkdir(parents=True, exist_ok=True)

    # Pre-existing suppression (simulating a prior `anakot auth remove`)
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("anakot_cli.auth._import_codex_cli_tokens", _fake_import)
    ...
    after = json.loads((anakot_home / "auth.json").read_text())
...
def test_auth_remove_env_seeded_suppresses_shell_exported_var(tmp_path, monkeypatch, capsys):
    """`anakot auth remove xai 1` must stick even when the env var is exported
    by the shell (not written into ~/.anakot/.env).  Before PR for #13371 the
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    """When the env var lives only in ~/.anakot/.env (not the shell), the
    ...
    (anakot_home / ".env").write_text("DEEPSEEK_API_KEY=sk-ds-only\n")
    ...
    assert (anakot_home / ".env").read_text().strip() == ""


def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    """Re-adding a credential via `anakot auth add <provider>` clears any
...
def test_seed_from_env_respects_env_suppression(tmp_path, monkeypatch):
    ...
    via `anakot auth remove`.  This is the gate that prevents shell-exported
...
def test_seed_from_singletons_respects_copilot_suppression(tmp_path, monkeypatch):
    ...
    import anakot_cli.copilot_auth as ca
...
def test_auth_add_clears_all_suppressions_including_non_env(tmp_path, monkeypatch):
    """Re-adding a credential via `anakot auth add <provider>` clears ALL
...
def test_auth_remove_codex_manual_device_code_suppresses_canonical(tmp_path, monkeypatch):
    """Removing a manual:device_code entry (from `anakot auth add openai-codex`)
