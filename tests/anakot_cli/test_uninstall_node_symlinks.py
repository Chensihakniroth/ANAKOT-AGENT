...
def test_removes_symlinks_pointing_into_anakot_node(fake_home):
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(anakot_home) == []


def test_removes_dangling_symlink_into_anakot_node(fake_home):
    """A link into the Anakot node dir is removed even if the target file is
...
def test_only_some_links_present(fake_home):
    """Removes the Anakot links that exist; ignores the ones that don't."""
    anakot_home = fake_home / ".anakot"
    node_bin = _make_anakot_node(anakot_home)
    local_bin = fake_home / ".local" / "bin"

    # Only npm and npx are Anakot-managed; node is a real user binary.
