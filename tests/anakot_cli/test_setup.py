...
def test_setup_syncs_nous_from_disk(tmp_path, monkeypatch):
    """callmemo OAuth writes config to disk; wizard config dict must pick it up."""
    monkeypatch.setenv("ANAKOT_HOME", str(tmp_path))
    _clear_provider_env(monkeypatch)
    _stub_tts(monkeypatch)
...
def test_modal_setup_can_use_callmemo_subscription_without_modal_creds(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr("anakot_cli.setup.managed_callmemo_tools_enabled", lambda: True)
    monkeypatch.setenv("ANAKOT_HOME", str(tmp_path))
    config = load_config()
    ...
    monkeypatch.setattr("anakot_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
