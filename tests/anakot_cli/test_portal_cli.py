"""Tests for `anakot portal` dispatch.

`anakot portal` (no subcommand) is the human-readable alias for the callmemo Portal
one-shot onboarding (`anakot auth add nous --type oauth` / `anakot setup
--portal`). The prior status default moved to `anakot portal info`, with
...
from anakot_cli import portal_cli
...
def test_bare_portal_and_login_run_one_shot(monkeypatch, sub):
    """`anakot portal`, `anakot portal login` -> one-shot onboarding."""
    ...
    monkeypatch.setattr(
        "anakot_cli.setup._run_portal_one_shot", fake_one_shot
    )
...
def test_info_and_status_alias_run_status(monkeypatch, sub):
    """`anakot portal info` and the `status` back-compat alias -> status."""
    calls = {"login": 0, "status": 0}

    monkeypatch.setattr(
        "anakot_cli.setup._run_portal_one_shot",
...
def test_login_cancelled_returns_one(monkeypatch):
    ...
    monkeypatch.setattr("anakot_cli.setup._run_portal_one_shot", boom)
    monkeypatch.setattr(portal_cli, "load_config", lambda: {})
...
def test_one_shot_delegates_to_model_flow_callmemo(monkeypatch):
    """`anakot portal` must run the quick-setup callmemo flow (login + MODEL PICK +
    provider + Tool Gateway), i.e. delegate to `_model_flow_callmemo` — not the
    ...
    # _model_flow_callmemo lives in anakot_cli.main and is imported lazily inside
    # _run_portal_one_shot, so patch it at the source module.
    monkeypatch.setattr("anakot_cli.main._model_flow_callmemo", fake_model_flow)
    ...
    assert calls["model_flow"] == 1, (
        "`anakot portal` must route through _model_flow_callmemo so the model "
...
def test_one_shot_swallows_cancel_and_systemexit(monkeypatch, exc):
    """A cancel/abort from the delegated callmemo flow must NOT escape and kill the
    CLI. `_login_callmemo` raises SystemExit(130)/(1) on cancel/failure, and the
    expired-session re-login path inside `_model_flow_callmemo` only catches
    ...
    monkeypatch.setattr("anakot_cli.main._model_flow_callmemo", boom)
    monkeypatch.setattr("anakot_cli.config.load_config", lambda: {})
