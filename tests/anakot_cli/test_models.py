...
from anakot_cli.models import (
    ...
    is_nous_free_tier, partition_callmemo_models_by_tier,
    check_callmemo_free_tier, _FREE_TIER_CACHE_TTL,
...
class TestFetchOpenRouterModels:
    ...
    def test_falls_back_to_static_snapshot_on_fetch_failure(self, monkeypatch):
        monkeypatch.setattr(_models_mod, "_openrouter_catalog_cache", None)
        with patch("anakot_cli.models.urllib.request.urlopen", side_effect=OSError("boom")):
            models = fetch_openrouter_models(force_refresh=True)
    ...
    def test_permissive_when_supported_parameters_missing(self, monkeypatch):
        ...
        Some OpenRouter-compatible gateways (callmemo Portal, private mirrors, older
...
class TestPartitionNousModelsByTier:
    """Tests for partition_callmemo_models_by_tier — free vs paid tier model split."""
    ...
    def test_paid_tier_all_selectable(self):
        ...
        sel, unav = partition_callmemo_models_by_tier(models, pricing, free_tier=False)
        assert sel == models
        assert unav == []
    ...
    def test_free_tier_splits_correctly(self):
        ...
        sel, unav = partition_callmemo_models_by_tier(models, pricing, free_tier=True)
    ...
    def test_no_pricing_returns_all(self):
        ...
        sel, unav = partition_callmemo_models_by_tier(models, {}, free_tier=True)
        assert sel == models
        assert unav == []
...
class TestUnionWithPortalFreeRecommendations:
    ...
    def test_adds_portal_free_model_missing_from_curated(self):
        ...
        with patch(
            "anakot_cli.models.fetch_callmemo_recommended_models",
    ...
    def test_then_partition_keeps_portal_free_model(self):
        ...
        with patch(
            ...
            ids, p = union_with_portal_free_recommendations(curated, pricing, "")
        sel, unav = partition_callmemo_models_by_tier(ids, p, free_tier=True)
    ...
    def test_empty_payload_returns_inputs_unchanged(self):
        ...
        with patch("anakot_cli.models.fetch_callmemo_recommended_models", return_value={}):
            ids, p = union_with_portal_free_recommendations(curated, pricing, "")
        assert ids == curated
        assert p == pricing
...
class TestUnionWithPortalPaidRecommendations:
    ...
    def test_does_not_synthesize_pricing_for_paid_models(self):
        ...
        :func:`partition_callmemo_models_by_tier` into treating them as free;
...
class TestCheckNousFreeTierCache:
    """Tests for the TTL cache on check_callmemo_free_tier()."""
    ...
    @patch("anakot_cli.callmemo_account.get_callmemo_portal_account_info")
    def test_result_is_cached(self, mock_account):
        ...
        result1 = check_callmemo_free_tier()
        result2 = check_callmemo_free_tier()
    ...
    def test_force_fresh_bypasses_cache(self, mock_account):
        ...
        assert check_callmemo_free_tier() is False
        assert check_callmemo_free_tier(force_fresh=True) is False
...
class TestNousRecommendedModels:
    """Tests for fetch_callmemo_recommended_models + get_callmemo_recommended_aux_model."""
    ...
    def test_fetch_caches_per_portal_url(self):
        from anakot_cli.models import fetch_callmemo_recommended_models
        mock_cm = self._mock_urlopen(self._SAMPLE_PAYLOAD)
        with patch("urllib.request.urlopen", return_value=mock_cm) as mock_urlopen:
            a = fetch_callmemo_recommended_models("https://portal.example.com")
            b = fetch_callmemo_recommended_models("https://portal.example.com")
    ...
    def test_fetch_cache_is_keyed_per_portal(self):
        ...
        mock_cm = self._mock_urlopen(self._SAMPLE_PAYLOAD)
        with patch("urllib.request.urlopen", return_value=mock_cm) as mock_urlopen:
            fetch_callmemo_recommended_models("https://portal.example.com")
            fetch_callmemo_recommended_models("https://portal.staging-callmemo.ai")
    ...
    def test_fetch_returns_empty_on_network_failure(self):
        ...
        with patch("urllib.request.urlopen", side_effect=OSError("boom")):
            result = fetch_callmemo_recommended_models("https://portal.example.com")
        assert result == {}
    ...
    def test_fetch_force_refresh_bypasses_cache(self):
        ...
        with patch("urllib.request.urlopen", return_value=mock_cm) as mock_urlopen:
            fetch_callmemo_recommended_models("https://portal.example.com")
            fetch_callmemo_recommended_models("https://portal.example.com", force_refresh=True)
    ...
    def test_get_aux_model_returns_vision_recommendation(self):
        ...
        with patch(
            ...
            model = get_callmemo_recommended_aux_model(vision=True, free_tier=True)
    ...
    def test_get_aux_model_returns_compaction_recommendation(self):
        ...
        with patch(
            ...
            model = get_callmemo_recommended_aux_model(vision=False, free_tier=True)
    ...
    def test_get_aux_model_returns_none_on_empty_payload(self):
        from anakot_cli.models import get_callmemo_recommended_aux_model
        with patch("anakot_cli.models.fetch_callmemo_recommended_models", return_value={}):
            assert get_callmemo_recommended_aux_model(vision=False, free_tier=True) is None
            assert get_callmemo_recommended_aux_model(vision=True, free_tier=False) is None
    ...
    def test_paid_tier_prefers_paid_recommendation(self):
        ...
        with patch("anakot_cli.models.fetch_callmemo_recommended_models", return_value=payload):
            text = get_callmemo_recommended_aux_model(vision=False, free_tier=False)
            vision = get_callmemo_recommended_aux_model(vision=True, free_tier=False)
    ...
    def test_auto_detects_tier_when_not_supplied(self):
        """Default behaviour: call check_callmemo_free_tier() to pick the tier."""
        from anakot_cli.models import get_callmemo_recommended_aux_model
        payload = {
        ...
        with (
            patch("anakot_cli.models.fetch_callmemo_recommended_models", return_value=payload),
            patch("anakot_cli.models.check_callmemo_free_tier", return_value=True),
        ):
            assert get_callmemo_recommended_aux_model(vision=False) == "free-model"
        with (
            patch("anakot_cli.models.fetch_callmemo_recommended_models", return_value=payload),
            patch("anakot_cli.models.check_callmemo_free_tier", return_value=False),
        ):
            assert get_callmemo_recommended_aux_model(vision=False) == "paid-model"
    ...
    def test_tier_detection_error_defaults_to_paid(self):
        ...
        with (
            patch("anakot_cli.models.fetch_callmemo_recommended_models", return_value=payload),
            patch("anakot_cli.models.check_callmemo_free_tier", side_effect=RuntimeError("boom")),
        ):
            assert get_callmemo_recommended_aux_model(vision=False) == "paid-model"
