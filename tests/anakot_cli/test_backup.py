...
def _make_anakot_tree(root: Path) -> None:
    ...
    (root / "anakot_state.db").write_bytes(b"fake-state")
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        # get_default_anakot_root needs this
    ...
    def test_excludes_anakot_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include anakot-agent/ directory."""
        anakot_home = tmp_path / ".anakot"
        anakot_home.mkdir()
        _make_anakot_tree(anakot_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "anakot-agent" in n]
            assert agent_files == [], f"anakot-agent files leaked into backup: {agent_files}"
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/anakot-backup-*.zip."""
        anakot_home = tmp_path / ".anakot"
        anakot_home.mkdir()
        ...
        zips = list(tmp_path.glob("anakot-backup-*.zip"))
        assert len(zips) == 1
...
class TestValidateBackupZip:
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only anakot_state.db (old wrong name) is rejected."""
        from anakot_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["anakot_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into anakot home."""
        anakot_home = tmp_path / ".anakot"
        anakot_home.mkdir()
        monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (anakot_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (anakot_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (anakot_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (anakot_home / "profiles" / "coder" / "config.yaml").exists()

    def test_strips_anakot_prefix(self, tmp_path, monkeypatch):
        """Import strips .anakot/ prefix if all entries share it."""
        anakot_home = tmp_path / ".anakot"
        anakot_home.mkdir()
        monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".anakot/config.yaml": "model: test\n",
            ".anakot/skills/a/SKILL.md": "# A\n",
        })
        ...
        assert (anakot_home / "config.yaml").read_text() == "model: test\n"
        assert (anakot_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_anakot_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a anakot backup."""
        anakot_home = tmp_path / ".anakot"
        anakot_home.mkdir()
        monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        # traversal file should NOT exist outside anakot home
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (anakot_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (anakot_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (anakot_home / "config.yaml").read_text() == "model: restored\n"
...
class TestFormatSize:
    def test_bytes(self):
        from anakot_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without anakot markers fails validation."""
        import io
        from anakot_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_anakot(self):
        """Detects .anakot/ prefix wrapping all entries."""
        import io
        from anakot_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".anakot/config.yaml", "test")
            zf.writestr(".anakot/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".anakot/"
...
class TestBackupEdgeCases:
    def test_nonexistent_anakot_home(self, tmp_path, monkeypatch):
        """Backup exits when anakot home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".anakot"
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("anakot-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_permission_error_during_backup(self, tmp_path, monkeypatch):
        ...
        bad_file = anakot_home / "secret.db"
    ...
    def test_skips_output_zip_inside_anakot(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside anakot root."""
        anakot_home = tmp_path / ".anakot"
        anakot_home.mkdir()
        (anakot_home / "config.yaml").write_text("model: test\n")
        ...
        # Output inside anakot home
        out_zip = anakot_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        anakot_home = tmp_path / ".anakot"
        anakot_home.mkdir()
        (anakot_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (anakot_home / "profiles" / "coder" / "config.yaml").exists()
        assert (anakot_home / "profiles" / "researcher" / "config.yaml").exists()
        ...
        assert "anakot -p coder" in coder_wrapper
...
class TestQuickSnapshot:
    ...
    def test_creates_snapshot(self, anakot_home):
    ...
    def test_label_in_id(self, anakot_home):
        from anakot_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", anakot_home=anakot_home)
    ...
    def test_copies_nested_files(self, anakot_home):
        from anakot_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(anakot_home=anakot_home)
        assert (anakot_home / "state-snapshots" / snap_id / "cron" / "jobs.json").exists()

    def test_missing_files_skipped(self, anakot_home):
        from anakot_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(anakot_home=anakot_home)
        with open(anakot_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(anakot_home=empty) is None

    def test_list_snapshots(self, anakot_home):
        from anakot_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", anakot_home=anakot_home)
        id2 = create_quick_snapshot(label="second", anakot_home=anakot_home)

        snaps = list_quick_snapshots(anakot_home=anakot_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, anakot_home):
        from anakot_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", anakot_home=anakot_home)
        snaps = list_quick_snapshots(limit=3, anakot_home=anakot_home)
        assert len(snaps) == 3

    def test_restore_config(self, anakot_home):
        from anakot_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(anakot_home=anakot_home)

        (anakot_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (anakot_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, anakot_home=anakot_home)
        assert result is True
        assert "openrouter" in (anakot_home / "config.yaml").read_text()

    def test_restore_state_db(self, anakot_home):
        from anakot_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(anakot_home=anakot_home)

        conn = sqlite3.connect(str(anakot_home / "state.db"))
    ...
    def test_restore_nonexistent(self, anakot_home):
        from anakot_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", anakot_home=anakot_home) is False

    def test_auto_prune(self, anakot_home):
        from anakot_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", anakot_home=anakot_home)
        snaps = list_quick_snapshots(limit=100, anakot_home=anakot_home)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, anakot_home):
        from anakot_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", anakot_home=anakot_home)
        deleted = prune_quick_snapshots(keep=3, anakot_home=anakot_home)
        assert deleted == 7
        assert len(list_quick_snapshots(anakot_home=anakot_home)) == 3
    ...
    def test_empty_pairing_dir_does_not_fail(self, anakot_home):
...
class TestPreUpdateBackup:
    ...
    def test_does_not_recurse_into_prior_backups(self, anakot_home):
        """The ``backups/`` directory must be excluded so that each backup
        doesn't grow exponentially by including all prior backups."""
        from anakot_cli.backup import create_pre_update_backup
        # First backup
        out1 = create_pre_update_backup(anakot_home=anakot_home)
        assert out1 is not None
        # Second backup — must not include the first
        out2 = create_pre_update_backup(anakot_home=anakot_home)
    ...
    def test_rotation_preserves_manual_files(self, anakot_home):
        """Hand-dropped zips in ``backups/`` must not be touched by
        rotation — it only prunes files matching ``pre-update-*.zip``."""
        import time as _t
        from anakot_cli.backup import create_pre_update_backup

        (anakot_home / "backups").mkdir(exist_ok=True)
        manual = anakot_home / "backups" / "my-manual.zip"
        manual.write_bytes(b"manual backup")

        for _ in range(5):
            create_pre_update_backup(anakot_home=anakot_home, keep=2)
            _t.sleep(1.05)
    ...
    def test_returns_none_if_root_missing(self, tmp_path):
        from anakot_cli.backup import create_pre_update_backup
        assert create_pre_update_backup(anakot_home=tmp_path / "does-not-exist") is None

    def test_keep_zero_does_not_delete_freshly_created_backup(self, anakot_home):
        ...
        """
        from anakot_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(anakot_home=anakot_home, keep=0)
        assert out is not None
        assert out.exists(), (
    ...
    def test_keep_negative_does_not_delete_freshly_created_backup(self, anakot_home):
    ...
    def test_keep_zero_still_prunes_older_backups(self, anakot_home):
        ...
        first = create_pre_update_backup(anakot_home=anakot_home, keep=5)
        _t.sleep(1.05)
        second = create_pre_update_backup(anakot_home=anakot_home, keep=5)
        _t.sleep(1.05)
        third = create_pre_update_backup(anakot_home=anakot_home, keep=0)
...
class TestRunPreUpdateBackup:
    ...
    def anakot_home(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for anakot_cli.config + anakot_constants so they pick up ANAKOT_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("anakot_cli.config") or mod == "anakot_constants":
                del __import__("sys").modules[mod]
        return root

    def test_backup_flag_creates_backup(self, anakot_home, capsys):
        """--backup forces the pre-update backup for one run even when config is off."""
        from anakot_cli.main import _run_pre_update_backup
        _run_pre_update_backup(Namespace(no_backup=False, backup=True))
        out = capsys.readouterr().out
        assert "Creating pre-update backup" in out
        assert "Saved:" in out
        assert "Restore:" in out
        assert "anakot import" in out
        assert "Disable:" in out
        # Actual backup was created
        backups = list((anakot_home / "backups").glob("pre-update-*.zip"))
        assert len(backups) == 1

    def test_default_disabled_is_silent(self, anakot_home, capsys):
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not (anakot_home / "backups").exists() or not list(
            (anakot_home / "backups").glob("pre-update-*.zip")
        )

    def test_no_backup_flag_skips(self, anakot_home, capsys):
        from anakot_cli.main import _run_pre_update_backup
    ...
    def test_config_enabled_creates_backup(self, anakot_home, capsys):
        ...
        for mod in list(_sys.modules.keys()):
            if mod.startswith("anakot_cli.config"):
    ...
    def test_config_disabled_is_silent(self, anakot_home, capsys):
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not list((anakot_home / "backups").glob("pre-update-*.zip")) \
            if (anakot_home / "backups").exists() else True

    def test_cli_flag_overrides_enabled_config(self, anakot_home, capsys):
...
class TestPreMigrationBackup:
    ...
    def test_creates_backup_under_backups_dir(self, anakot_home):
        ...
        assert out is not None
        assert out.exists()
        # Shares the backups/ directory with pre-update backups so `anakot
        # import` and the update-backup listing both pick them up.
        assert out.parent == anakot_home / "backups"
        assert out.name.startswith("pre-migration-")
        assert out.suffix == ".zip"

    def test_backup_uses_shared_exclusion_rules(self, anakot_home):
        """Pre-migration backup reuses the same exclusion rules as
        ``anakot backup`` / ``create_pre_update_backup`` — no drift."""
        from anakot_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(anakot_home=anakot_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_recurse_into_prior_backups(self, anakot_home):
        from anakot_cli.backup import create_pre_migration_backup
        out1 = create_pre_migration_backup(anakot_home=anakot_home)
        assert out1 is not None
        out2 = create_pre_migration_backup(anakot_home=anakot_home)
        assert out2 is not None
        with zipfile.ZipFile(out2) as zf:
            names = zf.namelist()
        assert not any(n.startswith("backups/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, anakot_home):
        ...
        created = []
        for _ in range(7):
            out = create_pre_migration_backup(anakot_home=anakot_home, keep=3)
            if out is not None:
                created.append(out)
            _t.sleep(1.05)  # timestamp resolution

        remaining = sorted((anakot_home / "backups").glob("pre-migration-*.zip"))
        assert len(remaining) <= 3, f"expected <=3 backups retained, got {len(remaining)}"

    def test_missing_anakot_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.anakot yet — nothing to back up."""
        from anakot_cli.backup import create_pre_migration_backup
        missing = tmp_path / "does-not-exist"
        out = create_pre_migration_backup(anakot_home=missing)
        assert out is None
...
class TestRestoreCronJobsIfEmptied:
    ...
    def test_noop_when_snapshot_id_missing(self, tmp_path):
        ...
        jobs_path = anakot_home / "cron" / "jobs.json"
        self._seed_jobs(jobs_path, [])
        assert restore_cron_jobs_if_emptied(None, anakot_home=anakot_home) is None
        assert restore_cron_jobs_if_emptied("", anakot_home=anakot_home) is None
