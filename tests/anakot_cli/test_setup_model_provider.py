...
def test_setup_summary_marks_anthropic_auth_as_vision_available(tmp_path, monkeypatch, capsys):
    ...
    assert "missing run 'anakot setup' to configure" not in output
...
def test_setup_summary_shows_camofox_when_browser_feature_is_camofox(tmp_path, monkeypatch, capsys):
    ...
    _clear_provider_env(monkeypatch)
    monkeypatch.setattr(
        "anakot_cli.setup.get_callmemo_subscription_features",
...
def test_setup_summary_local_browser_unavailable_without_chromium(
    ...
    ``get_callmemo_subscription_features`` so the surface stays aligned with the
    ...
    monkeypatch.setattr("anakot_cli.callmemo_subscription._has_agent_browser", lambda: True)
    monkeypatch.setattr(
