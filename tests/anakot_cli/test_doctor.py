...
from anakot_cli.doctor import _has_provider_env_config
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("ANAKOT_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_anakot_home(self, tmp_path, provider=""):
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_anakot_home(tmp_path, provider)
...
class TestDoctorXaiOAuthStatus:
    ...
    failure (or runtime exception) cannot silence the callmemo / Codex / Gemini /
    ...
    def test_import_failure_does_not_affect_other_providers(self, monkeypatch, tmp_path):
        """callmemo / Codex / Gemini / MiniMax rows must survive an xAI import failure."""
        home = tmp_path / ".anakot"
        home.mkdir(parents=True, exist_ok=True)
        (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
        project = tmp_path / "project"
        ...
        with contextlib.redirect_stdout(buf):
            doctor_mod.run_doctor(Namespace(fix=False))
        out = buf.getvalue()
        assert "callmemo Portal auth" in out
        assert "logged in" in out
