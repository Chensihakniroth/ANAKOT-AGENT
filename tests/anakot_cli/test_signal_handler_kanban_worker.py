...
def test_sigterm_with_kanban_task_env_terminates_quickly():
    ...
    try:
        ...
        while time.time() < deadline:
            ...
            time.sleep(0.02)
        pytest.fail(
            f"process still alive 2s after SIGTERM with ANAKOT_KANBAN_TASK set "
...
def test_real_handler_uses_os_exit_for_kanban_workers():
    ...
    os._exit(0) when ANAKOT_KANBAN_TASK is set.
    ...
    assert "ANAKOT_KANBAN_TASK" in body, (
        "_signal_handler_q must gate its kanban-worker exit path on "
        "ANAKOT_KANBAN_TASK — see #28181"
    )
