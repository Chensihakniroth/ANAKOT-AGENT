...
def test_gui_installs_packages_and_launches_desktop_app(tmp_path, monkeypatch):
    ...
    with patch("anakot_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("anakot_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("anakot_cli.main._desktop_build_needed", return_value=True), \
         patch("anakot_cli.main._write_desktop_build_stamp"), \
         patch("anakot_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("anakot_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
...
def test_gui_forwards_desktop_environment_overrides(tmp_path, monkeypatch):
    root = _make_desktop_tree(tmp_path)
    anakot_root = tmp_path / "custom-anakot"
    ...
    with patch("anakot_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("anakot_cli.main._run_npm_install_deterministic", return_value=ok), \
         patch("anakot_cli.main._desktop_build_needed", return_value=True), \
         patch("anakot_cli.main._write_desktop_build_stamp"), \
         patch("anakot_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("anakot_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns(
            fake_boot=True,
            ignore_existing=True,
            anakot_root=str(anakot_root),
            cwd=str(cwd),
        ))

    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["ANAKOT_DESKTOP_BOOT_FAKE"] == "1"
    assert launch_env["ANAKOT_DESKTOP_IGNORE_EXISTING"] == "1"
    assert launch_env["ANAKOT_DESKTOP_ANAKOT_ROOT"] == str(anakot_root)
    assert launch_env["ANAKOT_DESKTOP_CWD"] == str(cwd)
...
def test_gui_exits_when_npm_missing(tmp_path, monkeypatch, capsys):
    ...
    with patch("anakot_cli.main.shutil.which", return_value=None), \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_skip_build_launches_existing_packaged_app_without_npm(tmp_path, monkeypatch):
    ...
    with patch("anakot_cli.main.shutil.which", return_value=None), \
         patch("anakot_cli.main._run_npm_install_deterministic") as mock_install, \
...
def test_gui_linux_configures_sandbox_before_launch(tmp_path, monkeypatch):
    ...
    with patch("anakot_cli.main.shutil.which", return_value="/usr/bin/sudo"), \
         patch("anakot_cli.main.subprocess.run", return_value=ok) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_source_mode_uses_renderer_build_and_electron(tmp_path, monkeypatch):
    ...
    with patch("anakot_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("anakot_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("anakot_cli.main._desktop_build_needed", return_value=True), \
         patch("anakot_cli.main._write_desktop_build_stamp"), \
         patch("anakot_cli.main.subprocess.run", side_effect=[build_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
...
def test_desktop_build_stamp_skips_build_when_up_to_date(tmp_path, monkeypatch):
    ...
    with patch("anakot_cli.main._desktop_build_needed", return_value=False), \
         patch("anakot_cli.main._run_npm_install_deterministic") as mock_install, \
         patch("anakot_cli.main.subprocess.run", return_value=launch_ok) as mock_run, \
         patch("anakot_cli.main._desktop_macos_relaunchable_fixup"), \
         pytest.raises(SystemExit) as exc:
...
def test_desktop_force_build_overrides_stamp(tmp_path, monkeypatch):
    ...
    with patch("anakot_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("anakot_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("anakot_cli.main._desktop_build_needed", return_value=False), \
         patch("anakot_cli.main._write_desktop_build_stamp") as mock_stamp, \
         patch("anakot_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("anakot_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
...
def test_compute_desktop_content_hash_stable(tmp_path, monkeypatch):
    ...
    (root / "package.json").write_text('{"name":"anakot"}', encoding="utf-8")
...
def test_gui_retries_pack_once_after_purging_build_cache(tmp_path, monkeypatch):
    ...
    with patch("anakot_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("anakot_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("anakot_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("anakot_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("anakot_cli.main._write_desktop_build_stamp"), \
         patch("anakot_cli.main._purge_electron_build_cache", return_value=[Path("/c/electron.zip")]) as mock_purge, \
         patch("anakot_cli.main.subprocess.run", side_effect=[pack_fail, pack_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_does_not_retry_when_purge_finds_nothing(tmp_path, monkeypatch, capsys):
    ...
    with patch("anakot_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("anakot_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("anakot_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("anakot_cli.main._purge_electron_build_cache", return_value=[]) as mock_purge, \
