...
class TestEarlyMouseDisable:
    def test_writes_disable_sequence_when_tui_flag_in_argv(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["anakot", "--tui", "-c", "abc"])
    ...
    def test_writes_disable_sequence_when_anakot_tui_env_set(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["anakot"])
        monkeypatch.setenv("ANAKOT_TUI", "1")
        monkeypatch.delenv("ANAKOT_TUI_NO_EARLY_DISABLE", raising=False)
    ...
    def test_skips_when_stdout_is_not_a_tty(self, monkeypatch):
        # `anakot --tui … >log` or CI capture: pipe is fd 1, not a TTY. The
