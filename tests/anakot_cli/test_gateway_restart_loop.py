...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "anakot  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "ANAKOT GATEWAY RESTART",           # uppercase
    ])
    def test_anakot_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "systemctl start anakot-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill anakot gateway process",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.anakot.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
...
class TestGatewaySelfTargetingGuard:
    """Verify anakot gateway stop/restart refuse when _ANAKOT_GATEWAY=1."""
