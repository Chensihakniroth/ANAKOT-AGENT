...
def test_managed_install_takes_precedence(tmp_path):
    """When ANAKOT_MANAGED is set, that takes precedence over git detection."""
    (tmp_path / ".git").mkdir()
    with patch("anakot_cli.config.get_managed_system", return_value="NixOS"), \
         patch("anakot_cli.config.get_anakot_home", return_value=tmp_path):
        from anakot_cli.config import detect_install_method
        method = detect_install_method(project_root=tmp_path)
        assert method == "nixos"
...
def test_banner_warns_on_pip_install(tmp_path):
    ...
    with patch("anakot_cli.config.get_anakot_home", return_value=hh), \
         patch("anakot_constants.get_anakot_home", return_value=hh):
        buf = io.StringIO()
