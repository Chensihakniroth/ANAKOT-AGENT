"""Tests for normalized callmemo Portal account entitlement helpers."""
...
from anakot_cli.callmemo_account import (
    ...
    reset_callmemo_portal_account_info_cache,
)
...
def test_valid_jwt_with_paid_access_true(monkeypatch):
    token = _jwt(
        {
            ...
            "org_id": "org_123",
            "client_id": "anakot-cli",
            "product_id": "nous-anakot-agent",
            "nous_client": "anakot-agent",
    ...
    info = get_callmemo_portal_account_info()
    ...
    assert info.product_id == "nous-anakot-agent"
...
def test_expired_jwt_falls_back_to_fresh_account(monkeypatch):
    ...
    monkeypatch.setattr("anakot_cli.callmemo_account._fetch_callmemo_account_info", lambda *a, **kw: payload)
...
def test_entitlement_message_returns_none_for_paid_access():
    ...
    assert format_callmemo_portal_entitlement_message(info, capability="paid models") is None
...
def test_entitlement_message_for_inference_key_without_portal_login():
    ...
    assert "callmemo inference credentials are configured" in message
    assert "cannot verify your callmemo Portal paid access" in message
    assert "Log in with `anakot model`" in message
...
def test_entitlement_message_for_no_subscription_or_credits():
    ...
    message = format_callmemo_portal_entitlement_message(info, capability="paid models")
...
def test_entitlement_message_for_unknown_entitlement_is_explicit():
    ...
    message = format_callmemo_portal_entitlement_message(info, capability="Tool Gateway")

    assert message is not None
    assert "could not verify" in message
    assert "account_api_timeout" in message
    assert "Run `anakot model`" in message
...
def test_entitlement_message_for_account_missing():
    ...
    assert message is not None
    assert "could not find a callmemo Portal account or organisation" in message
