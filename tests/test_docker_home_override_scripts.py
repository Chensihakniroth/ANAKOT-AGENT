...
def test_main_wrapper_preserves_docker_workdir() -> None:
    ...
    assert "_anakot_orig_cwd" in text, (
        "main-wrapper.sh must save the original cwd before cd /opt/data"
    )
    assert 'ANAKOT_ORIG_CWD:-$PWD' in text, (
    ...
    # first exec / if-block.
    activate_idx = text.index("/opt/anakot/.venv/bin/activate")
    restore_idx = text.index('cd "$_anakot_orig_cwd"')
    exec_idx = text.index("if [ $# -eq 0 ]")
    assert activate_idx < restore_idx < exec_idx, (
        "cd $_anakot_orig_cwd must appear after venv activation and "
...
def test_dashboard_run_resets_home_before_dropping_privileges() -> None:
    ...
    assert "exec s6-setuidgid anakot anakot dashboard" in text
...
def test_dashboard_run_does_not_derive_insecure_from_bind_host() -> None:
    ...
    ``ANAKOT_DASHBOARD_HOST``. Doing so disables the OAuth auth gate on
    ...
    The opt-in is now explicit: ``ANAKOT_DASHBOARD_INSECURE=1`` (truthy).
    ...
    assert '127.0.0.1|localhost' not in text, (
        ...
        "is the authority now — opt in via ANAKOT_DASHBOARD_INSECURE instead."
    )
    ...
    assert "ANAKOT_DASHBOARD_INSECURE" in text, (
        "Explicit ANAKOT_DASHBOARD_INSECURE opt-in is missing."
    )
    # Truthy values aligned with the rest of the s6 scripts
    # (e.g. ANAKOT_DASHBOARD).
    for truthy in ("1", "true", "TRUE", "True", "yes", "YES", "Yes"):
        assert truthy in text, (
            f"ANAKOT_DASHBOARD_INSECURE should accept truthy value {truthy!r}"
        )
