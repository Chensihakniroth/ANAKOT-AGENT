...
Regression test for https://github.com/callmemo/anakot-agent/issues/18594.
...
class TestGetAnakotHomeProfileWarning:
    ...
    def test_default_active_profile_no_warning(
        ...
        """active_profile=default → still no warning, returns ~/.anakot."""
        anakot_dir = tmp_path / ".anakot"
        anakot_dir.mkdir()
        (anakot_dir / "active_profile").write_text("default\n")
        result = fresh_constants.get_anakot_home()
        assert result == tmp_path / ".anakot"
        assert "ANAKOT_HOME fallback" not in capsys.readouterr().err
    ...
    def test_empty_active_profile_no_warning(
        ...
        anakot_dir = tmp_path / ".anakot"
        anakot_dir.mkdir()
        (anakot_dir / "active_profile").write_text("")
