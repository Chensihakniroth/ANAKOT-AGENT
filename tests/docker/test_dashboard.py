...
def test_dashboard_slot_reports_down_when_disabled(
    ...
    """Without ANAKOT_DASHBOARD, s6-svstat should report the dashboard
    slot as DOWN (not up-with-sleep-infinity, which would
    false-positive `anakot doctor` and any other health check).
    ...
    ANAKOT_DASHBOARD is unset, so the slot reflects reality.
    """
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name, built_image,
         "sleep", "60"],
    ...
    assert "down" in r.stdout, (
        f"Dashboard slot should be 'down' without ANAKOT_DASHBOARD; "
...
def test_dashboard_slot_reports_up_when_enabled(
    ...
    """Symmetry: with ANAKOT_DASHBOARD=1, s6-svstat reports the slot as up."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         ...
         # ANAKOT_DASHBOARD_OAUTH_CLIENT_ID in this test env), start_server
...
def test_dashboard_opt_in_starts(
    ...
    """With ANAKOT_DASHBOARD=1, a dashboard process should be visible."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "ANAKOT_DASHBOARD=1",
    ...
    ok, _ = _poll(
        container_name, "pgrep -f 'anakot dashboard'", deadline_s=30.0,
    )
    assert ok, "Dashboard should be running with ANAKOT_DASHBOARD=1"


def test_dashboard_port_override(
    built_image: str, container_name: str,
) -> None:
    """ANAKOT_DASHBOARD_PORT changes the dashboard's listen port."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "ANAKOT_DASHBOARD=1", "-e", "ANAKOT_DASHBOARD_PORT=9120",
         # Default bind is 0.0.0.0; pin insecure opt-in so the auth gate
...
def test_dashboard_restarts_after_crash(
    ...
    for _attempt in range(10):
        first_pid_result = docker_exec(
            container_name, "pgrep", "-f", "anakot dashboard",
        )
    ...
    # Kill the dashboard. The dashboard process runs as anakot, so the
    # anakot user can kill it (same UID).
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "ANAKOT_DASHBOARD=1",
         "-e", "ANAKOT_DASHBOARD_HOST=0.0.0.0",
         "-e", "ANAKOT_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
         built_image, "sleep", "120"],
        check=True, capture_output=True, timeout=30,
    )
...
def test_dashboard_insecure_env_var_opts_out_of_gate(
    ...
    """``ANAKOT_DASHBOARD_INSECURE=1`` re-enables the legacy no-gate mode
    ...
    assert status.get("auth_required") is False, (
        "ANAKOT_DASHBOARD_INSECURE=1 must disable the auth gate (explicit "
