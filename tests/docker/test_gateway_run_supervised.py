...
def test_gateway_run_no_supervise_flag_preserves_legacy_behavior(
    ...
    Three positive assertions confirm we took the pre-s6 path:
      ...
      * The CMD process is a python ``anakot gateway run`` invocation
        (not ``sleep infinity``).
...
def test_gateway_run_no_supervise_env_var(
    ...
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "ANAKOT_GATEWAY_NO_SUPERVISE=1",
    ...
    if inspect.stdout.strip() == "running":
        assert not _svstat_wants_up(container_name, "gateway-default"), (
            "ANAKOT_GATEWAY_NO_SUPERVISE=1: gateway-default has "
...
def test_supervised_gateway_does_not_recurse(
    ...
    # Count python processes running `anakot gateway run`. If the
    ...
    r = _sh(container_name, "ps -eo pid,cmd | grep -v grep | grep -E 'python.*anakot.*gateway run' | wc -l")
    assert r.returncode == 0
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    log file under ``${ANAKOT_HOME}/logs/gateways/<profile>/current``.
    ...
    a poor user experience: the iconic "Anakot Gateway Starting…"
