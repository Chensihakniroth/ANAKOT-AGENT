...
def _reset_anakot_time_cache():
    """Reset the anakot_time module cache (replacement for removed reset_cache)."""
    anakot_time._cached_tz = None
    anakot_time._cached_tz_name = None
    anakot_time._cache_resolved = False
...
class TestAnakotTimeNow:
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["ANAKOT_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="anakot_time"):
            result = anakot_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_none_for_invalid(self):
        os.environ["ANAKOT_TIMEZONE"] = "Not/A/Timezone"
        tz = anakot_time.get_timezone()
        assert tz is None
...
class TestCronTimezone:
    ...
    def test_ensure_aware_normalizes_aware_to_anakot_tz(self):
        """Already-aware datetimes should be normalized to Anakot tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Anakot tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Anakot tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Anakot is UTC.
