...
class TestSaveSessionLogRedactsSecrets:
    ...
    def _ensure_redaction_enabled(self, monkeypatch):
        """Force redaction on regardless of host ANAKOT_REDACT_SECRETS state.
...
class TestBuildSystemPrompt:
    ...
    def test_includes_callmemo_subscription_prompt(self, agent, monkeypatch):
        monkeypatch.setattr(run_agent, "build_callmemo_subscription_prompt", lambda tool_names: "NOUS SUBSCRIPTION BLOCK")
        prompt = agent._build_system_prompt()
...
class TestBuildApiKwargs:
    ...
    def test_reasoning_sent_for_nous_route(self, agent):
        agent.provider = "nous"
        agent.base_url = "https://inference-api.callmemo.ai/v1"
        agent.model = "minimax/minimax-m2.5"
        messages = [{"role": "user", "content": "hi"}]
        kwargs = agent._build_api_kwargs(messages)
        assert kwargs["extra_body"]["reasoning"]["effort"] == "medium"
...
class TestExecuteToolCalls:
    ...
    def test_keyboard_interrupt_emits_cancelled_post_tool_hook(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("anakot_cli.plugins.invoke_hook", _capture_hook)
...
class TestConcurrentToolExecution:
    ...
    def test_invoke_tool_agent_level_tool_emits_terminal_post_tool_hook(self, agent, monkeypatch):
        ...
        hook_calls = []
        monkeypatch.setattr(
            "anakot_cli.plugins.get_pre_tool_call_block_message",
    ...
    def test_sequential_agent_level_tool_execution_middleware_wraps_inline_dispatch(self, agent, monkeypatch):
        ...
        monkeypatch.setattr(
            "anakot_cli.plugins.invoke_middleware",
...
class TestRunConversation:
    ...
    def test_request_scoped_api_hooks_fire_for_each_api_call(self, agent):
        ...
        with (
            ...
            patch(
                "anakot_cli.plugins.has_hook",
                side_effect=lambda name: name in {"pre_api_request", "post_api_request"},
            ),
            patch("anakot_cli.plugins.invoke_hook", side_effect=_record_hook),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_api_request_error_hook_skips_payload_work_without_listener(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("anakot_cli.plugins.invoke_hook", _invoke_hook)
    ...
    def test_request_scoped_api_hooks_skip_payload_work_without_listeners(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("anakot_cli.plugins.has_hook", _has_hook)
    ...
    def test_kanban_block_called_on_iteration_exhaustion(self, agent, monkeypatch):
        ...
        with (
            ...
            patch("anakot_cli.kanban_db._record_task_failure",
                  mock_record_failure),
            patch("anakot_cli.kanban_db.connect", mock_connect),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
...
class TestNousCredentialRefresh:
    """Verify callmemo credential refresh rebuilds the runtime client."""
    ...
    def test_try_refresh_nous_client_credentials_rebuilds_client(
        ...
        monkeypatch.setattr(
            "anakot_cli.auth.resolve_callmemo_runtime_credentials", _fake_resolve
        )
        ...
        assert (
            rebuilt["kwargs"]["base_url"] == "https://inference-api.callmemo.ai/v1"
        )
...
class TestGpt5ApiModeRouting:
    ...
    def test_nous_gpt5_stays_on_chat_completions(self, agent):
        """callmemo serves gpt-5.x on /chat/completions — must not upgrade to codex_responses."""
        agent.provider = "nous"
        agent.base_url = "https://inference-api.callmemo.ai/v1"
        agent.api_mode = "chat_completions"
