...
def test_nemo_relay_plugin_emits_llm_tool_and_exports_atif(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ANAKOT_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY", str(tmp_path / "atof"))
    monkeypatch.setenv("ANAKOT_NEMO_RELAY_ATIF_ENABLED", "1")
    monkeypatch.setenv("ANAKOT_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY", str(tmp_path / "atif"))
    ...
    assert (tmp_path / "atif" / "anakot-atif-s1.json").exists()
...
def test_nemo_relay_plugin_emits_approval_marks(monkeypatch):
    ...
    assert "anakot.approval.request" in mark_names
    assert "anakot.approval.response" in mark_names
...
def test_nemo_relay_plugin_emits_unmatched_fallback_marks(monkeypatch):
    ...
    mark_names = [event[1] for event in fake.events if event[0] == "scope.event"]
    assert "anakot.api.response.unmatched" in mark_names
    assert "anakot.api.error" in mark_names
    assert "anakot.tool.response.unmatched" in mark_names
...
def test_nemo_relay_plugin_metadata_promotes_trajectory_and_subagent_ids(monkeypatch):
    ...
    turn_mark = next(event for event in fake.events if event[0] == "scope.event" and event[1] == "anakot.turn.start")
    ...
    start_mark = next(event for event in fake.events if event[0] == "scope.event" and event[1] == "anakot.subagent.start")
    ...
    stop_mark = next(event for event in fake.events if event[0] == "scope.event" and event[1] == "anakot.subagent.stop")
...
def test_nemo_relay_plugin_reparents_child_session_scope_for_embedded_atif(monkeypatch):
    ...
    child_push = next(
        ...
        if event[0] == "scope.push" and event[1] == "anakot-session-child-session"
    )
    child_kwargs = child_push[3]
    assert child_kwargs["handle"] == ("scope", "anakot-session-parent-session")
...
def test_nemo_relay_plugin_skips_embedded_child_atif_file_by_default(tmp_path, monkeypatch):
    ...
    assert (tmp_path / "atif" / "anakot-atif-parent-session.json").exists()
    assert not (tmp_path / "atif" / "anakot-atif-child-session.json").exists()
...
def test_nemo_relay_plugin_can_write_embedded_child_atif_file_in_all_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ANAKOT_NEMO_RELAY_ATIF_ENABLED", "1")
    monkeypatch.setenv("ANAKOT_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY", str(tmp_path / "atif"))
    monkeypatch.setenv("ANAKOT_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE", "all")
    ...
    assert (tmp_path / "atif" / "anakot-atif-parent-session.json").exists()
    assert (tmp_path / "atif" / "anakot-atif-child-session.json").exists()
...
""",
    ...
    monkeypatch.setenv("ANAKOT_NEMO_RELAY_PLUGINS_TOML", str(plugins_toml))
