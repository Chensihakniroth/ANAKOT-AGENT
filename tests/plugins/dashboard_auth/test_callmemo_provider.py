"""Tests for the bundled callmemo dashboard-auth plugin.
...
class TestConstruction:
    ...
    def test_name_and_display(self):
        ...
        assert p.name == "nous"
        assert p.display_name == "callmemo"
...
class TestPluginRegister:
    ...
    def test_registers_with_default_portal_url_when_only_client_id_set(
        ...
        defaults to the production callmemo Portal. The user shouldn't have
        ...
        assert registered._portal_url == "https://portal.callmemo.ai"
    ...
    def test_skips_when_client_id_malformed(self, monkeypatch):
        monkeypatch.setenv("ANAKOT_DASHBOARD_OAUTH_CLIENT_ID", "anakot-dashboard")
        monkeypatch.setenv("ANAKOT_DASHBOARD_PORTAL_URL", "https://p.example")
        ctx = MagicMock()
        nous_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_not_called()
        # Skip reason names the offending value + contract shape.
        assert "agent:" in nous_plugin.LAST_SKIP_REASON
        assert "anakot-dashboard" in nous_plugin.LAST_SKIP_REASON
    ...
    def test_strips_whitespace_from_env_vars(self, monkeypatch):
        monkeypatch.setenv("ANAKOT_DASHBOARD_OAUTH_CLIENT_ID", "  agent:x  ")
        monkeypatch.setenv("ANAKOT_DASHBOARD_PORTAL_URL", "  https://p.example  ")
        ctx = MagicMock()
        nous_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_called_once()
...
class TestConfigYamlSource:
    ...
    def test_env_overrides_config_portal_url(self, patch_config, monkeypatch):
        monkeypatch.setenv("ANAKOT_DASHBOARD_OAUTH_CLIENT_ID", "agent:x")
        monkeypatch.setenv(
            "ANAKOT_DASHBOARD_PORTAL_URL", "https://env.portal.example",
        )
        patch_config({
            "client_id": "agent:x",
    ...
    def test_empty_env_string_does_not_shadow_config(
        ...
        """``ANAKOT_DASHBOARD_OAUTH_CLIENT_ID=`` (set but empty) is
        ...
        monkeypatch.setenv("ANAKOT_DASHBOARD_OAUTH_CLIENT_ID", "")
        patch_config({"client_id": "agent:from-config"})
        ctx = MagicMock()
        nous_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_called_once()
    ...
    def test_config_yaml_load_failure_falls_through_cleanly(
        ...
        monkeypatch.setattr(
            "anakot_cli.config.load_config", _broken_load
        )
        ctx = MagicMock()
        # Must not raise.
        nous_plugin.register(ctx)
