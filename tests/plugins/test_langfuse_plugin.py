...
class TestRuntimeGate:
    ...
    def test_get_langfuse_does_not_import_anakot_config(self, monkeypatch):
        ...
        # Drop any cached import of anakot_cli.config.
        sys.modules.pop("anakot_cli.config", None)

        langfuse_plugin = self._fresh_plugin()
        for _ in range(20):
            langfuse_plugin._get_langfuse()

        assert "anakot_cli.config" not in sys.modules, (
            "langfuse plugin imported anakot_cli.config — regression toward "
...
class TestPlaceholderKeyDetection:
    ...
    def test_validate_langfuse_key_rejects_wrong_prefix(self, monkeypatch):
        ...
        msg = plugin._validate_langfuse_key(
            "ANAKOT_LANGFUSE_PUBLIC_KEY", "placeholder"
        )
        assert msg is not None
        assert "ANAKOT_LANGFUSE_PUBLIC_KEY" in msg
        assert "pk-lf-" in msg
    ...
    def test_validate_langfuse_key_unknown_name_passes(self, monkeypatch):
        ...
        self._clear_env(monkeypatch)
        plugin = self._fresh_plugin()
        assert plugin._validate_langfuse_key("ANAKOT_LANGFUSE_BASE_URL", "anything") is None
    ...
    def test_common_placeholders_detected(self, monkeypatch, caplog, placeholder):
        ...
        with caplog.at_level(logging.WARNING, logger=self.LOGGER_NAME):
            assert plugin._get_langfuse() is None
        assert "ANAKOT_LANGFUSE_PUBLIC_KEY" in caplog.text

    def test_legacy_LANGFUSE_PUBLIC_KEY_also_validated(self, monkeypatch, caplog):
        """The plugin reads both the canonical ANAKOT_-prefixed env var and
        ...
        # ANAKOT_-prefixed one — operators set the ANAKOT_-prefixed one).
        assert "ANAKOT_LANGFUSE_PUBLIC_KEY" in caplog.text
