...
class TestGetDefaultAnakotRoot:
    ...
    def test_anakot_home_is_native(self, tmp_path, monkeypatch):
        """When ANAKOT_HOME = ~/.anakot, returns ~/.anakot."""
        native = tmp_path / ".anakot"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ANAKOT_HOME", str(native))
        assert get_default_anakot_root() == native

    def test_anakot_home_is_profile(self, tmp_path, monkeypatch):
        """When ANAKOT_HOME is a profile under ~/.anakot, returns ~/.anakot."""
        native = tmp_path / ".anakot"
    ...
    def test_anakot_home_is_docker(self, tmp_path, monkeypatch):
        """When ANAKOT_HOME points outside ~/.anakot (Docker), returns ANAKOT_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ANAKOT_HOME", str(docker_home))
        assert get_default_anakot_root() == docker_home

    def test_anakot_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any ANAKOT_HOME outside ~/.anakot is treated as the root."""
        custom = tmp_path / "my-anakot-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ANAKOT_HOME", str(custom))
        assert get_default_anakot_root() == custom
    ...
    def test_no_anakot_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/anakot without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("ANAKOT_HOME", raising=False)
        ...
        assert get_default_anakot_root() == home / "AppData" / "Local" / "anakot"
