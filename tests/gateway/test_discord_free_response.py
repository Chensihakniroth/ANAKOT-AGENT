...
async def test_discord_forum_threads_are_handled_as_threads(adapter, monkeypatch):
    ...
    thread = FakeThread(channel_id=456, name="Can Anakot reply here?", parent=forum)
    ...
    assert event.source.thread_id == "456"
    assert event.source.chat_type == "thread"
    assert event.source.chat_name == "Anakot Server / support-forum / Can Anakot reply here?"
