...
class TestBangPrefixCommands:
    ...
    async def test_bang_with_bot_suffix_resolves(self, adapter):
        """``!stop@anakot`` matches the get_command() ``@suffix`` stripping."""
        await adapter._handle_slack_message(self._make_event("!stop@anakot"))

        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.startswith("/stop@anakot")
        assert msg_event.message_type == MessageType.COMMAND
...
class TestSlashCommands:
    ...
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /anakot btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_anakot_freeform_question(self, adapter):
        """/anakot <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/anakot",
...
class TestSlashEphemeralAck:
    ...
    @pytest.mark.asyncio
    async def test_legacy_anakot_slash_stashes_context(self, adapter):
        """Legacy /anakot <subcommand> also stashes context."""
        command = {
            "command": "/anakot",
            "text": "help",
            "user_id": "U_H",
    ...
    @pytest.mark.asyncio
    async def test_freeform_anakot_question_does_not_stash_context(self, adapter):
        """Free-form /anakot <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/anakot",
