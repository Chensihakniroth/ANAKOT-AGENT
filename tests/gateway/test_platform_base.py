...
class TestMediaDeliveryPathValidation:
    ...
    def test_allows_operator_configured_extra_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ANAKOT_MEDIA_ALLOW_DIRS", str(extra_root))
    ...
    def test_recency_trust_rejects_old_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("ANAKOT_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("ANAKOT_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("ANAKOT_MEDIA_TRUST_RECENT_SECONDS", "60")
    ...
    def test_recency_trust_allows_pdf_in_project_dir(self, tmp_path, monkeypatch):
        ...
        files outside ~/.anakot/cache/* were silently dropped, leaving the
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_anakot_credentials(self, tmp_path, monkeypatch):
        """~/.anakot/.env and ~/.anakot/auth.json stay blocked even in
    ...
    def test_denylist_blocks_anakot_config_in_active_profile(self, tmp_path, monkeypatch):
        ...
        anakot_dir = fake_home / ".anakot"
        anakot_dir.mkdir(parents=True)
        config_file = anakot_dir / "config.yaml"
    ...
    def test_denylist_blocks_shared_anakot_root_config_for_profiles(self, tmp_path, monkeypatch):
        """Profile-mode gateways must still block the shared Anakot root config."""
        self._patch_roots(monkeypatch)

        fake_home = tmp_path / "home"
        profile_home = fake_home / ".anakot" / "profiles" / "work"
        profile_home.mkdir(parents=True)
        anakot_root = fake_home / ".anakot"
        config_file = anakot_root / "config.yaml"
    ...
    def test_strict_mode_truthy_aliases(self, monkeypatch, tmp_path):
        """``ANAKOT_MEDIA_DELIVERY_STRICT=true|yes|on|1`` all enable strict mode."""
        self._patch_roots(monkeypatch)
        from gateway.platforms.base import _media_delivery_strict_mode

        for raw in ("1", "true", "TRUE", "yes", "on"):
            monkeypatch.setenv("ANAKOT_MEDIA_DELIVERY_STRICT", raw)
    ...
    def test_root_home_anakot_env_still_blocked(self, tmp_path, monkeypatch):
        """``~/.anakot/.env`` stays blocked under the $HOME exception — it is a
...
class TestGetHumanDelay:
    def test_off_mode(self):
        with patch.dict(os.environ, {"ANAKOT_HUMAN_DELAY_MODE": "off"}):
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_default_is_off(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ANAKOT_HUMAN_DELAY_MODE", None)
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_natural_mode_range(self):
        with patch.dict(os.environ, {"ANAKOT_HUMAN_DELAY_MODE": "natural"}):
    ...
    def test_custom_mode_uses_env_vars(self):
        env = {
            ...
            "ANAKOT_HUMAN_DELAY_MIN_MS": "100",
            "ANAKOT_HUMAN_DELAY_MAX_MS": "200",
        }
        with patch.dict(os.environ, env):
            delay = BasePlatformAdapter._get_human_delay()
