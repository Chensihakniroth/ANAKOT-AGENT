...
@pytest.mark.asyncio
async def test_goal_verdict_done_sent_via_adapter_send(anakot_home):
    ...
    with patch("anakot_cli.goals.judge_goal", return_value=("done", "the feature shipped", False)):
...
async def test_goal_verdict_continue_enqueues_continuation(anakot_home):
    ...
    with patch("anakot_cli.goals.judge_goal", return_value=("continue", "still needs work", False)):
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
...
async def test_goal_verdict_budget_exhausted_sends_pause(anakot_home):
    ...
    with patch("anakot_cli.goals.judge_goal", return_value=("continue", "keep going", False)):
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
...
@pytest.mark.asyncio
async def test_goal_verdict_skipped_when_no_active_goal(anakot_home):
...
@pytest.mark.asyncio
async def test_goal_verdict_survives_adapter_without_send(anakot_home):
    ...
    with patch("anakot_cli.goals.judge_goal", return_value=("done", "ok", False)):
        # must not raise
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
