...
``~/.anakot/logs/`` (profile-aware via ``get_anakot_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``anakot_cli.web_server``,
    ``anakot_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
from anakot_constants import get_config_path, get_anakot_home
...
# Used by _ComponentFilter and exposed for ``anakot logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "anakot_plugins"),
...
def setup_logging(
    *,
    ...
    """Configure the Anakot logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    anakot_home
        Override for the Anakot home directory.  Falls back to
        ``get_anakot_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_anakot_verbose", False):
                return
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the anakot group. However, both ``_open()`` (initial creation)
