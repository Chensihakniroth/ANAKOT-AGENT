...
echo    [2] Rebuild Desktop App   (fixes blank/broken GUI)
echo    [3] Reinstall Python deps (fixes import errors)
echo    [4] Reinstall Node deps   (fixes npm-related errors)
echo    [5] Fix PATH              (fixes "anakot not recognized")
echo    [6] Fix permissions       (fixes access denied errors)
echo    [7] Clean rebuild         (rebuild everything from scratch)
echo    [8] Run diagnostics      (check what's wrong)
echo.
echo    [0] Exit
echo.
...
if "%CHOICE%"=="2" goto :REBUILD_DESKTOP
if "%CHOICE%"=="3" goto :REINSTALL_PYTHON
if "%CHOICE%"=="4" goto :REINSTALL_NODE
if "%CHOICE%"=="5" goto :FIX_PATH
...
if %ERRORLEVEL% EQU 0 (
    echo  [OK] Desktop App rebuilt successfully.
) else (
    echo  [FAIL] Desktop App build failed. Check error messages above.
)
...
if exist "%INSTALL_DIR%\apps\desktop\release\win-unpacked\Anakot.exe" (
    echo  [OK] Desktop App built
) else (
    echo  [WARN] Desktop App not built (rebuild required to use GUI)
)
echo.
