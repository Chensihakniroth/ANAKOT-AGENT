...
function Write-Banner {
    ...
    Write-Host ""
    Write-Host "       Backend + TUI + Desktop GUI" -ForegroundColor Gray
...
Write-Separator
Write-Host "    Building Desktop App (1-3 minutes)..."
Write-Host ""

Set-Location "$finalDir\apps\desktop"
...
npm run pack
$desktopBuilt = ($LASTEXITCODE -eq 0)
...
if ($desktopBuilt) {
    ...
    $shortcutPath = "$env:USERPROFILE\Desktop\Anakot Agent.lnk"
    $targetPath = "$finalDir\apps\desktop\release\win-unpacked\Anakot.exe"
    $workingDir = "$finalDir\apps\desktop\release\win-unpacked"
    ...
    Write-Host "    WARNING: Desktop build failed." -ForegroundColor Yellow
}
...
Write-Host "         4. Or launch 'Anakot Agent' from Desktop"
Write-Host "         ----------------------------------------------------"
Write-Host ""
