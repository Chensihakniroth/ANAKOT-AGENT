...
if npm run pack; then
    ...
    if [[ "$OS" == "macos" ]]; then
        APP_PATH="$INSTALL_DIR/apps/desktop/release/mac/Anakot Agent.app"
        if [[ -d "$APP_PATH" ]]; then
            ln -sf "$APP_PATH" ~/Desktop/ || true
            ok "Symlinked to Desktop."
        fi
    elif [[ "$OS" == "linux" ]]; then
        DESKTOP_ENTRY_DIR="$HOME/.local/share/applications"
        mkdir -p "$DESKTOP_ENTRY_DIR"
        cat > "$DESKTOP_ENTRY_DIR/anakot-agent.desktop" << EOF
[Desktop Entry]
Name=Anakot Agent
Exec=$INSTALL_DIR/apps/desktop/release/linux-unpacked/anakot
Type=Application
Categories=Utility;
Terminal=false
EOF
        chmod +x "$DESKTOP_ENTRY_DIR/anakot-agent.desktop"
        ok "Created desktop entry."
    elif [[ "$OS" == "windows" ]]; then
        # For Git Bash / WSL (though .bat should ideally be used for Windows)
        echo -e "    ${YELLOW}⚠ Skipping Windows shortcut in bash. Use install-anakot.bat for full Windows support.${NC}"
    fi
else
    echo -e "    ${YELLOW}⚠ Desktop build failed.${NC}"
fi
