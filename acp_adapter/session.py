...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Anakot itself is running in WSL.

    Windows ACP clients can launch ``anakot acp`` inside WSL while still sending
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Anakot from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["anakot-acp"]):
...
class SessionManager:
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Anakot runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.anakot/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``ANAKOT_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_anakot_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=anakot_home / "state.db")
            return self._db_instance
        except Exception:
