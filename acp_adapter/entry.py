...
Loads environment variables from ``~/.anakot/.env``, configures logging
...
def _load_env() -> None:
    ...
    if loaded:
        ...
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", anakot_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="anakot-acp",
        description="Run Anakot Agent as an ACP stdio server.",
    )
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``anakot postinstall`` and the runtime lazy installer.
