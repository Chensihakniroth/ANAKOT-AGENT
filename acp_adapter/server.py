...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Anakot running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Anakot cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
class AnakotACPAgent(acp.Agent):
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Anakot version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Anakot maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from anakot_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from anakot_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Anakot estimates ``used`` from the same
    ...
    async def _send_session_info_update(self, session_id: str) -> None:
        """Send ACP native session metadata after Anakot changes it."""
        if not self._conn:
            return
        try:
        ...
        # anakot_state.py schema — only started_at/ended_at). Use "now" as
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["anakot-acp"],
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Anakot' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Anakot setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Anakot
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        # ACP thought panes should not receive Anakot' local kawaii waiting/status
        ...
        # not the executor's. Also set ANAKOT_INTERACTIVE so approval.py
        # takes the CLI-interactive path (which calls the registered
        # callback via prompt_dangerous_approval) instead of the
        # non-interactive auto-approve branch (GHSA-96vc-wcxf-jjff).
        # ACP's conn.request_permission maps cleanly to the interactive
        # callback shape — not the gateway-queue ANAKOT_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind ANAKOT_SESSION_KEY for this session so per-session caches
            ...
            previous_interactive = os.environ.get("ANAKOT_INTERACTIVE")
            ...
            previous_session_id = os.environ.get("ANAKOT_SESSION_ID")
            ...
            try:
                ...
                # Restore ANAKOT_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("ANAKOT_INTERACTIVE", None)
                else:
                    os.environ["ANAKOT_INTERACTIVE"] = previous_interactive
                # Restore ANAKOT_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("ANAKOT_SESSION_ID", None)
                else:
                    os.environ["ANAKOT_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            ...
            # stomp on each other's ContextVar writes (ANAKOT_SESSION_KEY in
