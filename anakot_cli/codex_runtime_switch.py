...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Anakot'
...
The actual runtime resolution happens in anakot_cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "anakot"}:
        return "auto", []
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Anakot' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Anakot for
        ...
        try:
            ...
            # Tools/MCP servers (excluding the anakot-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "anakot-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            # Permissions + Anakot tool callback are always-on production
            ...
            if "anakot-tools" in mig_report.migrated:
                msg_lines.append(
