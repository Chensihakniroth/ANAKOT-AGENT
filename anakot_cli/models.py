...
Add, remove, or reorder entries here — both `anakot setup` and
`anakot` provider-selection will pick up the change automatically.
"""
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `anakot model`
    flow without maintaining a separate static list.
    """
    from anakot_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $ANAKOT_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $ANAKOT_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Anakot refreshes the cache from
...
# ---------------------------------------------------------------------------
# callmemo Portal free-model helper
# ---------------------------------------------------------------------------
# The callmemo Portal models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# callmemo Portal account tier detection
# ---------------------------------------------------------------------------
...
def partition_callmemo_models_by_tier(
    model_ids: list[str],
    pricing: dict[str, dict[str, str]],
    free_tier: bool,
) -> tuple[list[str], list[str]]:
    """Split callmemo models into (selectable, unavailable) based on user tier.
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    ``_PROVIDER_MODELS["callmemo"]`` list happens to contain or whether the
    ...
    older Anakot that doesn't ship that model in its hardcoded curated
    ...
    * ``pricing`` gets a synthetic ``{"prompt": "0", "completion": "0"}``
      ...
      map, so :func:`partition_callmemo_models_by_tier` keeps it.

    Failures (network, parse, missing field) are silent and degrade to
    returning the inputs unchanged.
    """
    try:
        payload = fetch_callmemo_recommended_models(
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    in-repo ``_PROVIDER_MODELS["callmemo"]`` list happens to contain or whether
    ...
    picker even if the user is running an older Anakot that doesn't ship
    ...
    * ``pricing`` is left untouched — we deliberately do NOT synthesize
      ...
      :func:`partition_callmemo_models_by_tier` keeps free models selectable;
...
def check_callmemo_free_tier(*, force_fresh: bool = False) -> bool:
    ...
    try:
        ...
        account_info = get_callmemo_portal_account_info(force_fresh=force_fresh)
...
def fetch_callmemo_recommended_models(
    ...
    *,
    ...
    """
    base = (portal_base_url or "https://portal.callmemo.ai").rstrip("/")
    now = time.monotonic()
...
# this list:  anakot model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `anakot model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `anakot model` TUI
...
CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ...
    ProviderEntry("groq", "Groq", "Groq (Ultra-fast inference via Groq Cloud)"),
...
#
# Some vendors expose several Anakot provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `anakot model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``anakot model``, the
...
# sensible default, but callmemo Portal is a per-token *metered aggregator* whose
...
# path. The *interactive* default (GUI onboarding / ``anakot model``) uses the
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``anakot auth add openai-codex`` without
    ``anakot model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``anakot model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    anakot-agent is tool-calling-first — every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (callmemo Portal, private mirrors, older catalog snapshots) don't populate
    ...
    *,
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — anakot-agent
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, callmemo Portal).
    """
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``anakot model``, ``/model``, etc.).
    """
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    Supports ``provider:model`` syntax to switch providers at runtime::
        ...
        nous:anakot-3                           →  ("callmemo", "anakot-3")
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (Codex, callmemo, etc. all support /models)
...
_AGGREGATOR_PROVIDERS = frozenset(
    {"callmemo", "openrouter", "copilot", "kilocode"}
)
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Anakot' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``anakot_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``anakot auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.anakot/.env``.
...
#   - "callmemo": curated list and Portal /models endpoint are the source of
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (Codex, callmemo),
    falling back to static lists. For providers in ``_MODELS_DEV_PREFERRED``
    (opencode-go/zen, xiaomi, deepseek, smaller inference providers, etc.),
    models.dev entries are merged on top of curated so new models released
    on the platform appear in ``/model`` without a Anakot release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Anakot release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "callmemo":
        # Try live callmemo Portal /models endpoint
        try:
            from anakot_cli.auth import fetch_nous_models, resolve_callmemo_runtime_credentials
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``anakot model`` shows.
...
#
# Cache strategy:
#   - One JSON file at $ANAKOT_HOME/provider_models_cache.json
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$ANAKOT_HOME/auth.json`` and external
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _ANAKOT_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Anakot' default Claude IDs elsewhere use
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Anakot will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
