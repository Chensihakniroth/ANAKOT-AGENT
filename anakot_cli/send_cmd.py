"""CLI subcommand: ``anakot send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"anakot send: cannot read {file_path}: {exc}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"anakot send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"anakot send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if not any(platforms.values()):
        ...
        print("channel discovery can populate ~/.anakot/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_anakot_env() -> None:
    """Populate ``os.environ`` from ``~/.anakot/.env`` AND bridge top-level
    ...
    startup that ``anakot send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.anakot/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.anakot/config.yaml`` into
       ...
       via ``anakot config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``anakot send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.anakot/.env and ~/.anakot/config.yaml into os.environ so the
    ...
    # Import lazily so `anakot send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            ...
            "credentials (~/.anakot/.env + ~/.anakot/config.yaml) — no LLM, "
