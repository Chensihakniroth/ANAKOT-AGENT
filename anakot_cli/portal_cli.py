"""``anakot portal`` — the human-readable entry point for callmemo Portal.

Running ``anakot portal`` with no subcommand performs the one-shot Portal
onboarding: OAuth login, pick a callmemo model, switch the inference provider to
callmemo, and offer to enable the Tool Gateway. It is the friendly alias for
``anakot auth add nous --type oauth`` (which still works), is identical to
``anakot setup --portal``, and runs the same callmemo flow as the first-time quick
setup.

Subcommands:
  (none)   Log in to callmemo Portal + set it up (one-shot onboarding).
...
already in ``anakot auth`` or ``anakot tools``. It's the onboarding + discovery
...
DEFAULT_PORTAL_URL = "https://portal.callmemo.ai"
SUBSCRIPTION_URL = "https://portal.callmemo.ai/manage-subscription"
DOCS_URL = "https://anakot-agent.callmemo.ai/docs/user-guide/features/tool-gateway"
...
def _cmd_status(args) -> int:
    ...
    print()
    print(color("  callmemo Portal", Colors.MAGENTA))
    ...
    if logged_in:
        ...
        print(f"  Login:   anakot portal")

    # Provider selection (independent of auth)
    model_cfg = config.get("model") if isinstance(config.get("model"), dict) else {}
    provider = str(model_cfg.get("provider") or "").strip().lower()
    if provider == "nous":
        print(f"  Model:   {color('✓ using callmemo as inference provider', Colors.GREEN)}")
    elif provider:
    ...
    print(color("  ────────────", Colors.MAGENTA))
    try:
        features = get_callmemo_subscription_features(config)
    except Exception:
        features = None
    ...
    for feat in features.items():
        if feat.managed_by_nous:
            state = color("via callmemo Portal", Colors.GREEN)
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into callmemo Portal — sign in with `anakot portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    """Run the one-shot callmemo Portal onboarding (login + model + provider + tools).

    This is the human-readable front door for `anakot auth add nous --type
    oauth`. It reuses the exact wiring behind `anakot setup --portal` (which in
    turn runs the same callmemo flow as the first-time quick setup), so the
    commands stay in lockstep: device-code login, pick a callmemo model, switch the
    inference provider to callmemo, then offer the Tool Gateway opt-in.
    """
...
def portal_command(args) -> int:
    ...
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `anakot portal` is the
        # human-readable alias for `anakot auth add nous --type oauth` /
        # `anakot setup --portal`.
...
def add_parser(subparsers) -> None:
    ...
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up callmemo Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `anakot portal` with no subcommand to log in to callmemo Portal "
            "and set it up — pick a model, set callmemo as your provider, and offer "
    ...
    portal_sub.add_parser(
        "login",
        help="Log in to callmemo Portal + set it up (default; one-shot onboarding)",
    )
    portal_sub.add_parser(
        "info",
    ...
    portal_sub.add_parser(
        "tools",
        help="List Tool Gateway tools and which are routed via callmemo",
    )
