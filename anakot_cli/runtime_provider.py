...
def _config_base_url_trustworthy_for_bare_custom(cfg_base_url: str, cfg_provider: str) -> bool:
    ...
    try:
        from anakot_cli.auth import resolve_provider as _resolve_provider
...
_VALID_API_MODES = {
    ...
    # instead of Anakot' tool dispatch. Gated behind config key
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = os.getenv("ANAKOT_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if not requested_norm.startswith("custom:"):
        try:
            ...
            # the built-in. See tests/anakot_cli/test_runtime_provider_resolution.py
...
def _resolve_explicit_runtime(
    *,
    ...
    if provider == "nous":
        ...
        base_url = (
            explicit_base_url
            or str(state.get("inference_base_url") or auth_mod.DEFAULT_CALLMEMO_INFERENCE_URL).strip().rstrip("/")
        )
        ...
        # by resolve_callmemo_runtime_credentials().
        api_key = explicit_api_key or (
            str(state.get("agent_key") or "").strip()
            if _agent_key_is_usable(
                state,
                max(60, int(os.getenv("ANAKOT_NOUS_MIN_KEY_TTL_SECONDS", "1800"))),
            )
            else ""
        )
...
def resolve_runtime_provider(
    *,
    ...
    if pool and pool.has_credentials():
        ...
        # For callmemo, the pool entry's runtime_api_key is the agent_key
        # compatibility field. It must be an invoke JWT. The pool doesn't
        # refresh it during selection (that would trigger network calls in
        # non-runtime contexts like `anakot auth list`).  If the key is
        # expired, clear pool_api_key so we fall through to
        # resolve_callmemo_runtime_credentials() which handles refresh.
        if provider == "nous" and entry is not None and pool_api_key:
            min_ttl = max(60, int(os.getenv("ANAKOT_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
            nous_state = {
            ...
            if not _agent_key_is_usable(nous_state, min_ttl):
                logger.debug("callmemo pool entry agent_key expired/missing, falling through to runtime resolution")
                pool_api_key = ""
    ...
    if provider == "nous":
        try:
            ...
            if requested_provider != "auto":
                raise
            # Auto-detected callmemo but credentials are stale/revoked —
            # fall through to env-var providers (e.g. OpenRouter).
            logger.info("Auto-detected callmemo provider but credentials failed; "
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "anakot-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from anakot_cli.auth import resolve_minimax_oauth_runtime_credentials
    ...
    if provider == "anthropic":
        ...
        if _is_azure_endpoint:
            ...
            # chain.  Accept both `key_env` (Anakot canonical — matches the
            # custom_providers field name) and `api_key_env` (documented in the
            # Azure Foundry guide and read by most Anakot-compatible importers).
