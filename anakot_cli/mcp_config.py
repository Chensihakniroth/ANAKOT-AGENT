...
Implements ``anakot mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.anakot/config.yaml under the ``mcp_servers`` key.
"""
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from anakot_cli.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.anakot/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``anakot mcp login`` to distinguish a genuine authentication
...
def cmd_mcp_add(args):
    ...
    # anakot_cli/main.py for why the dest is renamed.
    ...
    print()
    _success(f"Saved '{name}' to {display_anakot_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `anakot mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def mcp_command(args):
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `anakot plugin`.
        from anakot_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("anakot mcp                                    Open the catalog picker (default)")
        _info("anakot mcp catalog                            List callmemo-approved MCPs")
        _info("anakot mcp install <name>                     Install a catalog MCP")
        _info("anakot mcp serve                              Run as MCP server")
        _info("anakot mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("anakot mcp add <name> --command <cmd>         Add a stdio server")
        _info("anakot mcp add <name> --preset <preset>       Add from a known preset")
        _info("anakot mcp remove <name>                      Remove a server")
        _info("anakot mcp list                               List configured servers")
        _info("anakot mcp test <name>                        Test connection")
        _info("anakot mcp configure <name>                   Toggle tools")
        _info("anakot mcp login <name>                       Re-authenticate OAuth")
        print()
