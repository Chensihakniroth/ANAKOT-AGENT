...
Pure display functions with no AnakotCLI state dependency.
"""
...
ANAKOT_AGENT_LOGO = ANAKOT_AGENT_LOGO
ANAKOT_CADUCEUS = ANAKOT_ICON
...
# (e.g. nix-built anakot — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/callmemo/anakot-agent.git"
...
def _fetch_pypi_latest(package: str = "anakot-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    ...
    Two paths: if ``ANAKOT_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = anakot_home / ".update_check"
    embedded_rev = os.environ.get("ANAKOT_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # ANAKOT_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `anakot update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/anakot/update/check`
    ...
    if embedded_rev:
        ...
        # $ANAKOT_HOME/anakot-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = anakot_home / "anakot-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Anakot git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$ANAKOT_HOME/anakot-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``anakot_cli/build_info.py``) and return it as a frozen
...
_RELEASE_URL_BASE = "https://github.com/callmemo/anakot-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Anakot checkout. Cached per-process. Release URL always points at the
    canonical callmemo/anakot-agent repo (forks don't get a link).
    """
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else ANAKOT_CADUCEUS
    except Exception:
        _bskin = None
        _hero = ANAKOT_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Anakot[/]")
    ...
    try:
        ...
        if behind is not None and behind != 0:
            ...
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built anakot; we know an update
    ...
    # Pip-install warning — `pip install anakot-agent` is not the supported
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else ANAKOT_AGENT_LOGO
        console.print(_logo)
        console.print()
