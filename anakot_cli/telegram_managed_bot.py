...
manual BotFather token copy-paste. Anakot talks only to the callmemo onboarding
...
# Default pairing API base URL (callmemo-hosted Cloudflare Worker).
# Override for PoC/staging with TELEGRAM_ONBOARDING_URL.
DEFAULT_API_URL = "https://setup.anakot-agent.callmemo.ai"
TELEGRAM_ONBOARDING_URL_ENV = "TELEGRAM_ONBOARDING_URL"

# The callmemo-hosted manager bot username (without @). The backend returns the
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Anakot Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
