"""
Unified self-relaunch for Anakot CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``anakot sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``anakot`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
def resolve_anakot_bin() -> Optional[str]:
    """Find the anakot entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("anakot")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m anakot_cli.main``.
    ...
    ``anakot.exe`` on PATH or the ``sys.executable -m anakot_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def relaunch(
    ...
    *,
    ...
    relaunch contract wants: "run anakot again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``anakot.exe`` (a Python console-script shim that wraps
    ``python -m anakot_cli.main``) or a ``.cmd`` batch file, and both
    ...
    That's functionally equivalent — the user sees "anakot exited, then
    new anakot started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # cryptic.  Common causes: ``anakot`` not on PATH yet (install
