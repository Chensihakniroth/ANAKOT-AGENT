...
- ``anakot_cli.providers``        -- canonical provider identity + overlays
- ``anakot_cli.model_normalize``  -- per-provider name formatting
...
_ANAKOT_MODEL_WARNING = (
    "callmemo Anakot 3 & 4 models are NOT agentic and are not designed "
    "for use with Anakot Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real callmemo Anakot 3 / Anakot 4 chat families.
# The previous substring check (`"anakot" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``anakot-brain:qwen3-14b-ctx16k`` that just
# happen to carry "anakot" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   callmemo/Anakot-3-Llama-3.1-70B, anakot-4-405b, openrouter/anakot3:70b
# Negative examples it must NOT match:
#   anakot-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_ANAKOT_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])anakot[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
def is_nous_anakot_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real callmemo Anakot 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_ANAKOT_NON_AGENTIC_RE.search(model_name))


def _check_anakot_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a callmemo Anakot 3/4 chat model."""
    if is_nous_anakot_non_agentic(model_name):
        return _ANAKOT_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``anakot config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from anakot_cli.model_switch import
...
def resolve_display_context_length(
    ...
    about Codex OAuth, Copilot, callmemo, and falls back to models.dev for the
    rest.
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'anakot model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'anakot doctor' — config issues detected:"
        ...
        from anakot_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from anakot_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from anakot_cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in anakot_cli.runtime_provider.resolve_runtime_provider;
    ...
    anakot_warn = _check_anakot_model_warning(new_model)
    if anakot_warn:
        warnings.append(anakot_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from anakot_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from anakot_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by anakot provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://github.com/Chensihakniroth/ANAKOT-AGENT/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Anakot release. Falls back to the in-repo
    # _PROVIDER_MODELS["nous"] snapshot when the manifest is unreachable.
    curated["nous"] = get_curated_callmemo_model_ids()
    ...
    # --- 1. Check Anakot-mapped providers ---
    from anakot_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from anakot_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for anakot_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(anakot_id)
        if (
            _alias_target
            and _alias_target != anakot_id
        ...
        # section 2 (ANAKOT_OVERLAYS) with proper auth store checking.
        ...
        # /model picker sees the SAME list `anakot model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(anakot_id)
        if not model_ids:
            model_ids = curated.get(anakot_id, [])
            if anakot_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(anakot_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Anakot-only providers (nous, openai-codex, copilot, opencode-go) ---
    from anakot_cli.providers import ANAKOT_OVERLAYS
    from anakot_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Anakot provider ID.
    # ANAKOT_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Anakot IDs ("copilot").
    _mdev_to_anakot = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in ANAKOT_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Anakot slug — e.g. "github-copilot" → "copilot"
        anakot_slug = _mdev_to_anakot.get(pid, pid)
        if anakot_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(anakot_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, anakot_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or anakot_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds and anakot_slug == "anthropic":
            try:
                ...
                if (anakot_creds and anakot_creds.get("accessToken")) or \
        ...
        if anakot_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(anakot_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(anakot_slug)
                model_ids = _ids if _ids else (curated.get(anakot_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(anakot_slug, []) or curated.get(pid, [])
        elif anakot_slug == "nous":
            # callmemo serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `anakot model` picker deliberately shows ONLY the curated agentic
            ...
            # order. Mirror that exactly (see _model_flow_callmemo in main.py) so
            ...
            if not model_ids:
                model_ids = curated.get(anakot_slug, []) or curated.get(pid, [])
                if anakot_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(anakot_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": anakot_slug,
            "name": get_label(anakot_slug),
            "is_current": anakot_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "anakot",
        ...
        seen_slugs.add(anakot_slug.lower())
        _record_builtin_endpoint(anakot_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or ANAKOT_OVERLAYS (keeps /model in sync
    # with `anakot model`).
    try:
        from anakot_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Anakot's canonical write key (matches
            ...
            # Anakot writes ``models:`` as a dict keyed by model id
            # (see anakot_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Anakot's own writer uses; a hyphen variant is accepted
            ...
            # active model. Anakot's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, anakot_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`anakot_cli.models.fetch_openrouter_models`, which filters the
