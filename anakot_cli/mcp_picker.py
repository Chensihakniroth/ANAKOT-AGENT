...
``anakot mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `anakot plugin` picker UX: arrow keys to navigate, ENTER on a row
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `anakot mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: anakot mcp install <name>    Picker: anakot mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Anakot is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Anakot — run `anakot update` "
...
def run_picker() -> None:
    """`anakot mcp picker` (and default `anakot mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    ...
    if entry is None:
        print(color(
            ...
            "Run `anakot mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
