...
Outputs a compact, plain-text summary of the user's Anakot setup
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.anakot_build_sha`` by the Dockerfile's
    ``ANAKOT_GIT_SHA`` build-arg (see ``anakot_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``anakot dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def run_dump(args):
    ...
    lines.append(f"anakot_home:      {display_anakot_home()}")
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(anakot_home)}")
    lines.append(f"  skills:             {_count_skills(anakot_home)}")
