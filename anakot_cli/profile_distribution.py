...
A distribution is a Anakot profile published as a git repository (or
...
* ``anakot profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``anakot skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``anakot profile``, not a parallel tree):

    anakot profile install <source> [--name N] [--alias] [--force] [--yes]
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            anakot_requires=str(data.get("anakot_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.anakot_requires:
            out["anakot_requires"] = self.anakot_requires
        if self.author:
...
def check_anakot_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Anakot {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``anakot profile update`` can
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Anakot distribution."
        )

    # Version check up-front so we fail fast
    check_anakot_requires(manifest.anakot_requires, anakot_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.anakot).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
    ...
    with tempfile.TemporaryDirectory(prefix="anakot_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `anakot profile update` to upgrade in place, "
