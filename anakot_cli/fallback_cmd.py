"""
anakot fallback — manage the fallback provider chain.
...
Subcommands:
  anakot fallback [list]   Show the current fallback chain (default when no subcommand)
  anakot fallback add      Pick provider + model via the same picker as `anakot model`,
                           then append the selection to the chain
  anakot fallback remove   Pick an entry to delete from the chain
  anakot fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.anakot/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `anakot model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    print("  Docs: https://github.com/Chensihakniroth/ANAKOT-AGENT/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    ...
    print("  `anakot model` — select the provider + model you want as a fallback.")
    print()
