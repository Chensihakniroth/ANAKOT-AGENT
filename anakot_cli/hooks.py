...
Usage::
    ...
    anakot hooks list
    anakot hooks test <event> [--for-tool X] [--payload-file F]
    anakot hooks revoke <command>
    anakot hooks doctor

Consent records live under ``~/.anakot/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.anakot/config.yaml``
...
# stdin a script sees under `anakot hooks test` and `anakot hooks doctor`
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    # is already allowlisted.  Otherwise `anakot hooks doctor` would execute
