...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m anakot_cli.main gateway run --replace`` invocation) and
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "anakot_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("ANAKOT_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("ANAKOT_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("ANAKOT_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["ANAKOT_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["ANAKOT_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["ANAKOT_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("ANAKOT_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("ANAKOT_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("ANAKOT_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    Default profile: ``Anakot_Gateway``
    Named profile X: ``Anakot_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during anakot_cli boot.
    from anakot_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\anakot\\gateway-service\\<task_name>.cmd``
    (or ``<ANAKOT_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Anakot installs stay self-contained).
    """
    _assert_windows()
    from anakot_cli.config import get_anakot_home

    script_dir = Path(get_anakot_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``ANAKOT_HOME`` whenever it
    ...
    source checkout only if ``ANAKOT_HOME`` cannot be resolved yet.
    """
    from anakot_cli.config import get_anakot_home
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports ANAKOT_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m anakot_cli.main [--profile X] gateway run``
    ...
    lines.append(f'set "ANAKOT_HOME={anakot_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "ANAKOT_GATEWAY_DETACHED=1"')
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports anakot_constants-based logic during startup.
    venv_dir = str(Path(python_path).resolve().parent.parent)
    lines.append(f'set "VIRTUAL_ENV={venv_dir}"')

    pythonw_path = _derive_venv_pythonw(python_path)
    prog_args = [pythonw_path, "-m", "anakot_cli.main"]
    if profile_arg:
...
def _write_task_script() -> Path:
    ...
    anakot_home = str(Path(get_anakot_home()).resolve())
    profile_arg = _profile_arg(anakot_home)

    content = _build_gateway_cmd_script(python_path, working_dir, anakot_home, profile_arg)
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Anakot builds and failed
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m anakot_cli.main gateway run``
    ...
    log_dir = Path(get_anakot_home()) / "logs"
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("ANAKOT_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("ANAKOT_GATEWAY_INSTALL_START_ON_LOGIN")
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `anakot -p <profile> gateway install` must be safe.
    ...
    from anakot_cli.gateway import find_gateway_pids, _profile_arg
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: anakot gateway start")
        return
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from anakot_cli.config import get_anakot_home
        print(f"    type {Path(get_anakot_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_anakot_home()).resolve()}\\logs\\gateway-stdio.log")
...
def _print_next_steps() -> None:
    ...
    print(f"  type {anakot_home}\\logs\\gateway.log       # View logs")
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: anakot gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: anakot gateway start")
            return
...
def stop() -> None:
    ...
    """
    _assert_windows()
    from anakot_cli.gateway import kill_gateway_processes, _get_restart_drain_timeout
    from gateway.status import get_running_pid
