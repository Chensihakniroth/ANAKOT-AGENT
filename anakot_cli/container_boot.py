...
``$ANAKOT_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-anakot-setup
(the stage2 hook) has chowned the volume and seeded $ANAKOT_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$ANAKOT_HOME``,
    not under ``profiles/``). The dispatcher in ``anakot_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``anakot gateway start`` (no ``-p``) targets. Without it,
    bare ``anakot gateway start`` inside the container would land on
    ...
    ``$ANAKOT_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        anakot_home: The container's ANAKOT_HOME (typically /opt/data).
            Profiles live under ``<anakot_home>/profiles/<name>/``;
            the default profile lives at ``<anakot_home>`` itself.
    ...
    # ``anakot gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_prior_state(anakot_home)
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `anakot profile create` (config.yaml
            # is not — that comes later via `anakot setup`). Use it as the
            ...
            # via ``anakot -p default-named gateway start`` if they
...
def _is_legacy_gateway_run_request(argv: Sequence[str]) -> bool:
    ...
    if args and args[0].endswith("main-wrapper.sh"):
        args = args[1:]
    if args and Path(args[0]).name == "anakot":
        args = args[1:]
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `anakot -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit anakot
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the anakot user) won't hit EACCES. See
...
def _write_reconcile_log(
    anakot_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $ANAKOT_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    actions = reconcile_profile_gateways(
        anakot_home=anakot_home, scandir=scandir,
    )
    for a in actions:
        print(
