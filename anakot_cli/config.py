"""
Configuration management for Anakot Agent.

Config files are stored in ~/.anakot/ for easy access:
- ~/.anakot/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.anakot/.env         - API keys and secrets

This module provides:
- anakot config          - Show current configuration
- anakot config edit     - Open config in editor
- anakot config set      - Set a specific value
- anakot config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``anakot config set`` (which rewrites
    ...
    ``config.yaml`` in place: anakot never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.anakot/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``anakot logs``
...
#   the next ``subprocess.run([...])`` Anakot makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Anakot itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``anakot update``, the TUI build.
...
#   on every plugin install / ``anakot update``.
...
# * ``ANAKOT_HOME`` / ``ANAKOT_PROFILE`` / ``ANAKOT_CONFIG`` /
#   ``ANAKOT_ENV`` — Anakot runtime location flags. Writing these into
...
#
# IMPORTANT: ``ANAKOT_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (ANAKOT_GEMINI_CLIENT_ID,
# ANAKOT_LANGFUSE_PUBLIC_KEY, ANAKOT_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Anakot runtime location — never via dashboard env writer.
    # NOT a ANAKOT_* blanket: integration credentials (ANAKOT_GEMINI_*,
    # ANAKOT_LANGFUSE_*, ANAKOT_SPOTIFY_*, ...) ARE allowed.
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Anakot runtime location "
            "(ANAKOT_HOME, ANAKOT_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `anakot plugins enable
    # observability/langfuse` or `anakot tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "ANAKOT_LANGFUSE_ENV",
    "ANAKOT_LANGFUSE_RELEASE",
    "ANAKOT_LANGFUSE_SAMPLE_RATE",
    "ANAKOT_LANGFUSE_MAX_CHARS",
    "ANAKOT_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ANAKOT_MANAGED", "").strip()
    if raw:
...
def is_managed() -> bool:
    ...
    Two signals: the ANAKOT_MANAGED env var (set by the systemd service),
    or a .managed marker file in ANAKOT_HOME (set by the NixOS activation
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Anakot was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.anakot/.install_method`` file (written by installers)
    2. ANAKOT_MANAGED env / .managed marker (NixOS, Homebrew)
    ...
    so ``anakot update`` bailed with "doesn't apply inside the Docker
    ...
    """
    stamp = get_anakot_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.anakot/.install_method."""
    stamp = get_anakot_home() / ".install_method"
    try:
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Anakot lives in a ``uv tool`` layout.

    ``uv tool install anakot-agent`` places the install at
    ``.../uv/tools/anakot-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``anakot-agent`` happens to be uv-tool-installed on the machine while
    the *active* Anakot is a regular pip/venv install, causing
    ``anakot update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/anakot-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "pip":
        ...
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade anakot-agent"
        return "pip install --upgrade anakot-agent"
    return "anakot update"
...
# Long-form text for ``anakot update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# anakot_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Anakot, it doesn't update
...
def format_managed_message(action: str = "modify this Anakot installation") -> str:
    ...
    raw = os.getenv("ANAKOT_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Anakot installation is managed by NixOS "
            f"(ANAKOT_MANAGED={env_hint}).\n"
            "Edit services.anakot-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Anakot installation is managed by Homebrew "
            f"(ANAKOT_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade anakot-agent"
        )
...
def get_container_exec_info() -> Optional[dict]:
    ...
    Returns a dict with keys: backend, container_name, exec_user, anakot_bin
    or None if container mode is not active, we're already inside the
    container, or ANAKOT_DEV=1 is set.
    ...
    container_mode_file = get_anakot_home() / ".container-mode"
...
from anakot_constants import get_anakot_home  # noqa: F811,E402
...
def _resolve_anakot_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the ANAKOT_UID / ANAKOT_GID env vars set by Docker deployments.

    Docker containers running Anakot commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level ANAKOT_HOME once, but
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("ANAKOT_UID", "").strip()
    gid_str = os.environ.get("ANAKOT_GID", "").strip()
    try:
...
def _chown_to_anakot_uid(path) -> None:
    """Chown ``path`` to ``ANAKOT_UID:ANAKOT_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_anakot_home` on Docker deployments.
    See #34107.
    """
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the anakot group can
    share state with the gateway service.

    The mode can be overridden via the ANAKOT_HOME_MODE environment variable
    (e.g. ANAKOT_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ANAKOT_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``ANAKOT_UID``/``ANAKOT_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ANAKOT_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Anakot runs in a container with volume-mounted config files, forcing
...
def _secure_file(path):
    ...
    permissions.  Set ANAKOT_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ANAKOT_HOME if the user doesn't have one yet."""
...
def ensure_anakot_home():
    """Ensure ~/.anakot directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_anakot_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_anakot_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ANAKOT_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Anakot-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Anakot
        ...
        # identity slot (SOUL.md). Empty by default. The ANAKOT_ENVIRONMENT_HINT
    ...
    "terminal": {
        ...
        # skipped. When empty, Anakot auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Anakot sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Anakot
        ...
        # bundled Anakot image, which drops to the `anakot` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Anakot sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Anakot should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``anakot chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.anakot/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: anakot sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Anakot truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``anakot kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``anakot kanban decompose`` and the kanban
        ...
        # ``anakot profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `anakot model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `anakot` (and `anakot chat`) launches by default:
        ...
        # users aren't surprised.  ANAKOT_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `anakot --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/anakot`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # ``--insecure`` is not). The bundled callmemo Portal plugin reads
        # both keys at startup; they are the canonical surface for these
        # settings. Each can be overridden by an environment variable —
        # ``ANAKOT_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``ANAKOT_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # ``portal_url`` defaults to https://portal.callmemo.ai).
        "oauth": {
        ...
        # Public URL override (env: ``ANAKOT_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/anakot``) —
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.anakot/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.anakot/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # configured providers (OpenRouter, callmemo, Z.ai, Kimi, etc.) are supported.
    "delegation": {
    ...
    # response. If not, Anakot feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Anakot auto-pauses the goal and
    ...
    # always goes to ~/.anakot/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${ANAKOT_SKILL_DIR} and ${ANAKOT_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `anakot curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # normally restored on every `anakot update`, so pruning them only
        ...
        # skipped), snapshot ~/.anakot/skills/ into
        # ~/.anakot/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `anakot curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for anakot-specific overrides; everything else
 ...
 # Default (None) uses the built-in " *Anakot Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (ANAKOT_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.anakot/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ANAKOT_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `anakot doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``anakot_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Anakot to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via ANAKOT_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `anakot -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `anakot` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `anakot kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     anakot-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Anakot tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.anakot/logs/.
    ...
    # curated model lists for OpenRouter and callmemo Portal from this URL,
    # falling back to the in-repo snapshot on network failure.  Lets us
    # update model picker lists without shipping a anakot-agent release.
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.anakot/.env,
        ...
        # behavior: files must live under the Anakot cache, under
        ...
        # user. Bridged to ANAKOT_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Anakot
        # cache (~/.anakot/cache/{documents,images,audio,video,screenshots})
        ...
        # to ANAKOT_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # ANAKOT_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to ANAKOT_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.anakot/state.db.
    ...
    "sessions": {
        ...
        # default of ``anakot sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.anakot/sessions/session_{sid}.json`` on every turn
    ...
    # ``anakot update`` behaviour.
    "updates": {
        # Run a full ``anakot backup``-style zip of ANAKOT_HOME before every
        # ``anakot update``.  Backups land in ``<ANAKOT_HOME>/backups/`` and
        # can be restored with ``anakot import <path>``.  Off by default —
        # on large ANAKOT_HOME directories the zip can add minutes to every
        ...
        "backup_keep": 5,
        # What `anakot update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<ANAKOT_HOME>/lsp/bin/`` on first use.
    ...
    # `anakot tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.anakot/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.anakot/.env (or your shell) and never in config.yaml.
            ...
            # ~/.anakot/bin/ on first use.  When False you must install
...
# selection step (callmemo Portal / OpenRouter / Custom endpoint), so this
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'anakot model' for endpoint-specific config)",
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override for callmemo Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for callmemo Subscribers only, used to derive vendor hosts, e.g. callmemo.ai -> firecrawl-gateway.callmemo.ai",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme for callmemo Subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit callmemo Subscriber access token for tool-gateway requests (optional; otherwise read from the Anakot auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @anakot:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ANAKOT_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #anakot)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: anakot-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'anakot-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Anakot API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Anakot API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Anakot API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # ANAKOT_TOOL_PROGRESS and ANAKOT_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.anakot.config`` entries, then checks
    ...
    try:
        ...
        # should never break `anakot update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Anakot versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``anakot_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("ANAKOT_HOME", "~/.anakot")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("ANAKOT_TOOL_PROGRESS")
            old_mode = get_env_value("ANAKOT_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from ANAKOT_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from ANAKOT_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("ANAKOT_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from ANAKOT_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$ANAKOT_HOME/plugins/`` for currently installed user plugins.
    ...
    # settings in their `config.yaml`, and `anakot curator status` has no
    ...
    #   3. Creates `~/.anakot/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_anakot_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_anakot_home() / "logs" / "curator"
    ...
    # metadata.anakot.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _anakot_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.anakot/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.anakot/config.yaml.
    ...
    (which change ``ANAKOT_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   nous         (OAuth — anakot auth) — callmemo Portal
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.anakot/.env.
    ...
    interactive menu render (`anakot tools`, `anakot setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `anakot tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Anakot env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.anakot/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.anakot/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.anakot/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.anakot/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Anakot — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Anakot vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.anakot/.env or environment."""
...
def show_config():
    ...
    # Warn on stale ANAKOT_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("ANAKOT_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale ANAKOT_MAX_ITERATIONS={_env_ghost} "
                f"(run 'anakot doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: anakot config set <key> <value>")
            print()
            print("Examples:")
            print("  anakot config set model anthropic/claude-sonnet-4")
            print("  anakot config set terminal.backend docker")
            print("  anakot config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        print("  anakot config           Show current configuration")
        print("  anakot config edit      Open config in editor")
        print("  anakot config set <key> <value>   Set a config value")
        print("  anakot config check     Check for missing/outdated config")
        print("  anakot config migrate   Update config with new options")
        print("  anakot config path      Show config file path")
        print("  anakot config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``anakot config`` UI so users
