...
``<root>`` is the **shared Anakot root** (the parent of any active
...
``anakot -p <profile>`` joins the same board as the dispatcher that
...
the profile's Anakot home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``ANAKOT_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``ANAKOT_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``anakot kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.anakot``. In Docker / custom
deployments where ``ANAKOT_HOME`` points outside ``~/.anakot`` (e.g.
``/opt/anakot``), ``<root>`` is ``ANAKOT_HOME``. Legacy env-var
overrides still work:

* ``ANAKOT_KANBAN_DB`` — pin the database file path directly.
* ``ANAKOT_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``ANAKOT_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``ANAKOT_KANBAN_DB``,
``ANAKOT_KANBAN_WORKSPACES_ROOT``, and ``ANAKOT_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/anakot-kanban-v1-spec.pdf`` for the full
...
# ``ANAKOT_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``ANAKOT_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("ANAKOT_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``ANAKOT_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("ANAKOT_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``ANAKOT_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "anakot_kanban_current_board_override",
    default=None,
)
...
# enough that kebab-case names like ``atm10-server`` or ``anakot-agent``
...
def kanban_home() -> Path:
    """Return the shared Anakot root that anchors the kanban board.

    Resolution order:

    1. ``ANAKOT_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_anakot_root()``, which already returns ``<root>``
       when ``ANAKOT_HOME`` is ``<root>/profiles/<name>``, and returns
       ``ANAKOT_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``ANAKOT_HOME`` would silently fork the board per profile,
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``anakot kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``ANAKOT_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``anakot kanban boards
    ...
    env = os.environ.get("ANAKOT_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``anakot kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``ANAKOT_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("ANAKOT_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``ANAKOT_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``ANAKOT_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``anakot kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``ANAKOT_SESSION_ID``. NULL for
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("ANAKOT_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Anakot treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``ANAKOT_KANBAN_DB`` env → ``ANAKOT_KANBAN_BOARD`` env →
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See anakot_state._WAL_INCOMPAT_MARKERS for detection logic.
                from anakot_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``anakot kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``ANAKOT_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def create_task(
    ...
    *,
    ...
    each name to ``anakot --skills ...`` alongside the built-in
    ...
    # `anakot --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `anakot kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``anakot kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``anakot kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``anakot kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``anakot kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``ANAKOT_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Anakot' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Anakot-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``anakot kanban show <id>`` and the dashboard.
...
# without thrashing. Overridable via ``ANAKOT_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Anakot
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``anakot kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Anakot profile.
    ...
    if not rows:
        return False
    try:
        from anakot_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def dispatch_once(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from anakot_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Anakot profile.
        # `_default_spawn` invokes ``anakot -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Anakot
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `anakot kanban tail` — without
...
def _module_anakot_argv() -> list[str]:
    """Return the interpreter-bound Anakot CLI invocation."""
    # ``anakot_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``anakot`` package — there is no
    # ``anakot`` package to import.
...
def _anakot_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Anakot executable path.
...
def _resolve_anakot_argv() -> list[str]:
    """Resolve the ``anakot`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$ANAKOT_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("anakot")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\anakot.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m anakot_cli.main`` — fallback for setups where
       Anakot is launched from a venv and the ``anakot`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_anakot_bin`` for the same reason. Kept
    local (not imported from gateway) because ``anakot_cli`` sits below
    ...
    anakot_bin = _safe_which_no_cwd("anakot") if _IS_WINDOWS else shutil.which("anakot")
    if anakot_bin:
        return _anakot_path_argv(anakot_bin)
    return _module_anakot_argv()


def _kanban_worker_skill_available(anakot_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``anakot -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset ANAKOT_HOME means the worker falls back to the default root
    # home (``~/.anakot``), which ships the bundled skill.
    base = _Path(anakot_home) if anakot_home else (_Path.home() / ".anakot")
...
def _default_spawn(
    ...
    *,
    ...
    ``ANAKOT_KANBAN_DB`` / ``ANAKOT_KANBAN_BOARD`` / workspaces_root env
    ...
    # env, and when the child process starts `anakot -p <name>` the
    # _apply_profile_override() runs *before* anakot_constants is imported.
    # If ANAKOT_HOME is absent from the child's env, get_anakot_home() falls
    # back to Path.home() / ".anakot" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from anakot_cli.profiles import resolve_profile_env
    try:
        env["ANAKOT_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via ANAKOT_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # ANAKOT_HOME never had profiles created.
        pass
    if task.tenant:
    ...
    # that even when the worker activates a profile (`anakot -p <name>`
    # rewrites ANAKOT_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_anakot_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["ANAKOT_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["ANAKOT_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["ANAKOT_KANBAN_BOARD"] = resolved_board
    # ANAKOT_PROFILE is the author the kanban_comment tool defaults to.
    # `anakot -p <assignee>` activates the profile, but the env var is
    ...
    cmd = [
        *_resolve_anakot_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped ANAKOT_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("ANAKOT_HOME")):
    ...
    # `anakot kanban log` on a specific board reads its own file and
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``anakot kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled ANAKOT_PROFILE values like "anakot-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Anakot root exists

    Reads profile paths directly so this module has no import dependency on
    ``anakot_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from anakot_constants import get_default_anakot_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``anakot kanban assignees`` for the terminal.
