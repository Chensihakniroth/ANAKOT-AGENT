"""Migrate Anakot' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Anakot' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Anakot mcp_servers.<n>.command/args/env  → codex stdio transport
  Anakot mcp_servers.<n>.url/headers       → codex streamable_http transport
  Anakot mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Anakot mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Anakot-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Anakot' own
...
# Anakot keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Anakot' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, anakot_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Anakot MCP server config to the codex inline-table dict
    ...
    has_command = bool(anakot_cfg.get("command"))
    has_url = bool(anakot_cfg.get("url"))
    ...
    # Timeouts
    if "timeout" in anakot_cfg:
        try:
            out["tool_timeout_sec"] = float(anakot_cfg["timeout"])
        except (TypeError, ValueError):
            skipped.append("timeout (not numeric)")
    if "connect_timeout" in anakot_cfg:
        try:
            out["startup_timeout_sec"] = float(anakot_cfg["connect_timeout"])
        except (TypeError, ValueError):
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (ANAKOT_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Anakot)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Anakot' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Anakot' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Anakot
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a ANAKOT_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed anakot-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.anakot`` symlink that happens to live under ``/private/var/folders``
...
def _build_anakot_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Anakot' own
    ...
    so a anakot installed under /opt/, /usr/local/, or a venv all work.
    ANAKOT_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # ANAKOT_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_anakot_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever ANAKOT_HOME its launcher sets
    ...
    # — that would override the launcher's ANAKOT_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("ANAKOT_HOME", tmp_path) would
    ...
    if anakot_home and _looks_like_test_tempdir(anakot_home):
        anakot_home = ""
    if anakot_home:
        env["ANAKOT_HOME"] = anakot_home
    # PYTHONPATH passes through so a worktree-launched anakot finds the
...
def migrate(
    ...
    *,
    ...
    expose_anakot_tools: bool = True,
) -> MigrationReport:
    """Translate Anakot mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        anakot_config: full ~/.anakot/config.yaml dict
        ...
        expose_anakot_tools: when True (default), register Anakot' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Anakot for tools
    ...
    if not isinstance(anakot_servers, dict):
        report.errors.append(
            "mcp_servers in Anakot config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in anakot_servers.items():
    ...
    # Inject Anakot' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Anakot for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/anakot_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_anakot_tools:
        translated["anakot-tools"] = _build_anakot_tools_mcp_entry()
        if "anakot-tools" not in report.migrated:
            report.migrated.append("anakot-tools")
