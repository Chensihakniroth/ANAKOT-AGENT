...
the existing module-level functions in anakot_cli.gateway and
anakot_cli.gateway_windows directly. This protocol is a thin facade
...
``anakot gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``anakot gateway start/stop/restart``).
    """
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged anakot
    ...
    detection always fails. Since every Anakot runtime call inside the
    container drops to anakot via ``s6-setuidgid``, that silent failure
...
# in ``anakot_cli.gateway`` (systemd/launchd) and ``anakot_cli.gateway_windows``
...
# active (set via the ``anakot -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in anakot_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from anakot_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from anakot_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from anakot_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from anakot_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in anakot_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from anakot_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from anakot_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from anakot_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from anakot_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``anakot_cli.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `anakot profile create`
# runs inside the container (Phase 4). Static services (main-anakot, dashboard)
...
# UID/GID of the in-image ``anakot`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the anakot user.
    ...
    (the ``anakot`` user — UID 10000 — running every Anakot runtime
    ...
    is dispatched as the anakot user.
    ...
    with anakot ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           anakot:anakot, 0755 (parent must already exist)
    ``svc_dir/event/``                     anakot:anakot, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 anakot:anakot, 0755
    ``svc_dir/supervise/event/``           anakot:anakot, 03730
    ``svc_dir/supervise/control``          anakot:anakot, 0660    (FIFO)
    ...
    slot's supervise/ was anakot-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        try:
            ...
            # Running as the anakot user already — directory is anakot-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _ANAKOT_UID, _ANAKOT_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is anakot-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _ANAKOT_UID, _ANAKOT_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``anakot -p typo gateway start`` when no profile
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-anakot, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources ANAKOT_HOME (and any extra env) via with-contenv —
             so e.g. ``-e ANAKOT_HOME=/data/anakot`` is honored at run
          ...
          4. Drops to the anakot user and exec's
             ``anakot -p <profile> gateway run`` (or just ``anakot
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``anakot gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root ANAKOT_HOME profile" (the implicit profile that exists at
        the top of $ANAKOT_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``anakot_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$ANAKOT_HOME/profiles/default/`` — a
        ...
        ``anakot_cli.profiles._allocate_gateway_port``). PR #30136
        ...
        if profile == "default":
            gateway_cmd = "anakot gateway run"
        else:
            gateway_cmd = f"anakot -p {shlex.quote(profile)} gateway run"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${ANAKOT_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the ANAKOT_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e ANAKOT_HOME=/data/anakot``
        gets its logs under /data/anakot/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``anakot logs`` reads and what
        ...
        return (
            ...
            f'mkdir -p "$log_dir"\n'
            f'chown -R anakot:anakot "$log_dir" 2>/dev/null || true\n'
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with anakot ownership
            ...
            # the anakot user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is anakot-owned (see ``_seed_supervise_skeleton``), and on
