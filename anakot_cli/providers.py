...
2. **Anakot overlays** — transport type, auth patterns, aggregator flags,
...
# -- Anakot overlay ----------------------------------------------------------
# Anakot-specific metadata that models.dev doesn't provide.
...
ANAKOT_OVERLAYS: Dict[str, AnakotOverlay] = {
    ...
    "openai-api": AnakotOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": AnakotOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": AnakotOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="ANAKOT_QWEN_BASE_URL",
    ),
    "google-gemini-cli": AnakotOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": AnakotOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "omniroute": AnakotOverlay(
        transport="openai_chat",
        auth_type="api_key",
        extra_env_vars=("OMNIROUTE_API_KEY",),
        base_url_override="http://localhost:20128/v1",
        base_url_env_var="OMNIROUTE_BASE_URL",
    ),
    "copilot-acp": AnakotOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": AnakotOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": AnakotOverlay(
    ...
    "alibaba": AnakotOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": AnakotOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "opencode": AnakotOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": AnakotOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "huggingface": AnakotOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="HF_BASE_URL",
    ),
    "novita": AnakotOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": AnakotOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": AnakotOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": AnakotOverlay(
        transport="openai_chat",
    ...
    "azure-foundry": AnakotOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": AnakotOverlay(
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Anakot overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Anakot overlay
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + anakot extra
    ...
    if overlay is not None:
        # Anakot-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in ANAKOT_OVERLAYS
    if provider == "bedrock":
