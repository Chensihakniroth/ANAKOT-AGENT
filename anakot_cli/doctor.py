"""
Doctor command for anakot CLI.

Diagnoses issues with Anakot Agent setup.
"""
...
from anakot_cli.config import get_project_root, get_anakot_home, get_env_path
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized in {"google / gemini", "gemini"}:
        try:
            from anakot_cli.auth import get_gemini_oauth_auth_status
            return bool((get_gemini_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "minimax":
        try:
            from anakot_cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches anakot_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``anakot --version`` reporting a stale version while
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != anakot_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'anakot update', or set "
            "anakot_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-anakot and dashboard static services are up
    ...
    for static in ("main-anakot", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `anakot profile create <name>`")
        return
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `anakot`.
    os.environ.setdefault("ANAKOT_INTERACTIVE", "1")

    # Handle `anakot doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.anakot/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Anakot Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`anakot doctor --ack {hit.advisory.id}`."
                )
    ...
    # Detect drift between pyproject.toml and anakot_cli/__init__.py versions
    ...
    # Check ~/.anakot/.env (primary location for user config)
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'anakot setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                issues.append("Run 'anakot setup' to create .env")
    
    # Check ~/.anakot/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from anakot_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'anakot config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
                ...
                try:
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.anakot/.env or run 'anakot setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'anakot setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'anakot config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
    ...
    if config_path.exists():
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'anakot setup' to migrate config")
                else:
                    issues.append("Run 'anakot doctor --fix' or 'anakot setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'anakot doctor --fix'")
        except Exception:
            pass

        # Detect stale ANAKOT_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives ANAKOT_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            ...
            if drift:
                check_warn(
                    f"ANAKOT_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `anakot setup` run)",
                )
                if should_fix:
                    if remove_env_value("ANAKOT_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale ANAKOT_MAX_ITERATIONS from .env "
                        ...
                        manual_issues.append(
                            "Manually delete the ANAKOT_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale ANAKOT_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'anakot doctor --fix'"
                    )
        except Exception:
            pass
    ...
    try:
        ...
        if nous_status.get("logged_in"):
            check_ok("callmemo Portal auth", "(logged in)")
        else:
            check_warn("callmemo Portal auth", "(not logged in)")
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Anakot' own device-code flow — the Codex CLI is
    ...
    # disrupt the already-printed callmemo/Codex/Gemini/MiniMax rows above.
    try:
        from anakot_cli.auth import get_xai_oauth_auth_status
    ...
    for subdir_name in expected_subdirs:
        subdir_path = anakot_home / subdir_name
    ...
    wal_path = anakot_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'anakot doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "anakot"
        ...
        _cmd_link = _cmd_link_dir / "anakot"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(anakot not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/anakot → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/anakot points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/anakot → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/anakot — run 'anakot doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/anakot exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/anakot not found",
                    "(anakot command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/anakot → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/anakot symlink — run 'anakot doctor --fix'")
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `anakot doctor` invocation.
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``anakot doctor``.
    ...
    # never the legitimate source for `anakot doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'anakot setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: anakot skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = ANAKOT_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: anakot memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: anakot memory setup",
                    "No Honcho API key — run 'anakot memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run anakot memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: anakot memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: anakot memory setup")
        except Exception as _e:
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "anakot -p" in content:
                            _m = _re.search(r"anakot -p (\S+)", content)
