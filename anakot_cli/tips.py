...
TIPS = [
    ...
    "anakot chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    ...
    "The plan skill saves markdown plans under .anakot/plans/ in the active workspace.",
    ...
    # --- MCP ---
    "anakot mcp opens an interactive picker of callmemo-approved MCPs you can install in one keystroke.",
    "anakot mcp catalog lists callmemo-approved MCP servers shipped with the repo.",
    ...
    "Each profile gets its own subprocess HOME at ANAKOT_HOME/home/ — isolated git, ssh, npm, gh configs.",
    ...
    "Skills from trusted repos (callmemo) get a 'trusted' security level; community skills get extra scanning.",
    ...
    'anakot auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    'credential_pool_strategies.<provider>: round_robin cycles keys evenly instead of the fill_first default.',
    'use_gateway: true per-tool routes web, image, tts, or browser through your callmemo subscription — no extra keys.',
    ...
    'ANAKOT_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'ANAKOT_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    ...
    'anakot import restores a session export or profile archive produced by sessions export or profile export.',
