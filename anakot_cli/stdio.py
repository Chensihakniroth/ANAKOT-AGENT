...
on Japanese locales, etc.).  Anakot's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``anakot_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Anakot is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``ANAKOT_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("ANAKOT_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Anakot-managed Git install directories so
    ...
    # already-running shells don't see the change — which means anakot
    ...
    # install without Anakot-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Anakot's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Anakot's
    ``anakot config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Anakot (User env var in Windows Settings, or
    export in a PowerShell profile) and Anakot picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Anakot-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\anakot\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``anakot`` from right after install) retain their old PATH.

    Any subprocess Anakot spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Anakot-managed tool directories to our
    ...
    candidate_dirs = [
        ...
        # Anakot venv Scripts directory — host of the anakot.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes anakot, but harmless to include.
