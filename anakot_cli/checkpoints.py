...
store at ``~/.anakot/checkpoints/``.  Actions:

    anakot checkpoints               # same as `status`
    anakot checkpoints status        # total size, project count, breakdown
    anakot checkpoints list          # per-project checkpoint counts + workdir
    anakot checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    anakot checkpoints clear [-f]    # nuke the entire base (asks first)
    anakot checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    anakot checkpoints
    anakot checkpoints prune --retention-days 3 --max-size-mb 200
    anakot checkpoints clear -f
