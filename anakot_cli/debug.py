...
Currently supports:
    anakot debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.anakot/logs/*.log`` are not leaked into
...
def _pending_file() -> Path:
    """Path to ``~/.anakot/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``anakot debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``anakot debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, Anakot version, provider, which API keys
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "anakot-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``anakot debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.anakot/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``anakot debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "anakot-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``anakot dump`` is run
        internally.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``anakot debug share`` (CLI) and the
...
def run_debug_share(args):
    ...
    print(f"To delete now:  anakot debug delete <url>")
...
def run_debug_delete(args):
    ...
    if not urls:
        ...
        print("  Deletes paste.rs pastes uploaded by 'anakot debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``anakot debug`` call.
    ...
    # best-effort — any failure is swallowed so ``anakot debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: anakot debug <command>")
        print()
        print("Commands:")
