...
Mirrors the structure of ``anakot_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `anakot bundles list | grep …` doesn't
...
def register_cli(subparser) -> None:
    """Build the ``anakot bundles`` argparse tree.

    Called from ``anakot_cli/main.py`` where it owns the top-level
