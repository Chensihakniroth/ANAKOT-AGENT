"""callmemo Portal upstream adapter.

Reads the user's callmemo OAuth state from ``~/.anakot/auth.json`` through the
...
# Endpoints inference-api.callmemo.ai actually serves. Anything else
...
class NousPortalAdapter(UpstreamAdapter):
    """Proxy upstream for the callmemo Portal inference API."""

    def __init__(self) -> None:
        # Serialize proxy requests in this process; cross-process token refresh
        # and persistence are handled by resolve_callmemo_runtime_credentials().
    ...
    def get_retry_credential(
        ...
        *,
        ...
        if status_code != 401:
            return None
        logger.info("proxy: callmemo upstream rejected bearer; force-refreshing invoke JWT")
        return self._get_credential(
            force_refresh=True,
        )
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into callmemo Portal. Run `anakot auth add nous` first."
                )

            try:
                refreshed = resolve_callmemo_runtime_credentials(
                ...
                raise RuntimeError(
                    f"Failed to refresh callmemo Portal credentials: {exc}"
                ) from exc
            ...
            if not runtime_key:
                raise RuntimeError(
                    "callmemo Portal refresh did not return a usable inference JWT. "
    ...
    # to anakot_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
    ...
    def _save_state(
        ...
        *,
        ...
        try:
            ...
            logger.warning("proxy: failed to persist callmemo quarantine state: %s", exc)
