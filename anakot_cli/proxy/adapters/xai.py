...
# xAI's public API is OpenAI-compatible for the endpoints Anakot commonly
# uses. The Responses endpoint is included because Anakot' native xAI runtime
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`anakot auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`anakot auth reset xai-oauth` or re-authenticate with "
                    "`anakot auth add xai-oauth --type oauth`."
                )
