...
Powers both:
  - `anakot skills <subcommand>` (CLI argparse entry point)
...
def _resolve_short_name(name: str, sources, console: Console) -> str:
    ...
    if len(exact) > 1:
        ...
        # so users can copy it for `anakot skills install`.
...
def _existing_categories() -> List[str]:
    """Return sorted subdirectory names under ``~/.anakot/skills/`` that look
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print(
            "[bold]Category[/] [dim](optional — press Enter to install flat at ~/.anakot/skills/<name>/)[/]"
        )
    try:
...
def do_search(query: str, source: str = "all", limit: int = 10,
    ...
    # into `anakot skills install` fail.
...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    # skips the external API sources and serves everything from "anakot-index".
    ...
    if official_count > 0 and page == 1:
        c.print(f"[bright_cyan]★ {official_count} official optional skill(s) from callmemo[/]")
    c.print()
    ...
    # The identifier is what you pass to `anakot skills install`. Browse used
...
def do_install(identifier: str, category: str = "", force: bool = False,
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                "[bold bright_cyan]This is an official optional skill maintained by callmemo.[/]\n\n"
                "It ships with anakot-agent but is not activated by default.\n"
                "Installing will copy it to your skills directory where the agent can use it.\n\n"
                f"Files will be at: [cyan]{display_anakot_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]",
...
def browse_skills(page: int = 1, page_size: int = 20, source: str = "all") -> dict:
    ...
    page_size = max(1, min(page_size, 100))
    _TRUST_RANK = {"builtin": 3, "trusted": 2, "community": 1}
    # "anakot-index" must carry a high limit: when the index is available the
    ...
    _PER_SOURCE_LIMIT = {"anakot-index": 5000, "official": 100, "skills-sh": 100,
    ...
    # source-skip logic — querying anakot-index AND the external APIs at once
...
def do_list(source_filter: str = "all",
    ...
    config — ``anakot -p <profile> skills list`` reads that profile's
    ``skills.disabled`` list because ``-p`` swaps ``ANAKOT_HOME`` at process
...
def do_tap(action: str, repo: str = "", console: Optional[Console] = None) -> None:
    ...
    if action == "list":
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: anakot skills tap add owner/repo\n")
            return
        if mgr.add(repo):
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: anakot skills tap remove owner/repo\n")
            return
        if mgr.remove(repo):
    ...
    if target == "github":
        ...
        if not auth.is_authenticated():
            c.print("[bold red]Error:[/] GitHub authentication required.\n"
                    f"Set GITHUB_TOKEN in {display_anakot_home()}/.env or run 'gh auth login'.\n")
            return
...
def skills_command(args) -> None:
    ...
    if action == "browse":
        ...
        _console.print("Usage: anakot skills [browse|search|install|inspect|list|check|update|audit|uninstall|reset|opt-out|opt-in|publish|snapshot|tap]\n")
