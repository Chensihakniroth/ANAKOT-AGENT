...
_GATEWAY_LIFECYCLE_PATTERNS = re.compile(
    r"(?i)"
    r"(anakot\s+gateway\s+(restart|stop|start))"
    r"|(launchctl\s+(kickstart|unload|load|stop|restart)\s+.*anakot)"
    r"|(systemctl\s+(restart|stop|start)\s+.*anakot)"
    r"|(p?kill\s+.*anakot.*gateway)"
)
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'anakot cron create ...' or the /cron command in chat.", Colors.DIM))
        return
    ...
    if not find_gateway_pids():
        ...
        print(color("     Start it with: anakot gateway install", Colors.DIM))
        print(color("                    sudo anakot gateway install --system  # Linux servers", Colors.DIM))
        print()
...
def cron_status():
    ...
    if pids:
        ...
        print("    anakot gateway install    # Install as a user service")
        print("    sudo anakot gateway install --system  # Linux servers: boot-time system service")
        print("    anakot gateway            # Or run in foreground")
...
def cron_create(args):
    ...
    if _contains_gateway_lifecycle_command(combined):
        print(color(
            ...
            "Use `anakot gateway restart` from a shell outside the gateway.",
            Colors.RED,
        ))
        return 1
...
def cron_command(args):
    ...
    print("Usage: anakot cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
