...
def auth_add_command(args) -> None:
    ...
    if provider == "nous":
        # Codex-style auto-import: if a shared callmemo credential lives at
        # <anakot-root>/shared/nous_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `anakot --profile <name>
        ...
        if shared:
            ...
            print()
            if path:
                print(f"Found existing callmemo OAuth credentials at {path}")
            else:
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Anakot reads from
