...
- ``list_authenticated_providers`` already populates each row's
  ...
  curation and pulls in non-agentic models (callmemo /models returns ~400
...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      for callmemo, ``free_tier``/``unavailable_models`` so the GUI picker can
      show $/Mtok columns and gate paid models on free accounts —
      mirroring the ``anakot model`` CLI picker. Adds network calls
      (pricing fetch + callmemo tier check); only set for interactive pickers.
...
def _append_unconfigured_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    from anakot_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
...
def _apply_pricing(rows: list[dict]) -> None:
    """Enrich each provider row with per-model pricing + callmemo tier gating.
    ...
    For callmemo additionally adds::
