"""
Interactive setup wizard for Anakot Agent.
...
from tools.tool_backend_helpers import managed_callmemo_tools_enabled
...
# display_anakot_home imported lazily at call sites (stale-module safety during anakot update)
...
from anakot_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from anakot_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print_info("Configure Anakot using environment variables or config commands:")
    print_info("  anakot config set model.provider custom")
    print_info("  anakot config set model.base_url http://localhost:8080/v1")
    print_info("  anakot config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'anakot setup' in an interactive terminal to use the full wizard.")
    print()
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'anakot setup')")
...
def _print_setup_summary(config: dict, anakot_home):
    ...
    tool_status = []
    subscription_features = get_callmemo_subscription_features(config)
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'anakot setup' to configure"))
    ...
    if subscription_features.web.managed_by_nous:
        tool_status.append(("Web Search & Extract (callmemo subscription)", True, None))
    ...
    if subscription_features.browser.managed_by_nous:
        tool_status.append(("Browser Automation (callmemo Browser Use)", True, None))
    ...
    # Image generation — FAL (direct or via callmemo), or any plugin-registered
    # provider (OpenAI, etc.)
    if subscription_features.image_gen.managed_by_nous:
        tool_status.append(("Image Generation (callmemo subscription)", True, None))
    ...
    # Video generation — opt-in via `anakot tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    if subscription_features.video_gen.managed_by_nous:
        tool_status.append(("Video Generation (FAL via callmemo subscription)", True, None))
    else:
        try:
            from agent.video_gen_registry import list_providers as _list_video_providers
            from anakot_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        tool_status.append(("Text-to-Speech (OpenAI via callmemo subscription)", True, None))
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'anakot setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'anakot setup tts'"))
    else:
        tool_status.append(("Text-to-Speech (Edge TTS)", True, None))

    if subscription_features.modal.managed_by_nous:
        tool_status.append(("Modal Execution (callmemo subscription)", True, None))
    elif cfg_get(config, "terminal", "backend") == "modal":
        if subscription_features.modal.direct_override:
            tool_status.append(("Modal Execution (direct Modal)", True, None))
        else:
            tool_status.append(("Modal Execution", False, "run 'anakot setup terminal'"))
    elif managed_callmemo_tools_enabled() and subscription_features.nous_auth_present:
        tool_status.append(("Modal Execution (optional via callmemo subscription)", True, None))

    # Home Assistant
    if get_env_value("HASS_TOKEN"):
        tool_status.append(("Smart Home (Home Assistant)", True, None))

    # Spotify (OAuth via anakot auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'anakot setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {anakot_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('anakot setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('anakot setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('anakot setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('anakot setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('anakot setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('anakot config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('anakot config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('anakot config set <key> <value>', Colors.GREEN)}")
...
# between `anakot tools` and `anakot setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``anakot model``)
    ...
    provider added to ``anakot model`` is automatically available here.
    ...
    # Delegate to the shared anakot model flow — handles provider picker,
    ...
    try:
        ...
        print_info("You can try again later with: anakot model")
    ...
    # on demand via `anakot auth add`, `anakot setup` vision, and
    # `anakot setup tts`. This keeps both quick and full setup thin.

    # Tool Gateway prompt is already shown by _model_flow_callmemo() above.
    save_config(config)
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``anakot model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def _setup_tts_provider(config: dict):
    ...
    choices = []
    providers = []
    if managed_callmemo_tools_enabled() and subscription_features.nous_auth_present:
        choices.append("callmemo Subscription (managed OpenAI TTS, billed to your subscription)")
    ...
    if selected == "nous-openai":
        selected = "openai"
        print_info("OpenAI TTS will use the managed callmemo gateway and bill to your subscription.")
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.anakot/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Anakot auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via anakot setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Anakot runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `anakot setup terminal` / config.yaml.
        ...
        managed_modal_available = bool(
            managed_callmemo_tools_enabled()
            and
            get_callmemo_subscription_features(config).nous_auth_present
        ...
        if use_managed_modal:
            ...
            print_info("Modal execution will use the managed callmemo gateway and bill to your subscription.")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into ANAKOT_MAX_ITERATIONS at startup. We no longer write
    ...
    print_info("  Run `anakot setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # ANAKOT_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        ...
        if "/profiles/" in anakot_home:
            profile_name = anakot_home.rstrip("/").rsplit("/", 1)[-1]
    except Exception:
        pass
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Anakot delivers cron job results,")
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `anakot update`)",
                True,
            ):
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under ANAKOT_HOME, and print
    ...
    try:
        ...
        manifest = _build_full_manifest(
            bot_name="Anakot",
            bot_description="Your Anakot agent on Slack",
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Anakot delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Anakot to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from anakot_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Anakot from anywhere.")
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     anakot config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_anakot_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  anakot gateway")
        else:
            ...
            if is_container():
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   anakot gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `anakot setup tools` and `anakot tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``anakot_cli.auth`` — lists every supported
        provider along with its ``api_key_env_vars``.
      * ``active_provider`` in the auth store — covers OAuth device-code /
        external-OAuth providers (callmemo, Codex, Qwen, Gemini CLI, ...).
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from anakot_cli.config
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_anakot migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_anakot", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Anakot.
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Anakot config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Anakot.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Anakot's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(anakot_home: Path) -> bool:
    ...
    print_info("Anakot can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: anakot claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=anakot_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: anakot claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Anakot configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Anakot config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Anakot (use anakot claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot callmemo Portal setup — OAuth + model pick + provider + Tool Gateway.

    Wired into ``anakot setup --portal`` and ``anakot portal``. This is the
    callmemo-Portal slice of the first-time quick setup, collapsed into a single
    shareable command so a brand-new user goes from zero to a fully working
    Anakot session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``anakot setup`` and hunt for the quick-setup option.

    The login + model selection + provider switch + Tool Gateway opt-in are all
    delegated to ``_model_flow_callmemo`` — the exact same flow quick setup uses
    (``_run_first_time_quick_setup``) and the same one ``anakot model`` runs
    when you pick callmemo. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``anakot portal`` always offers a model picker,
    and there is a single source of truth for the callmemo onboarding steps.
    """
    ...
    print(color("│     ⚕ Anakot Setup — callmemo Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    print_info("    — all routed through your callmemo Portal sub.")
    print()
    print_info("  Sign up: https://portal.callmemo.ai/manage-subscription")
    print()

    # _model_flow_callmemo handles BOTH the logged-out path (device-code OAuth,
    # which selects a model internally) and the already-logged-in path (curated
    # callmemo model picker), then offers the Tool Gateway opt-in and sets
    # provider=nous via the login/model save. This is the same routine quick
    # setup calls, so `anakot portal` == quick setup's callmemo step.
    try:
        from anakot_cli.main import _model_flow_callmemo

        _model_flow_callmemo(config)
    except (KeyboardInterrupt, EOFError, SystemExit):
        # _login_callmemo raises SystemExit(130)/(1) on cancel/failure; the
        # logged-out path inside _model_flow_callmemo catches it, but the
        ...
        print_info("  You can retry later with `anakot portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_callmemo error during `anakot portal`: %s", exc)
        print()
        print_error(f"  callmemo Portal setup encountered an error: {exc}")
        print_info("  You can retry later with `anakot portal`.")
        return

    # Re-sync the in-memory config from disk — _model_flow_callmemo (and the
    ...
    print_info("  Run `anakot portal info` to inspect routing.")
    print_info("  Run `anakot` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      anakot setup           — full or quick (auto-detected)
    ...
    # --portal: one-shot callmemo Portal setup. Skips the rest of the wizard.
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Anakot Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│  Let's configure your Anakot Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, anakot_home)
            return
        ...
        print_info("")
        print_info("Tip: jump straight to a section with 'anakot setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(anakot_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Anakot?",
            [
                "Quick Setup (callmemo Portal) — free OAuth login, no API keys, model + tools (recommended)",
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, anakot_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {anakot_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'anakot config edit'")
    ...
    # Tune later with `anakot setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, anakot_home, is_existing: bool):
    """Streamlined first-time setup: choice between Local AI and callmemo Portal.

    Offers the user a fast path: either connect to a local provider (Ollama/LM Studio)
    or use the callmemo Portal (OAuth). Then configures the terminal backend.
    """
    from anakot_cli.config import load_config

    print()
    print_header("Quick Start")
    print_info("Choose your primary AI connection for a fast setup.")
    print()

    quick_choice = prompt_choice(
        "Select your connection type:",
        [
            "Local Host AI (Ollama, LM Studio) — private, free, runs on your machine",
            "callmemo Portal — 300+ models, no API keys, includes web/image tools (OAuth)",
        ],
        0,
    )
    ...
    if quick_choice == 0:
        # Local AI Path
        print()
        print_header("Local AI Setup")
        print_info("Connecting to your local model server...")
        
        local_choice = prompt_choice(
        ...
        if local_choice == 0:
            from anakot_cli.main import _model_flow_lmstudio
            _model_flow_lmstudio(config)
        elif local_choice == 1:
            # Ollama local is handled via custom flow or specific ollama-cloud
            # For quick start, we use the custom flow with Ollama defaults.
            from anakot_cli.main import _model_flow_custom
            print_info("Tip: Ensure Ollama is running with 'OLLAMA_ORIGINS=\"*\" ollama serve'")
            _model_flow_custom(config, default_url="http://127.0.0.1:11434/v1", name="Ollama (Local)")
        else:
            from anakot_cli.main import _model_flow_custom
            _model_flow_custom(config)
    else:
        # callmemo Portal Path
        print()
        print_header("callmemo Portal")
        print_info("One subscription, 300+ models, plus the Tool Gateway:")
        print_info("  web search, image generation, TTS, browser automation.")
        print_info("Sign up: https://portal.callmemo.ai/manage-subscription")
        print()
        try:
            ...
            print()
            print_info("callmemo Portal setup cancelled.")
        except Exception as exc:
            logger.debug("_model_flow_callmemo error during quick setup: %s", exc)
            print_warning(f"callmemo Portal setup encountered an error: {exc}")
            print_info("You can try again later with: anakot model")

    # Re-sync the wizard's config dict from disk
    _refreshed = load_config()
    config.clear()
    config.update(_refreshed)
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'anakot setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    anakot setup")
    if gateway_choice != 0:
...
def _run_quick_setup(config: dict, anakot_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'anakot setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Anakot to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'anakot setup gateway'.")
