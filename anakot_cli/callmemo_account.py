"""Normalized callmemo Portal account entitlement helpers."""
...
def callmemo_portal_billing_url(account_info: Optional[NousPortalAccountInfo] = None) -> str:
    """Return the billing URL for a normalized callmemo account snapshot."""
    try:
    ...
    if not isinstance(base, str) or not base.strip():
        base = DEFAULT_CALLMEMO_PORTAL_URL
...
def format_callmemo_portal_entitlement_message(
    ...
    *,
    ...
    """Return user-facing guidance for a missing callmemo tool-gateway entitlement.
    ...
    """
    billing_url = callmemo_portal_billing_url(account_info)
    ...
    if account_info is not None:
        if coverage_category is not None:
            ...
            if account_info.tool_gateway_entitled:
                ...
                return (
                    f"{capability} isn't included with your current callmemo Portal "
    ...
    if account_info is None:
        return (
            f"Anakot could not verify your callmemo Portal entitlement, so {capability} "
            f"is unavailable. Run `anakot model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"callmemo inference credentials are configured, but Anakot cannot verify "
                f"your callmemo Portal paid access for {capability}. Log in with "
                f"`anakot model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to callmemo Portal to use {capability}: run `anakot model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"Anakot could not verify your callmemo Portal paid access, so {capability} "
            f"is unavailable."
        )
    ...
    if reason == "account_missing":
        return (
            f"Anakot could not find a callmemo Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `anakot model` to "
            f"authenticate again; if the problem persists, contact callmemo support."
        )
    ...
    return (
        f"Your callmemo Portal account does not currently have paid service access, "
...
def _no_paid_access_message(
    ...
    if has_active_subscription and active_subscription_is_paid:
        ...
        return (
            f"Your callmemo Portal credits are exhausted{credit_detail}, so {capability} "
    ...
    if has_active_subscription and active_subscription_is_paid is False:
        return (
            f"Your current callmemo Portal plan does not include paid service access, "
    ...
    if has_active_subscription is False:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your callmemo Portal account has no active subscription or usable credits"
    ...
    credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
    return (
        f"Your callmemo Portal account has no usable paid credits{credit_detail}, so "
...
def get_callmemo_portal_account_info(
    *,
    ...
    """Return normalized callmemo Portal account entitlement information.
...
def _fresh_account_info(
    *,
    ...
    try:
        ...
        payload = _fetch_callmemo_account_info(access_token, portal_base_url)
        if not payload:
            return _error_info(
...
def _info_from_inference_key_pool(
    ...
    """Return an explicit unknown-entitlement snapshot for opaque callmemo keys."""
    try:
...
def _info_from_oauth_pool(
    *,
    ...
    try:
        payload = _fetch_callmemo_account_info(access_token, entry_portal_url)
    except Exception as exc:
        return _error_info(
            error=exc,
            logged_in=True,
...
def _fetch_callmemo_account_info(
    access_token: str,
    portal_base_url: Optional[str] = None,
) -> dict[str, Any]:
