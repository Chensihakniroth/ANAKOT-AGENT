...
Usage examples::
    ...
    anakot logs                    # last 50 lines of agent.log
    anakot logs -f                 # follow agent.log in real time
    anakot logs errors             # last 50 lines of errors.log
    anakot logs gateway -n 100    # last 100 lines of gateway.log
    anakot logs gui -f            # follow gui.log (dashboard/pty/ws)
    anakot logs desktop -f        # follow desktop.log (Electron app boot/backend)
    anakot logs --level WARNING    # only WARNING+ lines
    anakot logs --session abc123   # filter by session ID substring
    anakot logs --component tools  # only tool-related lines
    anakot logs --since 1h         # lines from the last hour
    anakot logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print(f"(Logs are created when Anakot runs — try 'anakot chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_anakot_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_anakot_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
