...
Automates what a user otherwise does by hand: open the callmemo Portal
``/local-dashboards`` page in a browser, click "register", copy the
resulting ``agent:{id}`` OAuth client ID, and paste it into ``~/.anakot/.env``
as ``ANAKOT_DASHBOARD_OAUTH_CLIENT_ID``.

This command:
  1. Resolves a fresh callmemo Portal access token from the existing login
     (``~/.anakot/auth.json``), refreshing it if needed. Fails fast with a
     "run `anakot setup`" hint when the user isn't logged in.
  ...
  3. Writes ``ANAKOT_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``ANAKOT_DASHBOARD_PORTAL_URL`` into ``~/.anakot/.env`` idempotently.
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    Precedence:
      1. ``override`` — explicit ``--portal-url`` flag or
         ``ANAKOT_DASHBOARD_PORTAL_URL`` env (used for testing against a
      ...
      2. The ``portal_base_url`` stored on the callmemo login — this is the
    ...
    try:
        from anakot_cli.auth import DEFAULT_CALLMEMO_PORTAL_URL, get_provider_auth_state
...
def _register_self_hosted_client(
    *,
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                "callmemo Portal rejected the access token (401). "
                "Try `anakot auth login nous` to re-authenticate."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
        ...
        raise RuntimeError(
            f"Could not reach callmemo Portal at {portal_base_url}: {exc.reason}"
        ) from exc
...
def _print_post_register_hint(
    *,
    ...
    print(f"    ANAKOT_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
    if wrote_portal_url:
        print(f"    ANAKOT_DASHBOARD_PORTAL_URL={portal_base_url}")
    print()
    print(
        "  Heads up — callmemo login only *engages* on a non-loopback bind. A plain\n"
    ...
    if custom_redirect_uri:
        ...
        try:
            ...
            host = "your-host"
        print("  To require callmemo login on your registered host, run the dashboard")
        print(f"  bound publicly (it must be reachable at https://{host}) and log in")
        print("  at its /login page.")
    else:
        print("  To require callmemo login (e.g. exposing on your LAN or a public host):")
        print("    anakot dashboard --host 0.0.0.0")
...
def cmd_dashboard_register(args) -> None:
    ...
    from anakot_cli.config import get_env_value, is_managed, save_env_value

    # Managed (Docker/hosted) installs get their dashboard OAuth client_id
    # stamped in by the orchestrator (NAS sets ANAKOT_DASHBOARD_OAUTH_CLIENT_ID
    ...
    if is_managed():
        print(
            "✗ `anakot dashboard register` is not available in a managed/hosted "
            "install.\n"
            "  The dashboard OAuth client is provisioned by the hosting platform."
        )
        sys.exit(1)

    # 1. Resolve a fresh callmemo access token (refreshes if near expiry). Fail fast
    #    with a setup hint when the user isn't logged in.
    try:
        access_token = resolve_nous_access_token()
    except AuthError as exc:
        if getattr(exc, "relogin_required", False):
            print("✗ You're not logged into callmemo Portal.")
            print("  Run `anakot setup` (or `anakot auth login nous`) first, then retry.")
        else:
            print(f"✗ Could not resolve a callmemo Portal access token: {exc}")
        sys.exit(1)
    ...
    # ANAKOT_DASHBOARD_PORTAL_URL env var, else the stored login's portal.
    ...
    try:
        ...
        print(f"  Set it manually:  ANAKOT_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
        sys.exit(1)

    wrote_portal_url = False
    default_portal = "https://portal.callmemo.ai"
    existing_portal = None
    try:
        existing_portal = get_env_value("ANAKOT_DASHBOARD_PORTAL_URL")
    except Exception:
        existing_portal = None
    if not existing_portal and portal_base_url.rstrip("/") != default_portal:
        try:
            save_env_value("ANAKOT_DASHBOARD_PORTAL_URL", portal_base_url)
