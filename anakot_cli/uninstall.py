...
- Keep data: Remove code but keep ~/.anakot/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing anakot-agent or anakot PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Anakot Agent" comment and following line
                ...
                # Remove any PATH line containing anakot
...
def remove_wrapper_script():
    ...
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "anakot",
        Path("/usr/local/bin/anakot"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains anakot_cli reference)
                content = wrapper.read_text()
                if 'anakot_cli' in content or 'anakot-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(anakot_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``anakot`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Anakot home's ``node``
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``anakot gateway run`` processes
...
#
#   1. Sets User-scope env vars ``ANAKOT_HOME`` and ``ANAKOT_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\anakot\git\cmd``, ``%LOCALAPPDATA%\anakot\git\bin``,
#      ``%LOCALAPPDATA%\anakot\git\usr\bin``, ``%LOCALAPPDATA%\anakot\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\anakot\git\`` and Node to
#      ``%LOCALAPPDATA%\anakot\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``anakot dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\anakot\gateway-service\`` and sometimes
...
def _anakot_path_markers(anakot_home: Path) -> list[str]:
    """Path-entry substrings that identify Anakot-owned User-PATH entries."""
    root = str(anakot_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare anakot-agent install dir.
    markers = [root + "\\anakot-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if ANAKOT_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "anakot".  We don't want to catch
    # unrelated entries like "canakot-foo" or "ephermeral", so we look for
    # backslash-anakot as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(anakot_home: Path) -> list[str]:
    """Strip Anakot-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _anakot_path_markers(anakot_home)
            kept: list[str] = []
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ANAKOT_HOME directory.

    We shell out to ``anakot -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current ANAKOT_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m anakot_cli.main` so we don't depend on a `anakot`
    #    wrapper that may be half-removed mid-uninstall.
    anakot_invocation = [_sys.executable, "-m", "anakot_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                anakot_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
...
def run_gui_uninstall(args):
    ...
    Mirrors ``anakot uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$ANAKOT_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from anakot_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    print(color("│         ⚕ Anakot Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.anakot/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.anakot/ for future reinstall
    """
    project_root = get_project_root()
    anakot_home = get_anakot_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie ANAKOT_HOMEs
    ...
    # full wipe (code + ~/.anakot data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Anakot Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    # their alias wrappers, and wiping their ANAKOT_HOME dirs. Otherwise
...
def _perform_uninstall(
    *,
    ...
    ``anakot`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$ANAKOT_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in anakot_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\anakot\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(anakot_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Anakot-owned PATH entries in User environment")

        log_info("Removing ANAKOT_HOME / ANAKOT_GIT_BASH_PATH User env vars...")
        removed_env = remove_anakot_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Anakot-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing anakot command...")
    ...
    #     (only when they still point into this Anakot home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Anakot-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(anakot_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Anakot-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(anakot_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE ANAKOT_HOME and
    ...
    try:
        ...
        gui_removed = uninstall_gui(anakot_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.anakot/, just remove the anakot-agent subdir
            if anakot_home in project_root.parents or project_root.parent == anakot_home:
    ...
    #     under ANAKOT_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(anakot_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(anakot_home)
    ...
    if full_uninstall:
        ...
        #     alias wrapper. The profile ANAKOT_HOME dirs live under
        ...
        try:
            if anakot_home.exists():
                shutil.rmtree(anakot_home)
                log_success(f"Removed {anakot_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {anakot_home}: {e}")
            log_info("You may need to manually remove it")
    else:
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m anakot_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import anakot_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``anakot_constants`` + ``anakot_cli.colors``
    (and lazily ``anakot_cli.gui_uninstall``), so it runs fine under a bare
