"""Helpers for callmemo subscription managed-tool capabilities."""
...
# coverage category (anakot_cli.callmemo_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `anakot tools` picker scope its entitlement gate to the selected backend, so a
# free-tool-pool user is allowed image gen but denied video gen at select time —
# consistent with the per-category feature gates in get_callmemo_subscription_features.
    ...
    *,
    ...
    try:
        if force_fresh:
            ...
            account_info = get_callmemo_portal_account_info()
    except Exception:
        account_info = None
    ...
    # Tool Gateway via `anakot model`, so direct credentials should NOT
    ...
    features = {
        ...
        "image_gen": NousFeatureState(
            ...
            current_provider="FAL" if direct_fal else ("callmemo Subscription" if image_managed else ""),
        ...
        "video_gen": NousFeatureState(
            ...
            current_provider="FAL" if direct_fal_video else ("callmemo Subscription" if video_managed else ""),
...
def get_gateway_eligible_tools(
    ...
    *,
    ...
    All lists are empty when the user is not a paid callmemo subscriber or
    is not using callmemo as their provider.
    """
    ...
    # Quick provider check without the heavy get_callmemo_subscription_features call
    model_cfg = config.get("model")
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        account_info = get_callmemo_portal_account_info(force_fresh=False)
    except Exception:
        account_info = None
    pool_only = bool(
        account_info
    ...
    source_label = "free tool pool" if pool_only else "callmemo subscription"
    ...
    if pool_only:
        title = "Your free callmemo tool pool — pick the tools to enable:"
    else:
        title = (
            "Your callmemo subscription includes the Tool Gateway — "
...
# ---------------------------------------------------------------------------
# Inline callmemo Portal login for the Tool Gateway picker (`anakot tools`)
# ---------------------------------------------------------------------------
...
def ensure_callmemo_portal_access(
    *,
    ...
    """Make sure the user is entitled to the callmemo Tool Gateway, logging in if
    needed.

    Used by ``anakot tools`` when a user selects a callmemo-managed Tool Gateway
    backend (e.g. "Firecrawl (callmemo Portal)").  Unlike ``anakot model``'s callmemo
    login, this:
    ...
    It only performs the callmemo Portal device-code OAuth (when the user isn't
    ...
    try:
        info = get_callmemo_portal_account_info(force_fresh=True)
    except Exception:
        info = None
    if _entitled(info):
        return True
    ...
    if info is None or not info.logged_in:
        if not _run_callmemo_portal_login_only(capability=capability):
            return False
        try:
            info = get_callmemo_portal_account_info(force_fresh=True)
        except Exception:
...
def _run_callmemo_portal_login_only(*, capability: str) -> bool:
    """Run the callmemo Portal device-code OAuth and persist credentials only.
    ...
    try:
        ...
        print(f"  Could not start callmemo Portal login: {exc}")
        return False

    print()
    print(f"  {capability} requires a callmemo Portal login.")
    try:
        proceed = input("  Log in to callmemo Portal now? [Y/n]: ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        print()
        return False
    if proceed not in {"", "y", "yes"}:
        print("  Skipped callmemo Portal login.")
        return False

    try:
        # Snapshot the active_provider so a tool-config login never silently
        # switches the user's inference provider to callmemo.
        ...
        if shared:
            try:
                do_import = input(
                    "  Found existing callmemo OAuth credentials. Import them? [Y/n]: "
                ).strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
        ...
        _sync_nous_pool_from_auth_store()
        print("  callmemo Portal login successful.")
        return True
        ...
        print(f"  callmemo Portal login failed: {exc}")
        return False
