...
# display_anakot_home is imported lazily at call sites to avoid ImportError
# when anakot_constants is cached from a pre-update version during `anakot update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "anakot CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``anakot gateway status`` from falsely counting the ``anakot`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``anakot gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "anakot_cli.main gateway",
        "anakot_cli.main --profile",
        "anakot_cli.main -p",
        "anakot_cli/main.py gateway",
        "anakot_cli/main.py --profile",
        "anakot_cli/main.py -p",
        "anakot gateway",
        ...
        # ``anakot.exe --profile`` / ``anakot.exe -p`` would also match
        # ``anakot.exe --profile foo dashboard`` and other CLI subcommands,
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        # the command doesn't advertise *some other* profile. ANAKOT_HOME
        ...
        # ANAKOT_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "anakot_home=" in command_lc
            and f"anakot_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``anakot update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Anakot profile are returned.
    """
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``anakot update`` users
    # with no running gateway until they re-invoke ``anakot gateway``
...
def _sync_anakot_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``ANAKOT_HOME``.

    Under ``sudo``, ``ANAKOT_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_anakot_home` falls back to ``/root/.anakot`` — the wrong
    profile. The unit file pins ``ANAKOT_HOME`` for the actual gateway
    ...
    unit_home = env.get("ANAKOT_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("ANAKOT_HOME", "").strip()
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}anakot gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}anakot gateway restart{scope_flag}"
    )
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``anakot gateway status`` output so users with
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (ANAKOT_HOME-scoped).
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Anakot Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``anakot gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``ANAKOT_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("ANAKOT_GATEWAY_DETACHED", "").strip().lower()
...
def _profile_suffix() -> str:
    """Derive a service-name suffix from the current ANAKOT_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (ANAKOT_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    default = get_default_anakot_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary ANAKOT_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(anakot_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when ANAKOT_HOME is a named profile.

    For ``~/.anakot/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        anakot_home: Optional explicit ANAKOT_HOME path. Defaults to the current
            ``get_anakot_home()`` value. Should be passed when generating a
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this ANAKOT_HOME.

    Default ``~/.anakot`` returns ``anakot-gateway`` (backward compatible).
    Profile ``~/.anakot/profiles/coder`` returns ``anakot-gateway-coder``.
    Any other ANAKOT_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Anakot installs that predate the
# anakot-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (anakot-gateway-*.service) and unrelated third-party
# "anakot" units are never matched.
...
def _find_legacy_anakot_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Anakot gateway units.

    Detects unit files installed by older Anakot versions that used a
    different service name (e.g. ``anakot.service`` before the rename to
    ``anakot-gateway.service``). When both a legacy unit and the current
    ``anakot-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``anakot-gateway-coder.service`` and unrelated third-party
      ``anakot-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``anakot.service`` running an unrelated
...
def print_legacy_unit_warning() -> None:
    """Warn about legacy Anakot gateway unit files if any are installed.
...
def remove_legacy_anakot_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Anakot gateway unit files.

    Iterates over whatever ``_find_legacy_anakot_units()`` returns — which is
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so Anakot can't create it from this user session."
            )
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo anakot gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo anakot gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo anakot gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Anakot often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.anakot`` → ``ai.anakot.gateway.plist`` (backward compatible).
    Profile ``~/.anakot/profiles/coder`` → ``ai.anakot.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.anakot/anakot-agent  -> /home/alice/.anakot/anakot-agent
      /opt/anakot                 -> /opt/anakot  (kept as-is)
...
def _anakot_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current ANAKOT_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_anakot_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.anakot                    → /home/alice/.anakot
      /root/.anakot/profiles/coder     → /home/alice/.anakot/profiles/coder
      /opt/custom-anakot               → /opt/custom-anakot  (kept as-is)
    """
    current_anakot = get_anakot_home().resolve()
    current_default = (Path.home() / ".anakot").resolve()
    target_default = Path(target_home_dir) / ".anakot"

    # Default ~/.anakot → remap to target user's default
    if current_anakot == current_default:
        return str(target_default)

    # Profile or subdir of ~/.anakot → preserve the relative structure
    try:
        relative = current_anakot.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.anakot) — keep as-is
        return str(current_anakot)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    anakot_node = anakot_home / "node" / "bin"
    if _is_dir(anakot_node):
        candidates.append(str(anakot_node))
    anakot_nm = anakot_home / "node_modules" / ".bin"
    if _is_dir(anakot_nm):
        candidates.append(str(anakot_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m anakot_cli.main``, so module
    ...
    dir, or a clone that ``anakot update`` later relocates/removes — the path
    ...
    ``ANAKOT_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if ANAKOT_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        # Anchor cwd to the target user's ANAKOT_HOME (stable, always exists)
...
User={username}
Group={group_name}
ExecStart={python_path} -m anakot_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only ANAKOT_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp ANAKOT_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../anakot_test`` path into
    # ``Environment="ANAKOT_HOME=..."``. Writing that to the developer's
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Anakot install"
    )
    return True
...
def systemd_install(
    ...
    # Offer to remove legacy units (anakot.service from pre-rename installs)
    # before installing the new anakot-gateway.service. If both remain, they
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}anakot gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}anakot gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}anakot gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}anakot gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}anakot gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}anakot gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}anakot gateway start{scope_flag}"
        )
...
# (the documented `nohup anakot gateway run` workaround). See #23387.
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m anakot_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active ANAKOT_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "anakot_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup anakot gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup anakot gateway run --replace "
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Anakot install"
    )
    return True
...
def launchd_stop():
    ...
    # `anakot gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("ANAKOT_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Anakot gateway as root inside the official Docker image."
    )
    print(
    ...
    print(
        "  Set ANAKOT_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `anakot gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set ANAKOT_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `anakot gateway restart` which already
    ...
    # `anakot gateway start/restart` — leaving the gateway vulnerable to
    ...
    # ANAKOT_GATEWAY_EXIT_DIAG env var (default: on while we're still
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Anakot will log in directly",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Full Matrix user ID, e.g. @anakot:matrix.example.org",
            },
            {
    ...
    },
    {
        ...
        "vars": [
            ...
            },
            {
                ...
                "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)",
                "password": False,
                "help": "Channel ID where Anakot delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Anakot agent",
        ...
        "vars": [
            {
                ...
                "password": False,
                "help": "The email address Anakot will use (e.g., anakot@gmail.com).",
            },
            {
                "name": "EMAIL_PASSWORD",
...
def _all_platforms() -> list[dict]:
    ...
    ``anakot setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run anakot under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.anakot/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_anakot_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: anakot pairing approve <platform> <code>"
                    )
                else:
                    print_info(
                        "  Skipped — configure later with 'anakot gateway setup'"
                    )
            continue
...
def _setup_whatsapp():
    ...
    from anakot_cli.main import cmd_whatsapp
    import argparse
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Anakot will open Tencent iLink QR login in this terminal.")
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `anakot pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "AnakotAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``anakot_cli.setup`` (which
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.anakot/plugins/
        ...
        if service_running:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    ...
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo anakot gateway install --system"
                        )
                    print_info("  Or run in foreground:  anakot gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: anakot gateway run")
                print_info(
                ...
                print_info("  Run in foreground: anakot gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup anakot gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`anakot_cli.service_manager` are caught and surfaced as
    ...
    if profile is None:
        ...
        # ANAKOT_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``anakot gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``anakot gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``anakot gateway run`` are unaffected.
      2. ``ANAKOT_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``anakot gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``ANAKOT_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    if no_supervise:
        return False
    if os.environ.get("ANAKOT_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${ANAKOT_HOME}/logs/gateways/<profile>/current,
    ...
    #
    # Prefer `sleep infinity` (matches the static main-anakot service's
    # pattern in docker/s6-rc.d/main-anakot/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: anakot gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  anakot profile create <name>     # creates the s6 service slot")
                print("  anakot -p <name> gateway start   # bring it up via s6")
                print("  anakot status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: anakot gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: anakot gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: anakot gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  anakot profile delete <name>     # tears down the s6 service slot")
                print("  anakot -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `anakot -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: anakot gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_ANAKOT_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `anakot gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_ANAKOT_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `anakot gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: anakot gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  anakot gateway install")
                else:
                    print("To install as a service:")
                    print("  anakot gateway install")
                    print("  sudo anakot gateway install --system")
            else:
                ...
                if is_termux():
                    ...
                    print(
                        "  anakot gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  anakot gateway install  # Install as user service")
                    print(
        ...
        # Stop, disable, and remove legacy Anakot gateway unit files from
        # pre-rename installs (e.g. anakot.service). Profile units and
        # unrelated third-party services are never touched.
