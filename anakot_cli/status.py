...
Shows the status of all Anakot Agent components.
"""
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``anakot config``'s
...
def show_status(args):
    ...
    print(color("│                 ⚕ Anakot Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if (
        ...
        try:
            callmemo_account_info = get_callmemo_portal_account_info()
        except Exception:
            callmemo_account_info = None

    nous_logged_in = bool(
        nous_status.get("logged_in")
        or (callmemo_account_info and callmemo_account_info.logged_in)
    )
    nous_inference_present = bool(
        nous_status.get("inference_credential_present")
        or (callmemo_account_info and callmemo_account_info.inference_credential_present)
    )
    nous_error = nous_status.get("error")
    if nous_logged_in:
        nous_label = "logged in"
    elif nous_inference_present:
        nous_label = "not logged in (callmemo inference key configured)"
    else:
        nous_label = "not logged in (run: anakot portal)"
    print(
        f"  {'callmemo Portal':<12}  {check_mark(nous_logged_in)} "
        f"{nous_label}"
    )
    portal_url = nous_status.get("portal_base_url") or "(unknown)"
    inference_url = (
        nous_status.get("inference_base_url")
        or (callmemo_account_info.inference_base_url if callmemo_account_info else None)
    )
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: anakot model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: anakot auth add minimax-oauth)'}"
    )
    ...
    # disrupt the already-printed callmemo/Codex/Qwen/MiniMax rows above.
    try:
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: anakot auth add xai-oauth)'}"
    )
    ...
    # =========================================================================
    # callmemo Subscription Features
    # =========================================================================
    if managed_callmemo_tools_enabled():
        features = get_callmemo_subscription_features(config)
        print()
        print(color("◆ callmemo Tool Gateway", Colors.CYAN, Colors.BOLD))
        if not features.nous_auth_present:
            print("  callmemo Portal   ✗ not logged in")
        else:
            print("  callmemo Portal   ✓ managed tools available")
        for feature in features.items():
            if feature.managed_by_nous:
                state = "active via callmemo subscription"
            elif feature.active:
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: anakot model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from anakot_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from anakot_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   anakot gateway")
    ...
    print(color("  Run 'anakot doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'anakot setup' to configure", Colors.DIM))
    print()
