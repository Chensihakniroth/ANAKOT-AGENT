"""
Anakot Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m anakot_cli.main web          # Start on http://127.0.0.1:9119
    python -m anakot_cli.main web --port 8080
"""
...
# ANAKOT_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("ANAKOT_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
...
#
# This list is defined in ``anakot_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from anakot_cli.dashboard_auth.public_paths import (
...
async def get_status():
    ...
    # and which providers are registered so ``anakot status`` and the
    # SPA's StatusPage can show "OAuth gate ON via callmemo" or
    # "loopback only — no auth gate" with no extra round trips.
    auth_required = bool(getattr(app.state, "auth_required", False))
    auth_providers: list[str] = []
    try:
        from anakot_cli.dashboard_auth import list_providers as _list_providers
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the anakot home root).
    """
...
async def run_curator():
    ...
    try:
        proc = _spawn_anakot_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def run_prompt_size():
    try:
        proc = _spawn_anakot_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_anakot_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
# streamed to a per-action log file under ``~/.anakot/logs/<action>.log`` so
...
def _spawn_anakot_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``anakot <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``anakot_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "anakot_cli.main", *subcommand]
...
async def restart_gateway():
    ...
    try:
        proc = _spawn_anakot_action(["gateway", "restart"], "gateway-restart")
    except Exception as exc:
...
async def update_anakot():
    ...
    try:
        proc = _spawn_anakot_action(["update"], "anakot-update")
    except Exception as exc:
        _log.exception("Failed to spawn anakot update")
...
async def check_anakot_update(force: bool = False):
    ...
    ``POST /api/anakot/update`` actually runs ``anakot update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Anakot version string
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.anakot/config.yaml`` under ``tts.``.
    """
...
async def search_sessions(request: Request, q: str = "", limit: int = 20):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Anakot surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Anakot supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def get_model_options():
    ...
    try:
        ...
        # onboarding picker) sees the SAME full provider universe `anakot model`
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `anakot model` does so GUI onboarding lands on a
    sensible default instead of blindly taking the first curated entry. For
    callmemo this honors the user's free/paid tier: free users get a free model,
    ...
    where free_tier is True/False for callmemo and None otherwise. `model` may be
    ...
    if slug == "nous":
        try:
            ...
            if free_tier:
                ...
                model_ids, _unavailable = partition_callmemo_models_by_tier(
    ...
    # Non-callmemo: first curated model for the provider, matching prior behaviour.
    try:
        from anakot_cli.inventory import build_models_payload, load_picker_context
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.anakot/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if scope == "main":
            ...
            # When switching the main provider to callmemo, mirror the CLI's
            # post-model-selection behaviour (anakot_cli/main.py
            # prompt_enable_tool_gateway / tools_config apply_nous_managed_defaults):
            # auto-route any *unconfigured* tools through the callmemo Tool Gateway.
            ...
            if provider.strip().lower() == "nous":
                try:
                    from anakot_cli.callmemo_subscription import apply_nous_managed_defaults
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Anakot from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Anakot to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Anakot from Slack via Socket Mode.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Anakot in Matrix rooms and direct messages.",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Anakot to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Anakot inside Feishu / Lark.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Anakot to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Anakot to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.anakot-agent.callmemo.ai"
...
# Anthropic, device-code for callmemo/Codex) still runs in the CLI for now;
# Phase 2 will add in-browser flows. For unconnected providers we return
# the canonical ``anakot auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Anakot resolves Anthropic creds in this order at runtime:
    1. ``~/.anakot/.anthropic_oauth.json`` — Anakot-managed PKCE flow
    ...
    try:
        ...
        read_anakot_oauth_credentials = None  # type: ignore
        _ANAKOT_OAUTH_FILE = None  # type: ignore

    anakot_creds = None
    if read_anakot_oauth_credentials:
        try:
    ...
    if anakot_creds and anakot_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "anakot_pkce",
            "source_label": f"Anakot PKCE ({_ANAKOT_OAUTH_FILE})",
            "token_preview": _truncate_token(anakot_creds.get("accessToken")),
            "expires_at": anakot_creds.get("expiresAt"),
            "has_refresh_token": bool(anakot_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Anakot even
    when they also have a separate Anakot-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "device_code",
        "cli_command": "anakot auth add nous",
        "docs_url": "https://portal.callmemo.ai",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "anakot auth add openai-codex",
    ...
    },
    {
        ...
        # as callmemo's device-code flow; the PKCE bit is a security
        # extension that doesn't change the operator experience.
        "flow": "device_code",
        "cli_command": "anakot auth add minimax-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "anakot auth add anthropic",
    ...
    try:
        ...
        if provider_id == "nous":
            ...
            return {
                ...
                "source": "callmemo_portal",
                "source_label": raw.get("portal_base_url") or "callmemo Portal",
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("anakot_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Anakot-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _ANAKOT_OAUTH_FILE
            if _ANAKOT_OAUTH_FILE.exists():
                _ANAKOT_OAUTH_FILE.unlink()
        except Exception:
            pass
...
#          → persists to ~/.anakot/.anthropic_oauth.json AND credential pool
#          → returns { ok: true, status: "approved" }
#
#   Device code (callmemo, OpenAI Codex):
...
# Guarded so anakot web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Anakot file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``anakot auth add anthropic``.
    """
    from agent.anthropic_adapter import _ANAKOT_OAUTH_FILE
    payload = {
    ...
    _ANAKOT_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _ANAKOT_OAUTH_FILE.with_name(
        f"{_ANAKOT_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _ANAKOT_OAUTH_FILE)
        try:
            _ANAKOT_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            "Content-Type": "application/json",
            "User-Agent": "anakot-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    """Initiate a device-code flow (callmemo, OpenAI Codex, or MiniMax).
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("ANAKOT_PORTAL_BASE_URL")
    ...
    if provider_id == "minimax-oauth":
        ...
        # operator's perspective it's identical to callmemo's device-code
...
# the tokens exactly like `anakot auth add xai-oauth`.
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `anakot auth add xai-oauth`'s credential-pool insert.
...
def _nous_poller(session_id: str) -> None:
    """Background poller that drives a callmemo device-code flow to completion."""
    from anakot_cli.auth import (
        _poll_for_token,
...
def _minimax_poller(session_id: str) -> None:
    ...
    the ``device_code`` field used by callmemo. On success, builds the same
...
async def poll_oauth_session(provider_id: str, session_id: str):
    ...
    Shared by the device-code flows (callmemo, OpenAI Codex, MiniMax) and the
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["anakot_home"] = str(home)
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process ANAKOT_HOME at import time. The dashboard is a single
...
#
# Wraps the same config data layer the CLI uses (anakot_cli.mcp_config), so
# servers managed here show up under `anakot mcp list` and vice versa.  Secrets
...
async def list_mcp_catalog():
    """Browse the callmemo-approved MCP catalog (the optional-mcps/ manifests).
    ...
    `anakot mcp catalog` / `anakot mcp install` read.
    """
    try:
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, request: Request):
    ...
    if entry.install is not None:
        try:
            proc = _spawn_anakot_action(["mcp", "install", name], "mcp-install")
        except Exception as exc:
...
#
# Wraps the same JSON store the CLI uses (anakot_cli.webhook); the webhook
...
# spawn the real `anakot gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway():
    try:
        proc = _spawn_anakot_action(["gateway", "start"], "gateway-start")
    except Exception as exc:
...
async def stop_gateway():
    try:
        proc = _spawn_anakot_action(["gateway", "stop"], "gateway-stop")
    except Exception as exc:
...
# `anakot memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `anakot memory setup` to configure a new one.",
            )
...
async def run_doctor():
    try:
        proc = _spawn_anakot_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_anakot_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_anakot_action(args, "backup")
    except Exception as exc:
...
async def run_import(body: ImportRequest):
    ...
    try:
        proc = _spawn_anakot_action(["import", archive], "import")
    except Exception as exc:
...
async def list_checkpoints():
    ...
    # Checkpoints live under <anakot_home>/checkpoints/.  Surface a count +
...
async def prune_checkpoints():
    try:
        proc = _spawn_anakot_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
async def install_skill_hub(body: SkillInstallRequest):
    ...
    try:
        proc = _spawn_anakot_action(["skills", "install", identifier], "skills-install")
    except Exception as exc:
...
async def uninstall_skill_hub(body: SkillUninstallRequest):
    ...
    try:
        proc = _spawn_anakot_action(["skills", "uninstall", name, "--yes"], "skills-uninstall")
    except Exception as exc:
...
async def update_skills_hub():
    try:
        proc = _spawn_anakot_action(["skills", "update"], "skills-update")
    except Exception as exc:
...
# Human-readable labels for each hub source id (matches `anakot skills search`
# provenance).  Keep in sync with create_source_router()'s source list.
_SKILL_HUB_SOURCE_LABELS = {
    "official": "Official (callmemo)",
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local ANAKOT_HOME override so the write lands in the target
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``anakot profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from ANAKOT_HOME).
    """
    from anakot_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``anakot profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the ANAKOT_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``anakot profile describe
    <name> --auto``.
...
async def toggle_toolset(name: str, body: ToolsetToggle, request: Request):
    ...
    helper the CLI ``anakot tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str):
    ...
    Surfaces the same provider rows the CLI ``anakot tools`` picker shows
...
async def select_toolset_provider(name: str, body: ToolsetProviderSelect):
    ...
    core extracted from the CLI configurator — so the GUI and ``anakot tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate):
    ...
    Writes each ``key: value`` to ``~/.anakot/.env`` via ``save_env_value`` —
    the same store ``anakot tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(name: str, body: ToolsetPostSetup):
    ...
    ``anakot tools post-setup <key>`` and the frontend tails the log via
...
#
# The endpoint spawns the same ``anakot --tui`` binary the CLI uses, behind
...
def _resolve_chat_argv(
    ...
    Default: whatever ``anakot --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``ANAKOT_TUI_RESUME`` env var —
    matching what ``anakot_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``ANAKOT_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
...
# ANAKOT_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`anakot_cli.dashboard_auth.prefix.normalise_prefix`
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/anakot/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /anakot`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__ANAKOT_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/anakot``)
        ...
        ``__ANAKOT_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            ...
            bootstrap_script = (
                f'<script>window.__ANAKOT_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__ANAKOT_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__ANAKOT_BASE_PATH__="{prefix}";'
                f"window.__ANAKOT_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/anakot`` they'd hit ``mission-control.tilos.com/fonts/...``
...
_BUILTIN_DASHBOARD_THEMES = [
    ...
    {"name": "nous-blue",     "label": "callmemo Blue",           "description": "Light mode — vivid callmemo-blue accents on cream canvas"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.anakot/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.anakot/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_anakot_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.anakot/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as anakot_cli.plugins):
    1. User plugins:    ~/.anakot/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.anakot/plugins/  (only if ANAKOT_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()
    ...
    # ``anakot_cli/plugins.py`` and the documented user contract.
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"anakot auth {name}"
                        break
            except Exception:
                pass
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.anakot/plugins/``) ship with the CWD and are
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing ANAKOT_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Anakot Agent exposes a
