...
def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    ...
    Called from ``anakot_cli.main`` as part of building the top-level
    ``anakot secrets`` parser.
    """
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        "  Status:  [cyan]anakot secrets bitwarden status[/cyan]\n"
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "`anakot secrets bitwarden setup` first.[/yellow]"
        )
        return 1
