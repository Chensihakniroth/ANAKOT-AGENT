...
`anakot backup` creates a zip archive of the entire ~/.anakot/ directory
(excluding the anakot-agent repo and transient files).

`anakot import` restores from a backup zip, overlaying onto the current
ANAKOT_HOME root.
"""
...
from anakot_constants import get_default_anakot_root, get_anakot_home, display_anakot_home
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside ANAKOT_HOME.
...
def run_backup(args) -> None:
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"anakot-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"anakot-backup-{stamp}.zip"
    ...
    for dirpath, dirnames, filenames in os.walk(anakot_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(anakot_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(anakot_root)
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    ...
    # Look for telltale files that a anakot home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Anakot backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.anakot/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a anakot dir name
        if prefix in {".anakot", "anakot"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_anakot_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (anakot_root / "config.yaml").exists()
        has_env = (anakot_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Anakot configuration.")
        ...
        for member in members:
            ...
            try:
                target.resolve().relative_to(anakot_root.resolve())
            except ValueError:
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # anakot_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: anakot profile list  (after installing anakot)")

        # Guidance
        print()
        if not (anakot_root / "anakot-agent").is_dir():
            print("Note: The anakot-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: anakot update")
...
# Critical state files to include in quick snapshots (relative to ANAKOT_HOME).
...
# — `anakot update` snapshots this set before pulling so approved-user lists
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(anakot_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside ANAKOT_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    Args:
        ...
        anakot_home: Override for the Anakot home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(anakot_home), keep=keep)
...
def run_quick_backup(args) -> None:
    ...
    if snap_id:
        ...
        print(f"  {len(snaps)} snapshot(s) stored in {display_anakot_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, anakot_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``anakot_root`` to ``out_path``.
...
def _pre_update_backup_dir(anakot_home: Optional[Path] = None) -> Path:
    home = anakot_home or get_anakot_home()
...
def create_pre_update_backup(
    ...
    result = _write_full_zip_backup(out_path, anakot_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `anakot claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of ANAKOT_HOME under ``backups/`` before a
    ``anakot claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``anakot import <archive>``.  Writes to
    ``<ANAKOT_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `anakot import` and the
