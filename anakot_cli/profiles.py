...
Each profile is a fully independent ANAKOT_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.anakot/profiles/<name>/`` by default.

The "default" profile is ``~/.anakot`` itself — backward compatible,
zero migration needed.

Usage::

    anakot profile create coder          # fresh profile + bundled skills
    anakot profile create coder --clone  # also copy config, .env, SOUL.md, skills
    anakot profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    anakot -p coder chat                 # or via flag
    anakot profile use coder             # set as sticky default
    anakot profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See anakot_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# default profile (``~/.anakot``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   anakot-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `anakot profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `anakot
...
# `anakot skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Anakot infrastructure directories that only the default profile
         (``~/.anakot``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_anakot_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.anakot) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "anakot-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "anakot", "default", "test", "tmp", "root", "sudo",
})
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the anakot root, NOT to the current ANAKOT_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ANAKOT_HOME points outside
    ``~/.anakot``, profiles live under ``ANAKOT_HOME/profiles/`` so
...
def _get_default_anakot_home() -> Path:
    """Return the default (pre-profile) ANAKOT_HOME path.

    In standard deployments this is ``~/.anakot``.
    In Docker/custom deployments where ANAKOT_HOME is outside ``~/.anakot``
    (e.g. ``/opt/data``), returns ANAKOT_HOME directly.
    """
...
def validate_profile_name(name: str) -> None:
    ...
    Also rejects names in :data:`_RESERVED_NAMES` (``anakot``, ``test``,
    ``tmp``, ``root``, ``sudo``) that would create confusing on-disk
    collisions (a ``anakot`` profile inside ``~/.anakot/``) or get refused
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, anakot subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``anakot -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``anakot profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``anakot profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Anakot config
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``anakot update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.anakot)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.anakot/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `anakot update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`anakot profile create --no-skills`).\n"
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `anakot profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `anakot -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ANAKOT_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``anakot profile create
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.anakot).\n"
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from anakot_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ANAKOT_HOME so _profile_suffix resolves correctly
    ...
    try:
        ...
        elif "ANAKOT_HOME" in os.environ:
            del os.environ["ANAKOT_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.anakot/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: anakot profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ANAKOT_HOME.

    Returns ``"default"`` if ANAKOT_HOME is not set or points to ``~/.anakot``.
    Returns the profile name if ANAKOT_HOME points into ``~/.anakot/profiles/<name>``.
    Returns ``"custom"`` if ANAKOT_HOME is set to an unrecognized path.
    """
    from anakot_constants import get_anakot_home
    anakot_home = get_anakot_home()
    resolved = anakot_home.resolve()

    default_resolved = _get_default_anakot_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.anakot itself — its parent is ~/ and its
        # directory name is ".anakot", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    # dir.  Importing as "default" would target ~/.anakot itself — disallow
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    candidates = [
        new_dir / "honcho.json",
        _get_default_anakot_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("anakot_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ANAKOT_HOME path string.

    Called early in the CLI entry point, before any anakot modules
    are imported, to set the ANAKOT_HOME environment variable.
    """
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    profile_dir = get_profile_dir(canon)
