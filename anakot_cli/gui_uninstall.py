"""
Anakot Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``anakot desktop`` / ``anakot gui``)
     Built inside the agent checkout under ``$ANAKOT_HOME/anakot-agent/``:
       ...
       - ``$ANAKOT_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Anakot.app`` or ``~/Applications/Anakot.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Anakot`` (NSIS per-user)
...
the app name ("Anakot"), separate from ``$ANAKOT_HOME``:
  - macOS:   ``~/Library/Application Support/Anakot``
  - Windows: ``%APPDATA%\\Anakot``
  - Linux:   ``$XDG_CONFIG_HOME/Anakot`` (default ``~/.config/Anakot``)
...
``anakot uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(anakot_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return anakot_home / "anakot-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Anakot"
...
def source_built_gui_artifacts(anakot_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``anakot desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``anakot-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(anakot_home)
    ...
    return [
        ...
        anakot_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Anakot.app"),
            home / "Applications" / "Anakot.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Anakot).
            local_base / "Programs" / "Anakot",
            # Older / alternate layout some builds used.
            local_base / "anakot-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Anakot")
    else:
        ...
        paths += [
            data_base / "applications" / "anakot.desktop",
            data_base / "applications" / "Anakot.desktop",
        ]
    return paths
...
def agent_is_installed(anakot_home: Path) -> bool:
    """Return True when a usable Python agent install exists under ANAKOT_HOME.
    ...
    if (agent_root / "anakot_cli").is_dir():
        return True
...
def gui_is_installed(anakot_home: Path) -> bool:
    ...
    for p in source_built_gui_artifacts(anakot_home):
        if p.exists():
            return True
...
def uninstall_gui(anakot_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``anakot-agent/anakot_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$ANAKOT_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove anakot' or "
            "'sudo dnf remove anakot'). AppImage builds are a single file you "
