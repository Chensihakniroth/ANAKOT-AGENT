...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``anakot doctor --ack <id>``; the ack is persisted to
...
1. ``anakot doctor`` (and ``anakot doctor --ack <id>``)
...
run in environments where the rest of Anakot failed to import.
"""
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.anakot/.env (OpenRouter, Anthropic, OpenAI, "
            "callmemo, GitHub, AWS, Google, Mistral, etc.).",
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — Anakot requires 3.10+ but defensive.
        return None
    try:
...
# ``~/.anakot/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `anakot doctor` on the gateway host for details.")
