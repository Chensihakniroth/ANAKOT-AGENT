"""CLI for the Anakot Kanban board — ``anakot kanban …`` subcommand.

Exposes the full Kanban command surface documented in the design spec
(``docs/anakot-kanban-v1-spec.pdf``).  All DB work is delegated to
...
from anakot_cli.profiles import get_active_profile_name, get_profile_dir, seed_profile_skills
...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    - ``running=True``: a gateway is alive for this ANAKOT_HOME and its
    ...
    Used by ``anakot kanban create`` (and callers) to warn when a task
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            "Durable SQLite-backed task board shared across Anakot profiles. "
            "Tasks are claimed atomically, can depend on other tasks, and "
            "are executed by a named profile in an isolated workspace. "
            "See https://github.com/Chensihakniroth/ANAKOT-AGENT/docs/user-guide/features/kanban "
            "or docs/anakot-kanban-v1-spec.pdf for the full design."
        ),
    )
    # --- global --board flag ---
    # Applies to every subcommand below. When set, scopes all reads and
    # writes to that board's DB. When omitted, resolves via the
    # ANAKOT_KANBAN_BOARD env var, then the persisted current-board
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "(set via `anakot kanban boards switch <slug>` or the "
            "ANAKOT_KANBAN_BOARD env var). Use `anakot kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    p_list.add_argument("--mine", action="store_true",
                        help="Filter by $ANAKOT_PROFILE as assignee")
    ...
    p_comment.add_argument("--author", default=None,
                           help="Author name (default: $ANAKOT_PROFILE or 'user')")
    ...
    p_asg = sub.add_parser(
        ...
        help="List known profiles + per-profile task counts "
             "(union of ~/.anakot/profiles/ and current assignees on the board)",
    )
    ...
    p_specify.add_argument(
        ...
        help="Author name recorded on the audit comment "
             "(default: $ANAKOT_PROFILE or 'specifier')",
    )
    p_specify.add_argument(
        "--json",
        action="store_true",
    ...
    p_decompose.add_argument(
        ...
        default=None,
        help="Author name recorded on the audit comment "
             "(default: $ANAKOT_PROFILE or 'decomposer')",
    )
    p_decompose.add_argument(
        "--json",
        action="store_true",
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``anakot kanban …`` argparse dispatch.
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: anakot kanban <action> [options]\n"
                "Run 'anakot kanban --help' for the full list of actions.",
                file=sys.stderr,
            )
        return 0
    ...
    # env-var pin for the duration of this call. Using ANAKOT_KANBAN_BOARD
    ...
    # ANAKOT_HOME. Previously only `init` and `daemon` triggered
...
# ---------------------------------------------------------------------------
# Boards management (anakot kanban boards …)
# ---------------------------------------------------------------------------

def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``anakot kanban boards <action>``.
    ...
    DB, so a fresh ANAKOT_HOME that has never called ``kanban init``
...
def _cmd_boards_rm(args: argparse.Namespace) -> int:
    # When the user runs `anakot kanban boards delete <slug>` (alias), the
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    # the kanban dispatcher can use them without a separate `anakot profile
    ...
    if profiles:
        ...
        print("Create one with `anakot -p <name> setup` before assigning tasks.")
    print()
    print("Next step: start the gateway so ready tasks actually get picked up.")
    print("  anakot gateway start")
    print()
    print(
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``anakot kanban show <id>``
...
def _worker_run_id_for(task_id: str) -> Optional[int]:
    if os.environ.get("ANAKOT_KANBAN_TASK") != task_id:
        return None
...
def _cmd_daemon(args: argparse.Namespace) -> int:
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"`anakot kanban list --status ready` / "
                    f"`anakot kanban list --status blocked` for recent "
    ...
    def _ready_queue_nonempty() -> bool:
        ...
        task whose assignee maps to a real Anakot profile (i.e. one the
