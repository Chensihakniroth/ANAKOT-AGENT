...
- **Aggregators** (OpenRouter, callmemo, AI Gateway, Kilo Code) need
...
- **DeepSeek** accepts ``deepseek-chat`` (V3), ``deepseek-reasoner``
  ...
  Anakot revisions folded every non-reasoner input into
...
# slug used by aggregator APIs (OpenRouter, callmemo, etc.).
#
...
def normalize_model_for_provider(model_input: str, target_provider: str) -> str:
    ...
    Args:
        ...
        target_provider: The canonical Anakot provider id, e.g.
            ``"openrouter"``, ``"anthropic"``, ``"copilot"``,
            ``"deepseek"``, ``"custom"``.  Should already be normalised
            via ``anakot_cli.models.normalize_provider()``.
