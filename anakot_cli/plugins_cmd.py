...
Plugins are installed from Git repositories into ``~/.anakot/plugins/``.
...
def _resolve_git_executable() -> Optional[str]:
    ...
    Matches other Anakot subprocess resolution: :func:`shutil.which` first,
...
def _missing_requires_env_names(manifest: dict) -> list[str]:
    """Return declared ``requires_env`` names that are unset in ``~/.anakot/.env``."""
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from anakot_cli.config import get_env_value, save_env_value  # noqa: F811
...
def _install_plugin_core(identifier: str, *, force: bool) -> tuple[Path, dict, str]:
    """Clone Git plugin into ``~/.anakot/plugins``.
...
def cmd_install(
    ...
    if not (target / "plugin.yaml").exists() and not (target / "plugin.yml").exists() and not (
        ...
        console.print(
            ...
            f"or __init__.py. It may not be a valid Anakot plugin.",
        )
    ...
    if user_dir.is_dir():
        ...
        for child in user_dir.iterdir():
            ...
            if manifest.get("name") == name:
                return True
    # Bundled: <repo>/plugins/<name>/ (or ANAKOT_BUNDLED_PLUGINS on Nix).
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    plugin-registered engines (third-party engines installed as Anakot
