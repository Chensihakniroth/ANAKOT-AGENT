...
from anakot_cli.dashboard_auth.cookies import read_session_cookies
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["anakot.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # sends only ``anakot_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if at:
        ...
        # tries to reach callmemo Portal's JWKS; if that's unreachable it raises,
    ...
    if session is None:
        ...
        from anakot_cli.dashboard_auth.cookies import clear_session_cookies
        from anakot_cli.dashboard_auth.prefix import prefix_from_request
