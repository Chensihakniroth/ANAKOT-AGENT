...
Three cookies in play:
  - anakot_session_at:   the OAuth access token
                         (HttpOnly, lifetime = token TTL, ~15 min)
  - anakot_session_rt:   the OAuth refresh token
                         (HttpOnly, lifetime = 24h, ROTATING + reuse-detected)
                         callmemo Portal issues a rotating refresh token for the
                         dashboard auth-code grant (Portal NAS #293 / anakot
  ...
  - anakot_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Gated HTTPS, direct deploy (Path=/) — ``__Host-`` prefix. Binds the
    ...
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/anakot) —
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/anakot`` covers
...
setter wrote ``__Secure-anakot_session_at`` would never find the value.
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /anakot`` we want ``Path=/anakot`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    ``refresh_token`` is written as the RT cookie when non-empty. callmemo Portal
    issues a 24h rotating refresh token (anakot #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/anakot``)
