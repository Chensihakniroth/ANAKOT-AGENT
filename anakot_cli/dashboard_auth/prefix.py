...
prefix (e.g. ``mission-control.tilos.com/anakot/*`` -> dashboard on
:9119), injecting ``X-Forwarded-Prefix: /anakot`` so the backend can
reconstruct prefixed URLs (Location: headers, OAuth redirect_uri,
cookie Path attributes, SPA asset URLs).

This module is also the home of the ``ANAKOT_DASHBOARD_PUBLIC_URL`` /
...
def normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/anakot"`` (no trailing slash) or ``""``
...
def resolve_public_url() -> str:
    ...
    Precedence (mirrors ``dashboard.oauth.client_id``):
      ...
      1. ``ANAKOT_DASHBOARD_PUBLIC_URL`` env var (when non-empty after
