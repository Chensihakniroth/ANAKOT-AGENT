...
Usage:
    ...
    anakot chat                # Interactive chat
    anakot gateway             # Run gateway in foreground
    anakot gateway start       # Start gateway as service
    anakot gateway stop        # Stop gateway service
    anakot gateway status      # Show gateway status
    anakot gateway install     # Install gateway service
    anakot gateway uninstall   # Uninstall gateway service
    anakot setup               # Interactive setup wizard
    anakot logout              # Clear stored authentication
    anakot status              # Show status of all components
    anakot cron                # Manage cron jobs
    anakot cron list           # List cron jobs
    anakot cron status         # Check if cron scheduler is running
    anakot doctor              # Check configuration and dependencies
    anakot honcho setup                    # Configure Honcho AI memory integration
    anakot honcho status                   # Show Honcho config and connection status
    anakot honcho sessions                 # List directory → session name mappings
    anakot honcho map <name>               # Map current directory to a session name
    anakot honcho peer                     # Show peer names and dialectic settings
    anakot honcho peer --user NAME         # Set user peer name
    anakot honcho peer --ai NAME           # Set AI peer name
    anakot honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    anakot honcho mode                     # Show current memory mode
    anakot honcho mode [hybrid|honcho|local]  # Set memory mode
    anakot honcho tokens                   # Show token budget settings
    anakot honcho tokens --context N       # Set session.context() token cap
    anakot honcho tokens --dialectic N     # Set dialectic result char cap
    anakot honcho identity                 # Show AI peer identity representation
    anakot honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    anakot honcho migrate                  # Step-by-step migration guide: OpenClaw native → Anakot + Honcho
    anakot version             Show version
    anakot update              Update to latest version
    anakot uninstall           Uninstall Anakot Agent
    anakot acp                 Run as an ACP server for editor integration
    anakot sessions browse     Interactive session picker with search

    anakot claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: anakot_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``anakot_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``anakot update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``anakot_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, anakot crashes on import and the user can't run
# ``anakot update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
def _set_process_title() -> None:
    """Set the process title to 'anakot' so tools like 'ps', 'top', and
...
# that run *before* anakot_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("ANAKOT_HOME") or os.environ.get("ANAKOT_HOME")
        if home:
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``ANAKOT_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
        return False
    if os.environ.get("ANAKOT_TUI") == "1" or os.environ.get("ANAKOT_TUI") == "1" or "--tui" in argv:
        return True
...
# `entry.tsx`; this is just the earlier cousin. ``ANAKOT_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("ANAKOT_TUI_NO_EARLY_DISABLE") == "1" or os.environ.get("ANAKOT_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`anakot --tui … >log`, CI capture):
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``anakot --version`` before config/logging imports on Termux."""
    if os.environ.get("ANAKOT_TERMUX_DISABLE_FAST_CLI") == "1" or os.environ.get("ANAKOT_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (anakot tools, anakot setup, anakot model) use
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any anakot module import.
#
# Many modules cache ANAKOT_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ANAKOT_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.anakot/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ANAKOT_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors anakot_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If ANAKOT_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.anakot/profiles/coder or
    # /opt/data/profiles/coder).  If ANAKOT_HOME points to the anakot root
    # instead (e.g. systemd hardcodes ANAKOT_HOME=/root/.anakot), we must
    # still read active_profile — the user may have switched profiles via
    # `anakot profile use` and the gateway should honour that choice.
    # See issue #22502.
    anakot_home_env = os.environ.get("ANAKOT_HOME", "")
    if profile_name is None and anakot_home_env:
        if Path(anakot_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the anakot root
    ...
    # 3. If we found a profile, resolve and set ANAKOT_HOME
...
# Bridge security.redact_secrets from config.yaml → ANAKOT_REDACT_SECRETS env
# var BEFORE anakot_logging imports agent.redact (which snapshots the flag at
...
# Initialize centralized file logging early — all `anakot` subcommands
...
def _has_any_provider_configured() -> bool:
    ...
    from anakot_cli.config import get_env_path, get_anakot_home, load_config
    from anakot_cli.auth import get_auth_status

    # Determine whether Anakot itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_anakot_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    # Check for callmemo Portal OAuth credentials
    auth_file = get_anakot_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Anakot has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Anakot to use their tokens.
    if _has_anakot_config:
        try:
...
def _print_tui_exit_summary(
    ...
    print(f"  anakot --tui --resume {target}")
    if title:
...
def _tui_need_npm_install(root: Path) -> bool:
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@anakot" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``ANAKOT_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
...
def _ensure_tui_node() -> None:
    ...
    ``ANAKOT_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    ...
    extras.extend([Path(anakot_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    if tui_dev:
        ...
        # packages/anakot-ink/dist/entry-exports.js. If that dist bundle is
...
def _launch_tui(
    ...
    env["ANAKOT_PYTHON_SRC_ROOT"] = os.environ.get(
        "ANAKOT_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("ANAKOT_PYTHON", sys.executable)
    env.setdefault("ANAKOT_CWD", os.getcwd())
    ...
    if model:
        env["ANAKOT_MODEL"] = model
        env["ANAKOT_INFERENCE_MODEL"] = model
    if provider:
        env["ANAKOT_TUI_PROVIDER"] = provider
        env["ANAKOT_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["ANAKOT_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["ANAKOT_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["ANAKOT_TUI_SKILLS"] = value
    if query:
        env["ANAKOT_TUI_QUERY"] = query
    if image:
        env["ANAKOT_TUI_IMAGE"] = image
    if checkpoints:
        env["ANAKOT_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["ANAKOT_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["ANAKOT_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["ANAKOT_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["ANAKOT_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["ANAKOT_ACCEPT_HOOKS"] = "1"
    ...
    # ANAKOT_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `anakot --tui` try to
    ...
    # still set ANAKOT_TUI_RESUME themselves.
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `anakot update` so
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``ANAKOT_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``anakot kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``anakot kanban boards switch`` from another session can flip the file
...
def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.anakot/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Anakot — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Anakot still functions without them; the user just sees an
...
def cmd_chat(args):
    ...
    # user's ~/.anakot/config.yaml and return built-in defaults. Set BEFORE
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Anakot?")
        print()
    ...
    # has no creds.json.  Every subsequent `anakot gateway` then paid a 30s
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: anakot gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `anakot gateway` skips it cleanly instead of paying a 30s
...
def cmd_postinstall(args):
    ...
    from anakot_cli.config import stamp_install_method
    from anakot_cli.dep_ensure import ensure_dependency
...
def select_provider_and_model(args=None):
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'anakot model' for "
    ...
    # PROVIDER_GROUPS in anakot_cli/models.py). A multi-member group shows one
    ...
    if selected_provider == "openrouter":
        ...
        _model_flow_lmstudio(config, current_model, args=args)
    elif selected_provider == "ollama-cloud":
        _model_flow_ollama_local(config, current_model, args=args)
    ...
    # a leftover OPENAI_BASE_URL in ~/.anakot/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.anakot/.env if the active provider is not 'custom'.
...
#
# Anakot uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `anakot model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `anakot model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`anakot_cli.plugins.PluginContext.register_auxiliary_task`) appear
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Anakot only falls back to a lightweight backend (OpenRouter,")
        print("  callmemo Portal) if the main model is unavailable.  Override a")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``anakot model`` flow, then route aux tasks to them here.
    """
    from anakot_cli.config import load_config
...
def _aux_flow_provider_model(
    ...
    from anakot_cli.models import get_pricing_for_provider
...
def _model_flow_openrouter(config, current_model=""):
    ...
    # in-flow (K/R/C) instead of having to edit ~/.anakot/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `anakot setup` from scratch. OpenRouter
    ...
    from anakot_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_callmemo(config, current_model="", args=None):
    ...
    from anakot_cli.auth import (
        ...
        resolve_callmemo_runtime_credentials,
        AuthError,
        format_auth_error,
        _login_callmemo,
        PROVIDER_REGISTRY,
    ...
    from anakot_cli.callmemo_subscription import prompt_enable_tool_gateway
    ...
    from anakot_cli.models import (
        get_curated_callmemo_model_ids,
    ...
    free_tier = check_callmemo_free_tier(force_fresh=True)
    if not free_tier:
        try:
            refreshed_creds = resolve_callmemo_runtime_credentials(
                force_refresh=True,
            )
            if refreshed_creds:
    ...
    if free_tier:
        try:
            from anakot_cli.callmemo_account import (
                format_callmemo_portal_entitlement_message,
                get_callmemo_portal_account_info,
            )

            _account_info = get_callmemo_portal_account_info(force_fresh=True)
            unavailable_message = (
                format_callmemo_portal_entitlement_message(
                    _account_info,
        ...
        model_ids, pricing = union_with_portal_free_recommendations(
            model_ids, pricing, _callmemo_portal_url,
        )
        model_ids, unavailable_models = partition_callmemo_models_by_tier(
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from anakot_cli.auth import DEFAULT_CALLMEMO_PORTAL_URL

            _url = (_callmemo_portal_url or DEFAULT_CALLMEMO_PORTAL_URL).rstrip("/")
    ...
    selected = _prompt_model_selection(
        ...
        portal_url=_callmemo_portal_url,
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `anakot auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    if status.get("logged_in"):
        ...
        if choice == "2":
            ...
            print()
            try:
                # Forward CLI flags from ``anakot model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``anakot model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``anakot auth add xai-oauth``).  Fall
...
def _model_flow_custom(config, default_url: str = "", name: str = ""):
    ...
    from anakot_cli.config import get_env_value, load_config, save_config
    from anakot_cli.secret_prompt import masked_secret_prompt

    current_url = default_url or get_env_value("OPENAI_BASE_URL") or ""
    ...
    if current_url:
        print(f"  Default URL: {current_url}")
    if current_key:
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Anakot URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from anakot_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from anakot_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    print("Anthropic-style API endpoints.  Anakot will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Anakot will install it now (the preflight below "
...
# downstream call sites read `anakot_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `anakot` invocation — including
# fast paths like `anakot --version` and slash-command dispatch that never
...
def _model_flow_lmstudio(config, current_model="", args=None):
    """LM Studio provider: auto-detect models at local URL."""
    from anakot_cli.main import _model_flow_custom
    _model_flow_custom(config, default_url="http://127.0.0.1:1234/v1", name="LM Studio")


def _model_flow_ollama_local(config, current_model="", args=None):
    """Ollama local provider: auto-detect models at local URL."""
    from anakot_cli.main import _model_flow_custom
    print("Tip: Ensure Ollama is running with 'OLLAMA_ORIGINS="*" ollama serve'")
    _model_flow_custom(config, default_url="http://127.0.0.1:11434/v1", name="Ollama (Local)")
...
def _model_flow_copilot(config, current_model=""):
    ...
    from anakot_cli.config import save_env_value, load_config, save_config
    from anakot_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Anakot turns to `copilot --acp`.")
    print("  Anakot currently starts its own ACP subprocess for each request.")
    print("  Anakot uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set ANAKOT_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``anakot setup`` / ``anakot model``.
    ...
    recover from a malformed paste without editing ``~/.anakot/.env`` by hand.
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `anakot setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so anakot knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Anakot typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Anakot, enable billing on your "
                ...
                print(
                    ...
                    "OpenRouter (free models), Groq, callmemo."
                )
                print()
    ...
    if provider_id == "lmstudio":
        ...
        from anakot_cli.models import fetch_lmstudio_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(
                f"    Anakot will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'anakot model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Anakot CLI with a provider."""
    from anakot_cli.auth import login_command
...
def cmd_auth(args):
    ...
    from anakot_cli.auth_commands import auth_command
...
def cmd_webhook(args):
    ...
    from anakot_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``anakot slack <subcommand>``. Currently supports:
...
def cmd_hooks(args):
    ...
    from anakot_cli.hooks import hooks_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from anakot_cli.backup import run_quick_backup
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from anakot_cli.banner import check_for_updates
...
# Critical files that every ``anakot`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``anakot update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "anakot_cli/main.py",
    "anakot_cli/config.py",
    "anakot_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``anakot`` startup; if any of them
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``anakot update --gateway`` so interactive prompts (stash restore,
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``anakot`` launcher present but ``anakot_cli`` not importable.
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``anakot update`` to stash the
    lockfile — repeatedly.
    """
...
#   - ``anakot update`` can unconditionally call ``anakot desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``anakot desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $ANAKOT_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $ANAKOT_HOME."""
    from anakot_constants import get_anakot_home
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``anakot update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        ...
        candidates = [
            release_dir / "win-unpacked" / "Anakot.exe",
            release_dir / "win-ia32-unpacked" / "Anakot.exe",
            release_dir / "win-arm64-unpacked" / "Anakot.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "anakot",
            release_dir / "linux-unpacked" / "Anakot",
            release_dir / "linux-arm64-unpacked" / "anakot",
            release_dir / "linux-arm64-unpacked" / "Anakot",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Anakot'`` desktop build failure: a corrupt zip in the
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Anakot is damaged and can't be opened." The
...
def cmd_gui(args: argparse.Namespace):
    ...
    if getattr(args, "fake_boot", False):
        env["ANAKOT_DESKTOP_BOOT_FAKE"] = "1"
    ...
    if getattr(args, "cwd", None):
        env["ANAKOT_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if build_result.returncode != 0 and not source_mode:
                ...
                # on the final `electron` -> `Anakot` rename: unpack-electron
            ...
            if not source_mode:
                ...
                # an in-place self-update (otherwise macOS reports "Anakot is
    ...
    print(f"→ Launching packaged Anakot Desktop: {packaged_executable}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``anakot dashboard`` processes other than ourselves.

    ``anakot dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``anakot update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Anakot Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``anakot dashboard`` as
    ...
    environment variable ``ANAKOT_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "anakot dashboard",
        "anakot_cli.main dashboard",
        "anakot_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "anakot.*dashboard"`) keeps us consistent
            # with `anakot_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `anakot update`.
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``anakot update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``anakot update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: anakot curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``anakot dashboard`` processes.

    Called at the end of ``anakot update`` (default ``reason``) and also
    from ``anakot dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the Anakot Desktop Electron app spawns this dashboard as a
    # backend child, it sets ANAKOT_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("ANAKOT_DESKTOP_CHILD_PID")
    if raw_pid:
...
def _update_via_zip(args):
    """Update Anakot Agent by downloading a ZIP archive.
    ...
    try:
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for anakot-agent itself
        ...
        # GitHub ZIPs extract to anakot-agent-<branch>/
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "anakot-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes anakot completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Anakot couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Anakot couldn't drop the saved stash entry."
            )
...
def _discard_stashed_changes(
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Anakot couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `anakot update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/callmemo/anakot-agent.git",
...
def _invalidate_update_cache():
    ...
    ``anakot update``, every profile is now current.
    """
    homes = []
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``anakot update`` is still progressing even if pip/uv itself is silent.
    """
...
def _anakot_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "anakot.exe",
        scripts_dir / "anakot-gateway.exe",
    ]
...
def _detect_concurrent_anakot_instances(
    ...
    The Anakot Desktop Electron app spawns ``anakot.EXE`` as a backend child,
    so during ``anakot update`` the user-invoked process and the desktop's
    ...
    shims (``anakot.exe`` / ``anakot-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``anakot update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``anakot.exe``) is a separate process from the Python
    ...
    # setuptools-generated anakot.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``anakot update``
    ...
    #      anakot.exe sitting *under* a non-Anakot parent (e.g. a Anakot
...
def _format_concurrent_instances_message(
    ...
    lines = ["✗ Another anakot.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Anakot Desktop, exit any open `anakot` REPLs, and")
    lines.append("  stop the gateway (`anakot gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `anakot update --force` if you've already")
...
def _quarantine_running_anakot_exe(
    ...
    """Pre-empt Windows file lock on the running ``anakot.exe``.
    ...
    when ``anakot update`` runs, ``anakot.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``anakot.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next anakot invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Anakot Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Anakot Desktop / gateway / REPL) and pointing to ``--force``.
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_anakot_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    shims, and on Windows the live ``anakot.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``anakot`` drops off PATH. This wraps
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_anakot_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``anakot.exe.old.*`` left by prior updates.

    Called early on every anakot invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``anakot update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `anakot update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``anakot.exe`` / ``anakot-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_anakot_exe`` for the rationale.
    """
    ...
    # hours later inside ``anakot update``'s desktop-rebuild or skill-sync
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``anakot update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``anakot.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``anakot`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `anakot update` output above.")
        return
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$ANAKOT_HOME/bin/uv``, but on Termux the official
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `anakot update`, so we install root-only first
...
class _UpdateOutputStream:
    """Stream wrapper used during ``anakot update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.anakot/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``anakot update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``anakot update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.anakot/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``anakot update --gateway``) the update is already
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``anakot update --check``: fetch and report without installing.
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``anakot update`` (apply
    ...
    if method == "pip":
        from anakot_cli.config import recommended_update_command
        from anakot_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``anakot update`` without requiring a reinstall.  The
    ...
    ``anakot`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/anakot.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v anakot'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/anakot, code at /usr/local/lib/anakot-agent).
    ...
    path_comment = (
        "# Anakot Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of ANAKOT_HOME before running the update.
    ...
    ANAKOT_HOME directories.  The ``--backup`` flag on ``anakot update``
    ...
    # Render path using display_anakot_home so the user sees ~/.anakot/...
    try:
        from anakot_constants import get_anakot_home, display_anakot_home

        home = get_anakot_home()
        try:
            display_path = f"{display_anakot_home()}/{out_path.relative_to(home)}"
        except ValueError:
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``anakot update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Anakot Agent to the latest version.
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `anakot update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Anakot via pip (for PyPI installs)."""
    from anakot_cli import __version__
    from anakot_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "anakot-agent"]
        ...
        cmd = [pipx, "upgrade", "anakot-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "anakot-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "anakot-agent"]
    ...
    print("✓ Update complete! Restart anakot to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_anakot_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        # If user is on a different branch than the update target, stay on
        # the current branch and merge the target.  This lets feature-branch
        # users (e.g. "notebookllm") pull in main's changes without being
        # forced onto main — they stay on their branch and see a clean merge.
        # When on the target branch already, just fast-forward pull.
        if current_branch != branch:
            ...
            print(f"  ⚠ Currently on {label} — will merge {branch} into it...")
        # Stash local changes so the merge/pull has a clean tree
        ...
        if commit_count == 0:
            ...
            # Restore stash — no branch switchback needed since we stayed
            # on the current branch throughout.
        ...
        # though `git pull` can't touch $ANAKOT_HOME, this is cheap
        ...
        # orphan merge-conflict markers in anakot_cli/config.py bricked
        # every user who ran ``anakot update`` for the 7 minutes between
        # the bad commit and the fix landing).
        pre_pull_sha = _capture_head_sha(git_cmd, PROJECT_ROOT)
        try:
            if current_branch != branch and current_branch != "HEAD":
                # Feature branch: merge the update target into the current
                # branch so the user stays on their branch with the latest
                # upstream changes.
                merge_result = subprocess.run(
                    git_cmd + ["merge", f"origin/{branch}"],
                    cwd=PROJECT_ROOT,
                    capture_output=True,
                    text=True,
                )
                if merge_result.returncode != 0:
                    print("  ⚠ Merge failed (conflicts or unrelated histories).")
                    stderr = merge_result.stderr.strip()
                    if stderr:
                        print(f"  {stderr.splitlines()[0]}")
                    print(
                        f"  Resolve manually: git merge origin/{branch}"
                    )
                    sys.exit(1)
            else:
                # Same branch or detached HEAD: fast-forward pull.
            ...
            # so the CLI stays bootable. The user can then retry ``anakot
        ...
        # the old bytecode (e.g. get_anakot_home added to anakot_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # never run ``anakot desktop`` shouldn't be forced into a full
        # Electron build by ``anakot update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        if (desktop_dir / "package.json").exists() and shutil.which("npm") and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "anakot_cli.main", "desktop", "--build-only"]
            ...
            if build_result.returncode != 0:
                build_result = subprocess.run(_desktop_build_cmd, cwd=PROJECT_ROOT, check=False)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `anakot desktop` to retry)")

        print()
        print("✓ Code updated!")

        # After git pull, source files on disk are newer than cached Python
        # modules in this process.  Reload anakot_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_anakot_home() added since the last release.
        try:
            import importlib
            import anakot_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit ANAKOT_HOME so
        # it is not affected by sync_skills()'s module-level ANAKOT_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's ANAKOT_HOME env var points at the default or a named profile.
        try:
        ...
        # check `anakot curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``anakot update`` gives users a predictable
        ...
        # When running as ``anakot update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_anakot_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all anakot-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. anakot-gateway.service or anakot-gateway-coder.service
                            ...
                            # path in `anakot gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    if unmapped_count > 1:
                        print(
                            "    (or: anakot -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Anakot gateway unit files are still installed.
        # When both anakot.service (from a pre-rename install) and the
        # current anakot-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `anakot update` surfaces the issue until the user migrates.
        try:
            from anakot_cli.gateway import (
                has_legacy_anakot_units,
                _find_legacy_anakot_units,
                supports_systemd_services,
            )
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``anakot -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → anakot -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.anakot)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  anakot profile alias {name} --name <custom>"
                    )
        ...
        if alias_name:
            ...
            print(f"Alias:   {alias_name} → anakot -p {name}  ({wrapper})")
        print()
        ...
        from anakot_cli.profiles import rename_profile
        ...
        from anakot_cli.profiles import import_profile
        ...
        try:
            ...
            if plan.has_cron:
                print(
                    ...
                    f"anakot -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        if data.get("anakot_requires"):
            print(f"Requires:     Anakot {data['anakot_requires']}")
        if data.get("source"):
...
def _report_dashboard_status() -> int:
    """Print ``anakot dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``anakot dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No anakot dashboard processes running.")
        return 0

    print(f"{len(pids)} anakot dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard_register(args):
    """Register a self-hosted dashboard OAuth client with callmemo Portal."""
    from anakot_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_completion(args, parser=None):
    ...
    from anakot_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from anakot_cli.prompt_size import cmd_prompt_size as _impl
...
# so that in ``anakot -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``anakot_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``anakot -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``anakot`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("ANAKOT_TUI") == "1")
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `anakot --tui` is the hot path on phones. The full parser setup imports
...
def main():
    ...
    # Cosmetic: make the process show up as 'anakot' instead of 'python3.11'
    ...
    # Sweep stale ``anakot.exe.old.*`` quarantine files left by previous
    # ``anakot update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_anakot_exe``.
    try:
    ...
    model_parser.add_argument(
        "--portal-url",
        help="Portal base URL for callmemo login (default: production portal)",
    )
    model_parser.add_argument(
        "--inference-url",
        help="Inference API base URL for callmemo login (default: production inference API)",
    )
    model_parser.add_argument(
        "--client-id",
        default=None,
    ...
    model_parser.add_argument(
        "--no-browser",
        action="store_true",
        help="Do not attempt to open the browser automatically during callmemo login",
    )
    model_parser.add_argument(
        "--manual-paste",
        action="store_true",
    ...
    model_parser.add_argument(
        ...
        type=float,
        default=15.0,
        help="HTTP request timeout in seconds for callmemo login (default: 15)",
    )
    model_parser.add_argument(
        "--ca-bundle", help="Path to CA bundle PEM file for callmemo TLS verification"
    )
    model_parser.add_argument(
        "--insecure",
        action="store_true",
        help="Disable TLS verification for callmemo login (testing only)",
    )
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy anakot.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Anakot gateway unit files "
    ...
    proxy_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "to an OAuth-authenticated provider (e.g. callmemo Portal). External "
    ...
    setup_parser = subparsers.add_parser(
        ...
        "Run a specific section: anakot setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        "--portal",
        action="store_true",
        help="One-shot callmemo Portal setup: log in via OAuth, pick a callmemo "
        "model, set callmemo as the inference provider, and opt into the Tool "
    ...
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Anakot")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Anakot CLI",
    )
    ...
    auth_add.add_argument("--portal-url", help="callmemo portal base URL")
    auth_add.add_argument("--inference-url", help="callmemo inference base URL")
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.anakot/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        "--profile",
        help="Anakot profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.anakot/scripts/. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        "--profile",
        help="Anakot profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.anakot/config.yaml, "
    ...
    security_parser = subparsers.add_parser(
        ...
        description=(
            "On-demand vulnerability scan against OSV.dev. Covers the Anakot "
    ...
    audit_parser.add_argument(
        "--skip-venv",
        action="store_true",
        help="Skip scanning the Anakot Python venv",
    )
    audit_parser.add_argument(
        "--skip-plugins",
        action="store_true",
    ...
    debug_parser = subparsers.add_parser(
        ...
        epilog="""\
Examples:
    anakot debug share              Upload debug report and print URL
...
""",
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'anakot debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Anakot home directory to a zip file",
    ...
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/anakot-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.anakot/checkpoints/",
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Anakot backup from a zip file",
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.anakot/skills/.bundled_manifest) "
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/anakot-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    def cmd_plugins(args):
        from anakot_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``anakot --help``, ``anakot version``, ``anakot logs``,
    ...
    # anakot tools list [--platform cli]
    ...
    # anakot tools disable <name...> [--platform cli]
    ...
    # anakot tools enable <name...> [--platform cli]
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Anakot as an MCP server",
        description=(
            "Manage MCP server connections and run Anakot as an MCP server.\n\n"
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Anakot as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `anakot mcp add ...` to fall through to
    # interactive chat.
    ...
    # ── Catalog (callmemo-approved MCPs shipped with the repo) ─────────────────
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `anakot mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
        help="List callmemo-approved MCPs available for one-click install",
    )
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `anakot mcp install n8n`)",
    )
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch anakot --resume <id> by replacing the current process
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Anakot",
    ...
    def cmd_claw(args):
        from anakot_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Anakot Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'anakot config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--branch",
        default=None,
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Windows: proceed with the update even when another anakot.exe is detected. The concurrent process will likely cause WinError 32 warnings and may leave a reboot-deferred .exe replacement.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Anakot Agent as an ACP (Agent Client Protocol) server",
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Anakot instances",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Anakot profile distribution. SOURCE can be a git URL "
    ...
    # for `anakot dashboard` cmdlines and SIGTERM them directly — the same
    # path `anakot update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running anakot dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running anakot dashboard processes and exit",
    )
    # Backward-compat shim: older Anakot desktop app shells (<= 0.15.x) spawn the
    # backend as `anakot dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Anakot couldn't start" with no actionable cause. Accept and silently
    ...
    # `anakot dashboard register` — register a self-hosted dashboard OAuth
    # client with callmemo Portal and write the client_id into ~/.anakot/.env.
    # Nested subparser so bare `anakot dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        "register",
        help="Register a self-hosted dashboard with callmemo Portal (writes the OAuth client ID to .env)",
        description=(
            "Register this install as a self-hosted dashboard with your callmemo "
    ...
    dashboard_register_parser.add_argument(
        ...
        default=None,
        help=(
            "Override the callmemo Portal base URL for registration (default: the "
    ...
    # for one release. The Anakot-Setup.exe success screen tells users to
    # run `anakot desktop` from a terminal, so the canonical name needs
    ...
    gui_parser = subparsers.add_parser(
        ...
        description=(
            "Launch the Anakot Electron desktop app. By default this installs "
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Anakot log files",
...
""",
    ...
    # e.g. ``anakot -c Pokemon Agent Dev`` → ``anakot -c 'Pokemon Agent Dev'``
    ...
    # fails (e.g. 'anakot -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (anakot hooks list, cron
