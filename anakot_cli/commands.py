"""Slash command definitions and autocomplete for the Anakot CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("busy", "Control what Enter does while Anakot is working", "Configuration",
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`anakot_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/anakot`` subcommand mapping, and
...
Telegram only displays a small BotCommand menu in practice.  The full Anakot
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``anakot skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``anakot skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/anakot <verb>`` subcommand.
    ...
    can still reach them via ``/anakot <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/anakot`` is always reserved as the first entry so the
    legacy ``/anakot <subcommand>`` form keeps working for anything that
    ...
    # Reserve /anakot as the catch-all top-level command.
    entries.append(("anakot", "Talk to Anakot or run a subcommand", "[subcommand] [args]"))
    seen.add("anakot")
...
def slack_app_manifest(request_url: str = "https://anakot-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /anakot handler.

    Maps both canonical names and aliases so /anakot bg do stuff works
    the same as /anakot background do stuff.

    Plugin-registered slash commands are included so ``/anakot <plugin-cmd>``
