...
`anakot tools` and `anakot setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.anakot/config.yaml under
...
from anakot_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _ANAKOT_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `anakot tools` → Video Generation, which walks
...
# `anakot tools` → X (Twitter) Search setup walks users through credential
# setup. The tool's check_fn means the schema still won't appear to the
# model if the credential later goes missing or expires.
_DEFAULT_OFF_TOOLSETS = {"moa", "homeassistant", "spotify", "discord", "discord_admin", "video", "video_gen", "x_search", "scrapling"}
...
# Platform-scoped toolsets: only appear in the `anakot tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``anakot tools`` → "reconfigure existing" would
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``anakot tools`` checklist actually offers
    for ``platform``.
    ...
    so ``anakot tools`` never claims to add or remove a toolset the user was
...
TOOL_CATEGORIES = {
    ...
    "video_gen": {
        ...
        "icon": "🎬",
        # "callmemo Subscription" row mirrors the image_gen pattern — managed
        # FAL video generation billed via the callmemo Portal.  Plugin-backed
    ...
    "x_search": {
        ...
        "setup_note": (
            "Anakot routes X searches through xAI's built-in x_search "
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. ANAKOT_CUA_DRIVER_VERSION is an
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "ANAKOT_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "ANAKOT_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "ANAKOT_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "ANAKOT_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "ANAKOT_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``anakot update`` and
      by ``anakot computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `anakot update` calls this for every
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_anakot_home()}/anakot-agent && npm install --workspaces=false")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in anakot-agent directory)")
            return
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.anakot/config.yaml")
        ...
        # Run the full `anakot auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.anakot/.env), then continues straight into PKCE. If they
        ...
        try:
            ...
            _print_info("    Run manually: anakot auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `anakot auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: anakot auth spotify")
        except Exception as exc:
        ...
        try:
            ...
            _print_info("    Run manually: anakot plugins enable observability/langfuse")
        _print_info("    Restart Anakot for tracing to take effect.")
        _print_info("    Verify: anakot plugins list")
        ...
        try:
            ...
            _print_info("    Run later: anakot auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: anakot auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: anakot auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``anakot tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``anakot tools post-setup <key>`` — non-interactive post-setup runner.
...
def _get_platform_tools(
    ...
    *,
    ...
    # "anakot-cli" (which include all _ANAKOT_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[anakot-cli, spotify]`` after enabling Spotify via ``anakot
        ...
        # (e.g. "anakot-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``anakot tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `anakot tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"anakot-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("anakot-")}
    ...
    # shipped as a bundled plugin but user must opt in via `anakot tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `anakot tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by anakot tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (anakot-cli, anakot-telegram, etc.)
    ...
    # Opening `anakot tools` is the user's opt-in to reconfigure tools, so treat
...
# ("callmemo Subscription" managed-gateway entry, "Firecrawl Self-Hosted")
...
# ("callmemo Subscription", "Local Browser", "Camofox") — see the comment block
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `anakot tools` gets a silent no-op
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `anakot tools` and `anakot setup tools`.
    ...
    if first_install:
        for pkey in enabled_platforms:
            ...
            for ts_key in sorted(auto_configured):
                ...
                print(color(f"  ✓ {label}: using your callmemo subscription defaults", Colors.GREEN))
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, anakot mcp
