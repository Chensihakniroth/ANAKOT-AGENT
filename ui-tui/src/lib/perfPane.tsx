...
const ENABLED = /^(?:1|true|yes|on)$/i.test((process.env.ANAKOT_DEV_PERF ?? process.env.ANAKOT_DEV_PERF ?? '').trim())
const THRESHOLD_MS = Number(process.env.ANAKOT_DEV_PERF_MS ?? process.env.ANAKOT_DEV_PERF_MS ?? '2') || 0
const LOG_PATH = (process.env.ANAKOT_DEV_PERF_LOG ?? process.env.ANAKOT_DEV_PERF_LOG ?? '').trim() || join(homedir(), '.anakot', 'perf.log')
