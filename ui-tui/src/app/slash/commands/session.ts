...
export const sessionCommands: SlashCommand[] = [
  ...
  {
    help: 'session usage + callmemo credits',
    name: 'usage',
    run: (_arg, ctx) => {
      ctx.gateway.rpc<SessionUsageResponse>('session.usage', { session_id: ctx.sid }).then(r => {
        ...
        // callmemo credits block is agent-independent (a portal fetch), so it shows
        ...
        if (creditsLines.length) {
          ctx.transcript.panel('callmemo credits', [{ text: creditsLines.join('\n') }])
        }
