...
export const LIGHT_THEME: Theme = {
  color: {
    ...
    muted: '#656d76',
    completionBg: '#ffffff',
    completionCurrentBg: '#ddf4ff',
    completionMetaBg: '#ffffff',
    completionMetaCurrentBg: '#ddf4ff',
...
// Terminal. Explicit ANAKOT_TUI_THEME / ANAKOT_TUI_THEME / COLORFGBG signals above still win,
...
export function detectLightMode(
  ...
  const lightFlag = (env.ANAKOT_TUI_LIGHT ?? env.ANAKOT_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.ANAKOT_TUI_THEME ?? env.ANAKOT_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.ANAKOT_TUI_BACKGROUND ?? env.ANAKOT_TUI_BACKGROUND ?? '')
