...
describe('createSlashHandler', () => {
  ...
  it('rejects unknown indicator styles before hitting the gateway', () => {
    ...
    expect(ctx.transcript.sys).toHaveBeenCalledWith('usage: /indicator [ascii|diamond|emoji|kaomoji|unicode]')
  })
  ...
  it('falls through to command.dispatch for skill commands and sends the message', async () => {
      ...
      gateway: {
        gw: {
          ...
          request: vi.fn((method: string) => {
            ...
            if (method === 'command.dispatch') {
              return Promise.resolve({ type: 'skill', message: skillMessage, name: 'anakot-agent-dev' })
            }
    ...
    await vi.waitFor(() => {
      expect(ctx.transcript.sys).toHaveBeenCalledWith('⚡ loading skill: anakot-agent-dev')
    })
