...
const STARTUP_TIMEOUT_MS = Math.max(5000, parseInt(process.env.ANAKOT_TUI_STARTUP_TIMEOUT_MS ?? process.env.ANAKOT_TUI_STARTUP_TIMEOUT_MS ?? '15000', 10) || 15000)
const REQUEST_TIMEOUT_MS = Math.max(30000, parseInt(process.env.ANAKOT_TUI_RPC_TIMEOUT_MS ?? process.env.ANAKOT_TUI_RPC_TIMEOUT_MS ?? '120000', 10) || 120000)
...
const resolveGatewayAttachUrl = () => {
  const raw = (process.env.ANAKOT_TUI_GATEWAY_URL ?? process.env.ANAKOT_TUI_GATEWAY_URL ?? '').trim()

  return raw ? raw : null
}

const resolveSidecarUrl = () => {
  const raw = (process.env.ANAKOT_TUI_SIDECAR_URL ?? process.env.ANAKOT_TUI_SIDECAR_URL ?? '').trim()

  return raw ? raw : null
}

const resolvePython = (root: string) => {
  const configured = (process.env.ANAKOT_PYTHON ?? process.env.ANAKOT_PYTHON ?? process.env.PYTHON ?? '').trim()
