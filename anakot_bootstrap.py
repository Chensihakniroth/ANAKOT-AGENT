"""Windows UTF-8 bootstrap for Anakot entry points.
...
should be imported at the very top of every Anakot entry point
(``anakot``, ``anakot-agent``, ``anakot-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import anakot_bootstrap``
# (or ``from anakot_bootstrap import apply_windows_utf8_bootstrap``) at
