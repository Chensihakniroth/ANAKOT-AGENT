...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/anakot/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when anakot
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# anakot process, the dashboard, and per-profile gateways.
RUN apt-get update && \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main anakot process, the dashboard,
...
RUN set -eu; \
    ...
    # 'Anakot WebUI' catalog). The image has moved to s6-overlay /init\
...
# Non-root user for runtime; UID can be overridden via ANAKOT_UID at runtime
RUN useradd -u 10000 -m -d /opt/data anakot
...
#
# ui-tui/packages/anakot-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/anakot-ink/ ui-tui/packages/anakot-ink/
...
# lock on the @anakot/ink entry, tripped the TUI launcher's
...
# for the same reason: it lazy-installs into /opt/anakot/.venv at first
...
# ---------- Permissions ----------
# Make install dir world-readable so any ANAKOT_UID can read it at runtime.
# The venv needs to be traversable too.
# node_modules trees additionally need to be writable by the anakot user
# so the runtime `npm install` triggered by _tui_need_npm_install() in
# anakot_cli/main.py succeeds (see #18800). /opt/anakot/web is build-time
# only (ANAKOT_WEB_DIST points at anakot_cli/web_dist) and is intentionally
# not chowned here.
# /opt/anakot/gateway is runtime-writable: Python may create __pycache__ and
# gateway state artifacts beneath the package after services drop privileges,
# especially when the anakot UID is remapped at boot (#27221).
# The .venv MUST remain anakot-writable so lazy_deps.py can install
# remaining optional platform packages and future pin bumps at first use.
# Without this, `uv pip install` fails with EACCES and adapters silently
# fail to load.  See tools/lazy_deps.py.
USER root
RUN chmod -R a+rX /opt/anakot && \
    chown -R anakot:anakot /opt/anakot/.venv /opt/anakot/ui-tui /opt/anakot/gateway /opt/anakot/node_modules
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the anakot user via
# `s6-setuidgid anakot` in its run script. If ANAKOT_UID is unset, services
# run as the default anakot user (UID 10000).

# ---------- Link anakot-agent itself (editable) ----------
...
# container always returns nothing — meaning `anakot dump` reports
# "(unknown)" and the startup banner drops its `· upstream <sha>` suffix.
# That makes support triage from container bug reports impossible:
# we can't tell which commit the user is actually running.
#
# Fix: write the commit SHA passed via the ANAKOT_GIT_SHA build-arg to
# /opt/anakot/.anakot_build_sha at build time, and have
# anakot_cli/build_info.py read it at runtime.  Both `anakot dump` and
...
ARG ANAKOT_GIT_SHA=
RUN if [ -n "${ANAKOT_GIT_SHA}" ]; then \
        printf '%s\n' "${ANAKOT_GIT_SHA}" > /opt/anakot/.anakot_build_sha && \
        chown anakot:anakot /opt/anakot/.anakot_build_sha; \
    fi

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-anakot + dashboard.
...
# `exec anakot`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $ANAKOT_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/anakot/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-anakot-setup && \
    chmod +x /etc/cont-init.d/01-anakot-setup
COPY --chmod=0755 docker/cont-init.d/015-supervise-perms /etc/cont-init.d/015-supervise-perms
COPY --chmod=0755 docker/cont-init.d/02-reconcile-profiles /etc/cont-init.d/02-reconcile-profiles

# ---------- Runtime ----------
ENV ANAKOT_WEB_DIST=/opt/anakot/anakot_cli/web_dist
# Point the TUI launcher at the prebuilt bundle baked at build time (Layer 8:
# `ui-tui && npm run build`). This makes _make_tui_argv take the prebuilt-bundle
# fast path (`node --expose-gc /opt/anakot/ui-tui/dist/entry.js`) and skip the
...
ENV ANAKOT_TUI_DIR=/opt/anakot/ui-tui
ENV ANAKOT_HOME=/opt/data

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> anakot ...` they default to root, and any file the
# command writes under $ANAKOT_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/anakot/bin/anakot, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid anakot`
# when invoked as root. Non-root callers (supervised processes,
# `--user anakot`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/anakot/.venv/bin/anakot). See the shim source for
# the opt-out env var (ANAKOT_DOCKER_EXEC_AS_ROOT=1).
COPY --chmod=0755 docker/anakot-exec-shim.sh /opt/anakot/bin/anakot

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> anakot ...` and any
# subprocess that doesn't activate the venv first still find anakot.
#
# /opt/anakot/bin is prepended ahead of the venv so the privilege-drop
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. anakot
# subcommand vs. no-args), drops to the anakot user via s6-setuidgid,
# and exec's the final program so its exit code becomes the container
# exit code. Without the wrapper-as-ENTRYPOINT, leading-dash args
# like `--version` would be intercepted by /init's POSIX shell.
ENTRYPOINT [ "/init", "/opt/anakot/docker/main-wrapper.sh" ]
CMD [ ]
