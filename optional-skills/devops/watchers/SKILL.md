...
Watch a GitHub repo (set `GITHUB_TOKEN` in `~/.anakot/.env` to avoid the 60 req/hr anonymous rate limit):

```bash
python $ANAKOT_HOME/skills/devops/watchers/scripts/watch_github.py \
  --name anakot-issues --repo callmemo/anakot-agent --scope issues
```
