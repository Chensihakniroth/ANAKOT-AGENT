---
name: agentmail
description: Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. anakot-agent@agentmail.to).
version: 1.0.0
...
Add to `~/.anakot/config.yaml` (paste your actual key — MCP env vars are not expanded from .env):
```yaml
mcp_servers:
  agentmail:
    command: "npx"
...
1. Create a dedicated inbox:
   - Use `create_inbox` with a username (e.g. `anakot-agent`)
   - The agent gets address: `anakot-agent@agentmail.to`
2. Send an email:
