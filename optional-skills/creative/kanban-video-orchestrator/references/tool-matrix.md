...
### Kanban infrastructure (`anakot-agent/skills/devops/`)
...
key is present in `~/.anakot/.env` (or macOS Keychain) before firing the kanban.
...
preference: macOS Keychain → `~/.anakot/.env` → environment variable.
