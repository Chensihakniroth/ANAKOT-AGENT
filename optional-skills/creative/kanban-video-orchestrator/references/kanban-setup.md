...
> [callmemo/kanban-video-pipeline](https://github.com/callmemo/kanban-video-pipeline).
...
`~/.anakot/.env` and macOS Keychain (if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    if grep -q "^${var}=" ~/.anakot/.env 2>/dev/null && \
       [ -n "$(grep "^${var}=" ~/.anakot/.env | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ~/.anakot/.env or Keychain (${kc_account}/${kc_service})"
    return 1
}
