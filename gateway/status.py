...
The PID file lives at ``{ANAKOT_HOME}/gateway.pid``.  ANAKOT_HOME defaults to
``~/.anakot`` but can be overridden via the environment variable.  This means
separate ANAKOT_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting ANAKOT_HOME."""
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Anakot gateway."""
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different ANAKOT_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``anakot.service`` + ``anakot-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # ``anakot gateway stop`` on Windows would be misclassified as an
