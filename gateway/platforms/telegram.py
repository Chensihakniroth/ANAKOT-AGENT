...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = float(os.getenv("ANAKOT_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", "0.8"))
        ...
        self._text_batch_delay_seconds = self._env_float_clamped(
            "ANAKOT_TELEGRAM_TEXT_BATCH_DELAY_SECONDS",
            0.3,
            min_value=0.08,
            max_value=2.0,
        )
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "ANAKOT_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS",
            1.0,
    ...
    def _thread_kwargs_for_send(
        ...
        metadata. Anakot-created private-chat topic lanes are marked with
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `anakot update` or `--replace` handoffs)
        ...
        message = (
            ...
            "To recover: ensure no other Anakot or OpenClaw instance is running "
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # readable by the Anakot process (shared mount, same machine, etc.).
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("ANAKOT_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("ANAKOT_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("ANAKOT_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("ANAKOT_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("ANAKOT_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            }
            ...
            try:
                ...
                from anakot_cli.commands import telegram_menu_commands
    ...
    def _build_provider_keyboard(self, providers: list):
        ...
        CLI ``anakot model`` picker via the shared ``group_providers`` fold,
        so all surfaces stay consistent.
        """
        try:
            from anakot_cli.models import group_providers
        except Exception:
    ...
    async def _handle_model_picker_callback(
        ...
        if data.startswith("mp:"):
            ...
            group_id = data[4:]
            try:
                from anakot_cli.models import PROVIDER_GROUPS
 ...
 text=self.format_message(f" Update prompt answered: *{label}*"),
    ...
    # Scripts live in ~/.anakot/scripts/gmail-triage/. `arg` from the callback
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # raw substring matches like "foo@anakot_bot.example" are not mentions
    ...
    def _explicit_bot_mentions_exclude_self(self, message: Message) -> bool:
        ...
        Telegram groups can contain several Anakot bot profiles. A message like
