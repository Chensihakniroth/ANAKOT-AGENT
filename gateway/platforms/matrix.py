...
def _resolve_matrix_bang_command(name: str) -> str | None:
    """Resolve a ``!command`` token to a dispatchable Anakot command token.

    Matrix clients often reserve leading ``/`` for local client commands.
    Anakot accepts ``!command`` as a Matrix-friendly alias, but only for
...
# Uses get_anakot_home() so each profile gets its own Matrix store.
from anakot_constants import get_anakot_dir as _get_anakot_dir

_STORE_DIR = _get_anakot_dir("platforms/matrix/store", "matrix/store")
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("ANAKOT_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("ANAKOT_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "Anakot Agent" become "Agent".
        """
        if not body:
            return ""
