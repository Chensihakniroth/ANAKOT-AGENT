...
class WhatsAppAdapter(BasePlatformAdapter):
 ...
 DEFAULT_REPLY_PREFIX = " *Anakot Agent*\n────────────\n"
    
    # Default bridge location relative to the anakot-agent install
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._session_path: Path = Path(config.extra.get(
            "session_path",
            get_anakot_dir("platforms/whatsapp/session", "whatsapp/session")
        ))
    ...
    async def connect(self) -> bool:
        ...
        if not check_whatsapp_requirements():
            ...
            self._set_fatal_error(
                ...
                "Node.js is not installed — install Node.js and re-run `anakot gateway`.",
                retryable=False,
            )
            return False
        ...
        # clear "run anakot whatsapp" message instead of the watcher
        ...
        if not creds_path.exists():
            logger.warning(
                ...
                "Run `anakot whatsapp` to pair, or remove WHATSAPP_ENABLED from "
                "your .env to disable.",
                self.name, creds_path,
            )
            self._set_fatal_error(
                "whatsapp_not_paired",
                "WhatsApp enabled but not paired — run `anakot whatsapp` to pair.",
                retryable=False,
            )
            return False
