...
# group mention gating without custom aliases, use conservative Anakot wake
# words so `require_mention: true` is a one-line enablement path.
DEFAULT_MENTION_PATTERNS = [
    r"(?<![\w@])@?anakot\s+agent\b[,:\-]?",
    r"(?<![\w@])@?anakot\b[,:\-]?",
]
...
class BlueBubblesAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> List[re.Pattern]:
        ...
        JSON list, or comma/newline-separated), or None (use Anakot defaults).
        """
        if raw is None:
