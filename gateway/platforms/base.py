...
# Audio file extensions Anakot recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Anakot' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Anakot-created Telegram private-chat
...
# Default location: {ANAKOT_HOME}/cache/images/ (legacy: image_cache/)
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; AnakotAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
...
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _ANAKOT_HOME / "image_cache",
    _ANAKOT_HOME / "audio_cache",
    _ANAKOT_HOME / "video_cache",
    _ANAKOT_HOME / "document_cache",
    _ANAKOT_HOME / "browser_screenshots",
    ...
    _ANAKOT_HOME / "cache" / "images",
    _ANAKOT_HOME / "cache" / "audio",
    _ANAKOT_HOME / "cache" / "videos",
    _ANAKOT_HOME / "cache" / "documents",
    _ANAKOT_HOME / "cache" / "screenshots",
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Anakot profile and shared Anakot root both contain control
    # files and credentials. Only cache subdirectories under them are
    # explicitly allowlisted above.
    for anakot_root in (_ANAKOT_HOME, _ANAKOT_ROOT):
        denied.append(anakot_root / ".env")
        denied.append(anakot_root / "auth.json")
        denied.append(anakot_root / "credentials")
        denied.append(anakot_root / "config.yaml")
    return denied
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Anakot
    secrets (``~/.anakot/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``ANAKOT_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Anakot-managed cache, under an operator-allowlisted root
    (``ANAKOT_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # The denylist still blocks /etc, /proc, ~/.ssh, ~/.aws, ~/.anakot/.env,
    # ~/.anakot/auth.json, etc. — so the obvious prompt-injection sites
...
class BasePlatformAdapter(ABC):
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("ANAKOT_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "ANAKOT_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "ANAKOT_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.anakot/media/generated/stale.png"}
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          ANAKOT_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          ANAKOT_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          ANAKOT_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        ...
        try:
            min_ms = int(os.getenv("ANAKOT_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("ANAKOT_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _anakot_run_generation is set on
