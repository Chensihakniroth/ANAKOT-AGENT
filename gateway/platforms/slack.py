...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # /model work directly without /anakot prefix). A single regex
            ...
            # manifest (see `anakot slack manifest`). In Socket Mode, Slack
            ...
            from anakot_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/anakot$")
            ...
            for _action_id in (
                "anakot_approve_once",
                "anakot_approve_session",
                "anakot_approve_always",
                "anakot_deny",
            ):
            ...
            for _action_id in (
                "anakot_confirm_once",
                "anakot_confirm_always",
                "anakot_confirm_cancel",
            ):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Anakot handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Anakot session — matching the per-thread behavior channels already
    ...
    async def _handle_slack_message(self, event: dict) -> None:
        ...
        if original_text.startswith("!"):
            try:
                ...
                # forms like ``!stop@anakot`` still resolve.
                ...
                },
                {
                    ...
                    "elements": [
                        ...
                        },
                        {
                            ...
                            "action_id": "anakot_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "anakot_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                ...
                },
                {
                    ...
                    "elements": [
                        ...
                        },
                        {
                            ...
                            "action_id": "anakot_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "anakot_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "anakot_confirm_once": "once",
            "anakot_confirm_always": "always",
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "anakot_approve_once": "once",
            "anakot_approve_session": "session",
            "anakot_approve_always": "always",
            "anakot_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/anakot <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/anakot
        ...
        if slash_name in {"anakot", ""}:
            # Legacy /anakot <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from anakot_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/anakot   ``).
        ...
        # "Running /cmd…" ack shown by handle_anakot_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/anakot <question>" must produce public replies so
