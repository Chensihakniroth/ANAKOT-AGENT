"""
Email platform adapter for the Anakot gateway.

Allows users to interact with Anakot by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "anakot-agent" "version" "{_anakot_version}" '
            '"vendor" "callmemo" '
            '"support-email" "noreply@callmemo.ai")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        msg_id = f"<anakot-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
