...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-Anakot-Session-Id header; opt-in long-term memory scoping via X-Anakot-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-Anakot-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists anakot-agent as an available model
- GET  /v1/capabilities            — machine-readable API capabilities for external UIs
- GET  /api/sessions               — list client-visible Anakot sessions
- POST /api/sessions               — create an empty Anakot session
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to anakot-agent
...
class ResponseStore:
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        # gracefully on NFS/SMB/FUSE-mounted ANAKOT_HOME (same filesystem
        # issue addressed for state.db/kanban.db — see
        # anakot_state._WAL_INCOMPAT_MARKERS).
...
def _derive_chat_session_id(
    ...
    the same Anakot session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through anakot-agent's AIAgent.
    """
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-Anakot-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-Anakot-Session-Id``: callers may send
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``anakot sessions list``
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the anakot-api-server default.

        ``gateway_session_key`` is a stable per-channel identifier supplied
        by the client (via ``X-Anakot-Session-Key``).  Unlike ``session_id``
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"status": "ok", "platform": "anakot-agent"})
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — return anakot-agent as an available model."""
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every Anakot version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions — create an empty Anakot session row."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        headers = {"X-Anakot-Session-Id": effective_session_id or session_id}
    ...
    async def _handle_session_chat_stream(self, request: "web.Request") -> "web.StreamResponse":
        ...
        headers = {
            ...
            "X-Anakot-Session-Id": session_id,
        }
        if gateway_session_key:
            headers["X-Anakot-Session-Key"] = gateway_session_key
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-Anakot-Session-Key.  This
        # is independent of X-Anakot-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-Anakot-Session-Id.
        ...
        if provided_session_id:
            ...
            # conversation map to the same Anakot session.  The first user
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``anakot.tool.progress`` with ``status: running``.
        ...
        response_headers = {
            "X-Anakot-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
        ...
        # truncation via finish_reason="length" + Anakot-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-Anakot-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-Anakot-Error"] = err_msg[:200]
    ...
    async def _write_sse_chat_completion(
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: anakot.tool.progress`` SSE event so
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = {"X-Anakot-Session-Id": session_id}
        if gateway_session_key:
            response_headers["X-Anakot-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
    ...
    async def _run_agent(
        ...
        def _run():
            ...
            # (e.g. X-Anakot-Session-Id header) can track compression-
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = (
            {"X-Anakot-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(
    ...
    async def _handle_run_approval(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "anakot.run.approval_response",
            "run_id": run_id,
            "choice": choice,
            "resolved": resolved,
        })
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # Store the adapter after native routes are registered. Local Anakot-Relay
