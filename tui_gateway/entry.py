...
# anakot modules.  anakot_cli sets ANAKOT_PYTHON_SRC_ROOT before spawning this
# subprocess; inserting it first ensures the installed packages win.
_src_root = os.environ.get("ANAKOT_PYTHON_SRC_ROOT", "")
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `ANAKOT_TUI_SIDECAR_URL` or `ANAKOT_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("ANAKOT_TUI_SIDECAR_URL") or os.environ.get("ANAKOT_TUI_SIDECAR_URL")
...
# tested; override via ``ANAKOT_TUI_GATEWAY_SHUTDOWN_GRACE_S`` or
# ``ANAKOT_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a slower environment needs
# more headroom (e.g. encrypted disks flushing checkpoints) and accept
# that a longer grace also means a longer wait when shutdown actually deadlocks.
_DEFAULT_SHUTDOWN_GRACE_S = 1.0


def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("ANAKOT_TUI_GATEWAY_SHUTDOWN_GRACE_S") or os.environ.get("ANAKOT_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``ANAKOT_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``anakot --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
...
def main():
    ...
    # server stalled the whole shell on "summoning anakot…" for the full
