...
load_anakot_dotenv(
    anakot_home=_anakot_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.anakot/logs/tui_gateway_crash.log
...
try:
    _slash_timeout = float(os.environ.get("ANAKOT_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _ws_orphan_reap_grace = float(
        os.environ.get("ANAKOT_TUI_WS_ORPHAN_REAP_GRACE_S") or "20"
    )
except (ValueError, TypeError):
...
_LONG_HANDLERS = frozenset(
    {
        ...
        "pet.hatch",
        "pet.cancel",
        # Wake-word mic open/halt/close can block up to ~5 s (start/resume wait
        # on the device; stop/pause join the capture thread), so run it off the
        # main loop — a flaky mic must not freeze approval.respond/heartbeat/
        # session streaming. wake.status stays fast-path; wake.feed (continuous
        # renderer audio frames) MUST stay inline on the main loop.
        "wake.pause",
        "wake.resume",
        "wake.start",
        "wake.stop",
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("ANAKOT_TUI_RPC_POOL_WORKERS") or "4")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 4
...
# ``profile`` on those calls; we open that profile's db and bind its ANAKOT_HOME
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `anakot` shows the prompt before constructing AIAgent; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            # ANAKOT_HOME so config/skills/model resolve to it, and hand the
...
def _load_cfg() -> dict:
    ...
    try:
        ...
        # launch profile's _anakot_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_anakot_home_override()
        home = override if isinstance(override, str) and override else _anakot_home
...
def _set_session_context(session_key: str, cwd: str | None = None) -> list:
    try:
        ...
        # Propagate the platform env var if set (e.g. web_server from the
        # web dashboard) so get_session_env("ANAKOT_SESSION_PLATFORM") resolves
        # correctly for tools that gate on it.
        _platform = os.environ.get("ANAKOT_SESSION_PLATFORM", "")
...
def _resolve_model() -> str:
    env = (
        os.environ.get("ANAKOT_MODEL", "")
        or os.environ.get("ANAKOT_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    model = _resolve_model()
    explicit_provider = os.environ.get("ANAKOT_TUI_PROVIDER", "").strip()
    ...
    try:
        ...
        current_provider = (
            ...
            or os.environ.get("ANAKOT_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _load_enabled_toolsets() -> list[str] | None:
    explicit = [
        item.strip()
        for item in os.environ.get("ANAKOT_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] ANAKOT_TUI_TOOLSETS=all enables every toolset; "
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown ANAKOT_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in ANAKOT_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid ANAKOT_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid ANAKOT_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(sid: str, session: dict, raw_input: str) -> dict:
    from anakot_cli.model_switch import parse_model_flags, switch_model
    ...
    os.environ["ANAKOT_MODEL"] = result.new_model
    os.environ["ANAKOT_INFERENCE_MODEL"] = result.new_model
    ...
    # in config or env.
    #
    # ANAKOT_TUI_PROVIDER is the canonical "explicit-this-process" carrier
    ...
    if result.target_provider:
        os.environ["ANAKOT_INFERENCE_PROVIDER"] = result.target_provider
        os.environ["ANAKOT_TUI_PROVIDER"] = result.target_provider
    if persist_global:
...
def _get_usage(agent) -> dict:
    ...
    # ANAKOT_DEV_CREDITS so the payload stays clean when the flag is off.
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from anakot_cli.config import load_config as _load_full_cfg
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("ANAKOT_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _make_agent(sid: str, key: str, session_id: str | None = None, session_db=None):
    ...
    return AIAgent(
        ...
        skip_context_files=is_truthy_value(os.environ.get("ANAKOT_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("ANAKOT_IGNORE_RULES")),
        **_agent_cbs(sid),
    )
...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind ANAKOT_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # custom `ANAKOT_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    _enable_gateway_prompts()
    home_token = (
        set_anakot_home_override(str(profile_home)) if profile_home is not None else None
    )
    try:
    ...
    with _session_resume_lock:
        ...
        try:
            ...
            if sid in _sessions:
                ...
                # Remember the profile home so each turn re-binds ANAKOT_HOME (the
...
def _(rid, params: dict) -> dict:
    ...
    # callmemo credits block — agent-independent (a portal fetch), so it shows even
    # with zero API calls or on a resumed session. The TUI /usage panel renders
    # these lines regardless of `calls`. Fail-open: [] when not logged into callmemo
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Anakot profile home
    # (~/.anakot/sessions/saved/) rather than the project/workspace CWD, and
...
#
# Layout:  $ANAKOT_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _run_prompt_submit(rid, sid: str, session: dict, text: Any) -> None:
    ...
    def run():
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn ANAKOT_HOME override for a resumed remote profile
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_anakot_home_override(_profile_home_str)
    ...
    if key == "yolo":
        ...
        try:
            if session:
                ...
                current = is_truthy_value(os.environ.get("ANAKOT_YOLO_MODE"))
                if current:
...
def _(rid, params: dict) -> dict:
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_anakot_home), "display": display_anakot_home()})
    if key == "project":
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Anakot provider is configured.",
                },
            )
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.anakot/.env`` into the gateway process via
    ``anakot_cli.config.reload_env``, matching classic CLI's ``/reload``
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`anakot setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`anakot gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`anakot sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`anakot config edit` needs $EDITOR in a real terminal"
    return None
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from anakot_cli.commands import SlashCommandCompleter
    ...
    try:
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `anakot model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.anakot/.env
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action == "tts":
        ...
        os.environ["ANAKOT_VOICE_TTS"] = "1" if new_value else "0"
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from anakot_cli.browser_connect import manual_chrome_debug_command
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from anakot_cli.browser_connect import try_launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("ANAKOT_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("ANAKOT_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_anakot_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "browse":
            from anakot_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from anakot_cli.skills_hub import inspect_skill
